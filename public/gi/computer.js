// Auto Generated

var _Kingdoms = [
    {
        "ID": 0,
        "Name": {
            "CH": "元素生命",
            "EN": "Elemental Life"
        },
        "Classes": [
            {
                "ID": 100010,
                "Name": {
                    "CH": "小史莱姆",
                    "EN": "Slimes"
                },
                "Monsters": [
                    101,
                    103,
                    105,
                    108,
                    110,
                    112,
                    114
                ]
            },
            {
                "ID": 100011,
                "Name": {
                    "CH": "大史莱姆",
                    "EN": "Big Slimes"
                },
                "Monsters": [
                    102,
                    104,
                    106,
                    107,
                    109,
                    111,
                    113,
                    115
                ]
            },
            {
                "ID": 10002,
                "Name": {
                    "CH": "元素凝核",
                    "EN": "Elem Core"
                },
                "Monsters": [
                    201
                ]
            },
            {
                "ID": 10004,
                "Name": {
                    "CH": "无相元素",
                    "EN": "Hypostases"
                },
                "Monsters": [
                    401,
                    402,
                    403,
                    405,
                    406,
                    404,
                    407
                ]
            },
            {
                "ID": 10005,
                "Name": {
                    "CH": "纯水幻形",
                    "EN": "Oceanid"
                },
                "Monsters": [
                    503,
                    509,
                    5020,
                    5070,
                    506,
                    5040,
                    508,
                    505,
                    5021,
                    5071,
                    5041
                ]
            },
            {
                "ID": 10008,
                "Name": {
                    "CH": "浊水幻灵",
                    "EN": "Phantasm"
                },
                "Monsters": [
                    510,
                    511,
                    512,
                    513,
                    516
                ]
            },
            {
                "ID": 10006,
                "Name": {
                    "CH": "飘浮灵",
                    "EN": "Specters"
                },
                "Monsters": [
                    601,
                    602,
                    603,
                    604,
                    605,
                    606,
                    607
                ]
            },
            {
                "ID": 10007,
                "Name": {
                    "CH": "雷音权现",
                    "EN": "Raijin"
                },
                "Monsters": [
                    701
                ]
            }
        ]
    },
    {
        "ID": 1,
        "Name": {
            "CH": "丘丘部族",
            "EN": "Hilichurls"
        },
        "Classes": [
            {
                "ID": 101010,
                "Name": {
                    "CH": "近战丘丘人",
                    "EN": "Melee"
                },
                "Monsters": [
                    10101,
                    10102,
                    10107,
                    10103,
                    10112,
                    10114
                ]
            },
            {
                "ID": 101011,
                "Name": {
                    "CH": "远程丘丘人",
                    "EN": "Ranged"
                },
                "Monsters": [
                    10104,
                    10105,
                    10109,
                    10110,
                    10106,
                    10113,
                    10116
                ]
            },
            {
                "ID": 101012,
                "Name": {
                    "CH": "Mimi domu!",
                    "EN": "Mimi domu!"
                },
                "Monsters": [
                    10115
                ]
            },
            {
                "ID": 101020,
                "Name": {
                    "CH": "丘丘暴徒",
                    "EN": "Mitachurls"
                },
                "Monsters": [
                    10202,
                    10207,
                    10201,
                    10203,
                    10206
                ]
            },
            {
                "ID": 101021,
                "Name": {
                    "CH": "丘丘王",
                    "EN": "Lawachurls"
                },
                "Monsters": [
                    10205,
                    10204,
                    10208
                ]
            },
            {
                "ID": 10103,
                "Name": {
                    "CH": "丘丘萨满",
                    "EN": "Samachurls"
                },
                "Monsters": [
                    10301,
                    10302,
                    10303,
                    10304,
                    10305,
                    10306
                ]
            },
            {
                "ID": 10104,
                "Name": {
                    "CH": "丘丘游侠",
                    "EN": "Rogues"
                },
                "Monsters": [
                    10401,
                    10402
                ]
            }
        ]
    },
    {
        "ID": 2,
        "Name": {
            "CH": "深渊势力",
            "EN": "The Abyss"
        },
        "Classes": [
            {
                "ID": 10201,
                "Name": {
                    "CH": "深渊法师",
                    "EN": "Mages"
                },
                "Monsters": [
                    20101,
                    20102,
                    20103,
                    20104
                ]
            },
            {
                "ID": 10202,
                "Name": {
                    "CH": "深渊使者",
                    "EN": "Invokers"
                },
                "Monsters": [
                    20201,
                    20202,
                    20301,
                    20302
                ]
            },
            {
                "ID": 10210,
                "Name": {
                    "CH": "深罪施洗",
                    "EN": "Baptists"
                },
                "Monsters": [
                    21001,
                    21002,
                    21003,
                    21004
                ]
            },
            {
                "ID": 10204,
                "Name": {
                    "CH": "兽境群狼",
                    "EN": "Rifthounds"
                },
                "Monsters": [
                    20401,
                    20402,
                    20501,
                    20502,
                    20601
                ]
            },
            {
                "ID": 10207,
                "Name": {
                    "CH": "黑蛇众",
                    "EN": "Khaenri'ah"
                },
                "Monsters": [
                    20701,
                    20702,
                    20703,
                    20801,
                    20802
                ]
            },
            {
                "ID": 10211,
                "Name": {
                    "CH": "隙境原体",
                    "EN": "Breacher"
                },
                "Monsters": [
                    21102,
                    21101,
                    21104,
                    21103
                ]
            }
        ]
    },
    {
        "ID": 3,
        "Name": {
            "CH": "愚人众",
            "EN": "The Fatui"
        },
        "Classes": [
            {
                "ID": 10301,
                "Name": {
                    "CH": "愚人众先遣队",
                    "EN": "Skirmishers"
                },
                "Monsters": [
                    30101,
                    30102,
                    30103,
                    30104,
                    30105,
                    30106
                ]
            },
            {
                "ID": 10302,
                "Name": {
                    "CH": "债务处理人",
                    "EN": "Agents"
                },
                "Monsters": [
                    30201
                ]
            },
            {
                "ID": 10303,
                "Name": {
                    "CH": "萤术士",
                    "EN": "Cicin Mages"
                },
                "Monsters": [
                    30301,
                    30401
                ]
            },
            {
                "ID": 10305,
                "Name": {
                    "CH": "冬国仕女",
                    "EN": "Maidens"
                },
                "Monsters": [
                    30501
                ]
            },
            {
                "ID": 10306,
                "Name": {
                    "CH": "愚人众役者",
                    "EN": "Seneschals"
                },
                "Monsters": [
                    30601,
                    30602
                ]
            }
        ]
    },
    {
        "ID": 4,
        "Name": {
            "CH": "自律机关",
            "EN": "Automatons"
        },
        "Classes": [
            {
                "ID": 104010,
                "Name": {
                    "CH": "原初机关",
                    "EN": "Primordial"
                },
                "Monsters": [
                    40101,
                    40102,
                    40103
                ]
            },
            {
                "ID": 10402,
                "Name": {
                    "CH": "拟生机关",
                    "EN": "Biomimetic"
                },
                "Monsters": [
                    40201,
                    40202,
                    40203,
                    40204,
                    40211
                ]
            },
            {
                "ID": 104011,
                "Name": {
                    "CH": "挖掘机关",
                    "EN": "Excavation"
                },
                "Monsters": [
                    40104
                ]
            },
            {
                "ID": 10403,
                "Name": {
                    "CH": "龙形机关",
                    "EN": "Drakes"
                },
                "Monsters": [
                    40301,
                    40302,
                    40303
                ]
            },
            {
                "ID": 10509,
                "Name": {
                    "CH": "人形机关",
                    "EN": "Swordmaster"
                },
                "Monsters": [
                    50901,
                    50902,
                    50903,
                    50904
                ]
            },
            {
                "ID": 10404,
                "Name": {
                    "CH": "赤王造物",
                    "EN": "Deshret"
                },
                "Monsters": [
                    40401,
                    40402,
                    40403,
                    40501
                ]
            },
            {
                "ID": 104060,
                "Name": {
                    "CH": "小机关 · 荒",
                    "EN": "Small Meka"
                },
                "Monsters": [
                    40603,
                    40605,
                    40606,
                    40611,
                    40601,
                    40602,
                    40604,
                    40607
                ]
            },
            {
                "ID": 104065,
                "Name": {
                    "CH": "小机关 · 芒",
                    "EN": "Small Meka"
                },
                "Monsters": [
                    40653,
                    40655,
                    40656,
                    40661,
                    40651,
                    40652,
                    40654,
                    40657
                ]
            },
            {
                "ID": 104061,
                "Name": {
                    "CH": "大机关 · 荒",
                    "EN": "Big Meka"
                },
                "Monsters": [
                    40609,
                    40608,
                    40610,
                    40612
                ]
            },
            {
                "ID": 104066,
                "Name": {
                    "CH": "大机关 · 芒",
                    "EN": "Big Meka"
                },
                "Monsters": [
                    40659,
                    40658,
                    40660,
                    40662
                ]
            },
            {
                "ID": 10407,
                "Name": {
                    "CH": "冰风组曲",
                    "EN": "Icewind"
                },
                "Monsters": [
                    40701,
                    40702
                ]
            },
            {
                "ID": 10408,
                "Name": {
                    "CH": "力场机器",
                    "EN": "Field Mek"
                },
                "Monsters": [
                    40801
                ]
            }
        ]
    },
    {
        "ID": 5,
        "Name": {
            "CH": "其他人类势力",
            "EN": "Other Human"
        },
        "Classes": [
            {
                "ID": 105010,
                "Name": {
                    "CH": "盗宝团瘦子",
                    "EN": "Slim Dudes"
                },
                "Monsters": [
                    50102,
                    50202
                ]
            },
            {
                "ID": 105011,
                "Name": {
                    "CH": "盗宝团药剂师",
                    "EN": "Potioneers"
                },
                "Monsters": [
                    50103,
                    50104,
                    50105,
                    50106
                ]
            },
            {
                "ID": 10502,
                "Name": {
                    "CH": "盗宝团胖子",
                    "EN": "Fat Dudes"
                },
                "Monsters": [
                    50107,
                    50302,
                    50303,
                    50601,
                    50701
                ]
            },
            {
                "ID": 10508,
                "Name": {
                    "CH": "野伏众",
                    "EN": "Nobushi"
                },
                "Monsters": [
                    50801,
                    50802,
                    50803
                ]
            },
            {
                "ID": 10510,
                "Name": {
                    "CH": "海乱鬼",
                    "EN": "Kairagi"
                },
                "Monsters": [
                    51001,
                    51002
                ]
            },
            {
                "ID": 10521,
                "Name": {
                    "CH": "镀金旅团低层",
                    "EN": "Eremite Low"
                },
                "Monsters": [
                    52102,
                    52103,
                    52101,
                    52104,
                    52105
                ]
            },
            {
                "ID": 10531,
                "Name": {
                    "CH": "镀金旅团中层",
                    "EN": "Eremite Mid"
                },
                "Monsters": [
                    53101,
                    53102,
                    53103
                ]
            },
            {
                "ID": 10541,
                "Name": {
                    "CH": "镀金旅团高层",
                    "EN": "Eremite Top"
                },
                "Monsters": [
                    54101,
                    54102,
                    54103,
                    54104
                ]
            },
            {
                "ID": 10511,
                "Name": {
                    "CH": "镀金旅团厄灵",
                    "EN": "Spirits"
                },
                "Monsters": [
                    51101,
                    51102,
                    51103,
                    51104
                ]
            }
        ]
    },
    {
        "ID": 6,
        "Name": {
            "CH": "异种魔兽",
            "EN": "Mystical Beasts"
        },
        "Classes": [
            {
                "ID": 10601,
                "Name": {
                    "CH": "骗骗花",
                    "EN": "Mimetics"
                },
                "Monsters": [
                    60101,
                    60102,
                    60103
                ]
            },
            {
                "ID": 10602,
                "Name": {
                    "CH": "巨藤树",
                    "EN": "Regisvines"
                },
                "Monsters": [
                    60201,
                    60202,
                    60203
                ]
            },
            {
                "ID": 10603,
                "Name": {
                    "CH": "岩之龙蜥",
                    "EN": "Geovishaps"
                },
                "Monsters": [
                    60301,
                    60401,
                    60402,
                    60403,
                    60404,
                    60501,
                    60502,
                    60503,
                    60504
                ]
            },
            {
                "ID": 10605,
                "Name": {
                    "CH": "深海龙蜥",
                    "EN": "Bathysmals"
                },
                "Monsters": [
                    60509,
                    60510,
                    60511,
                    60506,
                    60507,
                    60508
                ]
            },
            {
                "ID": 10606,
                "Name": {
                    "CH": "飞萤",
                    "EN": "Cicins"
                },
                "Monsters": [
                    60601,
                    60602,
                    60603
                ]
            },
            {
                "ID": 10609,
                "Name": {
                    "CH": "蕈兽",
                    "EN": "Fungi"
                },
                "Monsters": [
                    60901,
                    60902,
                    60903,
                    60904,
                    60905,
                    60906,
                    60907,
                    60908,
                    60909,
                    60910
                ]
            },
            {
                "ID": 10612,
                "Name": {
                    "CH": "真蕈",
                    "EN": "Shrooms"
                },
                "Monsters": [
                    61201,
                    61202,
                    61203,
                    61204
                ]
            },
            {
                "ID": 10611,
                "Name": {
                    "CH": "恐蕈",
                    "EN": "Shroomlord"
                },
                "Monsters": [
                    61101
                ]
            },
            {
                "ID": 10610,
                "Name": {
                    "CH": "圣骸兽",
                    "EN": "Consecrated"
                },
                "Monsters": [
                    61001,
                    61002,
                    61003,
                    61004,
                    61005
                ]
            },
            {
                "ID": 10613,
                "Name": {
                    "CH": "沙虫",
                    "EN": "Wenuts"
                },
                "Monsters": [
                    61301
                ]
            },
            {
                "ID": 10616,
                "Name": {
                    "CH": "原海异种",
                    "EN": "Aberrant"
                },
                "Monsters": [
                    61601,
                    61602,
                    61603
                ]
            },
            {
                "ID": 10617,
                "Name": {
                    "CH": "原海帝皇",
                    "EN": "Emperor"
                },
                "Monsters": [
                    61701,
                    61801
                ]
            },
            {
                "ID": 10619,
                "Name": {
                    "CH": "猊兽",
                    "EN": "Suanni"
                },
                "Monsters": [
                    61902,
                    61901
                ]
            }
        ]
    },
    {
        "ID": 11,
        "Name": {
            "CH": "<b>4.4 新增</b>",
            "EN": "<b>New in 4.4</b>"
        },
        "Classes": [
            {
                "ID": 99991,
                "Name": {
                    "CH": "沉玉谷",
                    "EN": "Chenyu Vale"
                },
                "Monsters": [
                    61902,
                    61901,
                    80221
                ]
            },
            {
                "ID": 99992,
                "Name": {
                    "CH": "地方传奇",
                    "EN": "Legends"
                },
                "Monsters": [
                    60406,
                    60407
                ]
            }
        ]
    },
    {
        "ID": 8,
        "Name": {
            "CH": "动物",
            "EN": "Animals"
        },
        "Classes": [
            {
                "ID": 10801,
                "Name": {
                    "CH": "龙脊雪山",
                    "EN": "Dragonspine"
                },
                "Monsters": [
                    80203
                ]
            },
            {
                "ID": 10802,
                "Name": {
                    "CH": "人类的好朋友",
                    "EN": "Sumpter"
                },
                "Monsters": [
                    806051,
                    806052,
                    806053,
                    806061,
                    806062,
                    806063
                ]
            },
            {
                "ID": 10806,
                "Name": {
                    "CH": "沉玉谷",
                    "EN": "Chenyu Vale"
                },
                "Monsters": [
                    80221
                ]
            },
            {
                "ID": 10803,
                "Name": {
                    "CH": "须弥雨林",
                    "EN": "Rainforest"
                },
                "Monsters": [
                    80601,
                    80602
                ]
            },
            {
                "ID": 10804,
                "Name": {
                    "CH": "须弥沙漠",
                    "EN": "Desert"
                },
                "Monsters": [
                    80603,
                    80604,
                    80615
                ]
            },
            {
                "ID": 10805,
                "Name": {
                    "CH": "枫丹",
                    "EN": "Fontaine"
                },
                "Monsters": [
                    61510,
                    61520,
                    61530,
                    61540,
                    61550,
                    61560,
                    61570
                ]
            }
        ]
    },
    {
        "ID": 9,
        "Name": {
            "CH": "值得铭记的强敌",
            "EN": "Weekly BOSS"
        },
        "Classes": [
            {
                "ID": 10901,
                "Name": {
                    "CH": "裂空的魔龙",
                    "EN": "Stormterror"
                },
                "Monsters": [
                    90101
                ]
            },
            {
                "ID": 10902,
                "Name": {
                    "CH": "北风的王狼",
                    "EN": "Boreas"
                },
                "Monsters": [
                    90201
                ]
            },
            {
                "ID": 10903,
                "Name": {
                    "CH": "「公子」",
                    "EN": "Childe"
                },
                "Monsters": [
                    90301,
                    90302,
                    90303
                ]
            },
            {
                "ID": 10904,
                "Name": {
                    "CH": "若陀龙王",
                    "EN": "Azhdaha"
                },
                "Monsters": [
                    90401
                ]
            },
            {
                "ID": 10905,
                "Name": {
                    "CH": "「女士」",
                    "EN": "La Signora"
                },
                "Monsters": [
                    90501,
                    90502
                ]
            },
            {
                "ID": 10906,
                "Name": {
                    "CH": "雷电将军",
                    "EN": "Raiden"
                },
                "Monsters": [
                    90602
                ]
            },
            {
                "ID": 10907,
                "Name": {
                    "CH": "「正机之神」",
                    "EN": "Scaramouche"
                },
                "Monsters": [
                    90701,
                    90702
                ]
            },
            {
                "ID": 109080,
                "Name": {
                    "CH": "翠草之龙",
                    "EN": "Apep"
                },
                "Monsters": [
                    90801,
                    90800
                ]
            },
            {
                "ID": 109081,
                "Name": {
                    "CH": "阿佩普的子嗣",
                    "EN": "Apep"
                },
                "Monsters": [
                    90802,
                    90803,
                    90804,
                    90805,
                    90806,
                    90807
                ]
            },
            {
                "ID": 10909,
                "Name": {
                    "CH": "吞星之鲸",
                    "EN": "Devourer"
                },
                "Monsters": [
                    90901,
                    90902,
                    90903
                ]
            }
        ]
    },
    {
        "ID": 10,
        "Name": {
            "CH": "地方传奇",
            "EN": "Local Legends"
        },
        "Classes": [
            {
                "ID": 11001,
                "Name": {
                    "CH": "「公义」",
                    "EN": "Masanori"
                },
                "Monsters": [
                    51003,
                    51004
                ]
            },
            {
                "ID": 11002,
                "Name": {
                    "CH": "武士狗",
                    "EN": "Doggie"
                },
                "Monsters": [
                    80209
                ]
            },
            {
                "ID": 11040,
                "Name": {
                    "CH": "4.0",
                    "EN": "4.0"
                },
                "Monsters": [
                    5101,
                    5111,
                    61611,
                    61621,
                    61613,
                    61623,
                    61612,
                    61622
                ]
            },
            {
                "ID": 11041,
                "Name": {
                    "CH": "4.1",
                    "EN": "4.1"
                },
                "Monsters": [
                    40622,
                    30603,
                    61624
                ]
            },
            {
                "ID": 11042,
                "Name": {
                    "CH": "4.2",
                    "EN": "4.2"
                },
                "Monsters": [
                    30604,
                    61625,
                    61635,
                    50108,
                    50602
                ]
            },
            {
                "ID": 11044,
                "Name": {
                    "CH": "4.4",
                    "EN": "4.4"
                },
                "Monsters": [
                    60406,
                    60407
                ]
            },
            {
                "ID": 11006,
                "Name": {
                    "CH": "深渊",
                    "EN": "Abyss"
                },
                "Monsters": [
                    5102,
                    5112,
                    40632,
                    30605,
                    30606,
                    60410,
                    60411
                ]
            },
            {
                "ID": 11003,
                "Name": {
                    "CH": "镇石",
                    "EN": "Monoliths"
                },
                "Monsters": [
                    70350450
                ]
            }
        ]
    },
    {
        "ID": 12,
        "Name": {
            "CH": "友方",
            "EN": "Friendly"
        },
        "Classes": [
            {
                "ID": 12130,
                "Name": {
                    "CH": "3.0 森林书",
                    "EN": "3.0 Quest"
                },
                "Monsters": [
                    35210101
                ]
            },
            {
                "ID": 12131,
                "Name": {
                    "CH": "3.1 沙漠书",
                    "EN": "3.1 Quest"
                },
                "Monsters": [
                    35310101,
                    35310301
                ]
            },
            {
                "ID": 12134,
                "Name": {
                    "CH": "3.4 后沙漠书",
                    "EN": "3.4 Quest"
                },
                "Monsters": [
                    35410101,
                    35410201
                ]
            },
            {
                "ID": 12135,
                "Name": {
                    "CH": "3.5 迪希雅",
                    "EN": "3.5 Dehya"
                },
                "Monsters": [
                    35210201,
                    35210402,
                    35210502
                ]
            },
            {
                "ID": 12136,
                "Name": {
                    "CH": "3.6 卡维",
                    "EN": "3.6 Kaveh"
                },
                "Monsters": [
                    35210202
                ]
            },
            {
                "ID": 12138,
                "Name": {
                    "CH": "3.8 地图",
                    "EN": "3.8 Map"
                },
                "Monsters": [
                    30050701
                ]
            }
        ]
    },
    {
        "ID": 23,
        "Name": {
            "CH": "4.4 11层",
            "EN": "4.4 F11"
        },
        "Disorder": {
            "CH": "队伍中所有角色获得75%风元素伤害加成。",
            "EN": "All party members receive a 75% Anemo DMG Bonus."
        },
        "Classes": [
            {
                "ID": 12111,
                "Name": {
                    "CH": "11-1 上半",
                    "EN": "11-1 Upper"
                },
                "Interval": "11-1",
                "Monsters": [
                    10202,
                    10102,
                    10107
                ]
            },
            {
                "ID": 12112,
                "Name": {
                    "CH": "11-1 下半",
                    "EN": "11-1 Lower"
                },
                "Interval": "11-1",
                "Monsters": [
                    112,
                    110,
                    105,
                    108
                ]
            },
            {
                "ID": 12121,
                "Name": {
                    "CH": "11-2 上半",
                    "EN": "11-2 Upper"
                },
                "Interval": "11-2",
                "Monsters": [
                    60402,
                    60404,
                    60506
                ]
            },
            {
                "ID": 12122,
                "Name": {
                    "CH": "11-2 下半",
                    "EN": "11-2 Lower"
                },
                "Interval": "11-2",
                "Monsters": [
                    20101,
                    20102,
                    113,
                    111
                ]
            },
            {
                "ID": 12131,
                "Name": {
                    "CH": "11-3 上半",
                    "EN": "11-3 Upper"
                },
                "Interval": "11-3",
                "Monsters": [
                    40301
                ]
            },
            {
                "ID": 12132,
                "Name": {
                    "CH": "11-3 下半",
                    "EN": "11-3 Lower"
                },
                "Interval": "11-3",
                "Monsters": [
                    30501,
                    50103,
                    50104,
                    50105,
                    50106
                ]
            }
        ]
    },
    {
        "ID": 25,
        "Name": {
            "CH": "4.4 12层",
            "EN": "4.4 F12"
        },
        "Disorder": {
            "CH": "仅在此层，地脉的涌流平缓通顺。",
            "EN": "For this floor only, the ley line flow will be normal."
        },
        "Classes": [
            {
                "ID": 12211,
                "Name": {
                    "CH": "12-1 上半",
                    "EN": "12-1 Upper"
                },
                "Interval": "12-1",
                "Monsters": [
                    40612,
                    21103
                ]
            },
            {
                "ID": 12212,
                "Name": {
                    "CH": "12-1 下半",
                    "EN": "12-1 Lower"
                },
                "Interval": "12-1",
                "Monsters": [
                    40104
                ]
            },
            {
                "ID": 12221,
                "Name": {
                    "CH": "12-2 上半",
                    "EN": "12-2 Upper"
                },
                "Interval": "12-2",
                "Monsters": [
                    54101,
                    54102,
                    54104,
                    54103
                ]
            },
            {
                "ID": 12222,
                "Name": {
                    "CH": "12-2 下半",
                    "EN": "12-2 Lower"
                },
                "Interval": "12-2",
                "Monsters": [
                    61301
                ]
            },
            {
                "ID": 12231,
                "Name": {
                    "CH": "12-3 上半",
                    "EN": "12-3 Upper"
                },
                "Interval": "12-3",
                "Monsters": [
                    40608,
                    40609
                ]
            },
            {
                "ID": 12232,
                "Name": {
                    "CH": "12-3 下半",
                    "EN": "12-3 Lower"
                },
                "Interval": "12-3",
                "Monsters": [
                    40303
                ]
            }
        ]
    },
    {
        "ID": 19,
        "Name": {
            "CH": "<b>4.5 11层</b>",
            "EN": "<b>4.5 F11</b>"
        },
        "Disorder": {
            "CH": "队伍中所有角色获得75%岩元素伤害加成。",
            "EN": "All party members receive a 75% Geo DMG Bonus."
        },
        "Classes": [
            {
                "ID": 12111,
                "Name": {
                    "CH": "11-1 上半",
                    "EN": "11-1 Upper"
                },
                "Interval": "11-1",
                "Monsters": [
                    10203,
                    10304,
                    10105,
                    10109,
                    10112
                ]
            },
            {
                "ID": 12112,
                "Name": {
                    "CH": "11-1 下半",
                    "EN": "11-1 Lower"
                },
                "Interval": "11-1",
                "Monsters": [
                    61201,
                    61203,
                    61202,
                    61204,
                    61601,
                    61602,
                    61603
                ]
            },
            {
                "ID": 12121,
                "Name": {
                    "CH": "11-2 上半",
                    "EN": "11-2 Upper"
                },
                "Interval": "11-2",
                "Monsters": [
                    61902,
                    40658,
                    40660
                ]
            },
            {
                "ID": 12122,
                "Name": {
                    "CH": "11-2 下半",
                    "EN": "11-2 Lower"
                },
                "Interval": "11-2",
                "Monsters": [
                    61002,
                    61003
                ]
            },
            {
                "ID": 12131,
                "Name": {
                    "CH": "11-3 上半",
                    "EN": "11-3 Upper"
                },
                "Interval": "11-3",
                "Monsters": [
                    5102,
                    5112
                ]
            },
            {
                "ID": 12132,
                "Name": {
                    "CH": "11-3 下半",
                    "EN": "11-3 Lower"
                },
                "Interval": "11-3",
                "Monsters": [
                    40602,
                    40612
                ]
            }
        ]
    },
    {
        "ID": 21,
        "Name": {
            "CH": "<b>4.5 12层</b>",
            "EN": "<b>4.5 F12</b>"
        },
        "Disorder": {
            "CH": "仅在此层，地脉的涌流平缓通顺。",
            "EN": "For this floor only, the ley line flow will be normal."
        },
        "Classes": [
            {
                "ID": 12211,
                "Name": {
                    "CH": "12-1 上半",
                    "EN": "12-1 Upper"
                },
                "Interval": "12-1",
                "Monsters": [
                    20101,
                    30605,
                    30606,
                    21103
                ]
            },
            {
                "ID": 12212,
                "Name": {
                    "CH": "12-1 下半",
                    "EN": "12-1 Lower"
                },
                "Interval": "12-1",
                "Monsters": [
                    61101
                ]
            },
            {
                "ID": 12221,
                "Name": {
                    "CH": "12-2 上半",
                    "EN": "12-2 Upper"
                },
                "Interval": "12-2",
                "Monsters": [
                    607,
                    605,
                    606,
                    60102,
                    20802,
                    20702,
                    20701,
                    20703
                ]
            },
            {
                "ID": 12222,
                "Name": {
                    "CH": "12-2 下半",
                    "EN": "12-2 Lower"
                },
                "Interval": "12-2",
                "Monsters": [
                    40211
                ]
            },
            {
                "ID": 12231,
                "Name": {
                    "CH": "12-3 上半",
                    "EN": "12-3 Upper"
                },
                "Interval": "12-3",
                "Monsters": [
                    40702
                ]
            },
            {
                "ID": 12232,
                "Name": {
                    "CH": "12-3 下半",
                    "EN": "12-3 Lower"
                },
                "Interval": "12-3",
                "Monsters": [
                    40632
                ]
            }
        ]
    },
    {
        "ID": 5001,
        "Name": {
            "CH": "4.4活动第一天",
            "EN": "4.4 Event Day 1"
        },
        "Disorder": {
            "CH": "<span style='font-size:0.9em;line-height:2;'>队伍中角色生命值提升或降低时，暴击率获得提升。<br>每个阶段试用：<color style='color:#2545BA;'>那维莱特 八重神子 枫原万叶 绮良良 班尼特 芭芭拉 / 林尼 迪希雅 行秋 迪奥娜 / 莱欧斯利 夜兰 / 娜维娅 芙宁娜</color><br>只能使用试用角色战斗。每阶段都可以用前面阶段的试用角色，但每个角色最多出场2次。<br>每阶段时间限制：600s（60原石）/ 150s（+20原石）/ 90s（+摩拉、矿）</span>",
            "EN": "<span style='font-size:0.9em;line-height:2;'>When party members' HP increases or decreases, gain Crit Rate boost.<br>Trial Each Stage: <color style='color:#2545BA;'>Neuvillette, Yae Miko, Kazuha, Kirara, Bennett, Barbara / Lyney, Dehya, Xingqiu, Diona / Wriothesley, Yelan / Navia, Furina</color><br>You can only use trial characters. In every stage you can use trial characters from previous stages, but each character can be used at most twice.<br>Time Limit Per Stage: 600s (60 Primo) / 150s (+20 Primo) / 90s (+ Ore, Mora)</span>"
        },
        "Classes": [
            {
                "ID": 0,
                "Name": {
                    "CH": "第一阶段",
                    "EN": "Stage 1"
                },
                "Interval": "44-1",
                "Monsters": [
                    21103,
                    60906,
                    60904,
                    61202,
                    61204
                ]
            },
            {
                "ID": 0,
                "Name": {
                    "CH": "第二阶段",
                    "EN": "Stage 2"
                },
                "Interval": "44-1",
                "Monsters": [
                    511,
                    10204,
                    30101
                ]
            },
            {
                "ID": 0,
                "Name": {
                    "CH": "第三阶段",
                    "EN": "Stage 3"
                },
                "Interval": "44-1",
                "Monsters": [
                    60506,
                    40103,
                    50801
                ]
            },
            {
                "ID": 0,
                "Name": {
                    "CH": "第四阶段",
                    "EN": "Stage 4"
                },
                "Interval": "44-1",
                "Monsters": [
                    40658,
                    40101,
                    40654,
                    40651
                ]
            }
        ]
    },
    {
        "ID": 5002,
        "Name": {
            "CH": "4.4活动第二天",
            "EN": "4.4 Event Day 2"
        },
        "Disorder": {
            "CH": "<span style='font-size:0.9em;line-height:2;'>角色下落攻击命中敌人时，将释放冲击波。<br>每个阶段试用：<color style='color:#2545BA;'>闲云 迪卢克 艾尔海森 班尼特 行秋 菲谢尔 / 神里绫华 莱欧斯利 芭芭拉 迪奥娜 / 枫原万叶 嘉明 / 魈 珐露珊</color><br>只能使用试用角色战斗。每阶段都可以用前面阶段的试用角色，但每个角色最多出场2次。<br>每阶段时间限制：600s（60原石）/ 150s（+20原石）/ 90s（+摩拉、矿）</span>",
            "EN": "<span style='font-size:0.9em;line-height:2;'>When characters' Plunge Attacks hit opponents, a shockwave will be unleashed.<br>Trial Each Stage: <color style='color:#2545BA;'>Xianyun, Diluc, Alhaitham, Bennett, Xingqiu, Fischl / Ayaka, Wriothesley, Barbara, Diona / Kazuha, Gaming / Xiao, Faruzan</color><br>You can only use trial characters. In every stage you can use trial characters from previous stages, but each character can be used at most twice.<br>Time Limit Per Stage: 600s (60 Primo) / 150s (+20 Primo) / 90s (+ Ore, Mora)</span>"
        },
        "Classes": [
            {
                "ID": 0,
                "Name": {
                    "CH": "第一阶段",
                    "EN": "Stage 1"
                },
                "Interval": "44-2",
                "Monsters": [
                    54104,
                    54103,
                    53103,
                    53101,
                    10202,
                    10207
                ]
            },
            {
                "ID": 0,
                "Name": {
                    "CH": "第二阶段",
                    "EN": "Stage 2"
                },
                "Interval": "44-2",
                "Monsters": [
                    510,
                    5020,
                    5070,
                    30103,
                    30105
                ]
            },
            {
                "ID": 0,
                "Name": {
                    "CH": "第三阶段",
                    "EN": "Stage 3"
                },
                "Interval": "44-2",
                "Monsters": [
                    60510,
                    10208,
                    61602,
                    61603
                ]
            },
            {
                "ID": 0,
                "Name": {
                    "CH": "第四阶段",
                    "EN": "Stage 4"
                },
                "Interval": "44-2",
                "Monsters": [
                    20101,
                    20102,
                    20103,
                    105,
                    108,
                    110,
                    112
                ]
            }
        ]
    },
    {
        "ID": 5003,
        "Name": {
            "CH": "4.4活动第三天",
            "EN": "4.4 Event Day 3"
        },
        "Disorder": {
            "CH": "<span style='font-size:0.9em;line-height:2;'>触发蒸发和融化反应时，角色会获得强化。<br>每个阶段试用：<color style='color:#2545BA;'>夜兰 芙宁娜 闲云 行秋 夏洛蒂 班尼特 / 神里绫人 神里绫华 香菱 莱依拉 / 宵宫 珊瑚宫心海 / 甘雨 迪希雅</color><br>只能使用试用角色战斗。每阶段都可以用前面阶段的试用角色，但每个角色最多出场2次。<br>每阶段时间限制：600s（60原石）/ 150s（+20原石）/ 90s（+摩拉、矿）</span>",
            "EN": "<span style='font-size:0.9em;line-height:2;'>Upon triggering Vaporize or Melt reactions, the characters will be buffed.<br>Trial Each Stage: <color style='color:#2545BA;'>Yelan, Furina, Xianyun, Xingqiu, Charlotte, Bennett / Ayato, Ayaka, Xiangling, Layla / Yoimiya, Kokomi / Ganyu, Dehya</color><br>You can only use trial characters. In every stage you can use trial characters from previous stages, but each character can be used at most twice.<br>Time Limit Per Stage: 600s (60 Primo) / 150s (+20 Primo) / 90s (+ Ore, Mora)</span>"
        },
        "Classes": [
            {
                "ID": 0,
                "Name": {
                    "CH": "第一阶段",
                    "EN": "Stage 1"
                },
                "Interval": "44-3",
                "Monsters": [
                    20101,
                    20302,
                    61601
                ]
            },
            {
                "ID": 0,
                "Name": {
                    "CH": "第二阶段",
                    "EN": "Stage 2"
                },
                "Interval": "44-3",
                "Monsters": [
                    30301,
                    30201,
                    50103,
                    50105,
                    50701
                ]
            },
            {
                "ID": 0,
                "Name": {
                    "CH": "第三阶段",
                    "EN": "Stage 3"
                },
                "Interval": "44-3",
                "Monsters": [
                    40302,
                    10401,
                    604
                ]
            },
            {
                "ID": 0,
                "Name": {
                    "CH": "第四阶段",
                    "EN": "Stage 4"
                },
                "Interval": "44-3",
                "Monsters": [
                    10208,
                    60401,
                    40301
                ]
            }
        ]
    },
    {
        "ID": 5004,
        "Name": {
            "CH": "4.4活动第四天",
            "EN": "4.4 Event Day 4"
        },
        "Disorder": {
            "CH": "<span style='font-size:0.9em;line-height:2;'>角色对敌人造成元素伤害时，队伍中所有角色的元素精通获得提升。<br>每个阶段试用：<color style='color:#2545BA;'>雷电将军 八重神子 珊瑚宫心海 菲谢尔 久岐忍 行秋 / 优菈 纳西妲 班尼特 罗莎莉亚 / 刻晴 闲云 / 神里绫人 枫原万叶</color><br>只能使用试用角色战斗。每阶段都可以用前面阶段的试用角色，但每个角色最多出场2次。<br>每阶段时间限制：600s（60原石）/ 150s（+20原石）/ 90s（+摩拉、矿）</span>",
            "EN": "<span style='font-size:0.9em;line-height:2;'>When characters deal Elemental DMG to opponents, all party members' Elemental Mastery is increased.<br>Trial Each Stage: <color style='color:#2545BA;'>Raiden Shogun, Yae Miko, Kokomi, Fischl, Shinobu, Xingqiu / Eula, Nahida, Bennett, Rosaria / Keqing, Xianyun / Ayato, Kazuha</color><br>You can only use trial characters. In every stage you can use trial characters from previous stages, but each character can be used at most twice.<br>Time Limit Per Stage: 600s (60 Primo) / 150s (+20 Primo) / 90s (+ Ore, Mora)</span>"
        },
        "Classes": [
            {
                "ID": 0,
                "Name": {
                    "CH": "第一阶段",
                    "EN": "Stage 1"
                },
                "Interval": "44-4",
                "Monsters": [
                    40302,
                    30602,
                    53103,
                    30102
                ]
            },
            {
                "ID": 0,
                "Name": {
                    "CH": "第二阶段",
                    "EN": "Stage 2"
                },
                "Interval": "44-4",
                "Monsters": [
                    511,
                    30501,
                    60506,
                    51001,
                    51002
                ]
            },
            {
                "ID": 0,
                "Name": {
                    "CH": "第三阶段",
                    "EN": "Stage 3"
                },
                "Interval": "44-4",
                "Monsters": [
                    30601,
                    10401,
                    10402,
                    40101,
                    40203
                ]
            },
            {
                "ID": 0,
                "Name": {
                    "CH": "第四阶段",
                    "EN": "Stage 4"
                },
                "Interval": "44-4",
                "Monsters": [
                    20302,
                    20101,
                    606,
                    607,
                    30106,
                    53101
                ]
            }
        ]
    },
    {
        "ID": 5005,
        "Name": {
            "CH": "4.4活动第五天",
            "EN": "4.4 Event Day 5"
        },
        "Disorder": {
            "CH": "<span style='font-size:0.9em;line-height:2;'>角色对触发草元素反应时，队伍中所有角色的元素精通获得提升。<br>每个阶段试用：<color style='color:#2545BA;'>芙宁娜 纳西妲 妮露 芭芭拉 柯莱 久岐忍 / 赛诺 夜兰 瑶瑶 菲谢尔 / 艾尔海森 白术 / 刻晴 闲云</color><br>只能使用试用角色战斗。每阶段都可以用前面阶段的试用角色，但每个角色最多出场2次。<br>每阶段时间限制：600s（60原石）/ 150s（+20原石）/ 90s（+摩拉、矿）</span>",
            "EN": "<span style='font-size:0.9em;line-height:2;'>When characters trigger Dendro Reactions, all party members' Elemental Mastery is increased.<br>Trial Each Stage: <color style='color:#2545BA;'>Furina, Nahida, Nilou, Barbara, Collei, Shinobu / Cyno, Yelan, Yaoyao, Fischl / Alhaitham, Baizhu / Keqing, Xianyun</color><br>You can only use trial characters. In every stage you can use trial characters from previous stages, but each character can be used at most twice.<br>Time Limit Per Stage: 600s (60 Primo) / 150s (+20 Primo) / 90s (+ Ore, Mora)</span>"
        },
        "Classes": [
            {
                "ID": 0,
                "Name": {
                    "CH": "第一阶段",
                    "EN": "Stage 1"
                },
                "Interval": "44-5",
                "Monsters": [
                    60509,
                    40658,
                    40302
                ]
            },
            {
                "ID": 0,
                "Name": {
                    "CH": "第二阶段",
                    "EN": "Stage 2"
                },
                "Interval": "44-5",
                "Monsters": [
                    40401,
                    40402,
                    40403,
                    40103,
                    10202
                ]
            },
            {
                "ID": 0,
                "Name": {
                    "CH": "第三阶段",
                    "EN": "Stage 3"
                },
                "Interval": "44-5",
                "Monsters": [
                    20502,
                    54101,
                    40612
                ]
            },
            {
                "ID": 0,
                "Name": {
                    "CH": "第四阶段",
                    "EN": "Stage 4"
                },
                "Interval": "44-5",
                "Monsters": [
                    10401,
                    54104,
                    40201,
                    40204
                ]
            }
        ]
    }
]

var _RESStateDescTextConfig = {
    "Alert": {
        "Text": {
            "CH": "入战动作期间",
            "EN": "While entering battle"
        }
    },
    "Weak": {
        "Text": {
            "CH": "虚弱",
            "EN": "Weak"
        }
    },
    "Improved": {
        "Text": {
            "CH": "强化",
            "EN": "Improved"
        }
    },
    "Hound_Activated": {
        "Text": {
            "CH": "猎犬嗜魔",
            "EN": "Activated"
        }
    },
    "Darkwraith_Fat_Rock_Angry": {
        "Text": {
            "CH": "打盾狂暴期间",
            "EN": "Fury mode after hitting shield"
        }
    },
    "Overcharge": {
        "Text": {
            "CH": "元素之环吟唱期间",
            "EN": "While charging elemental rings"
        }
    },
    "Skirmisher_Improved": {
        "Text": {
            "CH": "元素增幅期间",
            "EN": "Elemental Armor"
        }
    },
    "Nithhoggr_Ult": {
        "Text": {
            "CH": "大招蓄力期间",
            "EN": "While charging ultimate attack"
        }
    },
    "Gargoyle_Elemental": {
        "Text": {
            "CH": "吸收元素后",
            "EN": "After elemental absorption"
        }
    },
    "Gargoyle_Fafnir_Air": {
        "Text": {
            "CH": "龙兽空中状态",
            "EN": "Drake Air State"
        }
    },
    "Monolith_Invisible": {
        "Text": {
            "CH": "隐身期间",
            "EN": "During invisibility"
        }
    },
    "Eremite_Improve": {
        "Text": {
            "CH": "厄灵存在期间",
            "EN": "While Spirit of Omen exists"
        }
    },
    "Drake_Absorb": {
        "Text": {
            "CH": "吸收元素后",
            "EN": "After elemental absorption"
        }
    },
    "Drake_Absorb_1": {
        "Text": {
            "CH": "吸收第一元素后",
            "EN": "After 1st absorption"
        }
    },
    "Drake_Absorb_2": {
        "Text": {
            "CH": "吸收第二元素后",
            "EN": "After 2nd absorption"
        }
    },
    "Drake_Backfire": {
        "Text": {
            "CH": "成功反弹后5s",
            "EN": "5s after succesful counter"
        }
    },
    "Sacred_Enhance": {
        "Text": {
            "CH": "噬骸强化",
            "EN": "Consecrated Enhance"
        }
    },
    "Sandworm_Tower": {
        "Text": {
            "CH": "深渊沙虫",
            "EN": "Abyssal Wenut"
        }
    },
    "MitakeNarukami": {
        "Text": {
            "CH": "恶曜影向天魔",
            "EN": "Baleful Shadowlord"
        }
    },
    "Tartaglia_Weak": {
        "Text": {
            "CH": "一二阶段虚弱",
            "EN": "Phase 1 & 2 Weak"
        }
    },
    "Nada_Shield": {
        "Text": {
            "CH": "护盾未破除",
            "EN": "Shield not Broken"
        }
    },
    "Nada_Weak": {
        "Text": {
            "CH": "被新式虚空终端击破",
            "EN": "Weakened by Neo Akasha Terminal"
        }
    },
    "FauneAbyssale_Shield": {
        "Text": {
            "CH": "元素护甲",
            "EN": "Elemental Armor"
        }
    },
    "FauneAbyssale_Horn_Normal": {
        "Text": {
            "CH": "熔火双角增幅（大世界）",
            "EN": "Molten Horn (Overworld)"
        }
    },
    "FauneAbyssale_Horn_Abyss": {
        "Text": {
            "CH": "熔火双角增幅（深境螺旋）",
            "EN": "Molten Horn (Spiral Abyss)"
        }
    },
    "FauneAbyssale_Pearl_Normal": {
        "Text": {
            "CH": "千年珍珠增幅（大世界）",
            "EN": "Millenial Pearl (Overworld)"
        }
    },
    "FauneAbyssale_Pearl_Abyss": {
        "Text": {
            "CH": "千年珍珠增幅（深境螺旋）",
            "EN": "Millenial Pearl (Spiral Abyss)"
        }
    },
    "Coppelia_Route": {
        "Text": {
            "CH": "演出高潮 · 未削弱",
            "EN": "Climax - Before Weakened"
        }
    },
    "Adeptal_Power": {
        "Text": {
            "CH": "仙力凝聚",
            "EN": "Adeptal Power"
        }
    },
    "Gravita_Power": {
        "Text": {
            "CH": "荒性激发态",
            "EN": "Ousia Power"
        }
    },
    "Narcissusborn_Enlarge": {
        "Text": {
            "CH": "幻水狂暴",
            "EN": "Rage"
        }
    },
    "Apep_Provender": {
        "Text": {
            "CH": "提供养育",
            "EN": "Providing Nourishment"
        }
    }
}

var _MonsterDropTypes = {
    "1": {
        "Text": {
            "CH": "史莱姆凝液",
            "EN": "Slime Essence"
        }
    },
    "2": {
        "Text": {
            "CH": "飘浮灵核心",
            "EN": "Specter Core"
        }
    },
    "3": {
        "Text": {
            "CH": "骗骗花蜜",
            "EN": "Nectar"
        }
    },
    "4": {
        "Text": {
            "CH": "丘丘面具",
            "EN": "Hili Mask"
        }
    },
    "5": {
        "Text": {
            "CH": "丘丘箭矢",
            "EN": "Hili Arrow"
        }
    },
    "6": {
        "Text": {
            "CH": "丘丘萨满绘卷",
            "EN": "Samachurl Scroll"
        }
    },
    "7": {
        "Text": {
            "CH": "丘丘号角",
            "EN": "Mita/Lawa Horn"
        }
    },
    "8": {
        "Text": {
            "CH": "愚人众徽记",
            "EN": "Fatui Insignia"
        }
    },
    "9": {
        "Text": {
            "CH": "盗宝团鸦印",
            "EN": "TH Insignia"
        }
    },
    "10": {
        "Text": {
            "CH": "刀镡",
            "EN": "Handguard"
        }
    },
    "11": {
        "Text": {
            "CH": "常态 · 孢子",
            "EN": "Normal - Spore"
        }
    },
    "12": {
        "Text": {
            "CH": "激活 · 孢子",
            "EN": "Activated - Spore"
        }
    },
    "13": {
        "Text": {
            "CH": "激活 · 菌核",
            "EN": "Activated - Nucleus"
        }
    },
    "14": {
        "Text": {
            "CH": "地脉的枝叶",
            "EN": "Ley Line Sprout"
        }
    },
    "15": {
        "Text": {
            "CH": "幽暗刻像",
            "EN": "Statuette"
        }
    },
    "16": {
        "Text": {
            "CH": "兽境之爪",
            "EN": "Riftwolf Claw"
        }
    },
    "17": {
        "Text": {
            "CH": "机器人部件",
            "EN": "Ruin Parts"
        }
    },
    "18": {
        "Text": {
            "CH": "遗迹机兵部件",
            "EN": "Sentinel Parts"
        }
    },
    "19": {
        "Text": {
            "CH": "遗迹龙兽部件",
            "EN": "Drake Parts"
        }
    },
    "20": {
        "Text": {
            "CH": "债务处理人祭刀",
            "EN": "Agent Knife"
        }
    },
    "21": {
        "Text": {
            "CH": "雾虚花草",
            "EN": "Mist Grass"
        }
    },
    "22": {
        "Text": {
            "CH": "水光棱镜",
            "EN": "Hydro Prism"
        }
    },
    "23": {
        "Text": {
            "CH": "龙之骨片",
            "EN": "Bone Shard"
        }
    },
    "24": {
        "Text": {
            "CH": "镀金旅团红绸",
            "EN": "Red Brocade"
        }
    },
    "25": {
        "Text": {
            "CH": "元能棱晶",
            "EN": "Primal Prism"
        }
    },
    "26": {
        "Text": {
            "CH": "圣骸横脊",
            "EN": "Consecrated Shell"
        }
    },
    "27": {
        "Text": {
            "CH": "珍藏的野花",
            "EN": "Wanderer's Flower"
        }
    },
    "28": {
        "Text": {
            "CH": "异海结晶",
            "EN": "Transoceanics"
        }
    },
    "29": {
        "Text": {
            "CH": "发条齿轮",
            "EN": "Clockwork Gear"
        }
    },
    "30": {
        "Text": {
            "CH": "浊水",
            "EN": "Tainted Water"
        }
    },
    "31": {
        "Text": {
            "CH": "隙境之核",
            "EN": "Breacher Core"
        }
    },
    "32": {
        "Text": {
            "CH": "役人怀表",
            "EN": "Pocket Watch"
        }
    },
    "33": {
        "Text": {
            "CH": "渊光鳍翅",
            "EN": "Chasmlight Fin"
        }
    }
}

var _Monsters = {
    "101": {
        "_id": 101,
        "KingdomID": 0,
        "ClassID": 100010,
        "Name": {
            "CH": "草史莱姆",
            "EN": "Dendro Slime"
        },
        "Title": [
            {
                "CH": "圆滚滚的元素之力",
                "EN": "Rolling Elemental"
            }
        ],
        "Desc": {
            "CH": "因为充沛的草元素，而发生了生态上的质变的史莱姆。\n从生态而言，也获得了草元素的弱点。一旦遇上明火，就会剧烈地燃烧。",
            "EN": "A slime that has undergone ecological changes due to abundant Dendro in its environment.\nIt has also taken on the weaknesses of the Dendro element, and will burn intensely the moment it meets a bright flame."
        },
        "Color": "Grass",
        "HP": 0.8,
        "HPCurve": 1,
        "ATK": 0.6,
        "ATKCurve": 3,
        "Immune": [
            "Grass"
        ],
        "Element": {
            "Type": "Grass",
            "Value": 2,
            "Immutable": false
        },
        "Ball": {
            "Type": 5,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "dslm"
        ],
        "Grade": 1,
        "Weight": 60,
        "EndureTemplateID": 20,
        "Drop": [
            {
                "Type": 1,
                "Amount": [
                    0.6723,
                    0.1792,
                    0.0448,
                    0
                ]
            }
        ],
        "Csx": [
            "ECElementalLifeforms/草史莱姆"
        ]
    },
    "102": {
        "_id": 102,
        "KingdomID": 0,
        "ClassID": 100011,
        "Name": {
            "CH": "大型草史莱姆",
            "EN": "Large Dendro Slime"
        },
        "Title": [
            {
                "CH": "圆滚滚的元素之力",
                "EN": "Rolling Elemental"
            }
        ],
        "Desc": {
            "CH": "因为充沛的草元素，而发生了生态上的质变的史莱姆。\n但也有人认为，从热衷于通过欺骗性的伪装来偷袭猎物，类似骗骗花的捕猎行为来看，或许是被特殊的植物寄生的史莱姆。从这个角度来看，草史莱姆在某些文化当中，或许被认为具有特别的药用价值…？",
            "EN": "A slime that has undergone ecological changes due to abundant Dendro in its environment.\nThere are some who see it as something similar to the Whopperflowers, hiding its true form to trick and hunt its prey, and some regard it as a slime that has been parasitized by some special plant. From this point of view, might some cultures also see these slimes as having some special medicinal value?"
        },
        "Color": "Grass",
        "HP": 2.0,
        "HPCurve": 1,
        "ATK": 2.8,
        "ATKCurve": 3,
        "Immune": [
            "Grass"
        ],
        "Element": {
            "Type": "Grass",
            "Value": 8,
            "Immutable": false
        },
        "Ball": {
            "Type": 5,
            "Point": 3,
            "Drop": "113"
        },
        "Icon": [
            "ldslm"
        ],
        "Grade": 1,
        "Weight": 100,
        "EndureTemplateID": 150,
        "Drop": [
            {
                "Type": 1,
                "Amount": [
                    1.6808,
                    0.4482,
                    0.112,
                    0
                ]
            }
        ],
        "Csx": [
            "ECElementalLifeforms/大型草史莱姆"
        ]
    },
    "103": {
        "_id": 103,
        "KingdomID": 0,
        "ClassID": 100010,
        "Name": {
            "CH": "风史莱姆",
            "EN": "Anemo Slime"
        },
        "Title": [
            {
                "CH": "圆滚滚的元素之力",
                "EN": "Rolling Elemental"
            }
        ],
        "Desc": {
            "CH": "逸散在自然之中的风元素凝聚形成的小小魔物。\n因为风元素的力量，所以具有一定程度的飘浮能力。",
            "EN": "A small monster created by the coalescing of Anemo dispersed throughout nature.\nIt is able to float in the air to a certain degree due to the power of Anemo."
        },
        "Color": "Wind",
        "HP": 0.8,
        "HPCurve": 1,
        "ATK": 1.2,
        "ATKCurve": 3,
        "Immune": [
            "Wind"
        ],
        "Ball": {
            "Type": 6,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "aslm"
        ],
        "Grade": 1,
        "Weight": 60,
        "EndureTemplateID": 20,
        "Drop": [
            {
                "Type": 1,
                "Amount": [
                    0.6723,
                    0.1792,
                    0.0448,
                    0
                ]
            }
        ],
        "Csx": [
            "ECElementalLifeforms/风史莱姆"
        ]
    },
    "104": {
        "_id": 104,
        "KingdomID": 0,
        "ClassID": 100011,
        "Name": {
            "CH": "大型风史莱姆",
            "EN": "Large Anemo Slime"
        },
        "Title": [
            {
                "CH": "圆滚滚的元素之力",
                "EN": "Rolling Elemental"
            }
        ],
        "Desc": {
            "CH": "逸散在自然之中的风元素凝聚形成的魔物。\n相比风史莱姆，因为含有更浓厚的风元素，所以漂浮能力也更强。常年生活在荒野中的丘丘人似乎也会利用这种特性，制作特殊的、以大型风史莱姆为动力的载具，运输不知从何而来的物资。",
            "EN": "A monster created by the coalescing of Anemo dispersed throughout nature.\nOwing to being comprised of a greater concentration of Anemo, their flotation abilities are stronger than that of Anemo Slimes. The hilichurls living out in the wilds have made use of this special characteristic, creating special vehicles that use Large Anemo Slimes as a form of locomotion to transport goods from unknown sources."
        },
        "Color": "Wind",
        "HP": 2.0,
        "HPCurve": 1,
        "ATK": 2.8,
        "ATKCurve": 3,
        "Immune": [
            "Wind"
        ],
        "Ball": {
            "Type": 6,
            "Point": 3,
            "Drop": "113"
        },
        "Icon": [
            "laslm"
        ],
        "Grade": 1,
        "Weight": 100,
        "EndureTemplateID": 150,
        "Drop": [
            {
                "Type": 1,
                "Amount": [
                    1.6808,
                    0.4482,
                    0.112,
                    0
                ]
            }
        ],
        "Csx": [
            "ECElementalLifeforms/大型风史莱姆"
        ]
    },
    "105": {
        "_id": 105,
        "KingdomID": 0,
        "ClassID": 100010,
        "Name": {
            "CH": "雷史莱姆",
            "EN": "Electro Slime"
        },
        "Title": [
            {
                "CH": "圆滚滚的元素之力",
                "EN": "Rolling Elemental"
            }
        ],
        "Desc": {
            "CH": "逸散在自然之中的雷元素凝聚形成的小小魔物，能够放出微弱的电力。\n根据分析，雷史莱姆蹦蹦跳跳的运动方式能反映地面上的电势差。在雷元素极其丰沛的地方，可以通过观察雷史莱姆的异常运动，避开危险。",
            "EN": "A small monster created by the coalescing of Electro dispersed throughout nature.\nAccording to analyses, the jumping of Electro Slimes reflects the electric potential difference in the ground. In areas brimming over with Electro, their unusual movements can be observed and used to avoid danger."
        },
        "Color": "Elec",
        "HP": 0.8,
        "HPCurve": 1,
        "ATK": 0.6,
        "ATKCurve": 3,
        "Immune": [
            "Elec"
        ],
        "Element": {
            "Type": "Elec",
            "Value": 4,
            "Immutable": false
        },
        "Ball": {
            "Type": 3,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "eslm"
        ],
        "Grade": 1,
        "Weight": 60,
        "EndureTemplateID": 20,
        "Drop": [
            {
                "Type": 1,
                "Amount": [
                    0.6723,
                    0.1792,
                    0.0448,
                    0
                ]
            }
        ],
        "Csx": [
            "ECElementalLifeforms/雷史莱姆"
        ]
    },
    "106": {
        "_id": 106,
        "KingdomID": 0,
        "ClassID": 100011,
        "Name": {
            "CH": "大型雷史莱姆",
            "EN": "Large Electro Slime"
        },
        "Title": [
            {
                "CH": "圆滚滚的元素之力",
                "EN": "Rolling Elemental"
            }
        ],
        "Desc": {
            "CH": "逸散在自然之中的雷元素凝聚形成的魔物。\n因为体内充沛的雷元素，会间歇性向周围放电。目前，已经有人开始尝试利用这种能量去进行有益的生产活动了，新的学科或许不日也将应运而生…？",
            "EN": "A monster created by the coalescing of Electro dispersed throughout nature.\nDue to the abundant Electro within its form, it will deliver an electric shock to its surroundings from time to time. At present, some have attempted to harness this energy to aid in production activities. Perhaps some new science will be born from this...?"
        },
        "Color": "Elec",
        "HP": 2.0,
        "HPCurve": 1,
        "ATK": 4.2,
        "ATKCurve": 3,
        "Immune": [
            "Elec"
        ],
        "Element": {
            "Type": "Elec",
            "Value": 8,
            "Immutable": false
        },
        "Ball": {
            "Type": 3,
            "Point": 3,
            "Drop": "113"
        },
        "Icon": [
            "leslm"
        ],
        "Grade": 1,
        "Weight": 100,
        "EndureTemplateID": 150,
        "Drop": [
            {
                "Type": 1,
                "Amount": [
                    1.6808,
                    0.4482,
                    0.112,
                    0
                ]
            }
        ],
        "Csx": [
            "ECElementalLifeforms/大型雷史莱姆"
        ]
    },
    "107": {
        "_id": 107,
        "KingdomID": 0,
        "ClassID": 100011,
        "Name": {
            "CH": "变异雷史莱姆",
            "EN": "Mutated Electro Slime"
        },
        "Title": [
            {
                "CH": "圆滚滚的元素之力",
                "EN": "Rolling Elemental"
            }
        ],
        "Desc": {
            "CH": "逸散在自然之中的雷元素凝聚形成的魔物。\n雷史莱姆异变产生的，明黄色的史莱姆。因为体内充沛的雷元素，会间歇性向周围放电，并能向附近的雷史莱姆放出电弧；但紫色的雷史莱姆之间并不会通电。由此看来，雷史莱姆似乎具有两极。\n以此为基点，或许能成为某种伟大学科的开端…",
            "EN": "A monster created by the coalescing of Electro dispersed throughout nature.\nElectro Slimes can undergo mutations to become bright yellow. Due to the abundant Electro within its form, it will deliver an electric shock to its surroundings from time to time, and it can even cause nearby Electro Slimes to release electric arcs. This arc lightning will not bounce between purple Electro Slimes, which shows that Electro Slimes have two different polarities.\nOne wonders if some great new science could emerge from the study of such phenomena..."
        },
        "Color": "Elec",
        "HP": 2.0,
        "HPCurve": 1,
        "ATK": 4.2,
        "ATKCurve": 3,
        "Immune": [
            "Elec"
        ],
        "Element": {
            "Type": "Elec",
            "Value": 20,
            "Immutable": false
        },
        "Ball": {
            "Type": 3,
            "Point": 3,
            "Drop": "113"
        },
        "Icon": [
            "meslm"
        ],
        "Grade": 1,
        "Weight": 100,
        "EndureTemplateID": 150,
        "Drop": [
            {
                "Type": 1,
                "Amount": [
                    1.6808,
                    0.4482,
                    0.112,
                    0
                ]
            }
        ],
        "Csx": [
            "ECElementalLifeforms/变异雷史莱姆"
        ]
    },
    "108": {
        "_id": 108,
        "KingdomID": 0,
        "ClassID": 100010,
        "Name": {
            "CH": "冰史莱姆",
            "EN": "Cryo Slime"
        },
        "Title": [
            {
                "CH": "圆滚滚的元素之力",
                "EN": "Rolling Elemental"
            }
        ],
        "Desc": {
            "CH": "逸散在自然之中的冰元素凝聚形成的小小魔物。\n过去曾经有竞争对手恶意谣传说，「晨曦酒庄」会利用冰史莱姆来调控酒窖的温度，以此保证酒品的质量。",
            "EN": "A small monster created through the coagulation of Cryo in the natural environment.\nCompetitors once spread malicious rumors that the Dawn Winery used Cryo Slimes to control the temperature of their wine cellar and preserve the quality of their alcohol."
        },
        "Color": "Ice",
        "HP": 0.8,
        "HPCurve": 1,
        "ATK": 0.6,
        "ATKCurve": 3,
        "Immune": [
            "Ice"
        ],
        "Element": {
            "Type": "Frozen",
            "Value": 2,
            "Immutable": true
        },
        "Ball": {
            "Type": 4,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "cslm"
        ],
        "Grade": 1,
        "Weight": 60,
        "EndureTemplateID": 20,
        "Drop": [
            {
                "Type": 1,
                "Amount": [
                    0.6723,
                    0.1792,
                    0.0448,
                    0
                ]
            }
        ],
        "Csx": [
            "ECElementalLifeforms/冰史莱姆"
        ]
    },
    "109": {
        "_id": 109,
        "KingdomID": 0,
        "ClassID": 100011,
        "Name": {
            "CH": "大型冰史莱姆",
            "EN": "Large Cryo Slime"
        },
        "Title": [
            {
                "CH": "圆滚滚的元素之力",
                "EN": "Rolling Elemental"
            }
        ],
        "Desc": {
            "CH": "逸散在自然之中的冰元素凝聚形成的魔物。\n会利用体内凝聚的丰沛的冰元素，将大气中的水汽凝结成具有保护性的外壳；也会利用同样的原理，将周围的水面冻结。某种意义上是最自由的史莱姆，可以轻松自在地渡过汪洋大海。",
            "EN": "A monster created through the coagulation of Cryo in the natural environment.\nThese slimes can use the abundant Cryo energies within themselves to freeze the moisture in the atmosphere and form a protective shell around themselves. They can also utilize a similar principle to freeze the water around them. On some level, this can be considered to be the freest species of slime, as it can cross oceans and seas with ease."
        },
        "Color": "Ice",
        "HP": 2.0,
        "HPCurve": 1,
        "ATK": 4.2,
        "ATKCurve": 3,
        "Immune": [
            "Ice"
        ],
        "Element": {
            "Type": "Frozen",
            "Value": 2,
            "Immutable": true
        },
        "Ball": {
            "Type": 4,
            "Point": 3,
            "Drop": "113"
        },
        "Icon": [
            "lcslm"
        ],
        "Grade": 1,
        "Weight": 100,
        "EndureTemplateID": 150,
        "Drop": [
            {
                "Type": 1,
                "Amount": [
                    1.6808,
                    0.4482,
                    0.112,
                    0
                ]
            }
        ],
        "Csx": [
            "ECElementalLifeforms/大型冰史莱姆"
        ]
    },
    "110": {
        "_id": 110,
        "KingdomID": 0,
        "ClassID": 100010,
        "Name": {
            "CH": "水史莱姆",
            "EN": "Hydro Slime"
        },
        "Title": [
            {
                "CH": "圆滚滚的元素之力",
                "EN": "Rolling Elemental"
            }
        ],
        "Desc": {
            "CH": "逸散在自然之中的水元素沉积形成的小小魔物。\n传说中，有的人会将水史莱姆当作应急用水，在去往水源难寻的干旱地带、深邃的秘境之前储备好水史莱姆。但因为浓厚的水元素，直饮史莱姆其实对人体有害。",
            "EN": "A small monster created by the sedimentation of Hydro dispersed throughout nature.\nLegend has it that some people would use Hydro Slimes as an emergency water source, packing them in preparation for travels through dry regions or deep domains. But due to the high concentration of Hydro within these slimes, direct ingestion is, in fact, harmful to the human body."
        },
        "Color": "Water",
        "HP": 0.8,
        "HPCurve": 1,
        "ATK": 0.6,
        "ATKCurve": 3,
        "Immune": [
            "Water"
        ],
        "Element": {
            "Type": "Water",
            "Value": 1,
            "Immutable": true
        },
        "Ball": {
            "Type": 2,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "hslm"
        ],
        "Grade": 1,
        "Weight": 60,
        "EndureTemplateID": 20,
        "Drop": [
            {
                "Type": 1,
                "Amount": [
                    0.6723,
                    0.1792,
                    0.0448,
                    0
                ]
            }
        ],
        "Csx": [
            "ECElementalLifeforms/水史莱姆"
        ]
    },
    "111": {
        "_id": 111,
        "KingdomID": 0,
        "ClassID": 100011,
        "Name": {
            "CH": "大型水史莱姆",
            "EN": "Large Hydro Slime"
        },
        "Title": [
            {
                "CH": "圆滚滚的元素之力",
                "EN": "Rolling Elemental"
            }
        ],
        "Desc": {
            "CH": "逸散在自然之中的水元素沉积形成的魔物。\n力量更强大、体型更大的水史莱姆，可以产生水泡禁锢敌人。据说在某些力量充沛的地方，尺寸可以成长到非常夸张的程度。在奔狼领就曾有人目击过有一座小土丘那么大的水史莱姆。",
            "EN": "A monster created by the sedimentation of Hydro dispersed throughout nature.\nA larger and more powerful Hydro Slime that can create bubbles to entrap its enemies. It is said that they can grow to truly staggering sizes if given an energy-rich environment. Eyewitnesses have supposedly seen Hydro Slimes as large as a small mound in Wolvendom."
        },
        "Color": "Water",
        "HP": 2.0,
        "HPCurve": 1,
        "ATK": 2.8,
        "ATKCurve": 3,
        "Immune": [
            "Water"
        ],
        "Element": {
            "Type": "Water",
            "Value": 2,
            "Immutable": true
        },
        "Ball": {
            "Type": 2,
            "Point": 3,
            "Drop": "113"
        },
        "Icon": [
            "lhslm"
        ],
        "Grade": 1,
        "Weight": 100,
        "EndureTemplateID": 150,
        "Drop": [
            {
                "Type": 1,
                "Amount": [
                    1.6808,
                    0.4482,
                    0.112,
                    0
                ]
            }
        ],
        "Csx": [
            "ECElementalLifeforms/大型水史莱姆"
        ]
    },
    "112": {
        "_id": 112,
        "KingdomID": 0,
        "ClassID": 100010,
        "Name": {
            "CH": "火史莱姆",
            "EN": "Pyro Slime"
        },
        "Title": [
            {
                "CH": "圆滚滚的元素之力",
                "EN": "Rolling Elemental"
            }
        ],
        "Desc": {
            "CH": "逸散在自然之中的火元素沉积形成的小小魔物。\n具有非常基础的智能水平，但似乎具有很广泛的用途。对丘丘人而言。",
            "EN": "A small monster created by the sedimentation of Pyro dispersed throughout nature.\nIts intelligence is very basic, but its uses are just as broad — well, for hilichurls, that is."
        },
        "Color": "Fire",
        "HP": 0.8,
        "HPCurve": 1,
        "ATK": 0.6,
        "ATKCurve": 3,
        "Immune": [
            "Fire"
        ],
        "Element": {
            "Type": "Fire",
            "Value": 1,
            "Immutable": false
        },
        "Ball": {
            "Type": 1,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "pslm"
        ],
        "Grade": 1,
        "Weight": 60,
        "EndureTemplateID": 20,
        "Drop": [
            {
                "Type": 1,
                "Amount": [
                    0.6723,
                    0.1792,
                    0.0448,
                    0
                ]
            }
        ],
        "Csx": [
            "ECElementalLifeforms/火史莱姆"
        ]
    },
    "113": {
        "_id": 113,
        "KingdomID": 0,
        "ClassID": 100011,
        "Name": {
            "CH": "大型火史莱姆",
            "EN": "Large Pyro Slime"
        },
        "Title": [
            {
                "CH": "圆滚滚的元素之力",
                "EN": "Rolling Elemental"
            }
        ],
        "Desc": {
            "CH": "逸散在自然之中的火元素沉积形成的魔物。\n虽然智能水平并没有随着体型的成长而提高，但真的很热。在干燥的季节，可能会引发野火。愤怒的时候会喷吐炽热的火球，情绪低落的时候会变暗，是表达能力很强的史莱姆。",
            "EN": "A monster created by the sedimentation of Pyro dispersed throughout nature.\nThough its intelligence has not increased with its size, it is very hot. During dry seasons, it can cause wildfires. It will spit forth blazing fireballs when angered, and will turn dark when it is feeling down. A most expressive slime, indeed."
        },
        "Color": "Fire",
        "HP": 2.0,
        "HPCurve": 1,
        "ATK": 4.2,
        "ATKCurve": 3,
        "Immune": [
            "Fire"
        ],
        "Element": {
            "Type": "Fire",
            "Value": 8,
            "Immutable": false
        },
        "Ball": {
            "Type": 1,
            "Point": 3,
            "Drop": "113"
        },
        "Icon": [
            "lpslm"
        ],
        "Grade": 1,
        "Weight": 100,
        "EndureTemplateID": 150,
        "Drop": [
            {
                "Type": 1,
                "Amount": [
                    1.6808,
                    0.4482,
                    0.112,
                    0
                ]
            }
        ],
        "Csx": [
            "ECElementalLifeforms/大型火史莱姆"
        ]
    },
    "114": {
        "_id": 114,
        "KingdomID": 0,
        "ClassID": 100010,
        "Name": {
            "CH": "岩史莱姆",
            "EN": "Geo Slime"
        },
        "Title": [
            {
                "CH": "圆滚滚的元素之力",
                "EN": "Rolling Elemental"
            }
        ],
        "Desc": {
            "CH": "大地之中的岩元素凝聚形成的小小魔物。\n广义而言，地壳之中蕴含着丰富的岩元素。由此产生的岩史莱姆也让人有着脚踏实地的感觉。",
            "EN": "Small monsters created through the buildup of Geo within the earth.\nGenerally speaking, the crust of the earth is filled with Geo energy. Geo Slimes that are formed this way have a similarly \"down-to-earth\" sort of feeling."
        },
        "Color": "Rock",
        "HP": 0.8,
        "HPCurve": 1,
        "ATK": 0.6,
        "ATKCurve": 3,
        "Immune": [
            "Rock"
        ],
        "Ball": {
            "Type": 7,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "gslm"
        ],
        "Grade": 1,
        "Weight": 60,
        "EndureTemplateID": 20,
        "Drop": [
            {
                "Type": 1,
                "Amount": [
                    0.6723,
                    0.1792,
                    0.0448,
                    0
                ]
            }
        ],
        "Csx": [
            "ECElementalLifeforms/岩史莱姆"
        ]
    },
    "115": {
        "_id": 115,
        "KingdomID": 0,
        "ClassID": 100011,
        "Name": {
            "CH": "大型岩史莱姆",
            "EN": "Large Geo Slime"
        },
        "Title": [
            {
                "CH": "圆滚滚的元素之力",
                "EN": "Rolling Elemental"
            }
        ],
        "Desc": {
            "CH": "大地之中的岩元素凝聚形成的魔物。\n能将体内的力量凝结成岩壳，保护自己不受伤害。过去在盗宝团中一度流传着「大黄金岩史莱姆王」的说法，认为既然不少宝石与矿物中蕴含岩元素，那蕴含岩元素的史莱姆同样很可能是宝石与矿物。",
            "EN": "Monsters created through the buildup of Geo within the earth.\nThey can harden the energy within their bodies into a carapace of stone that protects them and prevents damage. The legend of a \"Golden Geo Slime King\" once circulated among the Treasure Hoarders. They believed that since gemstones and ores have Geo energy in them, Geo Slimes, which also contain Geo energy, may also be a type of gemstone or ore."
        },
        "Color": "Rock",
        "HP": 2.0,
        "HPCurve": 1,
        "ATK": 4.2,
        "ATKCurve": 3,
        "Immune": [
            "Rock"
        ],
        "Ball": {
            "Type": 7,
            "Point": 3,
            "Drop": "113"
        },
        "Icon": [
            "lgslm"
        ],
        "Grade": 1,
        "Weight": 100,
        "EndureTemplateID": 150,
        "Drop": [
            {
                "Type": 1,
                "Amount": [
                    1.6808,
                    0.4482,
                    0.112,
                    0
                ]
            }
        ],
        "Csx": [
            "ECElementalLifeforms/大型岩史莱姆"
        ]
    },
    "201": {
        "_id": 201,
        "KingdomID": 0,
        "ClassID": 10002,
        "Name": {
            "CH": "狂风之核",
            "EN": "Eye of the Storm"
        },
        "Title": [
            {
                "CH": "躁动的纯粹元素",
                "EN": "Restless Pure Elemental"
            }
        ],
        "Desc": {
            "CH": "质量极高的风元素结核后产生的形态。\n狂风之核的形成意味着地方的元素地脉阻塞，其中出现了淤积。会肆无忌惮地挥霍狂风的力量，扫荡周围的人，或许也是因为它象征着此间天地的病痛吧。",
            "EN": "The corporeal form of extremely high-quality Anemo energies.\nThe formation of an Eye of the Storm indicates that the elemental ley lines of a region are blocked, which has led to siltation. Its reckless, heedless wielding of storms that all but sweeps people off their feet may be due to its nature as a symbol of the world stricken by malaise."
        },
        "Color": "Wind",
        "HP": 5.0,
        "HPCurve": 1,
        "ATK": 5.0,
        "ATKCurve": 3,
        "Immune": [
            "Wind"
        ],
        "Ball": {
            "Type": 6,
            "Point": 3,
            "Drop": "333"
        },
        "Icon": [
            "eos"
        ],
        "Grade": 2,
        "Weight": 150,
        "EndureTemplateID": 4000,
        "Csx": [
            "ECElementalLifeforms/狂风之核"
        ]
    },
    "401": {
        "_id": 401,
        "KingdomID": 0,
        "ClassID": 10004,
        "Name": {
            "CH": "无相之雷",
            "EN": "Electro Hypostasis"
        },
        "Title": [
            {
                "CH": "「阿莱夫」",
                "EN": "Aleph"
            }
        ],
        "Desc": {
            "CH": "代号为「阿莱夫」的高级雷元素生命。\n无相元素是顶尖的元素构造体，会在元素能量喷涌的位置，或元素地脉阻塞的地点形成。\n不同的无相元素会基于其元素特性，进化出战斗的方式。",
            "EN": "Code name: Aleph. A high-purity Electro entity.\nElemental hypostases are the highest forms of elemental structures, usually formed either at a location bursting with elemental energy or at a clogged ley line.\nElemental hypostases have developed various attacks based on their elemental attributes."
        },
        "Color": "Elec",
        "HP": 7.0,
        "HPCurve": 1,
        "ATK": 6.0,
        "ATKCurve": 3,
        "Immune": [
            "Elec"
        ],
        "Element": {
            "Type": "Elec",
            "Value": 4,
            "Immutable": true
        },
        "Ball": {
            "Type": 3,
            "Point": 4,
            "Drop": "3333"
        },
        "Icon": [
            "eh"
        ],
        "Grade": 3,
        "Weight": 300,
        "EndureTemplateID": 4000,
        "Csx": [
            "ECElementalLifeforms/无相之雷"
        ]
    },
    "402": {
        "_id": 402,
        "KingdomID": 0,
        "ClassID": 10004,
        "Name": {
            "CH": "无相之风",
            "EN": "Anemo Hypostasis"
        },
        "Title": [
            {
                "CH": "「贝特」",
                "EN": "Beth"
            }
        ],
        "Desc": {
            "CH": "代号为「贝特」的高级风元素生命。\n「无相」指的是抛弃了其他元素生物的形态与生态，达到的纯粹境地。\n针对无相元素的研究，主要由须弥教令院的学者主导进行。但由于其危险性，除了代号与命名以外，基本没有取得什么值得参考的研究成果…",
            "EN": "Code name: Beth. A high-purity Anemo entity.\nElemental hypostases are life forms which have completely abandoned their former appearance and biological structure, making them able to reach the highest level of elemental purity.\nResearch into hypostases is mainly led by scholars of Sumeru Akademiya, but due to the level of danger that they pose, little of substance is known about hypostases beyond their scientific name and code name."
        },
        "Color": "Wind",
        "HP": 7.0,
        "HPCurve": 1,
        "ATK": 6.0,
        "ATKCurve": 3,
        "Immune": [
            "Wind"
        ],
        "Element": {
            "Type": "Wind",
            "Value": 1,
            "Immutable": true
        },
        "Ball": {
            "Type": 6,
            "Point": 4,
            "Drop": "3333"
        },
        "Icon": [
            "ah"
        ],
        "Grade": 3,
        "Weight": 300,
        "EndureTemplateID": 4000,
        "Csx": [
            "ECElementalLifeforms/无相之风"
        ]
    },
    "403": {
        "_id": 403,
        "KingdomID": 0,
        "ClassID": 10004,
        "Name": {
            "CH": "无相之岩",
            "EN": "Geo Hypostasis"
        },
        "Title": [
            {
                "CH": "「基末尔」",
                "EN": "Gimel"
            }
        ],
        "Desc": {
            "CH": "代号为「基末尔」的高级岩元素生命。\n无相元素本身是超浓缩的构造体，拥有极高的质量。\n浓缩成硬质固体的元素超质壳是惰性的，只有内部的核会活跃发生元素反应。",
            "EN": "Code name: Gimel. A high-purity Geo entity.\nElemental hypostases are ultra-compact structures with a high mass.\nConcentrated elemental energy forms a solid shell around the core of the hypostasis, leaving only the core reactive to elemental stimuli."
        },
        "Color": "Rock",
        "HP": 7.0,
        "HPCurve": 1,
        "ATK": 6.0,
        "ATKCurve": 4,
        "Immune": [
            "Rock"
        ],
        "Element": {
            "Type": "Rock",
            "Value": 4,
            "Immutable": true
        },
        "Ball": {
            "Type": 7,
            "Point": 4,
            "Drop": "3333"
        },
        "Icon": [
            "gh"
        ],
        "Grade": 3,
        "Weight": 300,
        "EndureTemplateID": 4000,
        "Csx": [
            "ECElementalLifeforms/无相之岩"
        ]
    },
    "404": {
        "_id": 404,
        "KingdomID": 0,
        "ClassID": 10004,
        "Name": {
            "CH": "无相之水",
            "EN": "Hydro Hypostasis"
        },
        "Title": [
            {
                "CH": "「希伊」",
                "EN": "He"
            }
        ],
        "Desc": {
            "CH": "代号为「希伊」的高级水元素生命。\n无相元素有强大的排异本能，会无情地驱逐试探它的人。\n排异习性的对象似乎甚至会延展到天然的元素上。",
            "EN": "Code Name: He. A high-purity Hydro entity.\nHypostases have strong rejection properties, and will mercilessly expel any that test them.\nIts rejection behavior even extends to natural elements."
        },
        "Color": "Water",
        "HP": 9.0,
        "HPCurve": 1,
        "ATK": 9.0,
        "ATKCurve": 3,
        "Immune": [
            "Water"
        ],
        "Element": {
            "Type": "Water",
            "Value": 4,
            "Immutable": true
        },
        "Ball": {
            "Type": 2,
            "Point": 4,
            "Drop": "3333"
        },
        "Icon": [
            "hh"
        ],
        "Grade": 3,
        "Weight": 300,
        "EndureTemplateID": 4000,
        "SkillData": [
            "0/404"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "爆炸产生水滴 + 水滴落地伤害",
                    "EN": "Explode to form slimes + Slime land"
                },
                "DMG": [
                    9,
                    3
                ]
            },
            {
                "Name": {
                    "CH": "自爆水滴自爆",
                    "EN": "Slime self-explode"
                },
                "DMG": [
                    2.8
                ]
            },
            {
                "Name": {
                    "CH": "鱼尾拍地 + 拍地产生的水波",
                    "EN": "Fish-tail slap ground + water waves created by slap"
                },
                "DMG": [
                    6,
                    6
                ]
            },
            {
                "Name": {
                    "CH": "化为鱼群跃出水面再入水",
                    "EN": "Form fish colony and leap out of floor"
                },
                "DMG": [
                    9
                ]
            },
            {
                "Name": {
                    "CH": "浪涛滚滚向中心 | 触碰浪涛 + 最后爆炸",
                    "EN": "Cubic waves roll towards the center | touch + explode"
                },
                "DMG": [
                    6,
                    9
                ]
            },
            {
                "Name": {
                    "CH": "插入地面，持续生成水泡",
                    "EN": "Dig into floor and continuously form bubbles"
                },
                "DMG": [
                    6
                ]
            },
            {
                "Name": {
                    "CH": "变成水泡爆炸 + 产生海豚出地 + 海豚入地",
                    "EN": "Form a bubble and explode + fish jump out + fish fall in"
                },
                "DMG": [
                    6,
                    1.5,
                    6
                ]
            },
            {
                "Name": {
                    "CH": "变成龙卷风，生成3个下雨领域，每0.5秒一次伤害",
                    "EN": "Form a tornado and create 3 rain areas, DMG every 0.5s"
                },
                "DMG": [
                    3
                ]
            }
        ],
        "Csx": [
            "ECElementalLifeforms/无相之水"
        ]
    },
    "405": {
        "_id": 405,
        "KingdomID": 0,
        "ClassID": 10004,
        "Name": {
            "CH": "无相之冰",
            "EN": "Cryo Hypostasis"
        },
        "Title": [
            {
                "CH": "「塔勒特」",
                "EN": "Daleth"
            }
        ],
        "Desc": {
            "CH": "代号为「塔勒特」的高级冰元素生命。\n根据研究，无相元素与常规的元素生物在物质构成上有着细微的差别。而这些细微的成分差别，或许就是它们高度稳定的形态与机械般的行动方式的由来吧。",
            "EN": "Code name: Daleth. A high-purity Cryo entity.\nResearch suggests that there are subtle differences between elemental hypostases and other elemental life forms in terms of their physical composition. Perhaps these differences can account for their high level of homeostasis and their almost mechanical movements."
        },
        "Color": "Ice",
        "HP": 7.0,
        "HPCurve": 1,
        "ATK": 6.0,
        "ATKCurve": 4,
        "Immune": [
            "Ice"
        ],
        "Element": {
            "Type": "Frozen",
            "Value": 4,
            "Immutable": true
        },
        "Ball": {
            "Type": 4,
            "Point": 4,
            "Drop": "3333"
        },
        "Icon": [
            "ch"
        ],
        "Grade": 3,
        "Weight": 300,
        "EndureTemplateID": 4000,
        "SkillData": [
            "0/405"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "车轮滚滚",
                    "EN": "Frozen wheel roll"
                },
                "DMG": [
                    3
                ]
            },
            {
                "Name": {
                    "CH": "发射向外扩散的冰刺圈",
                    "EN": "Circular barrage"
                },
                "DMG": [
                    6
                ]
            },
            {
                "Name": {
                    "CH": "冰刺圈留下的冰地板 | 每1秒伤害",
                    "EN": "Circular barrage ice floor | DMG every second"
                },
                "DMG": [
                    1.8
                ]
            },
            {
                "Name": {
                    "CH": "空中法阵：发射一堆小冰刺",
                    "EN": "Aerial totem: Fire lots of small icicles"
                },
                "DMG": [
                    3
                ]
            },
            {
                "Name": {
                    "CH": "地面法阵：冰刺地板偷袭",
                    "EN": "Ground totem: Ice spikes attack from floor"
                },
                "DMG": [
                    3
                ]
            },
            {
                "Name": {
                    "CH": "翅膀鼓掌",
                    "EN": "Wing clap"
                },
                "DMG": [
                    6
                ]
            },
            {
                "Name": {
                    "CH": "复活阶段：冰球砸",
                    "EN": "Reviving state: Ice ball drop"
                },
                "DMG": [
                    1.5
                ]
            },
            {
                "Name": {
                    "CH": "复活阶段：远程发射冰刺",
                    "EN": "Reviving state: Flying ice spikes"
                },
                "DMG": [
                    3
                ]
            },
            {
                "Name": {
                    "CH": "复活阶段：近程刺出冰刺",
                    "EN": "Reviving state: Icicles purge out from core"
                },
                "DMG": [
                    6
                ]
            }
        ],
        "Csx": [
            "ECElementalLifeforms/无相之冰"
        ]
    },
    "406": {
        "_id": 406,
        "KingdomID": 0,
        "ClassID": 10004,
        "Name": {
            "CH": "无相之火",
            "EN": "Pyro Hypostasis"
        },
        "Title": [
            {
                "CH": "「亚因」",
                "EN": "Ayin"
            }
        ],
        "Desc": {
            "CH": "代号为「亚因」的高级火元素生命。\n根据学者的研究，无相元素与当地元素脉络的异常有着密切的关联。但究竟是无相元素的存在影响了元素脉络，还是异变的元素能量创生了无相元素，就不得而知了。",
            "EN": "Code Name: Ayin. A high-purity Pyro entity.\nAccording to researchers, Elemental Hypostases are related to anomalies in local Elemental veins. But whether it is the existence of Elemental Hypostases that have affected Elemental veins or the mutated Elemental Energy that created Elemental Hypostases remains to be confirmed."
        },
        "Color": "Fire",
        "HP": 7.0,
        "HPCurve": 1,
        "ATK": 6.0,
        "ATKCurve": 3,
        "Immune": [
            "Fire"
        ],
        "Element": {
            "Type": "Fire",
            "Value": 4,
            "Immutable": true
        },
        "Ball": {
            "Type": 1,
            "Point": 4,
            "Drop": "3333"
        },
        "Icon": [
            "ph"
        ],
        "Grade": 3,
        "Weight": 300,
        "EndureTemplateID": 4000,
        "SkillData": [
            "0/406"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "火球滚滚",
                    "EN": "Rolling fireball"
                },
                "DMG": [
                    3
                ]
            },
            {
                "Name": {
                    "CH": "拳头砸地",
                    "EN": "Fist slam ground"
                },
                "DMG": [
                    6
                ]
            },
            {
                "Name": {
                    "CH": "拟态三连：牛 + 鱼 + 蝴蝶",
                    "EN": "Mimetic 3-Combo: Bull + Fish + Butterflies"
                },
                "DMG": [
                    6,
                    6,
                    6
                ]
            },
            {
                "Name": {
                    "CH": "蝴蝶持续伤害",
                    "EN": "Butterfly DoT"
                },
                "DMG": [
                    1.98
                ]
            },
            {
                "Name": {
                    "CH": "蛇头吐火球",
                    "EN": "Snake head spit small fireballs"
                },
                "DMG": [
                    6
                ]
            },
            {
                "Name": {
                    "CH": "<color style='color:#FF0000;'><b>终极爆炸</b></color>",
                    "EN": "<color style='color:#FF0000;'><b>Ultimate Explosion</b></color>"
                },
                "DMG": [
                    15
                ]
            },
            {
                "Name": {
                    "CH": "熄灭：创生火柱",
                    "EN": "Extinguished: Fire Columns"
                },
                "DMG": [
                    6
                ]
            },
            {
                "Name": {
                    "CH": "熄灭：空中法阵 发射火弹",
                    "EN": "Extinguished: Launch Fire Cubes"
                },
                "DMG": [
                    3
                ]
            },
            {
                "Name": {
                    "CH": "熄灭：翅膀鼓掌",
                    "EN": "Extinguished: Wing Clap"
                },
                "DMG": [
                    6
                ]
            }
        ],
        "Csx": [
            "ECElementalLifeforms/无相之火"
        ]
    },
    "407": {
        "_id": 407,
        "KingdomID": 0,
        "ClassID": 10004,
        "Name": {
            "CH": "无相之草",
            "EN": "Dendro Hypostasis"
        },
        "Title": [
            {
                "CH": "「扎因」",
                "EN": "Zayin"
            }
        ],
        "Desc": {
            "CH": "代号为「扎因」的高级草元素生命。\n依据学者的推想，无相元素具有「学习」的智慧，进而产生了利用「拟态」进行攻击的能力。但能够摄入信息、产生变化，就可能遭受信息的污染。在学习、变化的过程中，原本诞生的目的——尽管如今在学界中仍是未解之谜——或许也会被歪曲、变得无关紧要吧。",
            "EN": "Code Name: Zayin. A high-purity Dendro entity.\nAccording to the deductions of some researchers, Elemental Hypostases have the knowledge required to \"learn,\" which is how they use \"mimesis\" to initiate attacks. However, the ability to incorporate information to create change also opens one to the risk of information contamination. As this creature learns and changes, the purpose of its birth — no matter how academically unquantifiable it may still be — may yet become so distorted as to be rendered irrelevant."
        },
        "Color": "Grass",
        "HP": 9.0,
        "HPCurve": 1,
        "ATK": 6.0,
        "ATKCurve": 4,
        "Immune": [
            "Grass"
        ],
        "Element": {
            "Type": "Grass",
            "Value": 4,
            "Immutable": true
        },
        "Ball": {
            "Type": 5,
            "Point": 4,
            "Drop": "3333"
        },
        "Icon": [
            "dh"
        ],
        "Grade": 3,
        "Weight": 300,
        "EndureTemplateID": 4000,
        "SkillData": [
            "0/407"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "变成花 吐出2+3+4脏弹",
                    "EN": "Flower: Spit 2+3+4 dirty bombs"
                },
                "DMG": [
                    2.4
                ]
            },
            {
                "Name": {
                    "CH": "脏弹持续伤害 每0.5秒",
                    "EN": "Dirty bomb area DoT every 0.5s"
                },
                "DMG": [
                    0.3
                ]
            },
            {
                "Name": {
                    "CH": "变成锥：生成4个荆棘，出现和消失都有伤害",
                    "EN": "Cone: Summon 4 nettles | Summon and disappear DMG"
                },
                "DMG": [
                    1.2,
                    3
                ]
            },
            {
                "Name": {
                    "CH": "变成鞭：打三次",
                    "EN": "Whip: 3-Hit"
                },
                "DMG": [
                    2.4,
                    3.6,
                    4.8
                ]
            },
            {
                "Name": {
                    "CH": "变成球：砸地三次",
                    "EN": "Ball: Drop 3 times"
                },
                "DMG": [
                    4.2,
                    4.2,
                    6
                ]
            },
            {
                "Name": {
                    "CH": "变成树：一圈荆棘向中心靠拢",
                    "EN": "Tree: A ring of nettles move towards center"
                },
                "DMG": [
                    15
                ]
            },
            {
                "Name": {
                    "CH": "净化期间：草弹攻击",
                    "EN": "Cleansing: Dendro bombs"
                },
                "DMG": [
                    3.6
                ]
            }
        ],
        "Csx": [
            "ECElementalLifeforms/无相之草"
        ]
    },
    "4071": {
        "_id": 4071,
        "KingdomID": 0,
        "ClassID": 10004,
        "Name": {
            "CH": "至纯的无相之草",
            "EN": "Pure Dendro Hypostasis"
        },
        "Title": [
            {
                "CH": "「卓柯玛」",
                "EN": "Chokhmah"
            }
        ],
        "Desc": {
            "CH": "",
            "EN": ""
        },
        "Color": "Grass",
        "HP": 4.5,
        "HPCurve": 2,
        "ATK": 6.0,
        "ATKCurve": 4,
        "Immune": [
            "Grass"
        ],
        "Element": {
            "Type": "Grass",
            "Value": 4,
            "Immutable": true
        },
        "Ball": {
            "Type": 5,
            "Point": 4,
            "Drop": "3333"
        },
        "Icon": [
            "dh"
        ],
        "Grade": 3,
        "Weight": 300,
        "EndureTemplateID": 4000,
        "Csx": [
            "ECElementalLifeforms/交响诗无相之草"
        ]
    },
    "501": {
        "_id": 501,
        "Name": {
            "CH": "纯水精灵",
            "EN": "Oceanid"
        },
        "Icon": [
            "oc"
        ],
        "Csx": [
            "ECElementalLifeforms/纯水精灵"
        ]
    },
    "5020": {
        "_id": 5020,
        "KingdomID": 0,
        "ClassID": 10005,
        "Name": {
            "CH": "幻形豕兽 · 水",
            "EN": "Hydro Mimic Boar"
        },
        "Title": [
            {
                "CH": "纯水的拟态生命",
                "EN": "Hydro Mimic"
            }
        ],
        "Color": "Water",
        "HP": 1.6,
        "HPCurve": 1,
        "ATK": 2.0,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": 0.15,
            "Fire": -0.4,
            "Water": 0.15,
            "Elec": 0.15,
            "Ice": 0.15,
            "Grass": 0.15,
            "Wind": 0.15,
            "Rock": 0.15
        },
        "Immune": [
            "Water"
        ],
        "Element": {
            "Type": "Water",
            "Value": 4,
            "Immutable": true
        },
        "Ball": {
            "Type": 2,
            "Point": 1,
            "Drop": "1"
        },
        "Icon": [
            "hm"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 4000,
        "SkillDMG": [
            {
                "Name": {
                    "CH": "撅",
                    "EN": "Nudge"
                },
                "DMG": [
                    2
                ]
            },
            {
                "Name": {
                    "CH": "冲",
                    "EN": "Rush"
                },
                "DMG": [
                    2
                ]
            }
        ],
        "Csx": [
            "ECNonCategorized/幻形豕兽·水"
        ]
    },
    "5021": {
        "_id": 5021,
        "KingdomID": 0,
        "ClassID": 10005,
        "Name": {
            "CH": "幻形豕兽 · 水 (强化)",
            "EN": "Hydro Mimic Boar (Enhanced)"
        },
        "Title": [
            {
                "CH": "纯水的拟态生命",
                "EN": "Hydro Mimic"
            }
        ],
        "Color": "Water",
        "HP": 3.4,
        "HPCurve": 1,
        "ATK": 3.0,
        "ATKCurve": 3,
        "Immune": [
            "Water"
        ],
        "Element": {
            "Type": "Water",
            "Value": 4,
            "Immutable": true
        },
        "Ball": {
            "Type": 2,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "hm"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 4000,
        "SkillDMG": [
            {
                "Name": {
                    "CH": "泼水",
                    "EN": "Splash water"
                },
                "DMG": [
                    3
                ]
            },
            {
                "Name": {
                    "CH": "撅",
                    "EN": "Nudge"
                },
                "DMG": [
                    3
                ]
            },
            {
                "Name": {
                    "CH": "冲",
                    "EN": "Rush"
                },
                "DMG": [
                    3
                ]
            }
        ],
        "Csx": [
            "ECNonCategorized/幻形豕兽·水"
        ]
    },
    "503": {
        "_id": 503,
        "KingdomID": 0,
        "ClassID": 10005,
        "Name": {
            "CH": "幻形鹤 · 水",
            "EN": "Hydro Mimic Crane"
        },
        "Title": [
            {
                "CH": "纯水的拟态生命",
                "EN": "Hydro Mimic"
            }
        ],
        "Color": "Water",
        "HP": 0.8,
        "HPCurve": 1,
        "ATK": 5.0,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": 0.15,
            "Fire": 0.15,
            "Water": 0.15,
            "Elec": -0.4,
            "Ice": 0.15,
            "Grass": 0.15,
            "Wind": 0.15,
            "Rock": 0.15
        },
        "Immune": [
            "Water"
        ],
        "Element": {
            "Type": "Water",
            "Value": 4,
            "Immutable": true
        },
        "Ball": {
            "Type": 2,
            "Point": 1,
            "Drop": "1"
        },
        "Icon": [
            "hm"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 4000,
        "SkillDMG": [
            {
                "Name": {
                    "CH": "水波",
                    "EN": "Water wave"
                },
                "DMG": [
                    5
                ]
            }
        ],
        "Csx": [
            "ECNonCategorized/幻形鹤·水"
        ]
    },
    "5040": {
        "_id": 5040,
        "KingdomID": 0,
        "ClassID": 10005,
        "Name": {
            "CH": "幻形蟹 · 水",
            "EN": "Hydro Mimic Crab"
        },
        "Title": [
            {
                "CH": "纯水的拟态生命",
                "EN": "Hydro Mimic"
            }
        ],
        "Color": "Water",
        "HP": 1.6,
        "HPCurve": 1,
        "ATK": 4.0,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.15,
            "Fire": 0.15,
            "Water": 0.15,
            "Elec": 0.15,
            "Ice": -0.4,
            "Grass": 0.15,
            "Wind": 0.15,
            "Rock": 0.15
        },
        "Immune": [
            "Water"
        ],
        "Element": {
            "Type": "Water",
            "Value": 4,
            "Immutable": true
        },
        "Ball": {
            "Type": 2,
            "Point": 1,
            "Drop": "1"
        },
        "Icon": [
            "hm"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 4000,
        "SkillDMG": [
            {
                "Name": {
                    "CH": "喷水 | 每0.2秒伤害",
                    "EN": "Spit water DMG every 0.2s"
                },
                "DMG": [
                    0.4
                ]
            }
        ],
        "Csx": [
            "ECNonCategorized/幻形蟹·水"
        ]
    },
    "5041": {
        "_id": 5041,
        "KingdomID": 0,
        "ClassID": 10005,
        "Name": {
            "CH": "幻形蟹 · 水 (强化)",
            "EN": "Hydro Mimic Crab (Enhanced)"
        },
        "Title": [
            {
                "CH": "纯水的拟态生命",
                "EN": "Hydro Mimic"
            }
        ],
        "Color": "Water",
        "HP": 0.6,
        "HPCurve": 1,
        "ATK": 2.0,
        "ATKCurve": 3,
        "Immune": [
            "Water"
        ],
        "Element": {
            "Type": "Water",
            "Value": 4,
            "Immutable": true
        },
        "Ball": {
            "Type": 2,
            "Point": 1,
            "Drop": "1"
        },
        "Icon": [
            "hm"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 4000,
        "SkillDMG": [
            {
                "Name": {
                    "CH": "喷水 | 每0.2秒伤害",
                    "EN": "Spit water DMG every 0.2s"
                },
                "DMG": [
                    0.2
                ]
            }
        ],
        "Csx": [
            "ECNonCategorized/幻形蟹·水"
        ]
    },
    "505": {
        "_id": 505,
        "KingdomID": 0,
        "ClassID": 10005,
        "Name": {
            "CH": "幻形雀 · 水",
            "EN": "Hydro Mimic Finch"
        },
        "Title": [
            {
                "CH": "纯水的拟态生命",
                "EN": "Hydro Mimic"
            }
        ],
        "Color": "Water",
        "HP": 3.0,
        "HPCurve": 1,
        "ATK": 5.0,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": 0.15,
            "Fire": 0.15,
            "Water": 0.15,
            "Elec": 0.15,
            "Ice": 0.15,
            "Grass": 0.15,
            "Wind": 0.15,
            "Rock": -0.4
        },
        "Immune": [
            "Water"
        ],
        "Element": {
            "Type": "Water",
            "Value": 4,
            "Immutable": true
        },
        "Ball": {
            "Type": 2,
            "Point": 3,
            "Drop": "113"
        },
        "Icon": [
            "hm"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 4000,
        "SkillDMG": [
            {
                "Name": {
                    "CH": "小跳",
                    "EN": "Small Jump"
                },
                "DMG": [
                    5
                ]
            },
            {
                "Name": {
                    "CH": "大跳",
                    "EN": "Big Jump"
                },
                "DMG": [
                    5
                ]
            },
            {
                "Name": {
                    "CH": "死亡自爆",
                    "EN": "Death Explode"
                },
                "DMG": [
                    5
                ]
            }
        ],
        "Csx": [
            "ECNonCategorized/幻形雀·水"
        ]
    },
    "506": {
        "_id": 506,
        "KingdomID": 0,
        "ClassID": 10005,
        "Name": {
            "CH": "幻形游禽 · 水",
            "EN": "Hydro Mimic Mallard"
        },
        "Title": [
            {
                "CH": "纯水的拟态生命",
                "EN": "Hydro Mimic"
            }
        ],
        "Color": "Water",
        "HP": 1.6,
        "HPCurve": 1,
        "ATK": 2.0,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": 0.15,
            "Fire": 0.15,
            "Water": 0.15,
            "Elec": 0.15,
            "Ice": -0.4,
            "Grass": 0.15,
            "Wind": 0.15,
            "Rock": 0.15
        },
        "Immune": [
            "Water"
        ],
        "Element": {
            "Type": "Water",
            "Value": 4,
            "Immutable": true
        },
        "Ball": {
            "Type": 2,
            "Point": 1,
            "Drop": "1"
        },
        "Icon": [
            "hm"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 4000,
        "SkillDMG": [
            {
                "Name": {
                    "CH": "周围水域 | 每0.75秒伤害",
                    "EN": "Nearby water area | DMG every 0.75s"
                },
                "DMG": [
                    1
                ]
            },
            {
                "Name": {
                    "CH": "撞人",
                    "EN": "Rush"
                },
                "DMG": [
                    2
                ]
            }
        ],
        "Csx": [
            "ECNonCategorized/幻形游禽·水"
        ]
    },
    "5070": {
        "_id": 5070,
        "KingdomID": 0,
        "ClassID": 10005,
        "Name": {
            "CH": "幻形花鼠 · 水",
            "EN": "Hydro Mimic Squirrel"
        },
        "Title": [
            {
                "CH": "纯水的拟态生命",
                "EN": "Hydro Mimic"
            }
        ],
        "Color": "Water",
        "HP": 1.4,
        "HPCurve": 1,
        "ATK": 3.0,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": 0.15,
            "Fire": -0.4,
            "Water": 0.15,
            "Elec": 0.15,
            "Ice": 0.15,
            "Grass": 0.15,
            "Wind": 0.15,
            "Rock": 0.15
        },
        "Immune": [
            "Water"
        ],
        "Element": {
            "Type": "Water",
            "Value": 4,
            "Immutable": true
        },
        "Ball": {
            "Type": 2,
            "Point": 1,
            "Drop": "1"
        },
        "Icon": [
            "hm"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 4000,
        "SkillDMG": [
            {
                "Name": {
                    "CH": "尾巴砸",
                    "EN": "Tail smash"
                },
                "DMG": [
                    3
                ]
            },
            {
                "Name": {
                    "CH": "尾巴扫",
                    "EN": "Tail whip"
                },
                "DMG": [
                    3
                ]
            }
        ],
        "Csx": [
            "ECNonCategorized/幻形花鼠·水"
        ]
    },
    "5071": {
        "_id": 5071,
        "KingdomID": 0,
        "ClassID": 10005,
        "Name": {
            "CH": "幻形花鼠 · 水 (强化)",
            "EN": "Hydro Mimic Squirrel (Enhanced)"
        },
        "Title": [
            {
                "CH": "纯水的拟态生命",
                "EN": "Hydro Mimic"
            }
        ],
        "Color": "Water",
        "HP": 3.4,
        "HPCurve": 1,
        "ATK": 3.0,
        "ATKCurve": 3,
        "Immune": [
            "Water"
        ],
        "Element": {
            "Type": "Water",
            "Value": 4,
            "Immutable": true
        },
        "Ball": {
            "Type": 2,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "hm"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 4000,
        "SkillDMG": [
            {
                "Name": {
                    "CH": "尾巴砸 + 水喷",
                    "EN": "Tail smash + Water jet"
                },
                "DMG": [
                    3,
                    2.4
                ]
            },
            {
                "Name": {
                    "CH": "尾巴扫 + 水波",
                    "EN": "Tail whip + Water wave"
                },
                "DMG": [
                    3,
                    1.5
                ]
            }
        ],
        "Csx": [
            "ECNonCategorized/幻形花鼠·水"
        ]
    },
    "508": {
        "_id": 508,
        "KingdomID": 0,
        "ClassID": 10005,
        "Name": {
            "CH": "幻形蛙 · 水",
            "EN": "Hydro Mimic Frog"
        },
        "Title": [
            {
                "CH": "纯水的拟态生命",
                "EN": "Hydro Mimic"
            }
        ],
        "Color": "Water",
        "HP": 3.0,
        "HPCurve": 1,
        "ATK": 5.0,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": 0.15,
            "Fire": 0.15,
            "Water": 0.15,
            "Elec": 0.15,
            "Ice": 0.15,
            "Grass": 0.15,
            "Wind": 0.15,
            "Rock": -0.4
        },
        "Immune": [
            "Water"
        ],
        "Element": {
            "Type": "Water",
            "Value": 4,
            "Immutable": true
        },
        "Ball": {
            "Type": 2,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "hm"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 4000,
        "SkillDMG": [
            {
                "Name": {
                    "CH": "水波",
                    "EN": "Water Wave"
                },
                "DMG": [
                    5
                ]
            },
            {
                "Name": {
                    "CH": "大跳",
                    "EN": "Big Jump"
                },
                "DMG": [
                    5
                ]
            },
            {
                "Name": {
                    "CH": "死亡自爆",
                    "EN": "Death Explode"
                },
                "DMG": [
                    5
                ]
            }
        ],
        "Csx": [
            "ECNonCategorized/幻形蛙·水"
        ]
    },
    "509": {
        "_id": 509,
        "KingdomID": 0,
        "ClassID": 10005,
        "Name": {
            "CH": "幻形飞鸢 · 水",
            "EN": "Hydro Mimic Raptor"
        },
        "Title": [
            {
                "CH": "纯水的拟态生命",
                "EN": "Hydro Mimic"
            }
        ],
        "Color": "Water",
        "HP": 0.8,
        "HPCurve": 1,
        "ATK": 4.0,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": 0.15,
            "Fire": 0.15,
            "Water": 0.15,
            "Elec": -0.4,
            "Ice": 0.15,
            "Grass": 0.15,
            "Wind": 0.15,
            "Rock": 0.15
        },
        "Immune": [
            "Water"
        ],
        "Element": {
            "Type": "Water",
            "Value": 4,
            "Immutable": true
        },
        "Ball": {
            "Type": 2,
            "Point": 1,
            "Drop": "1"
        },
        "Icon": [
            "hm"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 4000,
        "SkillDMG": [
            {
                "Name": {
                    "CH": "下扑",
                    "EN": "Rush"
                },
                "DMG": [
                    4
                ]
            }
        ],
        "Csx": [
            "ECNonCategorized/幻形飞鸢·水"
        ]
    },
    "510": {
        "_id": 510,
        "KingdomID": 0,
        "ClassID": 10008,
        "Name": {
            "CH": "浊水粉碎幻灵",
            "EN": "Tainted Water-Splitting Phantasm"
        },
        "Title": [
            {
                "CH": "无源之水",
                "EN": "Water Without Source"
            }
        ],
        "Desc": {
            "CH": "在纯水精灵如今在枫丹已经绝迹的当下，水中出现的奇异元素生命体。\n在灾难后才出现的这种元素生灵不具备往昔纯水精灵的优美，因此被称为「浊水」。尽管溶解在水中的毒已经被稀释、净化，但除非何处仍有纯粹之水的容器，枫丹的水中再也不会自然地诞生纯水精灵了。",
            "EN": "Today, with the Oceanids extirpated from Fontaine, strange elemental life forms have appeared in the waters.\nThe elemental spirits that appeared after the calamity lack the elegance and beauty of the Oceanids, and thus have been called \"tainted.\" Though the toxins dissolved in the water have long since been diluted and cleansed, unless there are still containers of pure water somewhere, Fontaine's waters will never again naturally birth Oceanids."
        },
        "Color": "Water",
        "HP": 2.5,
        "HPCurve": 2,
        "ATK": 4.0,
        "ATKCurve": 4,
        "Immune": [
            "Water"
        ],
        "Element": {
            "Type": "Water",
            "Value": 4,
            "Immutable": true
        },
        "Ball": {
            "Type": 2,
            "Point": 3,
            "Drop": "112"
        },
        "Icon": [
            "UI_MonsterIcon_Necalevia_Normal"
        ],
        "Grade": 2,
        "Weight": 101,
        "EndureTemplateID": 400,
        "EndureExtra": 100,
        "Tutorial": {
            "CH": "- 浊水幻灵常态经过15s后会进入狂暴，狂暴持续20~30s，结束后返回常态，以此循环。<br>- 狂暴期间无法冻结。<br>- 常态下冻结超过8s，则强制解除冻结，并立刻狂暴。",
            "EN": "- Water Phantasms will enter Rage after spending 15s in normal mode. Rage lasts 20~30s.<br>- When Rage ends, returns back to normal and starts the cycle again.<br>- Cannot be Frozen in Rage mode.<br>- If Frozen for more than 8s in normal mode, removes Frozen and enters Rage immediately."
        },
        "Drop": [
            {
                "Type": 30,
                "Amount": [
                    0,
                    5.2526,
                    1.4006,
                    0.3502
                ]
            }
        ],
        "Csx": [
            "ECElementalLifeforms/浊水粉碎幻灵"
        ]
    },
    "5101": {
        "_id": 5101,
        "KingdomID": 10,
        "ClassID": 11040,
        "Name": {
            "CH": "「湖畔的尼尼安涅」",
            "EN": "Ninianne of the Lake"
        },
        "Title": [
            {
                "CH": "地方传奇 · 浊水粉碎幻灵",
                "EN": "Local Legend: Tainted Water-Splitting Phantasm"
            }
        ],
        "Desc": {
            "CH": "",
            "EN": ""
        },
        "Color": "Water",
        "HP": 36.5,
        "HPCurve": 2,
        "ATK": 8.0,
        "ATKCurve": 4,
        "Immune": [
            "Water"
        ],
        "Element": {
            "Type": "Water",
            "Value": 4,
            "Immutable": true
        },
        "Ball": {
            "Type": 2,
            "Point": 4,
            "Drop": "1114"
        },
        "Icon": [
            "UI_MonsterIcon_Necalevia_Normal"
        ],
        "Grade": 3,
        "Weight": 101,
        "EndureTemplateID": 400,
        "EndureExtra": 100,
        "Tutorial": {
            "CH": "- 地方传奇幻灵：永久狂暴，永久免疫冻结。",
            "EN": "- Local Legend Phantasm: Always in Rage Mode, always immune to Frozen."
        },
        "Csx": [
            "ECLegends/地方传奇·浊水粉碎幻灵"
        ]
    },
    "5102": {
        "_id": 5102,
        "KingdomID": 10,
        "ClassID": 11006,
        "Name": {
            "CH": "历经百战的浊水粉碎幻灵",
            "EN": "Veteran Tainted Water-Splitting Phantasm"
        },
        "Title": [
            {
                "CH": "地方传奇 · 浊水粉碎幻灵",
                "EN": "Local Legend: Tainted Water-Splitting Phantasm"
            }
        ],
        "Desc": {
            "CH": "",
            "EN": ""
        },
        "Color": "Water",
        "HP": 7.3,
        "HPCurve": 2,
        "ATK": 4.0,
        "ATKCurve": 4,
        "Immune": [
            "Water"
        ],
        "Element": {
            "Type": "Water",
            "Value": 4,
            "Immutable": true
        },
        "Ball": {
            "Type": 2,
            "Point": 4,
            "Drop": "1114"
        },
        "Icon": [
            "UI_MonsterIcon_Necalevia_Normal"
        ],
        "Grade": 3,
        "Weight": 101,
        "EndureTemplateID": 400,
        "EndureExtra": 100,
        "Tutorial": {
            "CH": "- 地方传奇幻灵：永久狂暴，永久免疫冻结。",
            "EN": "- Local Legend Phantasm: Always in Rage Mode, always immune to Frozen."
        },
        "Csx": [
            "ECLegends/地方传奇·浊水粉碎幻灵"
        ]
    },
    "511": {
        "_id": 511,
        "KingdomID": 0,
        "ClassID": 10008,
        "Name": {
            "CH": "浊水喷吐幻灵",
            "EN": "Tainted Water-Spouting Phantasm"
        },
        "Title": [
            {
                "CH": "无源之水",
                "EN": "Water Without Source"
            }
        ],
        "Desc": {
            "CH": "在纯水精灵如今在枫丹已经绝迹的当下，水中出现的奇异元素生命体。\n据说在厄歌莉娅的时代，纯水精灵常常与人同行，也曾经散布各地的清澈泉水中，借此将世界的水脉相互联系。但她们大多都随她们的神一同死去了，余下的也不再有她们的神。",
            "EN": "Today, with the Oceanids extirpated from Fontaine, strange elemental life forms have appeared in the waters.\nIt is said that in Egeria's era, the Oceanids often lived alongside humans, and lived in the springs of clear waters dotted across the lands, and through this, they connected the waters of the world. But most died with their god, and for the survivors, the world no longer has any god they recognize."
        },
        "Color": "Water",
        "HP": 2,
        "HPCurve": 2,
        "ATK": 5.0,
        "ATKCurve": 4,
        "Immune": [
            "Water"
        ],
        "Element": {
            "Type": "Water",
            "Value": 4,
            "Immutable": true
        },
        "Ball": {
            "Type": 2,
            "Point": 3,
            "Drop": "112"
        },
        "Icon": [
            "UI_MonsterIcon_Necalevia_Big"
        ],
        "Grade": 2,
        "Weight": 101,
        "EndureTemplateID": 400,
        "EndureExtra": 100,
        "Tutorial": {
            "CH": "- 浊水幻灵常态经过15s后会进入狂暴，狂暴持续20~30s，结束后返回常态，以此循环。<br>- 狂暴期间无法冻结。<br>- 常态下冻结超过8s，则强制解除冻结，并立刻狂暴。",
            "EN": "- Water Phantasms will enter Rage after spending 15s in normal mode. Rage lasts 20~30s.<br>- When Rage ends, returns back to normal and starts the cycle again.<br>- Cannot be Frozen in Rage mode.<br>- If Frozen for more than 8s in normal mode, removes Frozen and enters Rage immediately."
        },
        "Drop": [
            {
                "Type": 30,
                "Amount": [
                    0,
                    5.2526,
                    1.4006,
                    0.3502
                ]
            }
        ],
        "Csx": [
            "ECElementalLifeforms/浊水喷吐幻灵"
        ]
    },
    "5111": {
        "_id": 5111,
        "KingdomID": 10,
        "ClassID": 11040,
        "Name": {
            "CH": "「湖畔的维维安涅」",
            "EN": "Vivianne of the Lake"
        },
        "Title": [
            {
                "CH": "地方传奇 · 浊水喷吐幻灵",
                "EN": "Local Legend: Tainted Water-Spouting Phantasm"
            }
        ],
        "Desc": {
            "CH": "",
            "EN": ""
        },
        "Color": "Water",
        "HP": 32.6,
        "HPCurve": 2,
        "ATK": 9.6,
        "ATKCurve": 4,
        "Immune": [
            "Water"
        ],
        "Element": {
            "Type": "Water",
            "Value": 4,
            "Immutable": true
        },
        "Ball": {
            "Type": 2,
            "Point": 4,
            "Drop": "1114"
        },
        "Icon": [
            "UI_MonsterIcon_Necalevia_Big"
        ],
        "Grade": 3,
        "Weight": 101,
        "EndureTemplateID": 400,
        "EndureExtra": 100,
        "Tutorial": {
            "CH": "- 地方传奇幻灵：永久狂暴，永久免疫冻结。",
            "EN": "- Local Legend Phantasm: Always in Rage Mode, always immune to Frozen."
        },
        "Csx": [
            "ECLegends/地方传奇·浊水喷吐幻灵"
        ]
    },
    "5112": {
        "_id": 5112,
        "KingdomID": 10,
        "ClassID": 11006,
        "Name": {
            "CH": "历经百战的浊水喷吐幻灵",
            "EN": "Veteran Tainted Water-Spouting Phantasm"
        },
        "Title": [
            {
                "CH": "地方传奇 · 浊水喷吐幻灵",
                "EN": "Local Legend: Tainted Water-Spouting Phantasm"
            }
        ],
        "Desc": {
            "CH": "",
            "EN": ""
        },
        "Color": "Water",
        "HP": 6.52,
        "HPCurve": 2,
        "ATK": 4.8,
        "ATKCurve": 4,
        "Immune": [
            "Water"
        ],
        "Element": {
            "Type": "Water",
            "Value": 4,
            "Immutable": true
        },
        "Ball": {
            "Type": 2,
            "Point": 4,
            "Drop": "1114"
        },
        "Icon": [
            "UI_MonsterIcon_Necalevia_Big"
        ],
        "Grade": 3,
        "Weight": 101,
        "EndureTemplateID": 400,
        "EndureExtra": 100,
        "Tutorial": {
            "CH": "- 地方传奇幻灵：永久狂暴，永久免疫冻结。",
            "EN": "- Local Legend Phantasm: Always in Rage Mode, always immune to Frozen."
        },
        "Csx": [
            "ECLegends/地方传奇·浊水喷吐幻灵"
        ]
    },
    "512": {
        "_id": 512,
        "KingdomID": 0,
        "ClassID": 10008,
        "Name": {
            "CH": "水形幻人",
            "EN": "Hydro Tulpa"
        },
        "Title": [
            {
                "CH": "苏醒的生命",
                "EN": "Awakened Egregore"
            }
        ],
        "Desc": {
            "CH": "由水凝聚成的初具人形的魔物。\n曾有无数的意志消融在水中，但水中原本就与宇宙一样充满了渴求被诞下的灵魂。被称为水形幻人的新生儿究竟是前者还是后者是难以分辨的；但两者之间的区别并没有人们所想象的那么大：太阳不会区分、时光不会区分、翻涌的潮水不会区分，渗透一切的黑暗也不会区分。",
            "EN": "A humanoid monster formed from the agglomeration of water.\nCountless wills dissolved in the water, which, like the universe, was full of souls that wished to be born. Are the newborn Hydro Tulpas from the former or the latter? They're not as different as we'd like to think, anyway. The sun can't distinguish them, and neither can time, the surging tides, or even the darkness that permeates everything."
        },
        "Color": "Water",
        "HP": 11,
        "HPCurve": 2,
        "ATK": 10.0,
        "ATKCurve": 4,
        "Immune": [
            "Water"
        ],
        "Element": {
            "Type": "Water",
            "Value": 4,
            "Immutable": true
        },
        "RESState": [
            {
                "State": "Narcissusborn_Enlarge",
                "All": 0.45
            }
        ],
        "Tutorial": {
            "CH": "- 每15秒，召唤一名「半幻人」。半幻人可被元素反应消灭。<br>- 在战斗中，会试图吸收半幻人以进入狂暴状态：<b>所有抗性提升45%，造成的伤害大幅提升，持续1个招式</b>。<br>- 深渊特化：<b>第一次吸收推迟，第一次锋刃之雨提前。狂暴状态下释放2招，无缝衔接</b>。",
            "EN": "- Summons a Half-Tulpa every 15s. Half-Tulpas can be destroyed by Elemental Reactions.<br>- During combat, will attempt to absorb Half-Tulpas to enter Rage State: <b>All RES increased by 45%, and greatly boosts DMG dealt. Rage lasts for one skill.</b><br>- Abyss Version: <b>First Absorb will be used later, while first Rain of Blades will be used earlier. In Rage state, will use 2 skills consecutively, with 0 interval.</b>"
        },
        "Ball": {
            "Type": 2,
            "Point": 3,
            "Drop": "332"
        },
        "Icon": [
            "UI_MonsterIcon_Narcissusborn_Normal_01"
        ],
        "Grade": 3,
        "Weight": 20000,
        "EndureTemplateID": 4000,
        "SkillDMG": [
            {
                "Name": {
                    "CH": "二段剑击",
                    "EN": "Sword Attack x2"
                },
                "DMG": [
                    6,
                    6.5
                ]
            },
            {
                "Name": {
                    "CH": "三段锤击",
                    "EN": "Hammer Attack x3"
                },
                "DMG": [
                    6,
                    6.25,
                    6.5
                ]
            },
            {
                "Name": {
                    "CH": "扔矛",
                    "EN": "Throw Spear"
                },
                "DMG": [
                    6
                ]
            },
            {
                "Name": {
                    "CH": "<b>天河泻</b>",
                    "EN": "<b>Heavenly Waters</b>"
                },
                "DMG": [
                    10
                ]
            },
            {
                "Name": {
                    "CH": "狂暴二段剑击",
                    "EN": "Rage Sword Attack x2"
                },
                "DMG": [
                    9,
                    9.5
                ]
            },
            {
                "Name": {
                    "CH": "狂暴三段锤击",
                    "EN": "Rage Hammer Attack x3"
                },
                "DMG": [
                    9,
                    9.25,
                    9.5
                ]
            },
            {
                "Name": {
                    "CH": "狂暴扔矛",
                    "EN": "Rage Throw Spear"
                },
                "DMG": [
                    9
                ]
            }
        ],
        "Csx": [
            "ECElementalLifeforms/水形幻人"
        ]
    },
    "513": {
        "_id": 513,
        "KingdomID": 0,
        "ClassID": 10008,
        "Name": {
            "CH": "半幻人",
            "EN": "Half-Tulpa"
        },
        "Title": [
            {
                "CH": "苏醒的生命",
                "EN": "Awakened Egregore"
            }
        ],
        "Desc": {
            "CH": "由水凝聚成的初具人形的魔物。\n曾有无数的意志消融在水中，但水中原本就与宇宙一样充满了渴求被诞下的灵魂。被称为水形幻人的新生儿究竟是前者还是后者是难以分辨的；但两者之间的区别并没有人们所想象的那么大：太阳不会区分、时光不会区分、翻涌的潮水不会区分，渗透一切的黑暗也不会区分。",
            "EN": "A humanoid monster formed from the agglomeration of water.\nCountless wills dissolved in the water, which, like the universe, was full of souls that wished to be born. Are the newborn Hydro Tulpas from the former or the latter? They're not as different as we'd like to think, anyway. The sun can't distinguish them, and neither can time, the surging tides, or even the darkness that permeates everything."
        },
        "Color": "Water",
        "HP": 1.2,
        "HPCurve": 2,
        "ATK": 2.0,
        "ATKCurve": 4,
        "Immune": [
            "Water"
        ],
        "Element": {
            "Type": "Water",
            "Value": 1,
            "Immutable": false
        },
        "Icon": [
            "keq"
        ],
        "Grade": 3,
        "Weight": 20000,
        "EndureTemplateID": 4000,
        "Csx": [
            "ECNonCategorized/半幻人"
        ]
    },
    "516": {
        "_id": 516,
        "KingdomID": 0,
        "ClassID": 10008,
        "Name": {
            "CH": "纳齐森科鲁兹",
            "EN": "Narzissenkreuz"
        },
        "Title": [
            {
                "CH": "苏醒的生命",
                "EN": "Awakened Egregore"
            }
        ],
        "Desc": {
            "CH": "由水凝聚成的初具人形的魔物。\n曾有无数的意志消融在水中，但水中原本就与宇宙一样充满了渴求被诞下的灵魂。被称为水形幻人的新生儿究竟是前者还是后者是难以分辨的；但两者之间的区别并没有人们所想象的那么大：太阳不会区分、时光不会区分、翻涌的潮水不会区分，渗透一切的黑暗也不会区分。",
            "EN": "A humanoid monster formed from the agglomeration of water.\nCountless wills dissolved in the water, which, like the universe, was full of souls that wished to be born. Are the newborn Hydro Tulpas from the former or the latter? They're not as different as we'd like to think, anyway. The sun can't distinguish them, and neither can time, the surging tides, or even the darkness that permeates everything."
        },
        "Color": "Water",
        "HP": 10.45,
        "HPCurve": 2,
        "ATK": 5.0,
        "ATKCurve": 4,
        "Immune": [
            "Water"
        ],
        "Element": {
            "Type": "Water",
            "Value": 4,
            "Immutable": true
        },
        "Ball": {
            "Type": 2,
            "Point": 3,
            "Drop": "332"
        },
        "Icon": [
            "UI_MonsterIcon_Narcissusborn_Normal_01"
        ],
        "Grade": 3,
        "Weight": 20000,
        "EndureTemplateID": 4000,
        "Csx": [
            "ECElementalLifeforms/水形幻人"
        ]
    },
    "601": {
        "_id": 601,
        "KingdomID": 0,
        "ClassID": 10006,
        "Name": {
            "CH": "水飘浮灵",
            "EN": "Hydro Specter"
        },
        "Title": [
            {
                "CH": "飘浮的元素之灵",
                "EN": "Wandering Elemental Spirit"
            }
        ],
        "Desc": {
            "CH": "高浓度的水元素孕育而生的魔物，能够飘浮在空中。\n在月夜里，飘荡在河边的水飘浮灵散发着幽光。鬼鬼祟祟的它们常被路过的旅人当作游魂，也因此成为了不少乡间民话的常客。",
            "EN": "A monster birthed from a high concentration of Hydro that can float in the air.\nOn moonlit nights, Hydro Specters will give off a ghostly glow as they float by the riverbank. Their furtive forms are often mistaken by passers-by for wandering spirits, which has made them mainstays in quite a few folktales."
        },
        "Color": "Water",
        "HP": 0.8,
        "HPCurve": 2,
        "ATK": 1.75,
        "ATKCurve": 4,
        "Immune": [
            "Water"
        ],
        "Tutorial": {
            "CH": "- 受到的单次伤害介于 18.75% 和 31.25% 之间：+20% 怒气<br>- 受到的单次伤害介于 31.25% 和 43.75% 之间：+40% 怒气<br>- 受到的单次伤害介于 43.75% 和 56.25% 之间：+70% 怒气<br>- 受到的单次伤害大于 56.25% ：+100% 怒气",
            "EN": "- Taking single instance DMG between 18.75% and 31.25%: +20% Rage.<br>- Taking single instance DMG between 31.25% and 43.75%: +40% Rage.<br>- Taking single instance DMG between 43.75% and 56.25%: +70% Rage.<br>- Taking single instance DMG greater than 56.25%: +100% Rage."
        },
        "Ball": {
            "Type": 2,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "hspc"
        ],
        "Grade": 1,
        "Weight": 80,
        "EndureTemplateID": 100,
        "SkillDMG": [
            {
                "Name": {
                    "CH": "普攻：水弹",
                    "EN": "Normal: Water Bullet"
                },
                "DMG": [
                    1.4
                ]
            },
            {
                "Name": {
                    "CH": "战技：水喷",
                    "EN": "Skill: Ground Water Splash"
                },
                "DMG": [
                    1.4
                ]
            },
            {
                "Name": {
                    "CH": "膨胀大招：水爆",
                    "EN": "Fury Ult: Water Burst"
                },
                "DMG": [
                    1.75
                ]
            },
            {
                "Name": {
                    "CH": "正常死亡爆炸",
                    "EN": "Normal Death Explode"
                },
                "DMG": [
                    1.75
                ]
            },
            {
                "Name": {
                    "CH": "膨胀死亡爆炸",
                    "EN": "Fury Death Explode"
                },
                "DMG": [
                    2.1
                ]
            }
        ],
        "Drop": [
            {
                "Type": 2,
                "Amount": [
                    1.2606,
                    0.3362,
                    0.084,
                    0
                ]
            }
        ],
        "Csx": [
            "ECElementalLifeforms/水飘浮灵"
        ]
    },
    "602": {
        "_id": 602,
        "KingdomID": 0,
        "ClassID": 10006,
        "Name": {
            "CH": "岩飘浮灵",
            "EN": "Geo Specter"
        },
        "Title": [
            {
                "CH": "飘浮的元素之灵",
                "EN": "Wandering Elemental Spirit"
            }
        ],
        "Desc": {
            "CH": "高浓度的岩元素孕育而生的魔物，能够飘浮在空中。\n岩飘浮灵在翅叶闭合时会拟态成某种植物果实的样子。因此在一些古老的传说中，它们被当作天上巨木结出的果实，被认为有着神奇的药用价值。",
            "EN": "A monster birthed from a high concentration of Geo that can float in the air.\nGeo Specters mimic the fruit of certain plants when their wings are closed together. As such, they are held by some ancient tales to be the fruit of a giant tree in the sky, and are regarded as having mysterious medical value."
        },
        "Color": "Rock",
        "HP": 0.8,
        "HPCurve": 2,
        "ATK": 1.75,
        "ATKCurve": 4,
        "Immune": [
            "Rock"
        ],
        "Tutorial": {
            "CH": "- 受到的单次伤害介于 18.75% 和 31.25% 之间：+20% 怒气<br>- 受到的单次伤害介于 31.25% 和 43.75% 之间：+40% 怒气<br>- 受到的单次伤害介于 43.75% 和 56.25% 之间：+70% 怒气<br>- 受到的单次伤害大于 56.25% ：+100% 怒气",
            "EN": "- Taking single instance DMG between 18.75% and 31.25%: +20% Rage.<br>- Taking single instance DMG between 31.25% and 43.75%: +40% Rage.<br>- Taking single instance DMG between 43.75% and 56.25%: +70% Rage.<br>- Taking single instance DMG greater than 56.25%: +100% Rage."
        },
        "Ball": {
            "Type": 7,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "gspc"
        ],
        "Grade": 1,
        "Weight": 80,
        "EndureTemplateID": 100,
        "SkillDMG": [
            {
                "Name": {
                    "CH": "普攻：岩弹x3",
                    "EN": "Normal: Geo Bullet x3"
                },
                "DMG": [
                    1.4,
                    1.4,
                    1.4
                ]
            },
            {
                "Name": {
                    "CH": "战技：突进",
                    "EN": "Skill: Rush"
                },
                "DMG": [
                    1.75
                ]
            },
            {
                "Name": {
                    "CH": "膨胀大招：地震",
                    "EN": "Fury Ult: Earthquake"
                },
                "DMG": [
                    1.75
                ]
            },
            {
                "Name": {
                    "CH": "正常死亡爆炸",
                    "EN": "Normal Death Explode"
                },
                "DMG": [
                    1.75
                ]
            },
            {
                "Name": {
                    "CH": "膨胀死亡爆炸",
                    "EN": "Fury Death Explode"
                },
                "DMG": [
                    2.1
                ]
            }
        ],
        "Drop": [
            {
                "Type": 2,
                "Amount": [
                    1.2606,
                    0.3362,
                    0.084,
                    0
                ]
            }
        ],
        "Csx": [
            "ECElementalLifeforms/岩飘浮灵"
        ]
    },
    "603": {
        "_id": 603,
        "KingdomID": 0,
        "ClassID": 10006,
        "Name": {
            "CH": "风飘浮灵",
            "EN": "Anemo Specter"
        },
        "Title": [
            {
                "CH": "飘浮的元素之灵",
                "EN": "Wandering Elemental Spirit"
            }
        ],
        "Desc": {
            "CH": "高浓度的风元素孕育而生的魔物，能够飘浮在空中。\n从理论上来说，风元素凝固而成的魔物本身就具有一定的飘浮能力。由此看来，它们花瓣一样的翅叶首先应该是一种拟态器官。",
            "EN": "A monster birthed from a high concentration of Anemo that can float in the air.\nAs reason might dictate, monsters made from coalesced Anemo already come equipped with the ability to float. As such, their petal-like wings are most likely to be a mimetic organ first and foremost."
        },
        "Color": "Wind",
        "HP": 0.8,
        "HPCurve": 2,
        "ATK": 1.75,
        "ATKCurve": 4,
        "Immune": [
            "Wind"
        ],
        "Tutorial": {
            "CH": "- 受到的单次伤害介于 18.75% 和 31.25% 之间：+20% 怒气<br>- 受到的单次伤害介于 31.25% 和 43.75% 之间：+40% 怒气<br>- 受到的单次伤害介于 43.75% 和 56.25% 之间：+70% 怒气<br>- 受到的单次伤害大于 56.25% ：+100% 怒气",
            "EN": "- Taking single instance DMG between 18.75% and 31.25%: +20% Rage.<br>- Taking single instance DMG between 31.25% and 43.75%: +40% Rage.<br>- Taking single instance DMG between 43.75% and 56.25%: +70% Rage.<br>- Taking single instance DMG greater than 56.25%: +100% Rage."
        },
        "Ball": {
            "Type": 6,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "aspc"
        ],
        "Grade": 1,
        "Weight": 80,
        "EndureTemplateID": 100,
        "SkillDMG": [
            {
                "Name": {
                    "CH": "普攻：风弹x2",
                    "EN": "Normal: Anemo Bullet x2"
                },
                "DMG": [
                    1.4,
                    1.4
                ]
            },
            {
                "Name": {
                    "CH": "战技：风压爆炸",
                    "EN": "Skill: Wind Burst"
                },
                "DMG": [
                    1.75
                ]
            },
            {
                "Name": {
                    "CH": "膨胀大招：喷吐风暴x3",
                    "EN": "Fury Ult: Spit Wind x3"
                },
                "DMG": [
                    1.75,
                    1.75,
                    1.75
                ]
            },
            {
                "Name": {
                    "CH": "正常死亡爆炸",
                    "EN": "Normal Death Explode"
                },
                "DMG": [
                    1.75
                ]
            },
            {
                "Name": {
                    "CH": "膨胀死亡爆炸",
                    "EN": "Fury Death Explode"
                },
                "DMG": [
                    2.1
                ]
            },
            {
                "Name": {
                    "CH": "膨胀死亡：风场每0.5s伤害",
                    "EN": "Fury Death: Wind Zone DoT every 0.5s"
                },
                "DMG": [
                    0.7
                ]
            }
        ],
        "Drop": [
            {
                "Type": 2,
                "Amount": [
                    1.2606,
                    0.3362,
                    0.084,
                    0
                ]
            }
        ],
        "Csx": [
            "ECElementalLifeforms/风飘浮灵"
        ]
    },
    "604": {
        "_id": 604,
        "KingdomID": 0,
        "ClassID": 10006,
        "Name": {
            "CH": "冰飘浮灵",
            "EN": "Cryo Specter"
        },
        "Title": [
            {
                "CH": "飘浮的元素之灵",
                "EN": "Wandering Elemental Spirit"
            }
        ],
        "Desc": {
            "CH": "高浓度的冰元素孕育而生的魔物，能够飘浮在空中。\n曾经有炼金术士调查过飘浮灵。他认为史莱姆和旅行者都能不借用神之眼使用元素，所以飘浮灵应该是史莱姆或者旅行者。他被导师训斥了。",
            "EN": "A monster born from highly-concentrated Cryo elements that can float in the air.\nA certain alchemist once studied these beings, speculating that they might be a kind of Slime or the Traveler, as none of the above needed a Vision to use the elements. He was reprimanded by his teacher."
        },
        "Color": "Ice",
        "HP": 0.8,
        "HPCurve": 2,
        "ATK": 1.75,
        "ATKCurve": 4,
        "Immune": [
            "Ice"
        ],
        "Tutorial": {
            "CH": "- 受到的单次伤害介于 18.75% 和 31.25% 之间：+20% 怒气<br>- 受到的单次伤害介于 31.25% 和 43.75% 之间：+40% 怒气<br>- 受到的单次伤害介于 43.75% 和 56.25% 之间：+70% 怒气<br>- 受到的单次伤害大于 56.25% ：+100% 怒气",
            "EN": "- Taking single instance DMG between 18.75% and 31.25%: +20% Rage.<br>- Taking single instance DMG between 31.25% and 43.75%: +40% Rage.<br>- Taking single instance DMG between 43.75% and 56.25%: +70% Rage.<br>- Taking single instance DMG greater than 56.25%: +100% Rage."
        },
        "Ball": {
            "Type": 4,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "cspc"
        ],
        "Grade": 1,
        "Weight": 80,
        "EndureTemplateID": 100,
        "SkillDMG": [
            {
                "Name": {
                    "CH": "普攻：慢冰锥",
                    "EN": "Normal: Slow Ice Cone"
                },
                "DMG": [
                    1.4
                ]
            },
            {
                "Name": {
                    "CH": "战技：快冰弹",
                    "EN": "Skill: Fast Ice Bullet"
                },
                "DMG": [
                    1.4
                ]
            },
            {
                "Name": {
                    "CH": "膨胀大招：大雪球x6",
                    "EN": "Fury Ult: Big Snowball x6"
                },
                "DMG": [
                    1.75
                ]
            },
            {
                "Name": {
                    "CH": "正常死亡爆炸",
                    "EN": "Normal Death Explode"
                },
                "DMG": [
                    1.75
                ]
            },
            {
                "Name": {
                    "CH": "膨胀死亡爆炸",
                    "EN": "Fury Death Explode"
                },
                "DMG": [
                    2.1
                ]
            },
            {
                "Name": {
                    "CH": "膨胀死亡：冰地板每0.5s伤害",
                    "EN": "Fury Death: Ice Floor DoT every 0.5s"
                },
                "DMG": [
                    0.7
                ]
            }
        ],
        "Drop": [
            {
                "Type": 2,
                "Amount": [
                    1.2606,
                    0.3362,
                    0.084,
                    0
                ]
            }
        ],
        "Csx": [
            "ECElementalLifeforms/冰飘浮灵"
        ]
    },
    "605": {
        "_id": 605,
        "KingdomID": 0,
        "ClassID": 10006,
        "Name": {
            "CH": "雷飘浮灵",
            "EN": "Electro Specter"
        },
        "Title": [
            {
                "CH": "飘浮的元素之灵",
                "EN": "Wandering Elemental Spirit"
            }
        ],
        "Desc": {
            "CH": "高浓度的雷元素孕育而生的魔物，能够飘浮在空中。\n曾经有学者对它们这个物种进行过研究。他认为无相元素有元素特征、会在元素浓度高的区域诞生，和飘浮灵完全一样。所以飘浮灵是一种无相。他后来被调查出了学术腐败。",
            "EN": "A monster born from highly-concentrated Electro elements that can float in the air.\nA certain researcher once studied these creatures. He believed that they must be Hypostases of some kind. After all, they both had elemental peculiarities and were birthed from areas saturated with elemental energy, right? By the way, he was later investigated for academic corruption."
        },
        "Color": "Elec",
        "HP": 0.8,
        "HPCurve": 2,
        "ATK": 2.8,
        "ATKCurve": 4,
        "Immune": [
            "Elec"
        ],
        "Tutorial": {
            "CH": "- 受到的单次伤害介于 18.75% 和 31.25% 之间：+20% 怒气<br>- 受到的单次伤害介于 31.25% 和 43.75% 之间：+40% 怒气<br>- 受到的单次伤害介于 43.75% 和 56.25% 之间：+70% 怒气<br>- 受到的单次伤害大于 56.25% ：+100% 怒气",
            "EN": "- Taking single instance DMG between 18.75% and 31.25%: +20% Rage.<br>- Taking single instance DMG between 31.25% and 43.75%: +40% Rage.<br>- Taking single instance DMG between 43.75% and 56.25%: +70% Rage.<br>- Taking single instance DMG greater than 56.25%: +100% Rage."
        },
        "Ball": {
            "Type": 3,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "espc"
        ],
        "Grade": 1,
        "Weight": 80,
        "EndureTemplateID": 100,
        "SkillDMG": [
            {
                "Name": {
                    "CH": "普攻：落雷x3",
                    "EN": "Normal: Lightning x3"
                },
                "DMG": [
                    1.4
                ]
            },
            {
                "Name": {
                    "CH": "战技：突进",
                    "EN": "Skill: Rush"
                },
                "DMG": [
                    1.75
                ]
            },
            {
                "Name": {
                    "CH": "膨胀大招：划x2+砸！",
                    "EN": "Fury Ult: Slash x2 + Smash!"
                },
                "DMG": [
                    0.42,
                    0.42,
                    2.8
                ]
            },
            {
                "Name": {
                    "CH": "正常死亡爆炸",
                    "EN": "Normal Death Explode"
                },
                "DMG": [
                    1.75
                ]
            },
            {
                "Name": {
                    "CH": "膨胀死亡爆炸",
                    "EN": "Fury Death Explode"
                },
                "DMG": [
                    2.1
                ]
            }
        ],
        "Drop": [
            {
                "Type": 2,
                "Amount": [
                    1.2606,
                    0.3362,
                    0.084,
                    0
                ]
            }
        ],
        "Csx": [
            "ECElementalLifeforms/雷飘浮灵"
        ]
    },
    "606": {
        "_id": 606,
        "KingdomID": 0,
        "ClassID": 10006,
        "Name": {
            "CH": "火飘浮灵",
            "EN": "Pyro Specter"
        },
        "Title": [
            {
                "CH": "飘浮的元素之灵",
                "EN": "Wandering Elemental Spirit"
            }
        ],
        "Desc": {
            "CH": "高浓度的火元素孕育而生的魔物，能够飘浮在空中。\n曾经有教令院的学生对它们的生态进行过研究。他认为晶蝶能飞、会在元素浓度高的区域诞生，和飘浮灵完全一样。所以飘浮灵是一种晶蝶。他现在也没毕业。",
            "EN": "A monster born from highly-concentrated Pyro elements that can float in the air.\nThere was once an Akademiya student who conducted research into their behavior. He believed that Specters were a form of Crystalfly; since they both had powers of flight and both were birthed in areas saturated with elemental energy. He has not graduated yet."
        },
        "Color": "Fire",
        "HP": 0.8,
        "HPCurve": 2,
        "ATK": 1.75,
        "ATKCurve": 4,
        "Immune": [
            "Fire"
        ],
        "Tutorial": {
            "CH": "- 受到的单次伤害介于 18.75% 和 31.25% 之间：+20% 怒气<br>- 受到的单次伤害介于 31.25% 和 43.75% 之间：+40% 怒气<br>- 受到的单次伤害介于 43.75% 和 56.25% 之间：+70% 怒气<br>- 受到的单次伤害大于 56.25% ：+100% 怒气",
            "EN": "- Taking single instance DMG between 18.75% and 31.25%: +20% Rage.<br>- Taking single instance DMG between 31.25% and 43.75%: +40% Rage.<br>- Taking single instance DMG between 43.75% and 56.25%: +70% Rage.<br>- Taking single instance DMG greater than 56.25%: +100% Rage."
        },
        "Ball": {
            "Type": 1,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "pspc"
        ],
        "Grade": 1,
        "Weight": 80,
        "EndureTemplateID": 100,
        "SkillDMG": [
            {
                "Name": {
                    "CH": "普攻：火球",
                    "EN": "Normal: Fireball"
                },
                "DMG": [
                    1.4
                ]
            },
            {
                "Name": {
                    "CH": "战技：闪现背刺",
                    "EN": "Skill: Teleport Backstab"
                },
                "DMG": [
                    1.75
                ]
            },
            {
                "Name": {
                    "CH": "膨胀大招：喷火x6",
                    "EN": "Fury Ult: Breath Fire x6"
                },
                "DMG": [
                    1.75
                ]
            },
            {
                "Name": {
                    "CH": "正常死亡爆炸",
                    "EN": "Normal Death Explode"
                },
                "DMG": [
                    1.75
                ]
            },
            {
                "Name": {
                    "CH": "膨胀死亡爆炸",
                    "EN": "Fury Death Explode"
                },
                "DMG": [
                    2.1
                ]
            },
            {
                "Name": {
                    "CH": "膨胀死亡：额外爆炸",
                    "EN": "Fury Death: Extra Explosion"
                },
                "DMG": [
                    4.2
                ]
            }
        ],
        "Drop": [
            {
                "Type": 2,
                "Amount": [
                    1.2606,
                    0.3362,
                    0.084,
                    0
                ]
            }
        ],
        "Csx": [
            "ECElementalLifeforms/火飘浮灵"
        ]
    },
    "607": {
        "_id": 607,
        "KingdomID": 0,
        "ClassID": 10006,
        "Name": {
            "CH": "草飘浮灵",
            "EN": "Dendro Specter"
        },
        "Title": [
            {
                "CH": "飘浮的元素之灵",
                "EN": "Wandering Elemental Spirit"
            }
        ],
        "Desc": {
            "CH": "高浓度的草元素孕育而生的魔物，能够飘浮在空中。\n曾经有因论派的研究员对草飘浮灵的生态特征进行过研究，发现与传说中的「兰那罗」非常相似。他在一番考据后得出结论，所谓兰那罗其实就是草飘浮灵。后来这位研究员成为了籍籍无名的童话作家。",
            "EN": "A monster born from high-density Dendro energy that can float in the air.\nA researcher from Vahumana once did research into the ecological peculiarities of Dendro Specters, finding them to be quite similar to the legendary \"Aranara.\" His conclusion was that the Aranara were actually Dendro Specters. This researcher would become a little-known writer of fairy tales."
        },
        "Color": "Grass",
        "HP": 0.8,
        "HPCurve": 2,
        "ATK": 1.4,
        "ATKCurve": 4,
        "Immune": [
            "Grass"
        ],
        "Tutorial": {
            "CH": "- 受到的单次伤害介于 18.75% 和 31.25% 之间：+20% 怒气<br>- 受到的单次伤害介于 31.25% 和 43.75% 之间：+40% 怒气<br>- 受到的单次伤害介于 43.75% 和 56.25% 之间：+70% 怒气<br>- 受到的单次伤害大于 56.25% ：+100% 怒气",
            "EN": "- Taking single instance DMG between 18.75% and 31.25%: +20% Rage.<br>- Taking single instance DMG between 31.25% and 43.75%: +40% Rage.<br>- Taking single instance DMG between 43.75% and 56.25%: +70% Rage.<br>- Taking single instance DMG greater than 56.25%: +100% Rage."
        },
        "Ball": {
            "Type": 5,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "dspc"
        ],
        "Grade": 1,
        "Weight": 80,
        "EndureTemplateID": 100,
        "SkillDMG": [
            {
                "Name": {
                    "CH": "普攻：草弹",
                    "EN": "Normal: Dendro Bullet"
                },
                "DMG": [
                    1.4
                ]
            },
            {
                "Name": {
                    "CH": "战技：草丛爆发",
                    "EN": "Skill: Shrub Blast"
                },
                "DMG": [
                    1.4
                ]
            },
            {
                "Name": {
                    "CH": "膨胀大招：自旋冲击波x3",
                    "EN": "Fury Ult: Self Spin Shockwave x3"
                },
                "DMG": [
                    1.4,
                    1.4,
                    1.4
                ]
            },
            {
                "Name": {
                    "CH": "正常死亡爆炸",
                    "EN": "Normal Death Explode"
                },
                "DMG": [
                    1.75
                ]
            },
            {
                "Name": {
                    "CH": "膨胀死亡爆炸",
                    "EN": "Fury Death Explode"
                },
                "DMG": [
                    2.1
                ]
            }
        ],
        "Drop": [
            {
                "Type": 2,
                "Amount": [
                    1.2606,
                    0.3362,
                    0.084,
                    0
                ]
            }
        ],
        "Csx": [
            "ECElementalLifeforms/草飘浮灵"
        ]
    },
    "701": {
        "_id": 701,
        "KingdomID": 0,
        "ClassID": 10007,
        "Name": {
            "CH": "雷音权现",
            "EN": "Thunder Manifestation"
        },
        "Title": [
            {
                "CH": "霆雷的忿恨意志",
                "EN": "Stormy Resentment"
            },
            {
                "CH": "天云峠的愤怒雷鸣",
                "EN": "Raging Thunder of Amakumo Peak"
            }
        ],
        "Desc": {
            "CH": "异常的雷元素生命。虽然外貌上与纯水精灵有相似之处，却不具备她的智慧与记忆。\n驱动雷音权现的是更为原始纯粹的愤怒。\n据说这种生物只会随着雷霆降生在遗留着强大忿恨的地方，譬如说支离破碎、雷云黝黝的天云峠。只要土地中的怨恨不消，那雷鸣也不会断绝吧。",
            "EN": "An abnormal Electro elemental life form. Although it might appear similar to an Oceanid, it does not possess her intelligence or memories.\nThat which drives the Thunder Manifestation is a much more primal fury.\nIt is said that these life forms will only descend like lightning upon places that play host to great lingering resentment, such as the fragmented Amakumo Peak, where the storm clouds ever roil. As long as the grudges of the land do not fade, so will the roaring thunder persist."
        },
        "Color": "Elec",
        "HP": 7.0,
        "HPCurve": 2,
        "ATK": 7.2,
        "ATKCurve": 4,
        "Immune": [
            "Elec"
        ],
        "Element": {
            "Type": "Elec",
            "Value": 4,
            "Immutable": true
        },
        "Ball": {
            "Type": 3,
            "Point": 4,
            "Drop": "3332"
        },
        "Icon": [
            "tm"
        ],
        "Grade": 3,
        "Weight": 150,
        "EndureTemplateID": 4000,
        "SkillData": [
            "0/701"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "未狂暴：探针落地",
                    "EN": "Before lock: Strike Probes land"
                },
                "DMG": [
                    1.2
                ]
            },
            {
                "Name": {
                    "CH": "未狂暴：中心大圈落雷",
                    "EN": "Before lock: Central big circle lightning"
                },
                "DMG": [
                    6
                ]
            },
            {
                "Name": {
                    "CH": "未狂暴：边缘小圈落雷",
                    "EN": "Before lock: Side small circle lightning"
                },
                "DMG": [
                    3.1176
                ]
            },
            {
                "Name": {
                    "CH": "未狂暴：反击雷爆",
                    "EN": "Before lock: Counterattack"
                },
                "DMG": [
                    2
                ]
            },
            {
                "Name": {
                    "CH": "转转转转转转 | 每0.1s伤害",
                    "EN": "Spin spin spin spin | DMG every 0.1s"
                },
                "DMG": [
                    2
                ]
            },
            {
                "Name": {
                    "CH": "狂暴：死亡雷墙",
                    "EN": "Locked: Thunder Walls"
                },
                "DMG": [
                    7.2
                ]
            },
            {
                "Name": {
                    "CH": "狂暴：死亡雷墙 | 每0.5s伤害",
                    "EN": "Locked: Thunder Cage | DMG every 0.5s"
                },
                "DMG": [
                    2
                ]
            },
            {
                "Name": {
                    "CH": "狂暴：发射5雷弹",
                    "EN": "Locked: Fire 5 lightning bullets"
                },
                "DMG": [
                    2.0736
                ]
            },
            {
                "Name": {
                    "CH": "狂暴：角色在身后，转身打",
                    "EN": "Locked: Turn attack when player is behind"
                },
                "DMG": [
                    4
                ]
            },
            {
                "Name": {
                    "CH": "狂暴：直线俯冲2次",
                    "EN": "Locked: Straight line rush x2"
                },
                "DMG": [
                    6,
                    6
                ]
            },
            {
                "Name": {
                    "CH": "狂暴：3+3+3 落雷",
                    "EN": "Locked: 3+3+3 Lightning bolts"
                },
                "DMG": [
                    3.1176
                ]
            }
        ],
        "Csx": [
            "ECElementalLifeforms/雷音权现"
        ]
    },
    "10101": {
        "_id": 10101,
        "KingdomID": 1,
        "ClassID": 101010,
        "Name": {
            "CH": "丘丘人",
            "EN": "Hilichurl"
        },
        "Title": [
            {
                "CH": "单纯的魔物",
                "EN": "A Simple Creature"
            }
        ],
        "Desc": {
            "CH": "提瓦特荒野中游荡的原始住民。\n与人类轮廓相似，却失去了智能与灵性。据记载，在大地上出没超过千年，却没有历史与文明。\n自五百年前漆黑的灾祸以来，便广泛地分布在大地各处。虽然力量弱小，缺乏组织，但有时也能给人们带来不小的麻烦。",
            "EN": "The primitive wandering inhabitants of Teyvat's wildernesses.\nThey look very similar to humanity but seem to have lost both their intellect and spirituality. Their presence on the continent has been recorded for over a thousand years, and yet they have neither history nor civilization.\nSince the pitch-black calamity from five hundred years ago, they have begun to spread in large numbers across the land. They are not very strong, and they lack organization, but they nonetheless bring sizable trouble to humans every once in a while."
        },
        "HP": 1.0,
        "HPCurve": 1,
        "ATK": 1.8,
        "ATKCurve": 3,
        "Ball": {
            "Type": 0,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "hil"
        ],
        "Grade": 1,
        "Weight": 75,
        "EndureTemplateID": 50,
        "Drop": [
            {
                "Type": 4,
                "Amount": [
                    0.4202,
                    0.112,
                    0.028,
                    0
                ]
            }
        ],
        "Csx": [
            "ECHilichurls/丘丘人"
        ]
    },
    "10102": {
        "_id": 10102,
        "KingdomID": 1,
        "ClassID": 101010,
        "Name": {
            "CH": "打手丘丘人",
            "EN": "Hilichurl Fighter"
        },
        "Title": [
            {
                "CH": "单纯的魔物",
                "EN": "A Simple Creature"
            }
        ],
        "Desc": {
            "CH": "提瓦特荒野中游荡的原始住民。\n坏脾气的部落战士，朴实地信奉着肌肉的力量。如果肌肉够结实，力量够强，那终有一天也能举起巨大的盾牌，成为「丘丘暴徒」…？",
            "EN": "The primitive wandering inhabitants of Teyvat's wildernesses.\nIll-tempered warriors for their tribes, they are simple-minded believers in the power of brute force. If their muscles grow strong enough and their strength great enough, then one day they too shall raise up a great shield and become a mitachurl... Or will they?"
        },
        "HP": 1.0,
        "HPCurve": 1,
        "ATK": 1.8,
        "ATKCurve": 3,
        "Ball": {
            "Type": 0,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "hil"
        ],
        "Grade": 1,
        "Weight": 75,
        "EndureTemplateID": 50,
        "Drop": [
            {
                "Type": 4,
                "Amount": [
                    0.4202,
                    0.112,
                    0.028,
                    0
                ]
            }
        ],
        "Csx": [
            "ECHilichurls/打手丘丘人"
        ]
    },
    "10103": {
        "_id": 10103,
        "KingdomID": 1,
        "ClassID": 101010,
        "Name": {
            "CH": "木盾丘丘人",
            "EN": "Wooden Shield Hilichurl Guard"
        },
        "Title": [
            {
                "CH": "单纯的魔物",
                "EN": "A Simple Creature"
            }
        ],
        "Desc": {
            "CH": "提瓦特荒野中游荡的原始住民。\n拥有木盾的丘丘人战士，木盾不仅是坚固的防具，也是属于部落的珍贵宝物。根据不少人的报告，丘丘人会冲向扎营烹饪的旅人，撵走他们，然后抢走他们的锅盖。",
            "EN": "The primitive wandering inhabitants of Teyvat's wildernesses.\nHilichurl warriors who have wooden shields. These shields are not only sturdy defensive tools but also treasured items within the tribe. According to many reports, hilichurls will often charge at travelers making camp to cook and will snatch their pot lids after chasing them away."
        },
        "Color": "Wood",
        "HP": 1.0,
        "HPCurve": 1,
        "ATK": 1.8,
        "ATKCurve": 3,
        "Ball": {
            "Type": 0,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "hil"
        ],
        "Grade": 1,
        "Weight": 75,
        "EndureTemplateID": 50,
        "Drop": [
            {
                "Type": 4,
                "Amount": [
                    0.4202,
                    0.112,
                    0.028,
                    0
                ]
            }
        ],
        "Csx": [
            "ECHilichurls/木盾丘丘人"
        ]
    },
    "10104": {
        "_id": 10104,
        "KingdomID": 1,
        "ClassID": 101011,
        "Name": {
            "CH": "射手丘丘人",
            "EN": "Hilichurl Shooter"
        },
        "Title": [
            {
                "CH": "单纯的魔物",
                "EN": "A Simple Creature"
            }
        ],
        "Desc": {
            "CH": "提瓦特荒野中游荡的原始住民。\n手持简陋弩机的丘丘人射手。丘丘人实际上并不具备制造弩的工艺水平。因此，一般认为，在丘丘人背后，存在着操控丘丘人，并为它们提供设备物资的组织。而与丘丘人走得很近的深渊法师尤为可疑。",
            "EN": "The primitive wandering inhabitants of Teyvat's wildernesses.\nHilichurl Shooters who wield simple crossbows. Hilichurls do not, in truth, possess the craftsmanship needed to build crossbows. As such, it is commonly believed that there is an organization that controls the hilichurls from behind the scenes and furnishes them with equipment and material supplies. The Abyss Mages that often accompany these hilichurls are especially suspicious."
        },
        "HP": 0.8,
        "HPCurve": 1,
        "ATK": 0.9,
        "ATKCurve": 3,
        "Ball": {
            "Type": 0,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "hil"
        ],
        "Grade": 1,
        "Weight": 75,
        "EndureTemplateID": 50,
        "Drop": [
            {
                "Type": 5,
                "Amount": [
                    0.8404,
                    0.2241,
                    0.056,
                    0
                ]
            },
            {
                "Type": 4,
                "Amount": [
                    0.1681,
                    0.0448,
                    0.0112,
                    0
                ]
            }
        ],
        "Csx": [
            "ECHilichurls/射手丘丘人"
        ]
    },
    "10105": {
        "_id": 10105,
        "KingdomID": 1,
        "ClassID": 101011,
        "Name": {
            "CH": "火箭丘丘人",
            "EN": "Pyro Hilichurl Shooter"
        },
        "Title": [
            {
                "CH": "单纯的魔物",
                "EN": "A Simple Creature"
            }
        ],
        "Desc": {
            "CH": "提瓦特荒野中游荡的原始住民。\n手持简陋弩机的射手，箭头上敷了一层薄薄的易燃材料，以便烧灼其目标。前代的艾尔 · 马斯克认为，这是头脑灵光的丘丘人想要在捕猎时顺便完成烹饪而为的创新之事。",
            "EN": "The primitive wandering inhabitants of Teyvat's wildernesses.\nThese archers wield simple crossbows, and their arrowheads are coated with a thin layer of flammable material that can set their targets on fire. El Musk of the previous generation believed that this is an innovation by brighter hilichurls who wished to cook their game after hunting it."
        },
        "Color": "Fire",
        "HP": 0.8,
        "HPCurve": 1,
        "ATK": 1.35,
        "ATKCurve": 3,
        "Ball": {
            "Type": 0,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "phil"
        ],
        "Grade": 1,
        "Weight": 75,
        "EndureTemplateID": 50,
        "Drop": [
            {
                "Type": 5,
                "Amount": [
                    0.8404,
                    0.2241,
                    0.056,
                    0
                ]
            },
            {
                "Type": 4,
                "Amount": [
                    0.1681,
                    0.0448,
                    0.0112,
                    0
                ]
            }
        ],
        "Csx": [
            "ECHilichurls/火箭丘丘人"
        ]
    },
    "10106": {
        "_id": 10106,
        "KingdomID": 1,
        "ClassID": 101011,
        "Name": {
            "CH": "爆弹丘丘人",
            "EN": "Hilichurl Grenadier"
        },
        "Title": [
            {
                "CH": "单纯的魔物",
                "EN": "A Simple Creature"
            }
        ],
        "Desc": {
            "CH": "提瓦特荒野中游荡的原始住民。\n擅长活用火史莱姆的狡猾家伙，把火史莱姆用作迎击来犯者的武器。利用火史莱姆的爆炸，投掷技巧精湛的爆弹丘丘人或许可以一次击碎五根木柱吧。",
            "EN": "The primitive wandering inhabitants of Teyvat's wildernesses.\nThese sneaky fellows are skilled in the use of Pyro Slimes, using them as weapons to attack intruders. Skilled grenadiers may be able to destroy four wooden dummies in a single throw."
        },
        "Color": "Fire",
        "HP": 1.0,
        "HPCurve": 1,
        "ATK": 3.6,
        "ATKCurve": 3,
        "Ball": {
            "Type": 0,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "phil"
        ],
        "Grade": 1,
        "Weight": 75,
        "EndureTemplateID": 50,
        "Drop": [
            {
                "Type": 4,
                "Amount": [
                    0.4202,
                    0.112,
                    0.028,
                    0
                ]
            }
        ],
        "Csx": [
            "ECHilichurls/爆弹丘丘人"
        ]
    },
    "10107": {
        "_id": 10107,
        "KingdomID": 1,
        "ClassID": 101010,
        "Name": {
            "CH": "冲锋丘丘人",
            "EN": "Hilichurl Berserker"
        },
        "Title": [
            {
                "CH": "单纯的魔物",
                "EN": "A Simple Creature"
            }
        ],
        "Desc": {
            "CH": "提瓦特荒野中游荡的原始住民。\n为了击退来犯的冒险者，会挥舞着熊熊燃烧的棒槌，一往无前地奋勇冲锋。草原或森林中发生野火的话，通常而言，并不是因为某名特别的「骑士」，而是因为这种丘丘人的犯行。",
            "EN": "The primitive wandering inhabitants of Teyvat's wildernesses.\nThese hilichurls wield flaming clubs and charge with reckless abandon to chase intruding adventurers off. Most wildfires in the grasslands or forests are not actually caused by a certain special Knight, but due to the actions of these hilichurls."
        },
        "Color": "Fire",
        "HP": 1.0,
        "HPCurve": 1,
        "ATK": 1.8,
        "ATKCurve": 3,
        "Ball": {
            "Type": 0,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "phil"
        ],
        "Grade": 1,
        "Weight": 75,
        "EndureTemplateID": 50,
        "Drop": [
            {
                "Type": 4,
                "Amount": [
                    0.4202,
                    0.112,
                    0.028,
                    0
                ]
            }
        ],
        "Csx": [
            "ECHilichurls/冲锋丘丘人"
        ]
    },
    "10109": {
        "_id": 10109,
        "KingdomID": 1,
        "ClassID": 101011,
        "Name": {
            "CH": "冰箭丘丘人",
            "EN": "Cryo Hilichurl Shooter"
        },
        "Title": [
            {
                "CH": "单纯的魔物",
                "EN": "A Simple Creature"
            }
        ],
        "Desc": {
            "CH": "提瓦特荒野中游荡的原始住民。\n手持简陋弩机的射手，箭头上凝着不化的寒冰。好像听说这种箭头留下的伤口很难愈合，因为寒冷会减缓血液流通的速度。",
            "EN": "The primitive wandering inhabitants of Teyvat's wildernesses.\nThese archers wield simple crossbows, and unmelting ice shrouds their arrowheads. It is said that wounds caused by these arrowheads heal very slowly, as the cold will slow the circulation of blood."
        },
        "Color": "Ice",
        "HP": 0.8,
        "HPCurve": 1,
        "ATK": 0.9,
        "ATKCurve": 3,
        "Ball": {
            "Type": 0,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "chil"
        ],
        "Grade": 1,
        "Weight": 75,
        "EndureTemplateID": 50,
        "Drop": [
            {
                "Type": 5,
                "Amount": [
                    0.8404,
                    0.2241,
                    0.056,
                    0
                ]
            },
            {
                "Type": 4,
                "Amount": [
                    0.1681,
                    0.0448,
                    0.0112,
                    0
                ]
            }
        ],
        "Csx": [
            "ECHilichurls/冰箭丘丘人"
        ]
    },
    "10110": {
        "_id": 10110,
        "KingdomID": 1,
        "ClassID": 101011,
        "Name": {
            "CH": "雷箭丘丘人",
            "EN": "Electro Hilichurl Shooter"
        },
        "Title": [
            {
                "CH": "单纯的魔物",
                "EN": "A Simple Creature"
            }
        ],
        "Desc": {
            "CH": "提瓦特荒野中游荡的原始住民。\n手持简陋弩机的射手，箭头以电气水晶的碎片磨削而成，以便命中时造成电击。据说射术百里挑一的丘丘人射手可以一次发射五枚箭矢，非常厉害。",
            "EN": "The primitive wandering inhabitants of Teyvat's wildernesses.\nThese archers wield simple crossbows, and their arrowheads are crafted from carved Electro Crystals that will deliver an electric shock upon impact. They say that Hilichurl Shooters of exceptional skill can fire five crossbow bolts at once — a most impressive feat indeed."
        },
        "Color": "Elec",
        "HP": 0.8,
        "HPCurve": 1,
        "ATK": 1.35,
        "ATKCurve": 3,
        "Ball": {
            "Type": 0,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "ehil"
        ],
        "Grade": 1,
        "Weight": 75,
        "EndureTemplateID": 50,
        "Drop": [
            {
                "Type": 5,
                "Amount": [
                    0.8404,
                    0.2241,
                    0.056,
                    0
                ]
            },
            {
                "Type": 4,
                "Amount": [
                    0.1681,
                    0.0448,
                    0.0112,
                    0
                ]
            }
        ],
        "Csx": [
            "ECHilichurls/雷箭丘丘人"
        ]
    },
    "10112": {
        "_id": 10112,
        "KingdomID": 1,
        "ClassID": 101010,
        "Name": {
            "CH": "岩盾丘丘人",
            "EN": "Rock Shield Hilichurl Guard"
        },
        "Title": [
            {
                "CH": "单纯的魔物",
                "EN": "A Simple Creature"
            }
        ],
        "Desc": {
            "CH": "提瓦特荒野中游荡的原始住民。\n岩元素富集的地域中顽强生存的丘丘人，其坚不可摧的盾牌上增生着层层磐岩。",
            "EN": "The primitive wandering inhabitants of Teyvat's wildernesses.\nHardy hilichurls who have congregated in a region where Geo energies are abundant. Their impregnable shields have layers of rocky growths on them."
        },
        "Color": "Rock",
        "HP": 1.0,
        "HPCurve": 1,
        "ATK": 1.8,
        "ATKCurve": 3,
        "Ball": {
            "Type": 0,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "hil"
        ],
        "Grade": 1,
        "Weight": 75,
        "EndureTemplateID": 50,
        "Drop": [
            {
                "Type": 4,
                "Amount": [
                    0.4202,
                    0.112,
                    0.028,
                    0
                ]
            }
        ],
        "Csx": [
            "ECHilichurls/岩盾丘丘人"
        ]
    },
    "10113": {
        "_id": 10113,
        "KingdomID": 1,
        "ClassID": 101011,
        "Name": {
            "CH": "冰弹丘丘人",
            "EN": "Cryo Hilichurl Grenadier"
        },
        "Title": [
            {
                "CH": "单纯的魔物",
                "EN": "A Simple Creature"
            }
        ],
        "Desc": {
            "CH": "提瓦特荒野中游荡的原始住民。\n发现了冰史莱姆的妙用，可以高效地保鲜食物。丘丘人有着活用各种史莱姆的智慧，无论是将火史莱姆、冰史莱姆制成炸药桶，还是用风史莱姆制作载具…可能这就是艰难生活带来的思维方式吧。",
            "EN": "The primitive wandering inhabitants of Teyvat's wildernesses.\nHilichurls have discovered the use of Cryo Slimes in effectively keeping food fresh. Hilichurls have surprising amounts of wisdom in the practical use of Slimes, whether it be the use of Pyro and Cryo Slimes to make Exploding Barrels, or the use of Anemo Slimes in vehicles... Perhaps this is their way of adapting to their hard lot in life."
        },
        "Color": "Ice",
        "HP": 0.8,
        "HPCurve": 1,
        "ATK": 0.9,
        "ATKCurve": 3,
        "Ball": {
            "Type": 0,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "chil"
        ],
        "Grade": 1,
        "Weight": 75,
        "EndureTemplateID": 50,
        "Drop": [
            {
                "Type": 4,
                "Amount": [
                    0.4202,
                    0.112,
                    0.028,
                    0
                ]
            }
        ],
        "Csx": [
            "ECHilichurls/冰弹丘丘人"
        ]
    },
    "10114": {
        "_id": 10114,
        "KingdomID": 1,
        "ClassID": 101010,
        "Name": {
            "CH": "冰盾丘丘人",
            "EN": "Ice Shield Hilichurl Guard"
        },
        "Title": [
            {
                "CH": "单纯的魔物",
                "EN": "A Simple Creature"
            }
        ],
        "Desc": {
            "CH": "提瓦特荒野中游荡的原始住民。\n丘丘人有着强大的环境适应性的证明。在冰元素富集的环境中，能设法利用坚冰制成盾牌保护自己。",
            "EN": "The primitive wandering inhabitants of Teyvat's wildernesses.\nHilichurls display a strong sense of adaptability to their surroundings. In icy environments, they come up with ways and means to fashion this hard ice into shields to protect themselves."
        },
        "Color": "Ice",
        "HP": 1.0,
        "HPCurve": 1,
        "ATK": 1.8,
        "ATKCurve": 3,
        "Ball": {
            "Type": 0,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "chil"
        ],
        "Grade": 1,
        "Weight": 75,
        "EndureTemplateID": 50,
        "Drop": [
            {
                "Type": 4,
                "Amount": [
                    0.4202,
                    0.112,
                    0.028,
                    0
                ]
            }
        ],
        "Csx": [
            "ECHilichurls/冰盾丘丘人"
        ]
    },
    "10115": {
        "_id": 10115,
        "KingdomID": 1,
        "ClassID": 101012,
        "Name": {
            "CH": "奇怪的丘丘人",
            "EN": "Unusual Hilichurl"
        },
        "Title": [
            {
                "CH": "别墅之灾",
                "EN": "Menace of the Mansion"
            }
        ],
        "Desc": {
            "CH": "「Homu, Homu, mimi domu!」",
            "EN": "\"Homu, homu, mimi domu!\""
        },
        "HP": 1.6,
        "HPCurve": 1,
        "ATK": 3.6,
        "ATKCurve": 3,
        "Ball": {
            "Type": 0,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "uhil"
        ],
        "Grade": 1,
        "Weight": 75,
        "EndureTemplateID": 100,
        "Csx": [
            "ECHilichurls/奇怪的丘丘人"
        ]
    },
    "10116": {
        "_id": 10116,
        "KingdomID": 1,
        "ClassID": 101011,
        "Name": {
            "CH": "雷弹丘丘人",
            "EN": "Electro Hilichurl Grenadier"
        },
        "Title": [
            {
                "CH": "单纯的魔物",
                "EN": "A Simple Creature"
            }
        ],
        "Desc": {
            "CH": "提瓦特荒野中游荡的原始住民。\n对于保持着蒙昧生活的丘丘人而言，雷史莱姆似乎不像火史莱姆、风史莱姆那样用途广泛。或许有一天他们会发现雷史莱姆、变异雷史莱姆之间的元素流动，然后取得突飞猛进的发展…？",
            "EN": "The primitive wandering inhabitants of Teyvat's wildernesses.\nElectro Slimes don't seem to have as many applications as Pyro Slimes or Anemo Slimes in the benighted lives of the Hilichurls. Perhaps one day they may discover the flow of Elements between Electro Slime and Mutant Electro Slime and make some great progress... ?"
        },
        "Color": "Elec",
        "HP": 1.0,
        "HPCurve": 1,
        "ATK": 1.8,
        "ATKCurve": 3,
        "Ball": {
            "Type": 0,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "ehil"
        ],
        "Grade": 1,
        "Weight": 75,
        "EndureTemplateID": 50,
        "Drop": [
            {
                "Type": 4,
                "Amount": [
                    0.4202,
                    0.112,
                    0.028,
                    0
                ]
            }
        ],
        "Csx": [
            "ECHilichurls/雷弹丘丘人"
        ]
    },
    "10201": {
        "_id": 10201,
        "KingdomID": 1,
        "ClassID": 101020,
        "Name": {
            "CH": "木盾丘丘暴徒",
            "EN": "Wooden Shieldwall Mitachurl"
        },
        "Title": [
            {
                "CH": "丘丘暴徒 · 木盾",
                "EN": "Mitachurl: Wooden Shieldwall"
            }
        ],
        "Desc": {
            "CH": "经历无数战斗历练的丘丘人大块头。\n会利用草史莱姆创造坚固的木盾，进行战斗。\n拥有强健的肌肉，油亮的鬃毛，可靠的宽厚背影，一顿饭吃下很多肉的豪放，能为丘丘人抵挡毫不留情的冒险者，在部族当中有着相当崇高的地位。",
            "EN": "A muscular hilichurl who has been forged in countless fights.\nThey know how to use Dendro Slimes to create tough shields which they wield in combat.\nWith their powerful muscles, glossy fur, and sturdy, reliable silhouettes, they help the hilichurls withstand the pitiless assaults of adventurers, and they hold a rather exalted rank amongst the tribes."
        },
        "Color": "Wood",
        "HP": 3.0,
        "HPCurve": 1,
        "ATK": 2.4,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.3
        },
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "1113"
        },
        "Icon": [
            "mita"
        ],
        "Grade": 2,
        "Weight": 150,
        "EndureTemplateID": 210,
        "Drop": [
            {
                "Type": 7,
                "Amount": [
                    0,
                    1.5758,
                    0.4202,
                    0.1051
                ]
            },
            {
                "Type": 4,
                "Amount": [
                    0.5253,
                    0.1401,
                    0.035,
                    0
                ]
            }
        ],
        "Csx": [
            "ECHilichurls/木盾丘丘暴徒"
        ]
    },
    "10202": {
        "_id": 10202,
        "KingdomID": 1,
        "ClassID": 101020,
        "Name": {
            "CH": "火斧丘丘暴徒",
            "EN": "Blazing Axe Mitachurl"
        },
        "Title": [
            {
                "CH": "丘丘暴徒 · 火斧",
                "EN": "Mitachurl: Blazing Axe"
            }
        ],
        "Desc": {
            "CH": "手持长柄大斧的丘丘人大块头。\n攻击大开大合，会利用火史莱姆为战斧附上火元素，具有强大的破坏力。\n通常，像火斧丘丘暴徒这样为武器反复淬火会导致钢铁的硬度下降，进而导致刃口容易受损。但丘丘暴徒的力量足够强大，斧子就算刃口没了也可以当锤子用。",
            "EN": "Muscular hilichurls who wield large two-handed axes.\nAttacks with large strikes, and will use Pyro Slimes to infuse their axes with Pyro. They are very lethal.\nUsually, repeatedly quenching a weapon the way a Blazing Axe Mitachurl does will decrease the hardness of the metal and cause the axe-head's edge to chip more easily. But such is the strength of the mitachurls that even should the edge disappear altogether, they can still use their axes as hammers."
        },
        "Color": "Fire",
        "HP": 3.0,
        "HPCurve": 1,
        "ATK": 4.8,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.3
        },
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "1113"
        },
        "Icon": [
            "mita"
        ],
        "Grade": 2,
        "Weight": 150,
        "EndureTemplateID": 210,
        "Drop": [
            {
                "Type": 7,
                "Amount": [
                    0,
                    1.5758,
                    0.4202,
                    0.1051
                ]
            },
            {
                "Type": 4,
                "Amount": [
                    0.5253,
                    0.1401,
                    0.035,
                    0
                ]
            }
        ],
        "Csx": [
            "ECHilichurls/火斧丘丘暴徒"
        ]
    },
    "10203": {
        "_id": 10203,
        "KingdomID": 1,
        "ClassID": 101020,
        "Name": {
            "CH": "岩盾丘丘暴徒",
            "EN": "Rock Shieldwall Mitachurl"
        },
        "Title": [
            {
                "CH": "丘丘暴徒 · 岩盾",
                "EN": "Mitachurl: Rock Shieldwall"
            }
        ],
        "Desc": {
            "CH": "以顽岩为盾的丘丘人大块头。\n对于丘丘人而言，最重要的事情就是吃肉和打架，让身体变得强壮。\n如果能成为丘丘暴徒这样又大又壮的战士，就能吃更多肉、打更多架、举起（几乎）坚不可摧的巨大岩盾了。",
            "EN": "A muscular hilichurl that uses a rock as a shield.\nFighting and eating meat are the two most important things to hilichurls, because those are what make them grow stronger.\nAll young hilichurls dream of growing up to become a big and strong mitachurl, because that way they can eat more meat, fight more battles, and wield those (almost) impregnable giant rock shields."
        },
        "Color": "Rock",
        "HP": 3.0,
        "HPCurve": 1,
        "ATK": 2.4,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.3
        },
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "1113"
        },
        "Icon": [
            "mita"
        ],
        "Grade": 2,
        "Weight": 150,
        "EndureTemplateID": 210,
        "Drop": [
            {
                "Type": 7,
                "Amount": [
                    0,
                    1.5758,
                    0.4202,
                    0.1051
                ]
            },
            {
                "Type": 4,
                "Amount": [
                    0.5253,
                    0.1401,
                    0.035,
                    0
                ]
            }
        ],
        "Csx": [
            "ECHilichurls/岩盾丘丘暴徒"
        ]
    },
    "10204": {
        "_id": 10204,
        "KingdomID": 1,
        "ClassID": 101021,
        "Name": {
            "CH": "丘丘霜铠王",
            "EN": "Frostarm Lawachurl"
        },
        "Title": [
            {
                "CH": "冰霜刻印的丘丘王者",
                "EN": "Ice Sealed Lawachurl"
            }
        ],
        "Desc": {
            "CH": "雪雾中漫行的神秘「王者」。\n被同胞称为「Lawa」的庞大丘丘人。锻炼至极致的肉身似乎已和环境中满溢的冰元素相互融合，沉默威严有如北国的冰川。",
            "EN": "A mysterious \"lord\" of hilichurls that stalks the snow and fog.\nThis huge hilichurl is referred to as a \"lawa\" by its brethren. Its body, honed to the very limit, almost seems to blend into its Cryo-filled environment, and its stoic nobility is like that of the frozen northland rivers."
        },
        "Color": "Ice",
        "HP": 9.0,
        "HPCurve": 1,
        "ATK": 12.0,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.5,
            "Ice": 0.7
        },
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "6666"
        },
        "Icon": [
            "flc"
        ],
        "Grade": 2,
        "Weight": 150,
        "EndureTemplateID": 400,
        "SkillDMG": [
            {
                "Name": {
                    "CH": "勾拳+砸地",
                    "EN": "Fist + Smash"
                },
                "DMG": [
                    4.5,
                    4.8
                ]
            },
            {
                "Name": {
                    "CH": "冲刺 + 留下炸弹",
                    "EN": "Rush + Leave bombs on ground"
                },
                "DMG": [
                    6,
                    3
                ]
            },
            {
                "Name": {
                    "CH": "手扫出半圈冰晶 + 冰晶爆炸",
                    "EN": "Swipe ice shards + ice shard explosion"
                },
                "DMG": [
                    4.5,
                    6
                ]
            },
            {
                "Name": {
                    "CH": "跳砸",
                    "EN": "Jump and land"
                },
                "DMG": [
                    12
                ]
            },
            {
                "Name": {
                    "CH": "二连砸地",
                    "EN": "Double smash"
                },
                "DMG": [
                    6,
                    7.5
                ]
            },
            {
                "Name": {
                    "CH": "扔史莱姆",
                    "EN": "Throw slime"
                },
                "DMG": [
                    7.5
                ]
            }
        ],
        "Drop": [
            {
                "Type": 7,
                "Amount": [
                    0,
                    5.2526,
                    1.4006,
                    0.3502
                ]
            },
            {
                "Type": 4,
                "Amount": [
                    0.5253,
                    0.1401,
                    0.035,
                    0
                ]
            }
        ],
        "Csx": [
            "ECHilichurls/丘丘霜铠王"
        ]
    },
    "10205": {
        "_id": 10205,
        "KingdomID": 1,
        "ClassID": 101021,
        "Name": {
            "CH": "丘丘岩盔王",
            "EN": "Stonehide Lawachurl"
        },
        "Title": [
            {
                "CH": "磐岩刻印的丘丘王者",
                "EN": "Stone Sealed Lawachurl"
            }
        ],
        "Desc": {
            "CH": "千嶂中漫行的神秘「王者」。\n被同胞称为「Lawa」的庞大丘丘人。环境中增生的岩元素化成了它们身体的一部分，仿佛血肉与皮肤皆由不移的山岩雕琢而成。",
            "EN": "A mysterious \"lord\" of hilichurls that walks between a thousand peaks.\nThis huge hilichurl is referred to as a \"lawa\" by its brethren. The abundant Geo energies in the environment have become part of their flesh, blood, and skin, which look as if they were chiseled out of mountain rock."
        },
        "Color": "Rock",
        "HP": 9.0,
        "HPCurve": 1,
        "ATK": 9.0,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.5,
            "Rock": 0.7
        },
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "6666"
        },
        "Icon": [
            "slc"
        ],
        "Grade": 2,
        "Weight": 150,
        "EndureTemplateID": 400,
        "SkillDMG": [
            {
                "Name": {
                    "CH": "勾拳+砸地",
                    "EN": "Fist + Smash"
                },
                "DMG": [
                    3.6,
                    4.8
                ]
            },
            {
                "Name": {
                    "CH": "单勾拳",
                    "EN": "Single fist strike"
                },
                "DMG": [
                    4.5
                ]
            },
            {
                "Name": {
                    "CH": "冲刺",
                    "EN": "Rush"
                },
                "DMG": [
                    6
                ]
            },
            {
                "Name": {
                    "CH": "跳砸",
                    "EN": "Jump and land"
                },
                "DMG": [
                    9
                ]
            },
            {
                "Name": {
                    "CH": "三连砸地",
                    "EN": "Triple smash"
                },
                "DMG": [
                    3,
                    3,
                    3
                ]
            },
            {
                "Name": {
                    "CH": "扔史莱姆",
                    "EN": "Throw slime"
                },
                "DMG": [
                    7.5
                ]
            }
        ],
        "Drop": [
            {
                "Type": 7,
                "Amount": [
                    0,
                    5.2526,
                    1.4006,
                    0.3502
                ]
            },
            {
                "Type": 4,
                "Amount": [
                    0.5253,
                    0.1401,
                    0.035,
                    0
                ]
            }
        ],
        "Csx": [
            "ECHilichurls/丘丘岩盔王"
        ]
    },
    "10206": {
        "_id": 10206,
        "KingdomID": 1,
        "ClassID": 101020,
        "Name": {
            "CH": "冰盾丘丘暴徒",
            "EN": "Ice Shieldwall Mitachurl"
        },
        "Title": [
            {
                "CH": "丘丘暴徒 · 冰盾",
                "EN": "Mitachurl: Ice Shieldwall"
            }
        ],
        "Desc": {
            "CH": "以严冰为盾的丘丘人大块头，会利用坚冰的巨盾战斗。\n普遍而言，体积庞大、活动频繁的生物需要消耗大量的热量来维持生存。由此，当代的艾尔 · 马斯克提出了「暴徒指数」，根据某地区中活动的丘丘暴徒数量在丘丘人总人口中占的比例，计算这些丘丘人食物获取的丰富、稳定性。",
            "EN": "A muscular hilichurl that uses solid ice as a shield, wielding it in battle.\nReason normally dictates that large creatures that see a lot of action should burn through a great number of calories to sustain themselves. As such, the present-day El Musk has proposed the \"Mitachurl Index.\" This index correlates the percentage of mitachurls within the hilichurl population to the stability and success of hilichurl foraging activity."
        },
        "Color": "Ice",
        "HP": 3.0,
        "HPCurve": 1,
        "ATK": 2.4,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.3
        },
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "1113"
        },
        "Icon": [
            "mita"
        ],
        "Grade": 2,
        "Weight": 150,
        "EndureTemplateID": 210,
        "Drop": [
            {
                "Type": 7,
                "Amount": [
                    0,
                    1.5758,
                    0.4202,
                    0.1051
                ]
            },
            {
                "Type": 4,
                "Amount": [
                    0.5253,
                    0.1401,
                    0.035,
                    0
                ]
            }
        ],
        "Csx": [
            "ECHilichurls/冰盾丘丘暴徒"
        ]
    },
    "10207": {
        "_id": 10207,
        "KingdomID": 1,
        "ClassID": 101020,
        "Name": {
            "CH": "雷斧丘丘暴徒",
            "EN": "Crackling Axe Mitachurl"
        },
        "Title": [
            {
                "CH": "丘丘暴徒 · 雷斧",
                "EN": "Mitachurl: Crackling Axe"
            }
        ],
        "Desc": {
            "CH": "持长柄大斧的丘丘人大块头。\n攻击大开大合，会利用雷史莱姆为战斧附上雷元素，具有强大的破坏力。\n钢铁锻造的斧头是非常良好的感电材料。高大魁梧的雷斧丘丘暴徒高举大斧，召引天雷的样子对丘丘人而言可是相当雄壮的景象。",
            "EN": "Muscular hilichurls who wield large, two-handed axes.\nAttacks with large strikes and uses Electro Slimes to infuse their axes with Electro. Their blows are very lethal.\nAxes forged from steel are highly conductive material. The image of those tall and strong Crackling Axe Mitachurls holding their axes up to summon lightning is quite majestic to the hilichurls."
        },
        "Color": "Elec",
        "HP": 3.0,
        "HPCurve": 1,
        "ATK": 4.8,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.3
        },
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "1113"
        },
        "Icon": [
            "mita"
        ],
        "Grade": 2,
        "Weight": 150,
        "EndureTemplateID": 210,
        "Drop": [
            {
                "Type": 7,
                "Amount": [
                    0,
                    1.5758,
                    0.4202,
                    0.1051
                ]
            },
            {
                "Type": 4,
                "Amount": [
                    0.5253,
                    0.1401,
                    0.035,
                    0
                ]
            }
        ],
        "Csx": [
            "ECHilichurls/雷斧丘丘暴徒"
        ]
    },
    "10208": {
        "_id": 10208,
        "KingdomID": 1,
        "ClassID": 101021,
        "Name": {
            "CH": "丘丘雷兜王",
            "EN": "Thunderhelm Lawachurl"
        },
        "Title": [
            {
                "CH": "狂雷刻印的丘丘王者",
                "EN": "Storm Sealed Lawachurl"
            }
        ],
        "Desc": {
            "CH": "雷暴中漫行的神秘「王者」。\n被同胞称为「Lawa」的庞大丘丘人。巨像般的身躯充满了丰富的雷元素，散逸出的元素能量以电气水晶的形式从身体的缝隙中生长出来。",
            "EN": "A mysterious \"lord\" of hilichurls that strides through the thunderstorm.\nThis huge hilichurl is referred to as \"Lawa\" by its brethren. Its giant body is so saturated with Electro energy that Elemental Energy seeps through the cracks in its body in the form of Electro Crystal growths."
        },
        "Color": "Elec",
        "HP": 9.0,
        "HPCurve": 1,
        "ATK": 12.0,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.5,
            "Elec": 0.7
        },
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "6666"
        },
        "Icon": [
            "tlc"
        ],
        "Grade": 2,
        "Weight": 150,
        "EndureTemplateID": 400,
        "SkillDMG": [
            {
                "Name": {
                    "CH": "勾拳+砸地",
                    "EN": "Fist + Smash"
                },
                "DMG": [
                    4.5,
                    4.8
                ]
            },
            {
                "Name": {
                    "CH": "张开双臂飞扑",
                    "EN": "Flying hug"
                },
                "DMG": [
                    7.5
                ]
            },
            {
                "Name": {
                    "CH": "手伏地冲刺",
                    "EN": "Rush"
                },
                "DMG": [
                    6
                ]
            },
            {
                "Name": {
                    "CH": "跳砸",
                    "EN": "Jump and land"
                },
                "DMG": [
                    12
                ]
            },
            {
                "Name": {
                    "CH": "二连砸地",
                    "EN": "Double smash"
                },
                "DMG": [
                    6,
                    7.5
                ]
            },
            {
                "Name": {
                    "CH": "扔史莱姆 + 史莱姆分裂炸",
                    "EN": "Throw slime + Slime split"
                },
                "DMG": [
                    7.5,
                    6
                ]
            },
            {
                "Name": {
                    "CH": "电气水晶 | 生成 + 爆炸",
                    "EN": "Electro Crystal | Formation + Explosion"
                },
                "DMG": [
                    6,
                    6
                ]
            }
        ],
        "Drop": [
            {
                "Type": 7,
                "Amount": [
                    0,
                    5.2526,
                    1.4006,
                    0.3502
                ]
            },
            {
                "Type": 4,
                "Amount": [
                    0.5253,
                    0.1401,
                    0.035,
                    0
                ]
            }
        ],
        "Csx": [
            "ECHilichurls/丘丘雷兜王"
        ]
    },
    "10301": {
        "_id": 10301,
        "KingdomID": 1,
        "ClassID": 10103,
        "Name": {
            "CH": "水丘丘萨满",
            "EN": "Hydro Samachurl"
        },
        "Title": [
            {
                "CH": "水丘丘萨满",
                "EN": "Hydro Samachurl"
            }
        ],
        "Desc": {
            "CH": "嘟嘟囔囔的老年丘丘人，水元素的吟颂者。\n在元素掌控方面颇富天资的丘丘人，其能力往往会在年老时达到顶峰。究竟是怎样的过去，令这些愚昧的生物竟能掌握生水唤雨的秘诀？",
            "EN": "A wizened, mumbling hilichurl, one that chants of the power of Hydro.\nHilichurls especially gifted in commanding the elements who often reach the zenith of their skill in their twilight years. What sort of life would lead to such simple creatures being able to summon water and rains?"
        },
        "Color": "Water",
        "HP": 1.0,
        "HPCurve": 1,
        "ATK": 4.0,
        "ATKCurve": 3,
        "RESBase": {
            "Water": 0.5
        },
        "Ball": {
            "Type": 2,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "hsc"
        ],
        "Grade": 1,
        "Weight": 75,
        "EndureTemplateID": 50,
        "Drop": [
            {
                "Type": 6,
                "Amount": [
                    2.5212,
                    0.6723,
                    0.168,
                    0
                ]
            },
            {
                "Type": 4,
                "Amount": [
                    0.1681,
                    0.0448,
                    0.0112,
                    0
                ]
            }
        ],
        "Csx": [
            "ECHilichurls/水丘丘萨满"
        ]
    },
    "10302": {
        "_id": 10302,
        "KingdomID": 1,
        "ClassID": 10103,
        "Name": {
            "CH": "草丘丘萨满",
            "EN": "Dendro Samachurl"
        },
        "Title": [
            {
                "CH": "草丘丘萨满",
                "EN": "Dendro Samachurl"
            }
        ],
        "Desc": {
            "CH": "嘟嘟囔囔的老年丘丘人，草元素的唤醒者。\n在元素掌控方面颇富天资的丘丘人，其能力往往会在年老时达到顶峰。承载着面具下的不甘，憎恨人心的毒藤随呢喃蔓生而出。",
            "EN": "A wizened, mumbling hilichurl, one that awakens the might of Dendro.\nHilichurls especially gifted in commanding the elements who often reach the zenith of their skill in their twilight years. A grudge stirs beneath that mask, and poison ivy sprouts as hateful whispers issue forth from it."
        },
        "Color": "Grass",
        "HP": 1.0,
        "HPCurve": 1,
        "ATK": 4.0,
        "ATKCurve": 3,
        "RESBase": {
            "Grass": 0.5
        },
        "Ball": {
            "Type": 5,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "dsc"
        ],
        "Grade": 1,
        "Weight": 75,
        "EndureTemplateID": 50,
        "Drop": [
            {
                "Type": 6,
                "Amount": [
                    2.5212,
                    0.6723,
                    0.168,
                    0
                ]
            },
            {
                "Type": 4,
                "Amount": [
                    0.1681,
                    0.0448,
                    0.0112,
                    0
                ]
            }
        ],
        "Csx": [
            "ECHilichurls/草丘丘萨满"
        ]
    },
    "10303": {
        "_id": 10303,
        "KingdomID": 1,
        "ClassID": 10103,
        "Name": {
            "CH": "风丘丘萨满",
            "EN": "Anemo Samachurl"
        },
        "Title": [
            {
                "CH": "风丘丘萨满",
                "EN": "Anemo Samachurl"
            }
        ],
        "Desc": {
            "CH": "嘟嘟囔囔的老年丘丘人，风元素的传信者。\n在元素掌控方面颇富天资的丘丘人，其能力往往会在年老时达到顶峰，其牵掣流风的能力，正是来源于黑暗晦暝的失落记忆。",
            "EN": "A wizened, mumbling hilichurl, one that spreads the message of Anemo.\nHilichurls especially gifted in commanding the elements who often reach the zenith of their skill in their twilight years. Their ability to command the flowing winds stems from dark, forgotten memories."
        },
        "Color": "Wind",
        "HP": 1.0,
        "HPCurve": 1,
        "ATK": 4.0,
        "ATKCurve": 3,
        "RESBase": {
            "Wind": 0.5
        },
        "Ball": {
            "Type": 6,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "asc"
        ],
        "Grade": 1,
        "Weight": 75,
        "EndureTemplateID": 50,
        "Drop": [
            {
                "Type": 6,
                "Amount": [
                    2.5212,
                    0.6723,
                    0.168,
                    0
                ]
            },
            {
                "Type": 4,
                "Amount": [
                    0.1681,
                    0.0448,
                    0.0112,
                    0
                ]
            }
        ],
        "Csx": [
            "ECHilichurls/风丘丘萨满"
        ]
    },
    "10304": {
        "_id": 10304,
        "KingdomID": 1,
        "ClassID": 10103,
        "Name": {
            "CH": "岩丘丘萨满",
            "EN": "Geo Samachurl"
        },
        "Title": [
            {
                "CH": "岩丘丘萨满",
                "EN": "Geo Samachurl"
            }
        ],
        "Desc": {
            "CH": "嘟嘟囔囔的老年丘丘人，岩元素的倾听者。\n在元素掌控方面颇富天资的丘丘人，其能力往往会在年老时达到顶峰。同千古的顽岩相比，即使最年老的丘丘人也不过孩童，但记忆与经验引导它们获得了操使岩石的权能。",
            "EN": "A wizened, mumbling hilichurl, one that listens to the strength of Geo.\nHilichurls especially gifted in commanding the elements who often reach the zenith of their skill in their twilight years. To the persevering mountains, these old hilichurls are but children, but their memories and experiences have guided them towards gaining power over rock and stone."
        },
        "Color": "Rock",
        "HP": 1.0,
        "HPCurve": 1,
        "ATK": 4.0,
        "ATKCurve": 3,
        "RESBase": {
            "Rock": 0.5
        },
        "Ball": {
            "Type": 7,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "gsc"
        ],
        "Grade": 1,
        "Weight": 75,
        "EndureTemplateID": 50,
        "Drop": [
            {
                "Type": 6,
                "Amount": [
                    2.5212,
                    0.6723,
                    0.168,
                    0
                ]
            },
            {
                "Type": 4,
                "Amount": [
                    0.1681,
                    0.0448,
                    0.0112,
                    0
                ]
            }
        ],
        "Csx": [
            "ECHilichurls/岩丘丘萨满"
        ]
    },
    "10305": {
        "_id": 10305,
        "KingdomID": 1,
        "ClassID": 10103,
        "Name": {
            "CH": "冰丘丘萨满",
            "EN": "Cryo Samachurl"
        },
        "Title": [
            {
                "CH": "冰丘丘萨满",
                "EN": "Cryo Samachurl"
            }
        ],
        "Desc": {
            "CH": "嘟嘟囔囔的老年丘丘人，冰元素的呼求者。\n在元素掌控方面颇富天资的丘丘人，其能力往往会在年老时达到顶峰。这些萨满熟忆寒霜的低语，更精通将之用于制敌的技艺。",
            "EN": "A wizened, mumbling hilichurl, one that calls upon the bite of Cryo.\nHilichurls especially gifted in commanding the elements who often reach the zenith of their skill in their twilight years. These shamans remember the frosty chants and are even more skilled in the art of using them to entrap their foes."
        },
        "Color": "Ice",
        "HP": 1.0,
        "HPCurve": 1,
        "ATK": 4.0,
        "ATKCurve": 3,
        "RESBase": {
            "Ice": 0.5
        },
        "Ball": {
            "Type": 4,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "csc"
        ],
        "Grade": 1,
        "Weight": 75,
        "EndureTemplateID": 50,
        "Drop": [
            {
                "Type": 6,
                "Amount": [
                    2.5212,
                    0.6723,
                    0.168,
                    0
                ]
            },
            {
                "Type": 4,
                "Amount": [
                    0.1681,
                    0.0448,
                    0.0112,
                    0
                ]
            }
        ],
        "Csx": [
            "ECHilichurls/冰丘丘萨满"
        ]
    },
    "10306": {
        "_id": 10306,
        "KingdomID": 1,
        "ClassID": 10103,
        "Name": {
            "CH": "雷丘丘萨满",
            "EN": "Electro Samachurl"
        },
        "Title": [
            {
                "CH": "雷丘丘萨满",
                "EN": "Electro Samachurl"
            }
        ],
        "Desc": {
            "CH": "嘟嘟囔囔的老年丘丘人，雷元素的祈求者。\n在元素掌控方面颇富天资的丘丘人，其能力往往会在年老时达到顶峰，既然掌握了掣电引雷的高超技艺，便用这霹雳之声谱写挽歌吧。",
            "EN": "A wizened, mumbling Hilichurl, one that prays to Electro.\nHilichurls especially gifted in commanding elements who often reach the zenith of their skill in their twilight years. Since they possess such advanced skill in creating electricity and summoning thunder, you may as well use the crackling sound to compose an elegy."
        },
        "Color": "Elec",
        "HP": 1.0,
        "HPCurve": 1,
        "ATK": 4.0,
        "ATKCurve": 3,
        "RESBase": {
            "Elec": 0.5
        },
        "Ball": {
            "Type": 3,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "esc"
        ],
        "Grade": 1,
        "Weight": 75,
        "EndureTemplateID": 50,
        "Drop": [
            {
                "Type": 6,
                "Amount": [
                    2.5212,
                    0.6723,
                    0.168,
                    0
                ]
            },
            {
                "Type": 4,
                "Amount": [
                    0.1681,
                    0.0448,
                    0.0112,
                    0
                ]
            }
        ],
        "Csx": [
            "ECHilichurls/雷丘丘萨满"
        ]
    },
    "10401": {
        "_id": 10401,
        "KingdomID": 1,
        "ClassID": 10104,
        "Name": {
            "CH": "丘丘风行游侠",
            "EN": "Anemo Hilichurl Rogue"
        },
        "Title": [
            {
                "CH": "永恒异乡人",
                "EN": "The Eternal Outsider"
            }
        ],
        "Desc": {
            "CH": "不属于任何部族，似乎是在追寻一族命运之肇始，寻找如今已经失落的光辉的丘丘人流浪者。\n假使这些只有简单语言，也不具备文书传承的魔物当中，有史诗、有传奇，它们应当就是其中的主角吧。",
            "EN": "Hilichurl wanderers who do not belong to any hilichurl tribe. It almost seems as if they are searching for the fated genesis of their kind, seeking to uncover some bygone splendor that once belonged to their people.\nIf these monsters capable of only crude language and bereft of missing history could record their own epics and legends, the hilichurl wanderers would be the heroes in said tales."
        },
        "Color": "Wind",
        "HP": 2.5,
        "HPCurve": 2,
        "ATK": 5.0,
        "ATKCurve": 4,
        "RESBase": {
            "Wind": 0.5
        },
        "Ball": {
            "Type": 6,
            "Point": 3,
            "Drop": "111"
        },
        "Icon": [
            "ahilr"
        ],
        "Grade": 2,
        "Weight": 150,
        "EndureTemplateID": 210,
        "SkillData": [
            "1/10401"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "大招：飞天三风刃",
                    "EN": "Ultimate: Fly & Triple Wind Blade"
                },
                "DMG": [
                    4,
                    4,
                    5
                ]
            },
            {
                "Name": {
                    "CH": "普通攻击x3",
                    "EN": "Normal Attack x3"
                },
                "DMG": [
                    3,
                    3,
                    4
                ]
            },
            {
                "Name": {
                    "CH": "旋转斩",
                    "EN": "Circular Strike"
                },
                "DMG": [
                    4
                ]
            },
            {
                "Name": {
                    "CH": "手里剑三连扔",
                    "EN": "Throw 3 Frisbees"
                },
                "DMG": [
                    3,
                    3,
                    4.35
                ]
            },
            {
                "Name": {
                    "CH": "手里剑回旋镖",
                    "EN": "Boomerang"
                },
                "DMG": [
                    4.25
                ]
            }
        ],
        "Drop": [
            {
                "Type": 27,
                "Amount": [
                    0,
                    5.2526,
                    1.4006,
                    0.3502
                ]
            }
        ],
        "Csx": [
            "ECHilichurls/丘丘风行游侠"
        ]
    },
    "10402": {
        "_id": 10402,
        "KingdomID": 1,
        "ClassID": 10104,
        "Name": {
            "CH": "丘丘水行游侠",
            "EN": "Hydro Hilichurl Rogue"
        },
        "Title": [
            {
                "CH": "永恒异乡人",
                "EN": "The Eternal Outsider"
            }
        ],
        "Desc": {
            "CH": "不属于任何部族的丘丘人流浪者，如同自我流放一般在荒野中四处漫游。\n会在因缘的指引下帮助需要帮助的丘丘人、丘丘聚落，并在完成使命、因缘结束后启程离开。相比寻常的丘丘人似乎有更高的智能水平。",
            "EN": "Hilichurl wanderers unaffiliated with any specific tribe. They can often be found roving the wilds as if in voluntary exile.\nThey may also lend their aid to other hilichurls or hilichurl tribes for various reasons and go their separate ways afterward. They seem to possess a higher level of intelligence than commonplace hilichurls."
        },
        "Color": "Water",
        "HP": 2.5,
        "HPCurve": 2,
        "ATK": 5.0,
        "ATKCurve": 4,
        "RESBase": {
            "Water": 0.5
        },
        "Ball": {
            "Type": 2,
            "Point": 3,
            "Drop": "111"
        },
        "Icon": [
            "hhilr"
        ],
        "Grade": 2,
        "Weight": 150,
        "EndureTemplateID": 210,
        "SkillData": [
            "1/10402"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "大招：水泡 + 扔史莱姆",
                    "EN": "Ultimate: Bubbles + Throw Slime"
                },
                "DMG": [
                    2.5,
                    5
                ]
            },
            {
                "Name": {
                    "CH": "普通攻击x2",
                    "EN": "Normal Attack x2"
                },
                "DMG": [
                    4,
                    5
                ]
            },
            {
                "Name": {
                    "CH": "冲刺砍",
                    "EN": "Rush towards you and strike!"
                },
                "DMG": [
                    5
                ]
            },
            {
                "Name": {
                    "CH": "掰直镰刀，戳",
                    "EN": "Pierce"
                },
                "DMG": [
                    4
                ]
            },
            {
                "Name": {
                    "CH": "回旋斩",
                    "EN": "Circular Strike"
                },
                "DMG": [
                    5
                ]
            },
            {
                "Name": {
                    "CH": "扔3史莱姆",
                    "EN": "Throw 3 Slimes"
                },
                "DMG": [
                    4
                ]
            }
        ],
        "Drop": [
            {
                "Type": 27,
                "Amount": [
                    0,
                    5.2526,
                    1.4006,
                    0.3502
                ]
            }
        ],
        "Csx": [
            "ECHilichurls/丘丘水行游侠"
        ]
    },
    "20101": {
        "_id": 20101,
        "KingdomID": 2,
        "ClassID": 10201,
        "Name": {
            "CH": "火深渊法师",
            "EN": "Pyro Abyss Mage"
        },
        "Title": [
            {
                "CH": "火之咏者",
                "EN": "Pyrochanter"
            }
        ],
        "Desc": {
            "CH": "拥有魔力，利用火元素战斗的深渊魔物。\n出于某种不为人知的缘由，深渊法师精通丘丘人的笨拙语言，能驾驭它们的心智。据说深渊法师略通如今几乎无人可解的失落古语，并利用这种语言的力量行使魔法。",
            "EN": "Abyss creatures who command the power of Pyro in battle.\nFor unknown reasons, Abyss Mages are able to communicate with the hilichurls, thus enabling them to manipulate their minds easily. They say that Abyss Mages utter a long-forgotten language from which they draw magical power."
        },
        "Color": "Fire",
        "HP": 2.0,
        "HPCurve": 1,
        "ATK": 4.5,
        "ATKCurve": 3,
        "Ball": {
            "Type": 1,
            "Point": 4,
            "Drop": "1113"
        },
        "Icon": [
            "pam"
        ],
        "Grade": 1,
        "Weight": 75,
        "EndureTemplateID": 100,
        "SkillDMG": [
            {
                "Name": {
                    "CH": "闪现伤害",
                    "EN": "Teleport DMG"
                },
                "DMG": [
                    3
                ]
            },
            {
                "Name": {
                    "CH": "小火弹",
                    "EN": "Fire Bullet"
                },
                "DMG": [
                    2.25
                ]
            },
            {
                "Name": {
                    "CH": "三角阵喷火",
                    "EN": "Triangle Fire Breath"
                },
                "DMG": [
                    3
                ]
            },
            {
                "Name": {
                    "CH": "生盾期间地火",
                    "EN": "Shield Regen Groundfire"
                },
                "DMG": [
                    4.5
                ]
            },
            {
                "Name": {
                    "CH": "生盾成功冲击波",
                    "EN": "Shield Regen Success Shockwave"
                },
                "DMG": [
                    4.5
                ]
            }
        ],
        "Drop": [
            {
                "Type": 14,
                "Amount": [
                    0,
                    2.2324,
                    0.5953,
                    0.1487
                ]
            }
        ],
        "Csx": [
            "ECAbyss/火深渊法师"
        ]
    },
    "20102": {
        "_id": 20102,
        "KingdomID": 2,
        "ClassID": 10201,
        "Name": {
            "CH": "冰深渊法师",
            "EN": "Cryo Abyss Mage"
        },
        "Title": [
            {
                "CH": "冰之咏者",
                "EN": "Cryochanter"
            }
        ],
        "Desc": {
            "CH": "驾驭着极寒的冰元素进行战斗的深渊魔物。\n尽管染指了强大的力量，甚至能将大气中的水蒸气凝结成冰锥，本身却佝偻而孱弱。一旦保护自身的护罩被打破，就只能任人鱼肉了。",
            "EN": "Abyss creatures that call upon Cryo in battle.\nThough they dabble with great power and can even create icicles by condensing and freezing water vapor in the atmosphere, their bodies are very vulnerable in and of themselves. Once their protective barrier has been broken, they are at your mercy."
        },
        "Color": "Ice",
        "HP": 2.0,
        "HPCurve": 1,
        "ATK": 3.0,
        "ATKCurve": 3,
        "Ball": {
            "Type": 4,
            "Point": 4,
            "Drop": "1113"
        },
        "Icon": [
            "cam"
        ],
        "Grade": 1,
        "Weight": 75,
        "EndureTemplateID": 100,
        "SkillDMG": [
            {
                "Name": {
                    "CH": "闪现伤害",
                    "EN": "Teleport DMG"
                },
                "DMG": [
                    3
                ]
            },
            {
                "Name": {
                    "CH": "小冰锥",
                    "EN": "Small Ice Cone"
                },
                "DMG": [
                    3
                ]
            },
            {
                "Name": {
                    "CH": "坠落冰锥",
                    "EN": "Dropping Icicle"
                },
                "DMG": [
                    3
                ]
            },
            {
                "Name": {
                    "CH": "（强攻强化）坠落冰锥生成冰雾 | 每0.5s伤害",
                    "EN": "(Attack Enhance) Icicle Ice Fog Area | DMG every 0.5s"
                },
                "DMG": [
                    0.9
                ]
            },
            {
                "Name": {
                    "CH": "生盾期间冰锥",
                    "EN": "Shield Regen Ice Cone"
                },
                "DMG": [
                    2.1
                ]
            },
            {
                "Name": {
                    "CH": "生盾成功冲击波",
                    "EN": "Shield Regen Success Shockwave"
                },
                "DMG": [
                    4.5
                ]
            }
        ],
        "Drop": [
            {
                "Type": 14,
                "Amount": [
                    0,
                    2.2324,
                    0.5953,
                    0.1487
                ]
            }
        ],
        "Csx": [
            "ECAbyss/冰深渊法师"
        ]
    },
    "20103": {
        "_id": 20103,
        "KingdomID": 2,
        "ClassID": 10201,
        "Name": {
            "CH": "水深渊法师",
            "EN": "Hydro Abyss Mage"
        },
        "Title": [
            {
                "CH": "水之咏者",
                "EN": "Hydrochanter"
            }
        ],
        "Desc": {
            "CH": "利用水元素战斗的深渊魔物。\n在冒险者当中有传说说，深渊法师作为「深渊教团」底层的执行者，活跃在世界各地、从事各种阴谋破坏。尽管他们还戏称这种水深渊法师的最大危害是会导致风湿发作，其实它们有着不容小觑的力量。",
            "EN": "Abyss creatures that utilize Hydro energy in battle.\nStories circulate among adventurers that Abyss Mages are at the lowest rung of the Abyss Order's hierarchy and that they roam the various lands to scheme fell deeds of destruction. Though they also often make fun of these Abyss Mages, saying that the most harm they can do is to cause rheumatism or a common cold, their power is not to be underestimated."
        },
        "Color": "Water",
        "HP": 2.0,
        "HPCurve": 1,
        "ATK": 3.0,
        "ATKCurve": 3,
        "Ball": {
            "Type": 2,
            "Point": 4,
            "Drop": "1113"
        },
        "Icon": [
            "ham"
        ],
        "Grade": 1,
        "Weight": 75,
        "EndureTemplateID": 100,
        "SkillDMG": [
            {
                "Name": {
                    "CH": "闪现伤害",
                    "EN": "Teleport DMG"
                },
                "DMG": [
                    3
                ]
            },
            {
                "Name": {
                    "CH": "小水弹",
                    "EN": "Small Ice Cone"
                },
                "DMG": [
                    3
                ]
            },
            {
                "Name": {
                    "CH": "水泡包裹",
                    "EN": "Water Bubble Encase"
                },
                "DMG": [
                    3
                ]
            },
            {
                "Name": {
                    "CH": "生盾期间水泡",
                    "EN": "Shield Regen Bubbles"
                },
                "DMG": [
                    3
                ]
            },
            {
                "Name": {
                    "CH": "生盾成功冲击波",
                    "EN": "Shield Regen Success Shockwave"
                },
                "DMG": [
                    4.5
                ]
            }
        ],
        "Drop": [
            {
                "Type": 14,
                "Amount": [
                    0,
                    2.2324,
                    0.5953,
                    0.1487
                ]
            }
        ],
        "Csx": [
            "ECAbyss/水深渊法师"
        ]
    },
    "20104": {
        "_id": 20104,
        "KingdomID": 2,
        "ClassID": 10201,
        "Name": {
            "CH": "雷深渊法师",
            "EN": "Electro Abyss Mage"
        },
        "Title": [
            {
                "CH": "雷之咏者",
                "EN": "Electrochanter"
            }
        ],
        "Desc": {
            "CH": "利用雷元素战斗的深渊魔物。\n曾经有一名学者阿尔瓦 · 尼古拉，在教令院学习时曾对雷深渊法师使用的雷元素模型进行过系统的研究，最终提出了名为「尼古拉线圈」的理论武器，可惜至今无人将其付诸实践。一说认为他的研究资料在蕈兽泛滥的时候被吃掉了。",
            "EN": "Abyss creatures that utilize Electro in battles.\nWhile studying at the Akademiya, researcher Alva Nikola once conducted a systematic study of the methods by which Electro Abyss Mages manipulate Electro and came up with a theoretical weapon known as the Nikola Coil. However, it has yet to be put into practice. Some say that his research materials were eaten during a fungal infestation."
        },
        "Color": "Elec",
        "HP": 2.0,
        "HPCurve": 1,
        "ATK": 3.75,
        "ATKCurve": 3,
        "Ball": {
            "Type": 3,
            "Point": 4,
            "Drop": "1113"
        },
        "Icon": [
            "eam"
        ],
        "Grade": 1,
        "Weight": 75,
        "EndureTemplateID": 100,
        "SkillDMG": [
            {
                "Name": {
                    "CH": "闪现伤害",
                    "EN": "Teleport DMG"
                },
                "DMG": [
                    3.75
                ]
            },
            {
                "Name": {
                    "CH": "小雷球",
                    "EN": "Small Ice Cone"
                },
                "DMG": [
                    3
                ]
            },
            {
                "Name": {
                    "CH": "雷圈开启",
                    "EN": "Electro Circle Appear DMG"
                },
                "DMG": [
                    3
                ]
            },
            {
                "Name": {
                    "CH": "雷圈触碰",
                    "EN": "Electro Circle Touch DMG"
                },
                "DMG": [
                    3
                ]
            },
            {
                "Name": {
                    "CH": "雷圈触碰（强攻强化）",
                    "EN": "Electro Circle Touch DMG (Attack Enhance)"
                },
                "DMG": [
                    3.75
                ]
            },
            {
                "Name": {
                    "CH": "生盾期间雷球",
                    "EN": "Shield Regen Elec Ball"
                },
                "DMG": [
                    1.5
                ]
            },
            {
                "Name": {
                    "CH": "生盾成功冲击波",
                    "EN": "Shield Regen Success Shockwave"
                },
                "DMG": [
                    4.5
                ]
            }
        ],
        "Drop": [
            {
                "Type": 14,
                "Amount": [
                    0,
                    2.2324,
                    0.5953,
                    0.1487
                ]
            }
        ],
        "Csx": [
            "ECAbyss/雷深渊法师"
        ]
    },
    "20201": {
        "_id": 20201,
        "KingdomID": 2,
        "ClassID": 10202,
        "Name": {
            "CH": "深渊使徒 · 激流",
            "EN": "Abyss Herald: Wicked Torrents"
        },
        "Title": [
            {
                "CH": "操纵激流的漆黑先兆",
                "EN": "Dark Usher of Wicked Torrents"
            }
        ],
        "Desc": {
            "CH": "利用水元素战斗的魔物，隶属于「深渊教团」。\n这些沉默的身影是深渊的先行者，挥舞着席卷一切的浪潮之锋，斩碎一切铠甲的防御与不信的异端之心，向无虑的愚氓降下宿命的惩戒。",
            "EN": "Servants of the Abyss Order that use Hydro in combat.\nThese silent figures are the vanguard of the Abyss, brandishing Tidal Blades that can engulf everything in an instant, crumpling armor and obliterating heretics. They shall bring preordained damnation upon the foolish masses."
        },
        "Color": "Water",
        "HP": 8.0,
        "HPCurve": 1,
        "ATK": 6.0,
        "ATKCurve": 3,
        "Ball": {
            "Type": 2,
            "Point": 3,
            "Drop": "332"
        },
        "Icon": [
            "ahwt"
        ],
        "Grade": 2,
        "Weight": 150,
        "EndureTemplateID": 210,
        "Tutorial": {
            "CH": "部分攻击具有「激流附魔」：对角色造成伤害时，所有正在冷却中的技能CD延长6秒。",
            "EN": "Some of its attacks have the \"Abyssal Infusion\". When such attacks deal DMG to the character, all skills currently in CD will have their CD increased by 6s."
        },
        "SkillData": [
            "2/20201"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "<b>开盾冲击</b>",
                    "EN": "<b>Shield Activation Shockwave</b>"
                },
                "DMG": [
                    6
                ]
            },
            {
                "Name": {
                    "CH": "普通攻击3段",
                    "EN": "Melee Attack 3 Strikes"
                },
                "DMG": [
                    1.5,
                    1.5,
                    2.1
                ]
            },
            {
                "Name": {
                    "CH": "普攻额外段：转身斩+双手斩",
                    "EN": "Extra Melee Attack: Turn Cut + 2 Hand Strike"
                },
                "DMG": [
                    2.1,
                    3
                ]
            },
            {
                "Name": {
                    "CH": "一字水刃",
                    "EN": "Single Water Blade"
                },
                "DMG": [
                    3
                ]
            },
            {
                "Name": {
                    "CH": "大旋风",
                    "EN": "Whirlwind"
                },
                "DMG": [
                    2.4
                ]
            },
            {
                "Name": {
                    "CH": "十字水刃（带DEBUFF）",
                    "EN": "Cross Water Blade (Has DEBUFF)"
                },
                "DMG": [
                    3
                ]
            },
            {
                "Name": {
                    "CH": "终结技：八方水刃（带DEBUFF）",
                    "EN": "Ultimate: 8 Water Blades (Has DEBUFF)"
                },
                "DMG": [
                    3
                ]
            }
        ],
        "Drop": [
            {
                "Type": 14,
                "Amount": [
                    0,
                    5.2526,
                    1.4006,
                    0.3502
                ]
            },
            {
                "Type": 15,
                "Amount": [
                    0,
                    0.5253,
                    0.1401,
                    0.035
                ]
            }
        ],
        "Csx": [
            "ECAbyss/深渊使徒·激流"
        ]
    },
    "20202": {
        "_id": 20202,
        "KingdomID": 2,
        "ClassID": 10202,
        "Name": {
            "CH": "深渊使徒 · 霜落",
            "EN": "Abyss Herald: Frost Fall"
        },
        "Title": [
            {
                "CH": "锋刃凝冰的守密敕使",
                "EN": "Emissary of Frigid Blades"
            }
        ],
        "Desc": {
            "CH": "以冰霜的锋刃进行战斗的魔物，隶属于「深渊教团」。\n既然有散播与礼赞深邃福音的圣徒，那就有将歪曲的颂词导正，将不协音去除的使者。\n常有人说穿透身体的钢刀如同冰一般寒冷，但这种漆黑的冰霜据说连灵魂都能冻结。",
            "EN": "A monster from the Abyss Order that wields frigid, icy blades in battle.\nJust as there are saints who will spread and glorify the profound gospel, there are envoys who must correct any twisted strains and remove any dissonance.\nPeople often say that steel feels like ice as it pierces the body, but this dark frost is said to freeze even the soul."
        },
        "Color": "Ice",
        "HP": 8.0,
        "HPCurve": 1,
        "ATK": 4.5,
        "ATKCurve": 4,
        "Ball": {
            "Type": 4,
            "Point": 3,
            "Drop": "332"
        },
        "Icon": [
            "ahff"
        ],
        "Grade": 2,
        "Weight": 150,
        "EndureTemplateID": 210,
        "Tutorial": {
            "CH": "部分攻击具有「渊寒附魔」：对角色造成伤害时，扣除50体力。在3s内重复触发，只扣除20体力。\n\n入战时，附魔概率为 0%。\n如果一次常规攻击没有附魔，则下一次常规攻击附魔的概率 +20%(无盾) 或 +27%(有盾)。\n如果一次常规攻击成功附魔了，则下一次附魔概率清零。\n如果释放了极寒冰锥(必附魔的特殊攻击)，则附魔概率 -61%。\n如果释放了九方之冻(必附魔的特殊攻击)，则附魔概率 -100%。\n开盾时，附魔概率清零，且开盾的伤害一定没有附魔。",
            "EN": "Some of its attacks have the \"Abyssal Infusion\". When such attacks deal DMG to the character, the character loses 50 stamina. If triggered again within 3s, will only deduct 20 stamina."
        },
        "SkillData": [
            "2/20202"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "<b>开盾冲击</b>",
                    "EN": "<b>Shield Activation Shockwave</b>"
                },
                "DMG": [
                    9
                ]
            },
            {
                "Name": {
                    "CH": "普通攻击3段",
                    "EN": "Melee Attack 3 Strikes"
                },
                "DMG": [
                    2.97,
                    2.475,
                    2.475
                ]
            },
            {
                "Name": {
                    "CH": "普攻额外段：后空翻斩",
                    "EN": "Extra Melee Attack: Backflip Strike"
                },
                "DMG": [
                    3.375
                ]
            },
            {
                "Name": {
                    "CH": "蓄力圆形斩 （无盾 / 有盾）",
                    "EN": "Circular Strike (No Shield VS Has Shield)"
                },
                "DMG": [
                    4.05,
                    4.5
                ]
            },
            {
                "Name": {
                    "CH": "冲刺斩",
                    "EN": "Rush Strike"
                },
                "DMG": [
                    3.15
                ]
            },
            {
                "Name": {
                    "CH": "冲刺斩额外段：砸地",
                    "EN": "Rush Strike Extra: Ground Pound"
                },
                "DMG": [
                    3.87
                ]
            },
            {
                "Name": {
                    "CH": "渊寒冰锥（100% DEBUFF）",
                    "EN": "Darkfrost Icicle (100% DEBUFF)"
                },
                "DMG": [
                    3.51
                ]
            },
            {
                "Name": {
                    "CH": "终结技：九方之冻（100% DEBUFF）",
                    "EN": "Ultimate: 4+5 Icicles (100% DEBUFF)"
                },
                "DMG": [
                    2.97
                ]
            }
        ],
        "Drop": [
            {
                "Type": 14,
                "Amount": [
                    0,
                    5.2526,
                    1.4006,
                    0.3502
                ]
            },
            {
                "Type": 15,
                "Amount": [
                    0,
                    0.5253,
                    0.1401,
                    0.035
                ]
            }
        ],
        "Csx": [
            "ECAbyss/深渊使徒·霜落"
        ]
    },
    "20301": {
        "_id": 20301,
        "KingdomID": 2,
        "ClassID": 10202,
        "Name": {
            "CH": "深渊咏者 · 紫电",
            "EN": "Abyss Lector: Violet Lightning"
        },
        "Title": [
            {
                "CH": "降下紫电的福音使者",
                "EN": "Evangelist of Striking Thunder"
            }
        ],
        "Desc": {
            "CH": "驱雷掣电，高颂着漆黑渊薮的魔物，隶属于「深渊教团」。\n它们是深渊的布道者、学士。以狂雷将冥冥长夜所扭曲的深黑智慧，灌入不信者的心中，以无边的紫电，昭告侵蚀人智的伟大力量的存在。",
            "EN": "A monster who serves the Abyss Order and commands lightning while singing the praises of the darkness.\nThese are the evangelists and the scholars of the Abyss. Their violent lightning strikes the hearts of unbelievers with dark wisdom, warped by the shadowy depths of an eternal night, its violet glow proclaiming the existence of a great power that corrodes human intellect."
        },
        "Color": "Elec",
        "HP": 8.0,
        "HPCurve": 1,
        "ATK": 4.5,
        "ATKCurve": 3,
        "Ball": {
            "Type": 3,
            "Point": 3,
            "Drop": "332"
        },
        "Icon": [
            "alvl"
        ],
        "Grade": 2,
        "Weight": 150,
        "EndureTemplateID": 210,
        "Tutorial": {
            "CH": "部分攻击具有「紫电附魔」：对角色造成伤害时，全队角色流失20元素能量。",
            "EN": "Some of its attacks have the \"Abyssal Infusion\". When such attacks deal DMG to the character, all party members lose 20 Elemental Energy."
        },
        "SkillData": [
            "2/20301"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "<b>开盾冲击</b>",
                    "EN": "<b>Shield Activation Shockwave</b>"
                },
                "DMG": [
                    6
                ]
            },
            {
                "Name": {
                    "CH": "小圈落雷",
                    "EN": "Small Circle Lightning"
                },
                "DMG": [
                    3
                ]
            },
            {
                "Name": {
                    "CH": "直线落雷",
                    "EN": "Straight Line Lightning"
                },
                "DMG": [
                    2.4
                ]
            },
            {
                "Name": {
                    "CH": "近身球形雷爆",
                    "EN": "Close-up Sphere Explosion"
                },
                "DMG": [
                    4.5
                ]
            },
            {
                "Name": {
                    "CH": "三嗜能雷球 深渊之光（带DEBUFF）",
                    "EN": "3 Elec Balls (Has DEBUFF)"
                },
                "DMG": [
                    1.5
                ]
            },
            {
                "Name": {
                    "CH": "终结技：八方之雷 高贵的升华（带DEBUFF）",
                    "EN": "Ultimate: 8 Elec Balls (Has DEBUFF)"
                },
                "DMG": [
                    1.5
                ]
            }
        ],
        "Drop": [
            {
                "Type": 14,
                "Amount": [
                    0,
                    5.2526,
                    1.4006,
                    0.3502
                ]
            },
            {
                "Type": 15,
                "Amount": [
                    0,
                    0.5253,
                    0.1401,
                    0.035
                ]
            }
        ],
        "Csx": [
            "ECAbyss/深渊咏者·紫电"
        ]
    },
    "20302": {
        "_id": 20302,
        "KingdomID": 2,
        "ClassID": 10202,
        "Name": {
            "CH": "深渊咏者 · 渊火",
            "EN": "Abyss Lector: Fathomless Flames"
        },
        "Title": [
            {
                "CH": "漆黑渊薮的噬身之火",
                "EN": "The Devouring Benthic Flame"
            }
        ],
        "Desc": {
            "CH": "驱使深渊之火，追寻着章典之意义的魔物。隶属于「深渊教团」。\n地上之人对他们的称呼其实和本意略有偏差。「咏者」也有「读经士」之意。诵读恩典之章，便会拥有强大的力量。",
            "EN": "Wielders of abyssal fire who pursue the meaning behind texts and scriptures. Is a member of the Abyss Order.\nThe surface people have a slightly distorted understanding of what their name means, calling them \"Lectors,\" when their name can also mean \"exegete.\" Reading the word of grace grants them great power."
        },
        "Color": "Fire",
        "HP": 8.0,
        "HPCurve": 1,
        "ATK": 4.5,
        "ATKCurve": 3,
        "Ball": {
            "Type": 1,
            "Point": 3,
            "Drop": "332"
        },
        "Icon": [
            "alff"
        ],
        "Grade": 2,
        "Weight": 150,
        "EndureTemplateID": 210,
        "Tutorial": {
            "CH": "部分攻击具有「渊火附魔」：对角色造成伤害时，全队流失部分生命值。护盾无法抵抗生命值流失。",
            "EN": "Some of its attacks have the \"Abyssal Infusion\". When such attacks deal DMG to the character, all party members lose a part of their HP. Shields do not resist HP loss."
        },
        "SkillData": [
            "2/20302"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "<b>开盾冲击</b>",
                    "EN": "<b>Shield Activation Shockwave</b>"
                },
                "DMG": [
                    6
                ]
            },
            {
                "Name": {
                    "CH": "小火球x5",
                    "EN": "Fireball x5"
                },
                "DMG": [
                    1.2
                ]
            },
            {
                "Name": {
                    "CH": "大火球",
                    "EN": "Big Fireball"
                },
                "DMG": [
                    3
                ]
            },
            {
                "Name": {
                    "CH": "近身圆形爆炸",
                    "EN": "Close-up Circle Explosion"
                },
                "DMG": [
                    4.5
                ]
            },
            {
                "Name": {
                    "CH": "深渊地火（带DEBUFF）",
                    "EN": "Ground Hellfire (Has DEBUFF)"
                },
                "DMG": [
                    1.2
                ]
            },
            {
                "Name": {
                    "CH": "终结技：黯火炉心降世 天云的启示",
                    "EN": "Ultimate: Hellfire Furnace Descends"
                },
                "DMG": [
                    3
                ]
            }
        ],
        "Drop": [
            {
                "Type": 14,
                "Amount": [
                    0,
                    5.2526,
                    1.4006,
                    0.3502
                ]
            },
            {
                "Type": 15,
                "Amount": [
                    0,
                    0.5253,
                    0.1401,
                    0.035
                ]
            }
        ],
        "Csx": [
            "ECAbyss/深渊咏者·渊火"
        ]
    },
    "20401": {
        "_id": 20401,
        "KingdomID": 2,
        "ClassID": 10204,
        "Name": {
            "CH": "嗜岩 · 兽境幼兽",
            "EN": "Rockfond Rifthound Whelp"
        },
        "Title": [
            {
                "CH": "隙间侵入的异兽",
                "EN": "Gap-Infiltrating Aliens"
            }
        ],
        "Desc": {
            "CH": "能够侵蚀世界边界的魔血之兽。「黄金」将其命名为「淋溶」。\n有伴随渊薮的渗透，吞噬元素的习性。值得庆幸的是这些魔兽如今数量并不多。",
            "EN": "A beast with monstrous blood that is capable of eroding the boundaries of the world. \"Gold\" classified them as \"Alfisol.\"\nThey follow the encroaching abyss to devour the elements. Fortunately, there are few of them left now in this world."
        },
        "Color": "Rock",
        "HP": 1.2,
        "HPCurve": 2,
        "ATK": 1.2,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.2,
            "Fire": 0.2,
            "Water": 0.2,
            "Elec": 0.2,
            "Ice": 0.2,
            "Grass": 0.2,
            "Wind": 0.2,
            "Rock": 0.2
        },
        "RESState": [
            {
                "State": "Hound_Activated",
                "Rock": -0.3
            }
        ],
        "Ball": {
            "Type": 7,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "rrhw"
        ],
        "Grade": 2,
        "Weight": 100,
        "EndureTemplateID": 150,
        "SkillData": [
            "2/20401"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "突刺嘴咬",
                    "EN": "Rush Mouth Bite"
                },
                "DMG": [
                    1.02
                ]
            },
            {
                "Name": {
                    "CH": "左手+右手划",
                    "EN": "Left Paw + Right Paw Strike"
                },
                "DMG": [
                    0.78,
                    0.78
                ]
            },
            {
                "Name": {
                    "CH": "闪现背刺",
                    "EN": "Teleport Backstab"
                },
                "DMG": [
                    1.2
                ]
            }
        ],
        "Drop": [
            {
                "Type": 16,
                "Amount": [
                    0,
                    2.6263,
                    0.7003,
                    0.1751
                ]
            }
        ],
        "Csx": [
            "ECAbyss/嗜岩·兽境幼兽"
        ]
    },
    "20402": {
        "_id": 20402,
        "KingdomID": 2,
        "ClassID": 10204,
        "Name": {
            "CH": "嗜雷 · 兽境幼兽",
            "EN": "Thundercraven Rifthound Whelp"
        },
        "Title": [
            {
                "CH": "隙间侵入的异兽",
                "EN": "Gap-Infiltrating Aliens"
            }
        ],
        "Desc": {
            "CH": "能够侵蚀世界边界的魔血之兽。「黄金」将其命名为「淋溶」。\n曾经的年代里，巨大的魔兽降临之时，会有大量猎犬提前显现，溶解世界的边界，打开通道。",
            "EN": "A beast with monstrous blood that is capable of eroding the boundaries of the world. \"Gold\" classified them as \"Alfisol.\"\nIn times long gone, when giant monstrous beasts descended, large swarms of these hounds would go before them to deliquesce the borders of the world and open the way."
        },
        "Color": "Elec",
        "HP": 1.2,
        "HPCurve": 2,
        "ATK": 1.2,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.2,
            "Fire": 0.2,
            "Water": 0.2,
            "Elec": 0.2,
            "Ice": 0.2,
            "Grass": 0.2,
            "Wind": 0.2,
            "Rock": 0.2
        },
        "RESState": [
            {
                "State": "Hound_Activated",
                "Elec": -0.3
            }
        ],
        "Ball": {
            "Type": 3,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "trhw"
        ],
        "Grade": 2,
        "Weight": 100,
        "EndureTemplateID": 150,
        "SkillData": [
            "2/20401"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "突刺嘴咬",
                    "EN": "Rush Mouth Bite"
                },
                "DMG": [
                    1.02
                ]
            },
            {
                "Name": {
                    "CH": "左手+右手划",
                    "EN": "Left Paw + Right Paw Strike"
                },
                "DMG": [
                    0.78,
                    0.78
                ]
            },
            {
                "Name": {
                    "CH": "闪现背刺",
                    "EN": "Teleport Backstab"
                },
                "DMG": [
                    1.2
                ]
            }
        ],
        "Drop": [
            {
                "Type": 16,
                "Amount": [
                    0,
                    2.6263,
                    0.7003,
                    0.1751
                ]
            }
        ],
        "Csx": [
            "ECAbyss/嗜雷·兽境幼兽"
        ]
    },
    "20501": {
        "_id": 20501,
        "KingdomID": 2,
        "ClassID": 10204,
        "Name": {
            "CH": "嗜岩 · 兽境猎犬",
            "EN": "Rockfond Rifthound"
        },
        "Title": [
            {
                "CH": "隙间侵入的异兽",
                "EN": "Gap-Infiltrating Aliens"
            }
        ],
        "Desc": {
            "CH": "能够侵蚀世界边界的魔血之兽。「黄金」的造物。\n会很微妙地表现出生物习性，就如同真正的狼一般。或许是对地上的「近亲」抱有仰慕或嫉妒的感情，而梦想取而代之吧。",
            "EN": "A beast with monstrous blood that is capable of eroding the boundaries of the world. They were created by \"Gold.\"\nThey do display certain biological behaviors similar to those of real wolves. Perhaps they feel jealousy towards these their \"next of kin,\" and dream of replacing them someday."
        },
        "Color": "Rock",
        "HP": 3.5,
        "HPCurve": 2,
        "ATK": 3.2,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.25,
            "Fire": 0.25,
            "Water": 0.25,
            "Elec": 0.25,
            "Ice": 0.25,
            "Grass": 0.25,
            "Wind": 0.25,
            "Rock": 0.25
        },
        "RESState": [
            {
                "State": "Hound_Activated",
                "Rock": -0.65
            }
        ],
        "Ball": {
            "Type": 7,
            "Point": 3,
            "Drop": "222"
        },
        "Icon": [
            "rrh"
        ],
        "Grade": 2,
        "Weight": 150,
        "EndureTemplateID": 210,
        "SkillData": [
            "2/20501"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "后撤前咬",
                    "EN": "Jump Back and Mouth Bite"
                },
                "DMG": [
                    2.88
                ]
            },
            {
                "Name": {
                    "CH": "左手+右手划",
                    "EN": "Left Paw + Right Paw Strike"
                },
                "DMG": [
                    2.72,
                    2.72
                ]
            },
            {
                "Name": {
                    "CH": "闪现背刺",
                    "EN": "Teleport Backstab"
                },
                "DMG": [
                    3.2
                ]
            },
            {
                "Name": {
                    "CH": "吐石头",
                    "EN": "Spit Piece of Rock"
                },
                "DMG": [
                    3.04
                ]
            },
            {
                "Name": {
                    "CH": "狂暴三连冲",
                    "EN": "Activated: Triple Rush"
                },
                "DMG": [
                    2.72,
                    2.72,
                    2.72
                ]
            }
        ],
        "Drop": [
            {
                "Type": 16,
                "Amount": [
                    0,
                    5.2526,
                    1.4006,
                    0.3502
                ]
            }
        ],
        "Csx": [
            "ECAbyss/嗜岩·兽境猎犬"
        ]
    },
    "20502": {
        "_id": 20502,
        "KingdomID": 2,
        "ClassID": 10204,
        "Name": {
            "CH": "嗜雷 · 兽境猎犬",
            "EN": "Thundercraven Rifthound"
        },
        "Title": [
            {
                "CH": "隙间侵入的异兽",
                "EN": "Gap-Infiltrating Aliens"
            }
        ],
        "Desc": {
            "CH": "能够侵蚀世界边界的魔血之兽。「黄金」的造物。\n曾经肆虐大陆，然后遭到反抗与猎捕而一度绝迹。但最近它们却又再度出现了。威胁到了清泉镇与奔狼领的黑狼之群，说的正是它们。",
            "EN": "A beast with monstrous blood that is capable of eroding the boundaries of the world. They were created by \"Gold.\"\nOnce upon a time, they ran riot across the continent, but they went extinct for a time due to resistance and mass hunts. Recently, however, they have re-emerged. The pack of black wolves that now threaten Springvale and Wolvendom are such creatures."
        },
        "Color": "Elec",
        "HP": 3.5,
        "HPCurve": 2,
        "ATK": 3.2,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.25,
            "Fire": 0.25,
            "Water": 0.25,
            "Elec": 0.25,
            "Ice": 0.25,
            "Grass": 0.25,
            "Wind": 0.25,
            "Rock": 0.25
        },
        "RESState": [
            {
                "State": "Hound_Activated",
                "Elec": -0.65
            }
        ],
        "Ball": {
            "Type": 3,
            "Point": 3,
            "Drop": "222"
        },
        "Icon": [
            "trh"
        ],
        "Grade": 2,
        "Weight": 150,
        "EndureTemplateID": 210,
        "SkillData": [
            "2/20501"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "后撤前咬",
                    "EN": "Jump Back and Mouth Bite"
                },
                "DMG": [
                    2.88
                ]
            },
            {
                "Name": {
                    "CH": "左手+右手划",
                    "EN": "Left Paw + Right Paw Strike"
                },
                "DMG": [
                    2.72,
                    2.72
                ]
            },
            {
                "Name": {
                    "CH": "闪现背刺",
                    "EN": "Teleport Backstab"
                },
                "DMG": [
                    3.2
                ]
            },
            {
                "Name": {
                    "CH": "尾巴甩雷轮",
                    "EN": "Tail Throws Electro Frisbee"
                },
                "DMG": [
                    3.04
                ]
            },
            {
                "Name": {
                    "CH": "狂暴三连冲",
                    "EN": "Activated: Triple Rush"
                },
                "DMG": [
                    2.72,
                    2.72,
                    2.72
                ]
            }
        ],
        "Drop": [
            {
                "Type": 16,
                "Amount": [
                    0,
                    5.2526,
                    1.4006,
                    0.3502
                ]
            }
        ],
        "Csx": [
            "ECAbyss/嗜雷·兽境猎犬"
        ]
    },
    "20601": {
        "_id": 20601,
        "KingdomID": 2,
        "ClassID": 10204,
        "Name": {
            "CH": "黄金王兽",
            "EN": "Golden Wolflord"
        },
        "Title": [
            {
                "CH": "无名的兽境之王",
                "EN": "Nameless King of the Rift"
            }
        ],
        "Desc": {
            "CH": "来自晦暗的异界，是漆黑群狼之中的王，具有号令隐爪之异兽为它溶解空间，创造通行的裂隙的权能。\n王兽并没有名字，因为本身不过是「黄金」无心的造物；正因如此，才会更执着于侵入不属于它的世界，以自己的力量争取名号吧。\n但狼群绝非痴钝之物。此前的入侵受挫后，选择了既无人、也无守护者的荒凉远岛，为王兽的降临做准备。",
            "EN": "Hailing from a dark world, this ruler of the black wolves has the power to call upon its followers, its hidden claws, to dissolve space itself and forge a rift through which it may enter.\nThe Wolflord has no name, for it was but one of \"Gold\"'s unintentional creations, and that is precisely why it is obsessed with invading worlds that do not belong to it and making a name for itself.\nThat said, the wolf pack is far from unintelligent. After suffering a defeat in its previous invasion, a deserted island where no humans or protectors dwell has been selected for them to prepare for their lord's descent."
        },
        "Color": "Rock",
        "HP": 6.0,
        "HPCurve": 2,
        "ATK": 8.0,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": 0.25,
            "Fire": 0.25,
            "Water": 0.25,
            "Elec": 0.25,
            "Ice": 0.25,
            "Grass": 0.25,
            "Wind": 0.25,
            "Rock": 0.25
        },
        "RESState": [
            {
                "State": "Alert",
                "All": 2.0
            },
            {
                "State": "Hound_Activated",
                "Rock": -0.45
            }
        ],
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "4444"
        },
        "Icon": [
            "gw"
        ],
        "Grade": 3,
        "Weight": 1000,
        "EndureTemplateID": 4000,
        "SkillData": [
            "2/20601"
        ],
        "Csx": [
            "ECAbyss/黄金王兽"
        ]
    },
    "20701": {
        "_id": 20701,
        "KingdomID": 2,
        "ClassID": 10207,
        "Name": {
            "CH": "黯色空壳 · 旗令",
            "EN": "Shadowy Husk: Standard Bearer"
        },
        "Title": [
            {
                "CH": "黑蛇骑士的终末",
                "EN": "The Black Serpent Knights' End"
            }
        ],
        "Desc": {
            "CH": "漆黑的士兵。曾经是地下古国的骄傲军人，如今已经失去了一切知觉与记忆吧。\n即使如此，也会高举着王家的旗号进行战斗。就算忘记了同侪的模样、王家的祝福，只有这枚旗印，对它而言是绝对不能放下的。",
            "EN": "A pitch-dark soldier. Once a proud warrior of an ancient underground nation, it has now lost any awareness of memory.\nBut even so, it holds the banner of the royal house aloft into battle. Though it has forgotten its comrades or any royal blessing they might have been given, it will never lay this banner down."
        },
        "Color": "Fire",
        "HP": 2.5,
        "HPCurve": 2,
        "ATK": 4.0,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": 0.3,
            "Fire": 0.3
        },
        "Ball": {
            "Type": 1,
            "Point": 4,
            "Drop": "1112"
        },
        "Icon": [
            "shsb"
        ],
        "Grade": 2,
        "Weight": 110,
        "EndureTemplateID": 210,
        "Tutorial": {
            "CH": "攻击未击破护盾时，为场上所有黯色空壳和黑蛇骑士赋予6元素量<color style='color: #FF0000;'>火元素护盾</color>，持续15秒。",
            "EN": "When its attacks fail to break a shield, provides 6U <color style='color: #FF0000;'>Pyro Shield</color> lasting 15s for all Shadowy Husks and Black Serpent Knights on field."
        },
        "SkillData": [
            "2/20701"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "火球",
                    "EN": "Fire Ball"
                },
                "DMG": [
                    4
                ]
            },
            {
                "Name": {
                    "CH": "抡旗",
                    "EN": "Flag Strike"
                },
                "DMG": [
                    4
                ]
            }
        ],
        "Drop": [
            {
                "Type": 15,
                "Amount": [
                    0,
                    6.5658,
                    1.7508,
                    0.4378
                ]
            }
        ],
        "Csx": [
            "ECAbyss/黯色空壳·旗令"
        ]
    },
    "20702": {
        "_id": 20702,
        "KingdomID": 2,
        "ClassID": 10207,
        "Name": {
            "CH": "黯色空壳 · 破阵",
            "EN": "Shadowy Husk: Line Breaker"
        },
        "Title": [
            {
                "CH": "黑蛇骑士的终末",
                "EN": "The Black Serpent Knights' End"
            }
        ],
        "Desc": {
            "CH": "漆黑的士兵。曾经是地下古国的骄傲军人，如今已经不再具备身为「人」的智性了。\n为了歼敌、为了守护而陷阵的意志似乎仍然刻在这幅空壳当中，就算已经没有人能告诉它们敌在何方，要守护的又是什么。",
            "EN": "A pitch-dark soldier. Once a proud warrior of an ancient underground nation, it no longer has any \"intelligence\" that it may still be called \"human.\"\nYet the desire to destroy the foe and plunge into battle to defend others is etched into this empty suit of armor, even if no one remains to tell it where the enemy is or who or what exactly must be defended."
        },
        "Color": "Water",
        "HP": 3.0,
        "HPCurve": 2,
        "ATK": 3.0,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": 0.3,
            "Water": 0.3
        },
        "Ball": {
            "Type": 2,
            "Point": 3,
            "Drop": "222"
        },
        "Icon": [
            "shlb"
        ],
        "Grade": 2,
        "Weight": 110,
        "EndureTemplateID": 210,
        "Tutorial": {
            "CH": "攻击未击破护盾时，为附近的黯色空壳和黑蛇骑士恢复10%生命值。",
            "EN": "When its attacks fail to break a shield, heals 10% Max HP for nearby Shadowy Husks and Black Serpent Knights."
        },
        "SkillData": [
            "2/20702"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "穿刺",
                    "EN": "Pierce"
                },
                "DMG": [
                    2.4
                ]
            },
            {
                "Name": {
                    "CH": "横扫",
                    "EN": "Slash"
                },
                "DMG": [
                    2.4
                ]
            },
            {
                "Name": {
                    "CH": "冲刺期间伤害",
                    "EN": "Rush"
                },
                "DMG": [
                    1.8
                ]
            },
            {
                "Name": {
                    "CH": "冲刺结束伤害",
                    "EN": "Rush End"
                },
                "DMG": [
                    3
                ]
            }
        ],
        "Drop": [
            {
                "Type": 15,
                "Amount": [
                    0,
                    6.5658,
                    1.7508,
                    0.4378
                ]
            }
        ],
        "Csx": [
            "ECAbyss/黯色空壳·破阵"
        ]
    },
    "20703": {
        "_id": 20703,
        "KingdomID": 2,
        "ClassID": 10207,
        "Name": {
            "CH": "黯色空壳 · 近卫",
            "EN": "Shadowy Husk: Defender"
        },
        "Title": [
            {
                "CH": "黑蛇骑士的终末",
                "EN": "The Black Serpent Knights' End"
            }
        ],
        "Desc": {
            "CH": "漆黑的士兵。曾经是地下古国的骄傲军人，如今不过是一具残留着战斗记忆的空壳罢了。\n万物尽头皆是废退，而这种空壳就是「黑蛇骑士」在罪、诅咒、时间的冲刷之下变成的模样。",
            "EN": "A pitch-dark soldier. Once a proud warrior of an ancient underground nation, it is now nothing but a husk that only bears memories of battle.\nAll shall decay in the end, and this is all that is left of the Black Serpent Knights amidst the merciless march of their sins, curses, and time itself."
        },
        "Color": "Ice",
        "HP": 2.0,
        "HPCurve": 2,
        "ATK": 5.0,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": 0.3,
            "Ice": 0.3
        },
        "Ball": {
            "Type": 4,
            "Point": 4,
            "Drop": "1111"
        },
        "Icon": [
            "shd"
        ],
        "Grade": 2,
        "Weight": 110,
        "EndureTemplateID": 210,
        "Tutorial": {
            "CH": "攻击未击破护盾时，攻击力提升15%，并获得可以抵御任何正面攻击的坚盾，持续30秒。此效果可叠加，最多叠加5层。被击飞时，移除此效果。",
            "EN": "When its attacks fail to break a shield, ATK is increased by 30% and gains a tough shield able to block all DMG from the front for 30s. This effect can stack, max 5 stacks. When launched intp the air, loses this effect."
        },
        "SkillData": [
            "2/20703"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "直射箭",
                    "EN": "Straight Arrow"
                },
                "DMG": [
                    5
                ]
            },
            {
                "Name": {
                    "CH": "坠落箭",
                    "EN": "Falling Area Arrow"
                },
                "DMG": [
                    5
                ]
            }
        ],
        "Drop": [
            {
                "Type": 15,
                "Amount": [
                    0,
                    6.5658,
                    1.7508,
                    0.4378
                ]
            }
        ],
        "Csx": [
            "ECAbyss/黯色空壳·近卫"
        ]
    },
    "20801": {
        "_id": 20801,
        "KingdomID": 2,
        "ClassID": 10207,
        "Name": {
            "CH": "黑蛇骑士 · 斩风之剑",
            "EN": "Black Serpent Knight: Windcutter"
        },
        "Title": [
            {
                "CH": "「曾经行于微风之中…」",
                "EN": "\"Once Upon a Gentle Wind...\""
            }
        ],
        "Desc": {
            "CH": "在宫廷当中颇具地位的近卫军人，以名唤「至真之术」的剑斗技巧，扫除王家的敌人。\n「末光之剑」也曾是他们当中的一员。在王国的光荣尽数失落、一切都无可挽回之前。",
            "EN": "A guard of some standing in the court, sundering the foes of the nation's rulers using a sword art known as \"Truthseeker.\"\nThe Twilight Sword was once one of their number. That realm's glory has been extinguished, never again to be reclaimed."
        },
        "Color": "Wind",
        "HP": 3.5,
        "HPCurve": 2,
        "ATK": 5.0,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": 0.3,
            "Wind": 0.5
        },
        "Ball": {
            "Type": 6,
            "Point": 3,
            "Drop": "222"
        },
        "Icon": [
            "bskw"
        ],
        "Grade": 2,
        "Weight": 150,
        "EndureTemplateID": 210,
        "Tutorial": {
            "CH": "攻击未击破护盾时，可以闪避掉下一次角色的伤害。闪避结束后造成的伤害提升。",
            "EN": "When its attacks fail to break a shield, becomes able to dodge the character's next attack. After dodging, DMG dealt is increased."
        },
        "SkillData": [
            "2/20801"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "普通攻击x3",
                    "EN": "Normal Attack x3"
                },
                "DMG": [
                    3,
                    4,
                    4
                ]
            },
            {
                "Name": {
                    "CH": "链剑：竖劈",
                    "EN": "Chain: Vertical Strike"
                },
                "DMG": [
                    5
                ]
            },
            {
                "Name": {
                    "CH": "链剑：发射风弹",
                    "EN": "Chain: Launch Anemo Bullets"
                },
                "DMG": [
                    1.8
                ]
            },
            {
                "Name": {
                    "CH": "链剑：甩出碎片",
                    "EN": "Chain: Swing Out Pieces"
                },
                "DMG": [
                    3
                ]
            },
            {
                "Name": {
                    "CH": "链剑：碎片持续伤害 每0.2s",
                    "EN": "Chain: Chain Piece DoT every 0.2s"
                },
                "DMG": [
                    2
                ]
            }
        ],
        "Drop": [
            {
                "Type": 15,
                "Amount": [
                    0,
                    6.5658,
                    1.7508,
                    0.4378
                ]
            }
        ],
        "Csx": [
            "ECAbyss/黑蛇骑士·斩风之剑"
        ]
    },
    "20802": {
        "_id": 20802,
        "KingdomID": 2,
        "ClassID": 10207,
        "Name": {
            "CH": "黑蛇骑士 · 摧岩之钺",
            "EN": "Black Serpent Knight: Rockbreaker Ax"
        },
        "Title": [
            {
                "CH": "「曾如磐石抵挡黑水奔流…」",
                "EN": "\"Once Stood Against the Tide of Dark Waters Like Bedrock...\""
            }
        ],
        "Desc": {
            "CH": "在宫廷当中颇具地位的近卫军人，以名唤「至真之术」的剑斗技巧，扫除王家的敌人——\n——虽说至真之术是剑斗技巧，但它的心得可以衍生应用于各种兵器上。在王国最黑暗的时刻，肩负守护职责的武人有时并没有选择的余裕。",
            "EN": "A guard of some standing in the court, sundering the foes of the nation's rulers using a sword art known as \"Truthseeker.\"\nThough Truthseeker is, strictly speaking, a sword art, its teachings can also be used with various other weapons — and in the realm's darkest hour, those who had to protect their homeland had little room to be fussy anyway."
        },
        "Color": "Rock",
        "HP": 4.0,
        "HPCurve": 2,
        "ATK": 6.0,
        "ATKCurve": 4,
        "RESBase": {
            "Rock": 0.5
        },
        "RESState": [
            {
                "State": "Darkwraith_Fat_Rock_Angry",
                "All": 0.5
            }
        ],
        "Ball": {
            "Type": 7,
            "Point": 3,
            "Drop": "222"
        },
        "Icon": [
            "bskr"
        ],
        "Grade": 2,
        "Weight": 150,
        "EndureTemplateID": 210,
        "Tutorial": {
            "CH": "攻击未击破护盾时，进入一段时间的狂暴模式：所有抗性提升50%，伤害转化为<color style='color:#D4A000;'>岩元素伤害</color>并大幅提升。",
            "EN": "When its attacks fail to break a shield, enter Rampage State: All RES +50%, all DMG are converted to <color style='color:#D4A000;'>Geo DMG</color> and are greatly increased."
        },
        "SkillData": [
            "2/20802"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "普通攻击x3",
                    "EN": "Normal Attack x3"
                },
                "DMG": [
                    4.2,
                    5.1,
                    6
                ]
            },
            {
                "Name": {
                    "CH": "转斧头",
                    "EN": "Axe Swirl"
                },
                "DMG": [
                    4.5,
                    5.1
                ]
            },
            {
                "Name": {
                    "CH": "盾砸",
                    "EN": "Shield Smash"
                },
                "DMG": [
                    6
                ]
            },
            {
                "Name": {
                    "CH": "狂暴：转转转",
                    "EN": "Rampage: Swirl Swirl"
                },
                "DMG": [
                    5.76
                ]
            },
            {
                "Name": {
                    "CH": "狂暴：转转转尾刀",
                    "EN": "Rampage: Swirl End Strike"
                },
                "DMG": [
                    7.2
                ]
            },
            {
                "Name": {
                    "CH": "狂暴：跑过来斧挑",
                    "EN": "Rampage: Rush and Hit"
                },
                "DMG": [
                    7.2
                ]
            }
        ],
        "Drop": [
            {
                "Type": 15,
                "Amount": [
                    0,
                    6.5658,
                    1.7508,
                    0.4378
                ]
            }
        ],
        "Csx": [
            "ECAbyss/黑蛇骑士·摧岩之钺"
        ]
    },
    "21001": {
        "_id": 21001,
        "KingdomID": 2,
        "ClassID": 10210,
        "Name": {
            "CH": "深罪浸礼者 · 火冰雷驱役者",
            "EN": "Iniquitous Baptist - Invoker of Flame, Frost, and Fulmination"
        },
        "Title": [
            {
                "CH": "漆黑乐园的引路人",
                "EN": "The Guide to the Dark Paradise"
            }
        ],
        "Desc": {
            "CH": "驾驭了多种元素力量的深渊魔物，因为「进化」而获得了多余的肢体。\n\n<i>「『深渊』之本质归根到底是与元素所不能相容的。但即使是有神的元素也会为其所裹挟、扭曲，成为深邃意志道具。从结果与效益而言，所谓的『深渊』或许与小至盗宝团手中充满元素力量的器皿、大到『神之眼』没有过多的区别吧…」</i>\n——谁人遗落在逾渊地墟的笔记",
            "EN": "A monster from the Abyss that wields multiple types of Elemental Power and has gained extra limbs from its \"evolution.\"\n\n<i>\"The nature of the 'Abyss' is essentially irreconcilable with the elements. However, even the elements of gods can be entrapped and distorted before they are subsumed to become tools of the abyssal will. Just judging from outcomes and practical use, \"Abyssal powers\" seem little different from elemental items, from ones as paltry as the Treasure Hoarders' elemental containers to those as powerful as Visions...\"</i>\n—Notes left behind in Hangeh Afrasiyab by someone."
        },
        "UseCustomColorName": true,
        "HP": 5.0,
        "HPCurve": 2,
        "ATK": 8.0,
        "ATKCurve": 4,
        "RESState": [
            {
                "State": "Alert",
                "All": 2.0
            },
            {
                "State": "Overcharge",
                "All": 2.0
            }
        ],
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "3333"
        },
        "Icon": [
            "ib"
        ],
        "Grade": 3,
        "Weight": 200,
        "EndureTemplateID": 400,
        "SkillData": [
            "2/21005"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "共鸣法阵：元素之环",
                    "EN": "Channel: Elemental Rings"
                },
                "DMG": [
                    16
                ]
            },
            {
                "Name": {
                    "CH": "换盾蓄力：双元素球",
                    "EN": "Switch Shield: Elemental Balls"
                },
                "DMG": [
                    8
                ]
            },
            {
                "Name": {
                    "CH": "火：四火球",
                    "EN": "Pyro: Fireballs"
                },
                "DMG": [
                    4
                ]
            },
            {
                "Name": {
                    "CH": "火：地狱渊火",
                    "EN": "Pyro: Ground Hellfire"
                },
                "DMG": [
                    4
                ]
            },
            {
                "Name": {
                    "CH": "冰：一字冰刃 + 十字冰刃",
                    "EN": "Cryo: Single Ice Blade + Cross Ice Blade"
                },
                "DMG": [
                    4,
                    5.6
                ]
            },
            {
                "Name": {
                    "CH": "冰：碎冰爆轰",
                    "EN": "Cryo: Close-up Ice Push"
                },
                "DMG": [
                    8
                ]
            },
            {
                "Name": {
                    "CH": "冰：渊寒冰锥",
                    "EN": "Cryo: Darkfrost Icicles"
                },
                "DMG": [
                    6.24
                ]
            },
            {
                "Name": {
                    "CH": "雷：双雷锥",
                    "EN": "Electro: Double Electro Pierce"
                },
                "DMG": [
                    5.6,
                    5.6
                ]
            },
            {
                "Name": {
                    "CH": "雷：苍雷激光",
                    "EN": "Electro: Fierce Laser"
                },
                "DMG": [
                    8
                ]
            },
            {
                "Name": {
                    "CH": "雷：嗜能雷球",
                    "EN": "Electro: Energy Consuming Balls"
                },
                "DMG": [
                    4
                ]
            }
        ],
        "Csx": [
            "ECAbyss/深罪浸礼者"
        ]
    },
    "21002": {
        "_id": 21002,
        "KingdomID": 2,
        "ClassID": 10210,
        "Name": {
            "CH": "深罪浸礼者 · 冰水火驱役者",
            "EN": "Iniquitous Baptist - Bringer of Frost, Flood, and Flame"
        },
        "Title": [
            {
                "CH": "漆黑乐园的引路人",
                "EN": "The Guide to the Dark Paradise"
            }
        ],
        "Desc": {
            "CH": "驾驭了多种元素力量的深渊魔物，因为「进化」而获得了多余的肢体。\n\n<i>「『深渊』之本质归根到底是与元素所不能相容的。但即使是有神的元素也会为其所裹挟、扭曲，成为深邃意志道具。从结果与效益而言，所谓的『深渊』或许与小至盗宝团手中充满元素力量的器皿、大到『神之眼』没有过多的区别吧…」</i>\n——谁人遗落在逾渊地墟的笔记",
            "EN": "A monster from the Abyss that wields multiple types of Elemental Power and has gained extra limbs from its \"evolution.\"\n\n<i>\"The nature of the 'Abyss' is essentially irreconcilable with the elements. However, even the elements of gods can be entrapped and distorted before they are subsumed to become tools of the abyssal will. Just judging from outcomes and practical use, \"Abyssal powers\" seem little different from elemental items, from ones as paltry as the Treasure Hoarders' elemental containers to those as powerful as Visions...\"</i>\n—Notes left behind in Hangeh Afrasiyab by someone."
        },
        "UseCustomColorName": true,
        "HP": 5.0,
        "HPCurve": 2,
        "ATK": 8.0,
        "ATKCurve": 4,
        "RESState": [
            {
                "State": "Alert",
                "All": 2.0
            },
            {
                "State": "Overcharge",
                "All": 2.0
            }
        ],
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "3333"
        },
        "Icon": [
            "ib"
        ],
        "Grade": 3,
        "Weight": 200,
        "EndureTemplateID": 400,
        "SkillData": [
            "2/21005"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "共鸣法阵：元素之环",
                    "EN": "Channel: Elemental Rings"
                },
                "DMG": [
                    16
                ]
            },
            {
                "Name": {
                    "CH": "换盾蓄力：双元素球",
                    "EN": "Switch Shield: Elemental Balls"
                },
                "DMG": [
                    8
                ]
            },
            {
                "Name": {
                    "CH": "冰：一字冰刃 + 十字冰刃",
                    "EN": "Cryo: Single Ice Blade + Cross Ice Blade"
                },
                "DMG": [
                    4,
                    5.6
                ]
            },
            {
                "Name": {
                    "CH": "冰：碎冰爆轰",
                    "EN": "Cryo: Close-up Ice Push"
                },
                "DMG": [
                    8
                ]
            },
            {
                "Name": {
                    "CH": "冰：渊寒冰锥",
                    "EN": "Cryo: Darkfrost Icicles"
                },
                "DMG": [
                    6.24
                ]
            },
            {
                "Name": {
                    "CH": "水：四水球",
                    "EN": "Hydro: Waterballs"
                },
                "DMG": [
                    4
                ]
            },
            {
                "Name": {
                    "CH": "水：一字水刃 + 十字水刃",
                    "EN": "Hydro: Single Water Blade + Cross Water Blade"
                },
                "DMG": [
                    4,
                    5.6
                ]
            },
            {
                "Name": {
                    "CH": "火：四火球",
                    "EN": "Pyro: Fireballs"
                },
                "DMG": [
                    4
                ]
            },
            {
                "Name": {
                    "CH": "火：地狱渊火",
                    "EN": "Pyro: Ground Hellfire"
                },
                "DMG": [
                    4
                ]
            }
        ],
        "Csx": [
            "ECAbyss/深罪浸礼者"
        ]
    },
    "21003": {
        "_id": 21003,
        "KingdomID": 2,
        "ClassID": 10210,
        "Name": {
            "CH": "深罪浸礼者 · 雷火水驱役者",
            "EN": "Iniquitous Baptist - Caller of Fulmination, Flame, and Flood"
        },
        "Title": [
            {
                "CH": "漆黑乐园的引路人",
                "EN": "The Guide to the Dark Paradise"
            }
        ],
        "Desc": {
            "CH": "驾驭了多种元素力量的深渊魔物，因为「进化」而获得了多余的肢体。\n\n<i>「『深渊』之本质归根到底是与元素所不能相容的。但即使是有神的元素也会为其所裹挟、扭曲，成为深邃意志道具。从结果与效益而言，所谓的『深渊』或许与小至盗宝团手中充满元素力量的器皿、大到『神之眼』没有过多的区别吧…」</i>\n——谁人遗落在逾渊地墟的笔记",
            "EN": "A monster from the Abyss that wields multiple types of Elemental Power and has gained extra limbs from its \"evolution.\"\n\n<i>\"The nature of the 'Abyss' is essentially irreconcilable with the elements. However, even the elements of gods can be entrapped and distorted before they are subsumed to become tools of the abyssal will. Just judging from outcomes and practical use, \"Abyssal powers\" seem little different from elemental items, from ones as paltry as the Treasure Hoarders' elemental containers to those as powerful as Visions...\"</i>\n—Notes left behind in Hangeh Afrasiyab by someone."
        },
        "UseCustomColorName": true,
        "HP": 5.0,
        "HPCurve": 2,
        "ATK": 8.0,
        "ATKCurve": 4,
        "RESState": [
            {
                "State": "Alert",
                "All": 2.0
            },
            {
                "State": "Overcharge",
                "All": 2.0
            }
        ],
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "3333"
        },
        "Icon": [
            "ib"
        ],
        "Grade": 3,
        "Weight": 200,
        "EndureTemplateID": 400,
        "SkillData": [
            "2/21005"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "共鸣法阵：元素之环",
                    "EN": "Channel: Elemental Rings"
                },
                "DMG": [
                    16
                ]
            },
            {
                "Name": {
                    "CH": "换盾蓄力：双元素球",
                    "EN": "Switch Shield: Elemental Balls"
                },
                "DMG": [
                    8
                ]
            },
            {
                "Name": {
                    "CH": "雷：双雷锥",
                    "EN": "Electro: Double Electro Pierce"
                },
                "DMG": [
                    5.6,
                    5.6
                ]
            },
            {
                "Name": {
                    "CH": "雷：苍雷激光",
                    "EN": "Electro: Fierce Laser"
                },
                "DMG": [
                    8
                ]
            },
            {
                "Name": {
                    "CH": "雷：嗜能雷球",
                    "EN": "Electro: Energy Consuming Balls"
                },
                "DMG": [
                    4
                ]
            },
            {
                "Name": {
                    "CH": "火：四火球",
                    "EN": "Pyro: Fireballs"
                },
                "DMG": [
                    4
                ]
            },
            {
                "Name": {
                    "CH": "火：地狱渊火",
                    "EN": "Pyro: Ground Hellfire"
                },
                "DMG": [
                    4
                ]
            },
            {
                "Name": {
                    "CH": "水：四水球",
                    "EN": "Hydro: Waterballs"
                },
                "DMG": [
                    4
                ]
            },
            {
                "Name": {
                    "CH": "水：一字水刃 + 十字水刃",
                    "EN": "Hydro: Single Water Blade + Cross Water Blade"
                },
                "DMG": [
                    4,
                    5.6
                ]
            }
        ],
        "Csx": [
            "ECAbyss/深罪浸礼者"
        ]
    },
    "21004": {
        "_id": 21004,
        "KingdomID": 2,
        "ClassID": 10210,
        "Name": {
            "CH": "深罪浸礼者 · 水冰雷驱役者",
            "EN": "Iniquitous Baptist - Summoner of Flood, Frost, and Fulmination"
        },
        "Title": [
            {
                "CH": "漆黑乐园的引路人",
                "EN": "The Guide to the Dark Paradise"
            }
        ],
        "Desc": {
            "CH": "驾驭了多种元素力量的深渊魔物，因为「进化」而获得了多余的肢体。\n\n<i>「『深渊』之本质归根到底是与元素所不能相容的。但即使是有神的元素也会为其所裹挟、扭曲，成为深邃意志道具。从结果与效益而言，所谓的『深渊』或许与小至盗宝团手中充满元素力量的器皿、大到『神之眼』没有过多的区别吧…」</i>\n——谁人遗落在逾渊地墟的笔记",
            "EN": "A monster from the Abyss that wields multiple types of Elemental Power and has gained extra limbs from its \"evolution.\"\n\n<i>\"The nature of the 'Abyss' is essentially irreconcilable with the elements. However, even the elements of gods can be entrapped and distorted before they are subsumed to become tools of the abyssal will. Just judging from outcomes and practical use, \"Abyssal powers\" seem little different from elemental items, from ones as paltry as the Treasure Hoarders' elemental containers to those as powerful as Visions...\"</i>\n—Notes left behind in Hangeh Afrasiyab by someone."
        },
        "UseCustomColorName": true,
        "HP": 5.0,
        "HPCurve": 2,
        "ATK": 8.0,
        "ATKCurve": 4,
        "RESState": [
            {
                "State": "Alert",
                "All": 2.0
            },
            {
                "State": "Overcharge",
                "All": 2.0
            }
        ],
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "3333"
        },
        "Icon": [
            "ib"
        ],
        "Grade": 3,
        "Weight": 200,
        "EndureTemplateID": 400,
        "SkillData": [
            "2/21005"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "共鸣法阵：元素之环",
                    "EN": "Channel: Elemental Rings"
                },
                "DMG": [
                    16
                ]
            },
            {
                "Name": {
                    "CH": "换盾蓄力：双元素球",
                    "EN": "Switch Shield: Elemental Balls"
                },
                "DMG": [
                    8
                ]
            },
            {
                "Name": {
                    "CH": "水：四水球",
                    "EN": "Hydro: Waterballs"
                },
                "DMG": [
                    4
                ]
            },
            {
                "Name": {
                    "CH": "水：一字水刃 + 十字水刃",
                    "EN": "Hydro: Single Water Blade + Cross Water Blade"
                },
                "DMG": [
                    4,
                    5.6
                ]
            },
            {
                "Name": {
                    "CH": "冰：一字冰刃 + 十字冰刃",
                    "EN": "Cryo: Single Ice Blade + Cross Ice Blade"
                },
                "DMG": [
                    4,
                    5.6
                ]
            },
            {
                "Name": {
                    "CH": "冰：碎冰爆轰",
                    "EN": "Cryo: Close-up Ice Push"
                },
                "DMG": [
                    8
                ]
            },
            {
                "Name": {
                    "CH": "冰：渊寒冰锥",
                    "EN": "Cryo: Darkfrost Icicles"
                },
                "DMG": [
                    6.24
                ]
            },
            {
                "Name": {
                    "CH": "雷：双雷锥",
                    "EN": "Electro: Double Electro Pierce"
                },
                "DMG": [
                    5.6,
                    5.6
                ]
            },
            {
                "Name": {
                    "CH": "雷：苍雷激光",
                    "EN": "Electro: Fierce Laser"
                },
                "DMG": [
                    8
                ]
            },
            {
                "Name": {
                    "CH": "雷：嗜能雷球",
                    "EN": "Electro: Energy Consuming Balls"
                },
                "DMG": [
                    4
                ]
            }
        ],
        "Csx": [
            "ECAbyss/深罪浸礼者"
        ]
    },
    "21101": {
        "_id": 21101,
        "KingdomID": 2,
        "ClassID": 10211,
        "Name": {
            "CH": "大型碎石隙境原体",
            "EN": "Large Shatterstone Breacher Primus"
        },
        "Title": [
            {
                "CH": "异界的生命组织",
                "EN": "Otherworldly Agglomerated Organism"
            }
        ],
        "Desc": {
            "CH": "主要出现在厄里那斯的奇异生命体。\n在美露莘的认知里，隙境原体是基本上无害的生物，甚至有美露莘认为它十分可爱。\n美露莘的视觉感知与人有很大的区别。",
            "EN": "A strange life form that mostly appears in Elynas.\nMelusines perceive Breacher Primuses as harmless creatures, and many even find them completely adorable.\nThe visual perception of Melusines is very different from humans."
        },
        "Color": "Rock",
        "HP": 3.6,
        "HPCurve": 2,
        "ATK": 7.0,
        "ATKCurve": 4,
        "RESBase": {
            "Rock": 0.5
        },
        "Ball": {
            "Type": 7,
            "Point": 4,
            "Drop": "1113"
        },
        "Icon": [
            "UI_MonsterIcon_Ovacua_Order_01"
        ],
        "Grade": 2,
        "Weight": 150,
        "EndureTemplateID": 150,
        "Tutorial": {
            "CH": "- 以25~35s为间隔释放<b>终结技</b>：在6U岩元素增幅的加持下，释放持续20s的岩石压制攻击。期间不可打断且免疫风力牵引。<br>- 破除其岩元素增幅，或是使用<b>1次荒性或芒性</b>攻击，可以使其瘫痪15s。<br>- 元素增幅不会降低其受到的伤害。深渊特化版本入战立刻释放终结技。",
            "EN": "- Every 25~35s, performs <b>Ultimate</b>: Under the Enhancement of 6U Geo Element, unleash deadly Stonefall attacks, lasting 20s.<br>- Breaking the Geo Enhancement with reactions or attacking it <b>1 time with Pnuema or Ousia</b> will paralyze it for 15s.<br>- The Enhancement does not affect its DMG taken. Spiral Abyss Version uses Ultimate immediately after entering battle."
        },
        "Drop": [
            {
                "Type": 31,
                "Amount": [
                    0,
                    6.5658,
                    1.7508,
                    0.4378
                ]
            }
        ],
        "Csx": [
            "ECAbyss/大型碎石隙境原体"
        ]
    },
    "21102": {
        "_id": 21102,
        "KingdomID": 2,
        "ClassID": 10211,
        "Name": {
            "CH": "碎石隙境原体",
            "EN": "Shatterstone Breacher Primus"
        },
        "Title": [
            {
                "CH": "异界的生命组织",
                "EN": "Otherworldly Agglomerated Organism"
            }
        ],
        "Desc": {
            "CH": "主要出现在厄里那斯的奇异生命体。\n在遇到荒性或芒性能量似乎会受到很强的刺激，因此尽管并不具有很强的攻击性与侵略性，这种手段似乎能够有效驱除隙境原体。",
            "EN": "A strange life form that mostly appears in Elynas.\nReceives a powerful shock from encountering ousia or pneuma energy, and thus Breacher Primuses can be driven away using this relatively nonaggressive and noninvasive means."
        },
        "Color": "Rock",
        "HP": 1.2,
        "HPCurve": 2,
        "ATK": 3.6,
        "ATKCurve": 4,
        "RESBase": {
            "Rock": 0.3
        },
        "Ball": {
            "Type": 7,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "UI_MonsterIcon_Ovacua_Order_02"
        ],
        "Grade": 2,
        "Weight": 100,
        "EndureTemplateID": 50,
        "Tutorial": {
            "CH": "- 以25~35s为间隔释放<b>终结技</b>：在2U岩元素增幅的加持下，释放持续20s的岩石压制攻击。期间不可打断且免疫风力牵引。<br>- 破除其岩元素增幅，或是使用<b>1次荒性或芒性</b>攻击，可以使其瘫痪15s。<br>- 元素增幅不会降低其受到的伤害。深渊特化版本入战立刻释放终结技。",
            "EN": "- Every 25~35s, performs <b>Ultimate</b>: Under the Enhancement of 2U Geo Element, unleash deadly Stonefall attacks, lasting 20s.<br>- Breaking the Geo Enhancement with reactions or attacking it <b>1 time with Pnuema or Ousia</b> will paralyze it for 15s.<br>- The Enhancement does not affect its DMG taken. Spiral Abyss Version uses Ultimate immediately after entering battle."
        },
        "Drop": [
            {
                "Type": 31,
                "Amount": [
                    0,
                    3.9394,
                    1.0505,
                    0.2626
                ]
            }
        ],
        "Csx": [
            "ECAbyss/碎石隙境原体"
        ]
    },
    "21103": {
        "_id": 21103,
        "KingdomID": 2,
        "ClassID": 10211,
        "Name": {
            "CH": "大型狂蔓隙境原体",
            "EN": "Large Overgrown Breacher Primus"
        },
        "Title": [
            {
                "CH": "异界的生命组织",
                "EN": "Otherworldly Agglomerated Organism"
            }
        ],
        "Desc": {
            "CH": "主要出现在厄里那斯的奇异生命体。\n隙境原体在基本的生物特征上超出了已知的范畴：不属于动物、植物、蕈类与元素类。须弥的学者因此关于它到底是什么的争论从未正式结束，直到它一同被禁止作为学术研究课题立项，因为就目前人类的知识体系而言，这个问题与「语言的定义与本质」一样，无法得出有意义的确切结论。",
            "EN": "A strange life form that mostly appears in Elynas.\nThe nature of a Breacher Primus defies known categories: Not animals, plants, Shroom-Kin, nor Elementals. The debate among scholars of Sumeru never formally reached a consensus on just what it is, until it was finally banned as a research topic. The reason is that based on the current system of knowledge, this problem is like \"the definitions presribed by language and the true nature of things,\" and it is impossible to have a meaningful, final answer."
        },
        "Color": "Grass",
        "HP": 4.2,
        "HPCurve": 2,
        "ATK": 5.0,
        "ATKCurve": 4,
        "RESBase": {
            "Grass": 0.5
        },
        "Ball": {
            "Type": 5,
            "Point": 4,
            "Drop": "1113"
        },
        "Icon": [
            "UI_MonsterIcon_Ovacua_Chaos_01"
        ],
        "Grade": 2,
        "Weight": 150,
        "EndureTemplateID": 150,
        "Tutorial": {
            "CH": "- 以25~35s为间隔释放<b>终结技</b>：在4U草元素增幅的加持下，释放持续20s的狂蔓飞弹攻击。期间不可打断且免疫风力牵引。<br>- 破除其草元素增幅，或是使用<b>1次荒性或芒性</b>攻击，可以使其瘫痪15s。<br>- 元素增幅不会降低其受到的伤害。深渊特化版本入战立刻释放终结技。",
            "EN": "- Every 25~35s, performs <b>Ultimate</b>: Under the Enhancement of 4U Dendro Element, unleash deadly Lushmissile attacks, lasting 20s.<br>- Breaking the Dendro Enhancement with reactions or attacking it <b>1 time with Pnuema or Ousia</b> will paralyze it for 15s.<br>- The Enhancement does not affect its DMG taken. Spiral Abyss Version uses Ultimate immediately after entering battle."
        },
        "Drop": [
            {
                "Type": 31,
                "Amount": [
                    0,
                    6.5658,
                    1.7508,
                    0.4378
                ]
            }
        ],
        "Csx": [
            "ECAbyss/大型狂蔓隙境原体"
        ]
    },
    "21104": {
        "_id": 21104,
        "KingdomID": 2,
        "ClassID": 10211,
        "Name": {
            "CH": "狂蔓隙境原体",
            "EN": "Overgrown Breacher Primus"
        },
        "Title": [
            {
                "CH": "异界的生命组织",
                "EN": "Otherworldly Agglomerated Organism"
            }
        ],
        "Desc": {
            "CH": "主要出现在厄里那斯的奇异生命体。\n在枫丹，最早观测到隙境原体是在灾难过后不久。当时同时留下了隙境原体攻击人与保护人的记录。",
            "EN": "A strange life form that mostly appears in Elynas.\nIn Fontaine, the earliest recorded observations of a Breacher Primus appear shortly after the cataclysm. Records from the time show instances of Breacher Primuses both attacking and protecting humans."
        },
        "Color": "Grass",
        "HP": 1.5,
        "HPCurve": 2,
        "ATK": 2.6,
        "ATKCurve": 4,
        "RESBase": {
            "Grass": 0.3
        },
        "Ball": {
            "Type": 5,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "UI_MonsterIcon_Ovacua_Chaos_02"
        ],
        "Grade": 2,
        "Weight": 100,
        "EndureTemplateID": 50,
        "Tutorial": {
            "CH": "- 以25~35s为间隔释放<b>终结技</b>：在2U草元素增幅的加持下，释放持续20s的狂蔓飞弹攻击。期间不可打断且免疫风力牵引。<br>- 破除其草元素增幅，或是使用<b>1次荒性或芒性</b>攻击，可以使其瘫痪15s。<br>- 元素增幅不会降低其受到的伤害。深渊特化版本入战立刻释放终结技。",
            "EN": "- Every 25~35s, performs <b>Ultimate</b>: Under the Enhancement of 2U Dendro Element, unleash deadly Lushmissile attacks, lasting 20s.<br>- Breaking the Dendro Enhancement with reactions or attacking it <b>1 time with Pnuema or Ousia</b> will paralyze it for 15s.<br>- The Enhancement does not affect its DMG taken. Spiral Abyss Version uses Ultimate immediately after entering battle."
        },
        "Drop": [
            {
                "Type": 31,
                "Amount": [
                    0,
                    3.9394,
                    1.0505,
                    0.2626
                ]
            }
        ],
        "Csx": [
            "ECAbyss/狂蔓隙境原体"
        ]
    },
    "30101": {
        "_id": 30101,
        "KingdomID": 3,
        "ClassID": 10301,
        "Name": {
            "CH": "愚人众先遣队 · 冰铳重卫士",
            "EN": "Fatui Skirmisher - Cryogunner Legionnaire"
        },
        "Title": [
            {
                "CH": "愚人众先遣队 · 冰铳重卫士",
                "EN": "Fatui Skirmisher - Cryogunner Legionnaire"
            }
        ],
        "Desc": {
            "CH": "愚人众战争机器中的士兵。\n配备着能在一定程度上制御元素的装备，可以利用铳形的武器喷射急冻的寒霜。\n虽然拥有常人不能企及的元素力量，但这种力量或许也是有其代价的吧…",
            "EN": "A soldier encased within a Fatui war machine.\nArmed with equipment that can control the elements to a certain extent, their guns are capable of firing jets of frost.\nThough this may confer elemental power beyond the ken of ordinary humans, that power may yet come at a price..."
        },
        "Color": "Ice",
        "HP": 4.0,
        "HPCurve": 1,
        "ATK": 3.0,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": -0.2
        },
        "RESState": [
            {
                "State": "Skirmisher_Improved",
                "All": 1.0
            }
        ],
        "Ball": {
            "Type": 4,
            "Point": 3,
            "Drop": "222"
        },
        "Icon": [
            "fsc2"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 210,
        "Drop": [
            {
                "Type": 8,
                "Amount": [
                    4.202,
                    1.1205,
                    0.28,
                    0
                ]
            }
        ],
        "Csx": [
            "ECFatui/冰铳重卫士"
        ]
    },
    "30102": {
        "_id": 30102,
        "KingdomID": 3,
        "ClassID": 10301,
        "Name": {
            "CH": "愚人众先遣队 · 水铳重卫士",
            "EN": "Fatui Skirmisher - Hydrogunner Legionnaire"
        },
        "Title": [
            {
                "CH": "愚人众先遣队 · 水铳重卫士",
                "EN": "Fatui Skirmisher - Hydrogunner Legionnaire"
            }
        ],
        "Desc": {
            "CH": "愚人众战争机器中的士兵。\n配备着能在一定程度上制御元素的装备，可以利用铳形的武器喷射激烈的水流。\n「先遣队」之名意味着生死不问，深入祖国以外的土地完成前期任务，为未来的某些行动夯实基础。",
            "EN": "A soldier encased within a Fatui war machine.\nArmed with equipment that can control the elements to a certain extent, their guns are capable of firing jets of water.\nThese skirmishers do not question if they shall live or die as they advance into nations far from home to complete reconaissance missions and lay the groundwork for operations to come."
        },
        "Color": "Water",
        "HP": 3.0,
        "HPCurve": 1,
        "ATK": 2.0,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": -0.2
        },
        "RESState": [
            {
                "State": "Skirmisher_Improved",
                "All": 1.0
            }
        ],
        "Ball": {
            "Type": 2,
            "Point": 3,
            "Drop": "222"
        },
        "Icon": [
            "fsh2"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 210,
        "Drop": [
            {
                "Type": 8,
                "Amount": [
                    4.202,
                    1.1205,
                    0.28,
                    0
                ]
            }
        ],
        "Csx": [
            "ECFatui/水铳重卫士"
        ]
    },
    "30103": {
        "_id": 30103,
        "KingdomID": 3,
        "ClassID": 10301,
        "Name": {
            "CH": "愚人众先遣队 · 雷锤前锋军",
            "EN": "Fatui Skirmisher - Electrohammer Vanguard"
        },
        "Title": [
            {
                "CH": "愚人众先遣队 · 雷锤前锋军",
                "EN": "Fatui Skirmisher - Electrohammer Vanguard"
            }
        ],
        "Desc": {
            "CH": "体格健壮的先遣队士兵，装备了附有雷元素的战锤。\n成为愚人众的军人意味着优渥的资源：军饷、武装与物资。\n但与此同时，也意味着远离故土与亲眷，将余命交给过去不曾谋面的战友。",
            "EN": "A sturdily-built skirmisher armed with an Electro-powered war-hammer.\nAs a Fatui soldier, one has access to munificent resources: salaries, armaments, and supplies.\nBut at the same time, it also means much time away from home and family and putting one's life in the hands of comrades one has never met."
        },
        "Color": "Elec",
        "HP": 4.0,
        "HPCurve": 1,
        "ATK": 6.0,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": -0.2
        },
        "RESState": [
            {
                "State": "Skirmisher_Improved",
                "All": 1.0
            }
        ],
        "Ball": {
            "Type": 3,
            "Point": 3,
            "Drop": "222"
        },
        "Icon": [
            "fse"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 210,
        "Drop": [
            {
                "Type": 8,
                "Amount": [
                    4.202,
                    1.1205,
                    0.28,
                    0
                ]
            }
        ],
        "Csx": [
            "ECFatui/雷锤前锋军"
        ]
    },
    "30104": {
        "_id": 30104,
        "KingdomID": 3,
        "ClassID": 10301,
        "Name": {
            "CH": "愚人众先遣队 · 岩使游击兵",
            "EN": "Fatui Skirmisher - Geochanter Bracer"
        },
        "Title": [
            {
                "CH": "愚人众先遣队 · 岩使游击兵",
                "EN": "Fatui Skirmisher - Geochanter Bracer"
            }
        ],
        "Desc": {
            "CH": "能驱使岩元素构成护罩，保护战友的先遣队士兵。\n不同于少数精锐的债务处理人与萤术士，先遣队的单兵实力与装备较为薄弱。\n为了完成任务，为了减少人员与物资的损耗，先遣队的战术更讲究团队协作。",
            "EN": "A skirmisher with the ability to generate a Geo Shield and defend their allies.\nUnlike the elite Mages and Agents, Fatui Skirmishers are soldiers of lesser individual might.\nTo complete their tasks and reduce losses in men and materiel, their tactics are thus more reliant on teamwork."
        },
        "Color": "Rock",
        "HP": 3.0,
        "HPCurve": 1,
        "ATK": 3.0,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": -0.2
        },
        "RESState": [
            {
                "State": "Skirmisher_Improved",
                "All": 1.0
            }
        ],
        "Ball": {
            "Type": 7,
            "Point": 3,
            "Drop": "222"
        },
        "Icon": [
            "fsg"
        ],
        "Grade": 1,
        "Weight": 80,
        "EndureTemplateID": 150,
        "Drop": [
            {
                "Type": 8,
                "Amount": [
                    4.202,
                    1.1205,
                    0.28,
                    0
                ]
            }
        ],
        "Csx": [
            "ECFatui/岩使游击兵"
        ]
    },
    "30105": {
        "_id": 30105,
        "KingdomID": 3,
        "ClassID": 10301,
        "Name": {
            "CH": "愚人众先遣队 · 风拳前锋军",
            "EN": "Fatui Skirmisher - Anemoboxer Vanguard"
        },
        "Title": [
            {
                "CH": "愚人众先遣队 · 风拳前锋军",
                "EN": "Fatui Skirmisher - Anemoboxer Vanguard"
            }
        ],
        "Desc": {
            "CH": "能利用风的力量，复苏战友的先遣队士兵。\n远离故土又为了故乡。告别了亲人却获得了能相托以生死的战友。\n——或许他们以任务的名义干着丑恶的勾当时，也带着这样的想法。",
            "EN": "A skirmisher armed with the ability to manipulate Anemo and heal their comrades.\nThey departed from their motherland and did so for its sake. They left their family, yet gained allies to stick through thick and thin with.\n—Perhaps such thoughts are what get them through the unscrupulous work that their missions entail."
        },
        "Color": "Wind",
        "HP": 3.0,
        "HPCurve": 1,
        "ATK": 6.0,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": -0.2
        },
        "Ball": {
            "Type": 6,
            "Point": 3,
            "Drop": "222"
        },
        "Icon": [
            "fsa"
        ],
        "Grade": 1,
        "Weight": 120,
        "EndureTemplateID": 210,
        "Drop": [
            {
                "Type": 8,
                "Amount": [
                    4.202,
                    1.1205,
                    0.28,
                    0
                ]
            }
        ],
        "Csx": [
            "ECFatui/风拳前锋军"
        ]
    },
    "30106": {
        "_id": 30106,
        "KingdomID": 3,
        "ClassID": 10301,
        "Name": {
            "CH": "愚人众先遣队 · 火铳游击兵",
            "EN": "Fatui Skirmisher - Pyroslinger Bracer"
        },
        "Title": [
            {
                "CH": "愚人众先遣队 · 火铳游击兵",
                "EN": "Fatui Skirmisher - Pyroslinger Bracer"
            }
        ],
        "Desc": {
            "CH": "操使着烈焰之铳的愚人众士兵。\n先遣队配置的武器在一定程度上能制御元素，战斗策略上则会利用元素反应。\n因为拥有这样的武装与知识，先遣队团队作战时能带来无法小觑的威胁。",
            "EN": "A Fatui soldier who wields a flaming gun.\nSkirmisher weapons can control the elements to a certain extent, and their combat tactics utilize elemental reactions.\nTheir ordnance and knowledge of squad-based combat thus constitute no small threat in battle."
        },
        "Color": "Fire",
        "HP": 3.0,
        "HPCurve": 1,
        "ATK": 4.5,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": -0.2
        },
        "RESState": [
            {
                "State": "Skirmisher_Improved",
                "All": 1.0
            }
        ],
        "Ball": {
            "Type": 1,
            "Point": 3,
            "Drop": "222"
        },
        "Icon": [
            "fsp"
        ],
        "Grade": 1,
        "Weight": 80,
        "EndureTemplateID": 150,
        "Drop": [
            {
                "Type": 8,
                "Amount": [
                    4.202,
                    1.1205,
                    0.28,
                    0
                ]
            }
        ],
        "Csx": [
            "ECFatui/火铳游击兵"
        ]
    },
    "30201": {
        "_id": 30201,
        "KingdomID": 3,
        "ClassID": 10302,
        "Name": {
            "CH": "愚人众 · 火之债务处理人",
            "EN": "Fatui Pyro Agent"
        },
        "Title": [
            {
                "CH": "债务处理人 · 焚毁之锋",
                "EN": "Agent - Incinerating Blades"
            }
        ],
        "Desc": {
            "CH": "隶属愚人众的特务人员。\n债务处理人所处理的「债务」，并不局限于钱货往来的清算。\n「以眼还眼，以牙还牙」也是一种债。无论是谁，只要损害了愚人众的利益，那就要十倍奉还。",
            "EN": "A Fatui secret agent.\nThe duty of a Fatui agent is to settle debts — but not only those of a monetary or goods-in-kind nature.\nThey also ensure dues are paid when it comes to the principle of \"an eye for an eye.\" If there is one thing the Fatui are not known for, it is leniency, and whoever dares to oppose them will invoke the full force of their wrath."
        },
        "Color": "Fire",
        "HP": 5.0,
        "HPCurve": 1,
        "ATK": 8.0,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": -0.2,
            "Fire": 0.5
        },
        "Ball": {
            "Type": 0,
            "Point": 3,
            "Drop": "333"
        },
        "Icon": [
            "fpa"
        ],
        "Grade": 2,
        "Weight": 80,
        "EndureTemplateID": 210,
        "Drop": [
            {
                "Type": 20,
                "Amount": [
                    0,
                    5.2526,
                    1.4006,
                    0.3502
                ]
            },
            {
                "Type": 8,
                "Amount": [
                    0.5253,
                    0.1401,
                    0.035,
                    0
                ]
            }
        ],
        "Csx": [
            "ECFatui/债务处理人"
        ]
    },
    "30301": {
        "_id": 30301,
        "KingdomID": 3,
        "ClassID": 10303,
        "Name": {
            "CH": "愚人众 · 雷萤术士",
            "EN": "Fatui Electro Cicin Mage"
        },
        "Title": [
            {
                "CH": "萤术士 · 拥雷的少女",
                "EN": "Cicin Mage - Thunder Embracer"
            }
        ],
        "Desc": {
            "CH": "操控着雷萤进行战斗，由来与面具下的容貌都是谜的愚人众少女术士。\n会利用飞萤喜爱「雾虚草」的特性，操控它们为自己而战。正如雾虚草的气味会令雷萤迷醉，嗜虐的术士也贪恋着戏弄对手的快感。",
            "EN": "A Fatui mage who can command Electro Cicins in battle. Their origins and what they look like under that mask are both mysteries.\nSimilarly to the way that Electro Cicins go crazy for Mist Grass, Cicin Mages take great pleasure in toying with their prey."
        },
        "Color": "Elec",
        "HP": 5.0,
        "HPCurve": 1,
        "ATK": 4.0,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": -0.2,
            "Elec": 0.5
        },
        "Ball": {
            "Type": 0,
            "Point": 3,
            "Drop": "333"
        },
        "Icon": [
            "fecm"
        ],
        "Grade": 2,
        "Weight": 75,
        "EndureTemplateID": 100,
        "Drop": [
            {
                "Type": 21,
                "Amount": [
                    0,
                    5.2526,
                    1.4006,
                    0.3502
                ]
            },
            {
                "Type": 8,
                "Amount": [
                    0.5253,
                    0.1401,
                    0.035,
                    0
                ]
            }
        ],
        "Csx": [
            "ECFatui/雷萤术士"
        ]
    },
    "30401": {
        "_id": 30401,
        "KingdomID": 3,
        "ClassID": 10303,
        "Name": {
            "CH": "愚人众 · 冰萤术士",
            "EN": "Fatui Cryo Cicin Mage"
        },
        "Title": [
            {
                "CH": "萤术士 · 降霜的少女",
                "EN": "Cicin Mage - Hoarfrost Wielder"
            }
        ],
        "Desc": {
            "CH": "操控着冰萤进行战斗，由来与面具下的容貌都是谜的愚人众少女术士。\n没有神之眼的人类，理应是无法像她们这样自如地操控元素的；但她们却在利用雾虚草操控冰萤之余，肆无忌惮地挥霍着寒冰的力量。看似漫无目的地四处游荡的她们，又领受着何种不为人知的使命呢…",
            "EN": "A Fatui mage who can command Cryo Cicins in battle. Their origins and what they look like under that mask are both mysteries.\nReason dictates that those who lack Visions should not be able to control the elements as these mages do. However, in addition to using Mist Grass to control the Cryo Cicins, they also boldly wield the might of frost. Seeing them as they wander the land in their seemingly aimless way, one can't help but wonder about the duty that must compel them..."
        },
        "Color": "Ice",
        "HP": 5.0,
        "HPCurve": 1,
        "ATK": 4.0,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": -0.2,
            "Ice": 0.5
        },
        "Ball": {
            "Type": 0,
            "Point": 3,
            "Drop": "333"
        },
        "Icon": [
            "fccm"
        ],
        "Grade": 2,
        "Weight": 75,
        "EndureTemplateID": 100,
        "Drop": [
            {
                "Type": 21,
                "Amount": [
                    0,
                    5.2526,
                    1.4006,
                    0.3502
                ]
            },
            {
                "Type": 8,
                "Amount": [
                    0.5253,
                    0.1401,
                    0.035,
                    0
                ]
            }
        ],
        "Csx": [
            "ECFatui/冰萤术士"
        ]
    },
    "30501": {
        "_id": 30501,
        "KingdomID": 3,
        "ClassID": 10305,
        "Name": {
            "CH": "愚人众 · 藏镜仕女",
            "EN": "Mirror Maiden"
        },
        "Title": [
            {
                "CH": "冬国仕女 · 水镜的使者",
                "EN": "Snezhnayan Maiden - Mirror Envoy"
            }
        ],
        "Desc": {
            "CH": "操纵着水镜进行战斗，有着仿如侍奉神明般的端庄姿态的愚人众仕女。\n优雅自如地操控着水镜的力量，让为之所炫的人，心甘情愿地坠入她的幻境。似乎一切都逃不过她的耳朵。被蒙住的双眼，不知见识过什么景象，也不知其中会倒映出怎样的执妄…",
            "EN": "A Fatui mage who can command a Hydro Mirror in battle. Her posture is highly dignified, as if she is one who worships a god.\nThe Hydro Mirror she manipulates lures those enchanted by her to willingly fall for her illusion. Nothing seems to escape her ears. No one knows what those covered eyes have seen, nor what kind of obsession might be reflected in them..."
        },
        "Color": "Water",
        "HP": 3.0,
        "HPCurve": 2,
        "ATK": 2.4,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": -0.2,
            "Water": 0.5
        },
        "Ball": {
            "Type": 0,
            "Point": 3,
            "Drop": "332"
        },
        "Icon": [
            "fmm"
        ],
        "Grade": 2,
        "Weight": 80,
        "EndureTemplateID": 150,
        "Drop": [
            {
                "Type": 22,
                "Amount": [
                    0,
                    5.2526,
                    1.4006,
                    0.3502
                ]
            }
        ],
        "Csx": [
            "ECFatui/藏镜仕女"
        ]
    },
    "30601": {
        "_id": 30601,
        "KingdomID": 3,
        "ClassID": 10306,
        "Name": {
            "CH": "愚人众 · 霜剑律从",
            "EN": "Frost Operative"
        },
        "Title": [
            {
                "CH": "愚人众 · 霜役人",
                "EN": "Rimesword Servant"
            }
        ],
        "Desc": {
            "CH": "愚人众当中精锐的特务。\n自幼就被选中的人，经长久年月的教化与训练，在无数次的汰换后才能成为所谓的「役人」。是愚人众的阵列中最优秀的士兵，有着出色的身手与不渝的忠诚心。",
            "EN": "An elite agent among the Fatui.\nSelected while still children, they can only become an \"Operative\" after many long years of training and teaching, after countless rounds have winnowed the field. They are the most outstanding soldiers among the Fatui, with extraordinary skill and unswerving loyalty."
        },
        "Color": "Ice",
        "HP": 4,
        "HPCurve": 2,
        "ATK": 4.8,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": -0.2,
            "Ice": 0.5
        },
        "Ball": {
            "Type": 4,
            "Point": 4,
            "Drop": "1112"
        },
        "Icon": [
            "UI_MonsterIcon_Fatuus_Escadron_Ice"
        ],
        "Grade": 2,
        "Weight": 85,
        "EndureTemplateID": 100,
        "Tutorial": {
            "CH": "- 会以20s为周期进行「猎杀」：<b>消耗5%生命值</b>进行猎杀攻击。若自身生命值较低，生命值消耗量也会降低。<br>- 猎杀攻击命中角色后，役人会<b>恢复10%生命值</b>。猎杀攻击的每次伤害命中角色，都会施加一定量的「生命之契」，可叠加。<br>-「生命之契」会无视护盾使角色<b>持续流失生命值</b>，并抵消角色受到的治疗量，使其无法被治疗。要消除「生命之契」，累计治疗量必须达到施加的契约量。<br>- 猎杀命中具有「生命之契」的角色时，会<b>造成原本250%的伤害</b>。<br>生命之契量：16%攻击力，可叠加。生命值流失量：每秒0.4%+72，后台5%以下停止流失。",
            "EN": "- Every 20s, will perform Hunt Down: <b>Consume 5% HP</b> to unleash Hunting Attacks. It its HP is low, the comsumed amount will decrease.<br>- When Hunting Attacks hit a character, it <b>regenerates 10% HP</b>, and inflicts Bond of Life on the character, stackable.<br>- Bond of Life causes the character to <b>lose HP over time</b> even under a shield, and absorbs incoming healing, making the character unable to be healed. To clear the Bond of Life, total healing received must reach the Bond's amount.<br>- When Hunting Attacks hit a character with Bond of Life, it <b>deals 250% of the original DMG</b>.<br>Bond of Life amount: 16% ATK, stackable. HP Loss: 0.4%+72 every second, off-field stops at 5%."
        },
        "SkillDMG": [
            {
                "Name": {
                    "CH": "猎杀（目标无生命之契）",
                    "EN": "Hunt Down (Target no Bond of Life)"
                },
                "DMG": [
                    1.92
                ]
            },
            {
                "Name": {
                    "CH": "猎杀追加攻击（目标无生命之契）",
                    "EN": "Hunting Follow-Up (Target no Bond of Life)"
                },
                "DMG": [
                    2.4,
                    4.8
                ]
            },
            {
                "Name": {
                    "CH": "猎杀（目标有生命之契）",
                    "EN": "Hunt Down (Target has Bond of Life)"
                },
                "DMG": [
                    4.8
                ]
            },
            {
                "Name": {
                    "CH": "猎杀追加攻击（目标有生命之契）",
                    "EN": "Hunting Follow-Up (Target has Bond of Life)"
                },
                "DMG": [
                    6,
                    12
                ]
            }
        ],
        "Drop": [
            {
                "Type": 32,
                "Amount": [
                    0,
                    5.2526,
                    1.4006,
                    0.3502
                ]
            }
        ],
        "Csx": [
            "ECFatui/愚人众·霜役人"
        ]
    },
    "30602": {
        "_id": 30602,
        "KingdomID": 3,
        "ClassID": 10306,
        "Name": {
            "CH": "愚人众 · 幽风铃兰",
            "EN": "Wind Operative"
        },
        "Title": [
            {
                "CH": "愚人众 · 风役人",
                "EN": "Gale-Lily Servant"
            }
        ],
        "Desc": {
            "CH": "愚人众当中精锐的特务。\n役人在愚人众中并没有常规的军种编制。依据命令与任务的内容，她们既可以与士兵同进退，也能独自或者与埋在各地的「钉子」一同，进行谍报、反侦察、护卫、抹杀等工作。为了那些更为特殊的任务，役人往往也需要掌握多种多样的技能乃至才艺。",
            "EN": "An elite agent among the Fatui.\n\"Operatives\" have no ordinary military rank among the Fatui. Based on their orders and the content of their missions, they can work in concert with other soldiers, work alone, or in tandem with the \"tacks\" hidden and scattered across the world. They spy, conduct counter-espionage, protect, assassinate, and conduct other tasks. In order to effectively carry out missions of a more special nature, \"Operatives\" also need to master all kinds of other skills and even various arts and talents."
        },
        "Color": "Wind",
        "HP": 3.5,
        "HPCurve": 2,
        "ATK": 6,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": -0.2,
            "Wind": 0.5
        },
        "Ball": {
            "Type": 6,
            "Point": 4,
            "Drop": "1112"
        },
        "Icon": [
            "UI_MonsterIcon_Fatuus_Escadron_Wind"
        ],
        "Grade": 2,
        "Weight": 85,
        "EndureTemplateID": 100,
        "Tutorial": {
            "CH": "- 会以20s为周期进行「猎杀」：<b>消耗5%生命值</b>进行猎杀攻击。若自身生命值较低，生命值消耗量也会降低。<br>- 猎杀攻击命中角色后，役人会<b>恢复10%生命值</b>。猎杀攻击的每次伤害命中角色，都会施加一定量的「生命之契」，可叠加。<br>-「生命之契」会无视护盾使角色<b>持续流失生命值</b>，并抵消角色受到的治疗量，使其无法被治疗。要消除「生命之契」，累计治疗量必须达到施加的契约量。<br>- 猎杀命中具有「生命之契」的角色时，会<b>造成原本250%的伤害</b>。<br>生命之契量：16%攻击力，可叠加。生命值流失量：每秒0.4%+72，后台5%以下停止流失。",
            "EN": "- Every 20s, will perform Hunt Down: <b>Consume 5% HP</b> to unleash Hunting Attacks. It its HP is low, the comsumed amount will decrease.<br>- When Hunting Attacks hit a character, it <b>regenerates 10% HP</b>, and inflicts Bond of Life on the character, stackable.<br>- Bond of Life causes the character to <b>lose HP over time</b> even under a shield, and absorbs incoming healing, making the character unable to be healed. To clear the Bond of Life, total healing received must reach the Bond's amount.<br>- When Hunting Attacks hit a character with Bond of Life, it <b>deals 250% of the original DMG</b>.<br>Bond of Life amount: 16% ATK, stackable. HP Loss: 0.4%+72 every second, off-field stops at 5%."
        },
        "SkillDMG": [
            {
                "Name": {
                    "CH": "猎杀（目标无生命之契）",
                    "EN": "Hunt Down (Target no Bond of Life)"
                },
                "DMG": [
                    5.4
                ]
            },
            {
                "Name": {
                    "CH": "猎杀追加攻击（目标无生命之契）",
                    "EN": "Hunting Follow-Up (Target no Bond of Life)"
                },
                "DMG": [
                    6
                ]
            },
            {
                "Name": {
                    "CH": "猎杀（目标有生命之契）",
                    "EN": "Hunt Down (Target has Bond of Life)"
                },
                "DMG": [
                    13.5
                ]
            },
            {
                "Name": {
                    "CH": "猎杀追加攻击（目标有生命之契）",
                    "EN": "Hunting Follow-Up (Target has Bond of Life)"
                },
                "DMG": [
                    15
                ]
            }
        ],
        "Drop": [
            {
                "Type": 32,
                "Amount": [
                    0,
                    5.2526,
                    1.4006,
                    0.3502
                ]
            }
        ],
        "Csx": [
            "ECFatui/愚人众·风役人"
        ]
    },
    "30603": {
        "_id": 30603,
        "KingdomID": 10,
        "ClassID": 11041,
        "Name": {
            "CH": "「以索忒」",
            "EN": "Yseut"
        },
        "Title": [
            {
                "CH": "地方传奇 · 愚人众 · 霜役人",
                "EN": "Local Legend: Frost Operative"
            }
        ],
        "Color": "Ice",
        "HP": 33,
        "HPCurve": 2,
        "ATK": 9,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": -0.2,
            "Ice": 0.5
        },
        "Ball": {
            "Type": 4,
            "Point": 4,
            "Drop": "1114"
        },
        "Icon": [
            "UI_MonsterIcon_Fatuus_Escadron_Ice"
        ],
        "Grade": 3,
        "Weight": 85,
        "EndureTemplateID": 100,
        "Tutorial": {
            "CH": "- 具有90%冻结抵抗。<br>- 会以20s为周期进行「猎杀」：<b>消耗5%生命值</b>进行猎杀攻击。若自身生命值较低，生命值消耗量也会降低。<br>- 猎杀攻击命中角色后，役人会<b>恢复10%生命值</b>。猎杀攻击的每次伤害命中角色，都会施加一定量的「生命之契」，可叠加。<br>-「生命之契」会无视护盾使角色<b>持续流失生命值</b>，并抵消角色受到的治疗量，使其无法被治疗。要消除「生命之契」，累计治疗量必须达到施加的契约量。<br>- 猎杀命中具有「生命之契」的角色时，会<b>造成原本250%的伤害</b>。<br>生命之契量：16%攻击力，可叠加。生命值流失量：每秒0.4%+72，后台5%以下停止流失。",
            "EN": "- Has 90% resistance to Freeze.<br>- Every 20s, will perform Hunt Down: <b>Consume 5% HP</b> to unleash Hunting Attacks. It its HP is low, the comsumed amount will decrease.<br>- When Hunting Attacks hit a character, it <b>regenerates 10% HP</b>, and inflicts Bond of Life on the character, stackable.<br>- Bond of Life causes the character to <b>lose HP over time</b> even under a shield, and absorbs incoming healing, making the character unable to be healed. To clear the Bond of Life, total healing received must reach the Bond's amount.<br>- When Hunting Attacks hit a character with Bond of Life, it <b>deals 250% of the original DMG</b>.<br>Bond of Life amount: 16% ATK, stackable. HP Loss: 0.4%+72 every second, off-field stops at 5%."
        },
        "SkillDMG": [
            {
                "Name": {
                    "CH": "猎杀（目标无生命之契）",
                    "EN": "Hunt Down (Target no Bond of Life)"
                },
                "DMG": [
                    3.6
                ]
            },
            {
                "Name": {
                    "CH": "猎杀追加攻击（目标无生命之契）",
                    "EN": "Hunting Follow-Up (Target no Bond of Life)"
                },
                "DMG": [
                    4.5,
                    9
                ]
            },
            {
                "Name": {
                    "CH": "猎杀（目标有生命之契）",
                    "EN": "Hunt Down (Target has Bond of Life)"
                },
                "DMG": [
                    9
                ]
            },
            {
                "Name": {
                    "CH": "猎杀追加攻击（目标有生命之契）",
                    "EN": "Hunting Follow-Up (Target has Bond of Life)"
                },
                "DMG": [
                    11.25,
                    22.5
                ]
            }
        ],
        "Csx": [
            "ECLegends/地方传奇·愚人众·霜役人"
        ]
    },
    "30604": {
        "_id": 30604,
        "KingdomID": 10,
        "ClassID": 11042,
        "Name": {
            "CH": "「雪奈茨芙娜的蝶安奈拉」",
            "EN": "Deianeira of Snezhevna"
        },
        "Title": [
            {
                "CH": "地方传奇 · 愚人众 · 风役人",
                "EN": "Local Legend: Wind Operative"
            }
        ],
        "Color": "Wind",
        "HP": 31.3,
        "HPCurve": 2,
        "ATK": 10,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": -0.2,
            "Wind": 0.5
        },
        "Ball": {
            "Type": 6,
            "Point": 4,
            "Drop": "1114"
        },
        "Icon": [
            "UI_MonsterIcon_Fatuus_Escadron_Wind"
        ],
        "Grade": 3,
        "Weight": 85,
        "EndureTemplateID": 100,
        "Tutorial": {
            "CH": "- 具有90%冻结抵抗。<br>- 会以20s为周期进行「猎杀」：<b>消耗5%生命值</b>进行猎杀攻击。若自身生命值较低，生命值消耗量也会降低。<br>- 猎杀攻击命中角色后，役人会<b>恢复10%生命值</b>。猎杀攻击的每次伤害命中角色，都会施加一定量的「生命之契」，可叠加。<br>-「生命之契」会无视护盾使角色<b>持续流失生命值</b>，并抵消角色受到的治疗量，使其无法被治疗。要消除「生命之契」，累计治疗量必须达到施加的契约量。<br>- 猎杀命中具有「生命之契」的角色时，会<b>造成原本250%的伤害</b>。<br>生命之契量：16%攻击力，可叠加。生命值流失量：每秒0.4%+72，后台5%以下停止流失。",
            "EN": "- Has 90% resistance to Freeze.<br>- Every 20s, will perform Hunt Down: <b>Consume 5% HP</b> to unleash Hunting Attacks. It its HP is low, the comsumed amount will decrease.<br>- When Hunting Attacks hit a character, it <b>regenerates 10% HP</b>, and inflicts Bond of Life on the character, stackable.<br>- Bond of Life causes the character to <b>lose HP over time</b> even under a shield, and absorbs incoming healing, making the character unable to be healed. To clear the Bond of Life, total healing received must reach the Bond's amount.<br>- When Hunting Attacks hit a character with Bond of Life, it <b>deals 250% of the original DMG</b>.<br>Bond of Life amount: 16% ATK, stackable. HP Loss: 0.4%+72 every second, off-field stops at 5%."
        },
        "SkillDMG": [
            {
                "Name": {
                    "CH": "猎杀（目标无生命之契）",
                    "EN": "Hunt Down (Target no Bond of Life)"
                },
                "DMG": [
                    9
                ]
            },
            {
                "Name": {
                    "CH": "猎杀追加攻击（目标无生命之契）",
                    "EN": "Hunting Follow-Up (Target no Bond of Life)"
                },
                "DMG": [
                    10
                ]
            },
            {
                "Name": {
                    "CH": "猎杀（目标有生命之契）",
                    "EN": "Hunt Down (Target has Bond of Life)"
                },
                "DMG": [
                    22.5
                ]
            },
            {
                "Name": {
                    "CH": "猎杀追加攻击（目标有生命之契）",
                    "EN": "Hunting Follow-Up (Target has Bond of Life)"
                },
                "DMG": [
                    25
                ]
            }
        ],
        "Csx": [
            "ECLegends/地方传奇·愚人众·风役人"
        ]
    },
    "30605": {
        "_id": 30605,
        "KingdomID": 10,
        "ClassID": 11006,
        "Name": {
            "CH": "历经百战的霜剑律从",
            "EN": "Veteran Frost Operative"
        },
        "Title": [
            {
                "CH": "地方传奇 · 愚人众 · 霜役人",
                "EN": "Local Legend: Frost Operative"
            }
        ],
        "Color": "Ice",
        "HP": 6.6,
        "HPCurve": 2,
        "ATK": 2.7,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": -0.2,
            "Ice": 0.5
        },
        "Ball": {
            "Type": 4,
            "Point": 4,
            "Drop": "1114"
        },
        "Icon": [
            "UI_MonsterIcon_Fatuus_Escadron_Ice"
        ],
        "Grade": 3,
        "Weight": 85,
        "EndureTemplateID": 100,
        "Tutorial": {
            "CH": "- 具有90%冻结抵抗。<br>- 会以20s为周期进行「猎杀」：<b>消耗5%生命值</b>进行猎杀攻击。若自身生命值较低，生命值消耗量也会降低。<br>- 猎杀攻击命中角色后，役人会<b>恢复10%生命值</b>。猎杀攻击的每次伤害命中角色，都会施加一定量的「生命之契」，可叠加。<br>-「生命之契」会无视护盾使角色<b>持续流失生命值</b>，并抵消角色受到的治疗量，使其无法被治疗。要消除「生命之契」，累计治疗量必须达到施加的契约量。<br>- 猎杀命中具有「生命之契」的角色时，会<b>造成原本250%的伤害</b>。<br>生命之契量：16%攻击力，可叠加。生命值流失量：每秒0.4%+72，后台5%以下停止流失。",
            "EN": "- Has 90% resistance to Freeze.<br>- Every 20s, will perform Hunt Down: <b>Consume 5% HP</b> to unleash Hunting Attacks. It its HP is low, the comsumed amount will decrease.<br>- When Hunting Attacks hit a character, it <b>regenerates 10% HP</b>, and inflicts Bond of Life on the character, stackable.<br>- Bond of Life causes the character to <b>lose HP over time</b> even under a shield, and absorbs incoming healing, making the character unable to be healed. To clear the Bond of Life, total healing received must reach the Bond's amount.<br>- When Hunting Attacks hit a character with Bond of Life, it <b>deals 250% of the original DMG</b>.<br>Bond of Life amount: 16% ATK, stackable. HP Loss: 0.4%+72 every second, off-field stops at 5%."
        },
        "SkillDMG": [
            {
                "Name": {
                    "CH": "猎杀（目标无生命之契）",
                    "EN": "Hunt Down (Target no Bond of Life)"
                },
                "DMG": [
                    1.08
                ]
            },
            {
                "Name": {
                    "CH": "猎杀追加攻击（目标无生命之契）",
                    "EN": "Hunting Follow-Up (Target no Bond of Life)"
                },
                "DMG": [
                    1.35,
                    2.7
                ]
            },
            {
                "Name": {
                    "CH": "猎杀（目标有生命之契）",
                    "EN": "Hunt Down (Target has Bond of Life)"
                },
                "DMG": [
                    2.7
                ]
            },
            {
                "Name": {
                    "CH": "猎杀追加攻击（目标有生命之契）",
                    "EN": "Hunting Follow-Up (Target has Bond of Life)"
                },
                "DMG": [
                    3.375,
                    6.75
                ]
            }
        ],
        "Csx": [
            "ECLegends/地方传奇·愚人众·霜役人"
        ]
    },
    "30606": {
        "_id": 30606,
        "KingdomID": 10,
        "ClassID": 11006,
        "Name": {
            "CH": "历经百战的幽风铃兰",
            "EN": "Veteran Wind Operative"
        },
        "Title": [
            {
                "CH": "地方传奇 · 愚人众 · 风役人",
                "EN": "Local Legend: Wind Operative"
            }
        ],
        "Color": "Wind",
        "HP": 6.26,
        "HPCurve": 2,
        "ATK": 3,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": -0.2,
            "Wind": 0.5
        },
        "Ball": {
            "Type": 6,
            "Point": 4,
            "Drop": "1114"
        },
        "Icon": [
            "UI_MonsterIcon_Fatuus_Escadron_Wind"
        ],
        "Grade": 3,
        "Weight": 85,
        "EndureTemplateID": 100,
        "Tutorial": {
            "CH": "- 具有90%冻结抵抗。<br>- 会以20s为周期进行「猎杀」：<b>消耗5%生命值</b>进行猎杀攻击。若自身生命值较低，生命值消耗量也会降低。<br>- 猎杀攻击命中角色后，役人会<b>恢复10%生命值</b>。猎杀攻击的每次伤害命中角色，都会施加一定量的「生命之契」，可叠加。<br>-「生命之契」会无视护盾使角色<b>持续流失生命值</b>，并抵消角色受到的治疗量，使其无法被治疗。要消除「生命之契」，累计治疗量必须达到施加的契约量。<br>- 猎杀命中具有「生命之契」的角色时，会<b>造成原本250%的伤害</b>。<br>生命之契量：16%攻击力，可叠加。生命值流失量：每秒0.4%+72，后台5%以下停止流失。",
            "EN": "- Has 90% resistance to Freeze.<br>- Every 20s, will perform Hunt Down: <b>Consume 5% HP</b> to unleash Hunting Attacks. It its HP is low, the comsumed amount will decrease.<br>- When Hunting Attacks hit a character, it <b>regenerates 10% HP</b>, and inflicts Bond of Life on the character, stackable.<br>- Bond of Life causes the character to <b>lose HP over time</b> even under a shield, and absorbs incoming healing, making the character unable to be healed. To clear the Bond of Life, total healing received must reach the Bond's amount.<br>- When Hunting Attacks hit a character with Bond of Life, it <b>deals 250% of the original DMG</b>.<br>Bond of Life amount: 16% ATK, stackable. HP Loss: 0.4%+72 every second, off-field stops at 5%."
        },
        "SkillDMG": [
            {
                "Name": {
                    "CH": "猎杀（目标无生命之契）",
                    "EN": "Hunt Down (Target no Bond of Life)"
                },
                "DMG": [
                    2.7
                ]
            },
            {
                "Name": {
                    "CH": "猎杀追加攻击（目标无生命之契）",
                    "EN": "Hunting Follow-Up (Target no Bond of Life)"
                },
                "DMG": [
                    3
                ]
            },
            {
                "Name": {
                    "CH": "猎杀（目标有生命之契）",
                    "EN": "Hunt Down (Target has Bond of Life)"
                },
                "DMG": [
                    6.75
                ]
            },
            {
                "Name": {
                    "CH": "猎杀追加攻击（目标有生命之契）",
                    "EN": "Hunting Follow-Up (Target has Bond of Life)"
                },
                "DMG": [
                    7.5
                ]
            }
        ],
        "Csx": [
            "ECLegends/地方传奇·愚人众·风役人"
        ]
    },
    "40101": {
        "_id": 40101,
        "KingdomID": 4,
        "ClassID": 104010,
        "Name": {
            "CH": "遗迹守卫",
            "EN": "Ruin Guard"
        },
        "Title": [
            {
                "CH": "遗迹守卫",
                "EN": "Ruin Guard"
            }
        ],
        "Desc": {
            "CH": "古老的人形战斗机械。\n传说是已经覆灭的国度留下的战争机械，会徘徊至世界各地的遗迹废都，并攻击冒失的旅人。研究表明遗迹守卫与这些废墟并非同源，表明两者的时代也相差至少数千年，为何遗迹守卫会驻留在其中，并以「爆炸性的火力」守护这些残垣断壁呢…",
            "EN": "An ancient humanoid war machine.\nLegend has it that these machines were combat automatons created by a long-destroyed nation. They now wander ruins and ancient detritus, attacking offending travelers. Research indicates that the Ruin Guards and the ruins they operate in do not originate from the same source, suggesting that there may be a difference of as much as several thousand years between them on the timeline. So why then do the Ruin Guards continue staying among these ruins, using their \"explosive firepower\" to protect the crumbling walls and broken tiles?"
        },
        "HP": 7.0,
        "HPCurve": 1,
        "ATK": 4.8,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.7
        },
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "4443"
        },
        "Icon": [
            "rgi"
        ],
        "Grade": 2,
        "Weight": 300,
        "EndureTemplateID": 4000,
        "SkillData": [
            "4/40101"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "脚踩+手拍",
                    "EN": "Foot stomp + Hand clap"
                },
                "DMG": [
                    3,
                    4.2
                ]
            },
            {
                "Name": {
                    "CH": "左手+右手+跳砸",
                    "EN": "Left hand + Right hand + Jump"
                },
                "DMG": [
                    2.4,
                    3.6,
                    4.8
                ]
            },
            {
                "Name": {
                    "CH": "发射导弹",
                    "EN": "Shoot missiles"
                },
                "DMG": [
                    1.5
                ]
            },
            {
                "Name": {
                    "CH": "大风车",
                    "EN": "Whirlwind"
                },
                "DMG": [
                    3.6
                ]
            }
        ],
        "Drop": [
            {
                "Type": 17,
                "Amount": [
                    0,
                    2.6263,
                    0.7003,
                    0.1751
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/遗迹守卫"
        ]
    },
    "40102": {
        "_id": 40102,
        "KingdomID": 4,
        "ClassID": 104010,
        "Name": {
            "CH": "遗迹猎者",
            "EN": "Ruin Hunter"
        },
        "Title": [
            {
                "CH": "遗迹猎者",
                "EN": "Ruin Hunter"
            }
        ],
        "Desc": {
            "CH": "巨大的异形战斗机械。\n传说是已经覆灭的国度留下的战争机械。似乎是为了提高战斗性能，设计上放弃了低效的拟人形态。无论是白刃战还是远程炮火的威力都相当恐怖。\n仅仅是废弃的古代兵器就有如此能力，曾经能生产这种兵器的文明又是如何呢…",
            "EN": "A giant alien-looking war machine.\nThey are said to be remnants from a lost ancient nation. It seems that the creators opted to forgo the low-performing humanoid-design in pursuit of improved combat effectiveness. They are extremely dangerous both in melee and ranged combat.\nConsidering these mere relics possess such extraordinary power, one cannot help but wonder what the civilization that produced them must have looked like at its height."
        },
        "HP": 6.0,
        "HPCurve": 1,
        "ATK": 14.0,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.5
        },
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "4443"
        },
        "Icon": [
            "rhu"
        ],
        "Grade": 2,
        "Weight": 300,
        "EndureTemplateID": 4000,
        "SkillData": [
            "4/40102"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "单划一次",
                    "EN": "Slash once"
                },
                "DMG": [
                    5.6
                ]
            },
            {
                "Name": {
                    "CH": "划3次",
                    "EN": "Slash 3 times"
                },
                "DMG": [
                    4.2,
                    5.6,
                    8.4
                ]
            },
            {
                "Name": {
                    "CH": "毒龙钻",
                    "EN": "Charge and PIERCE"
                },
                "DMG": [
                    14
                ]
            },
            {
                "Name": {
                    "CH": "空中：红圈轰炸",
                    "EN": "Air: Red circle bombardment"
                },
                "DMG": [
                    3.5
                ]
            },
            {
                "Name": {
                    "CH": "空中：机关枪 + 小导弹 + 大火炮",
                    "EN": "Air: Gun + Small missile + HUGE BOOM"
                },
                "DMG": [
                    1.4,
                    0.7,
                    14
                ]
            },
            {
                "Name": {
                    "CH": "空中：垂直激光",
                    "EN": "Air: Vertical Lasers"
                },
                "DMG": [
                    7
                ]
            }
        ],
        "Drop": [
            {
                "Type": 17,
                "Amount": [
                    0,
                    2.6263,
                    0.7003,
                    0.1751
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/遗迹猎者"
        ]
    },
    "40103": {
        "_id": 40103,
        "KingdomID": 4,
        "ClassID": 104010,
        "Name": {
            "CH": "遗迹重机",
            "EN": "Ruin Grader"
        },
        "Title": [
            {
                "CH": "遗迹重机",
                "EN": "Ruin Grader"
            }
        ],
        "Desc": {
            "CH": "古老的人形战斗机械。\n形态与遗迹守卫相似，但形态更为扭曲、力量更为强大。似乎是为了驱动过于沉重的躯体，在双脚上还额外装配了能量核心。",
            "EN": "An ancient humanoid war machine.\nTheir form is similar to that of Ruin Guards, but is more distorted and more powerful. Energy cores have been installed in both its legs, as though to power its overly heavy form."
        },
        "HP": 9.0,
        "HPCurve": 1,
        "ATK": 7.0,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.7
        },
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "4443"
        },
        "Icon": [
            "rgr"
        ],
        "Grade": 2,
        "Weight": 300,
        "EndureTemplateID": 4000,
        "SkillDMG": [
            {
                "Name": {
                    "CH": "冲刺",
                    "EN": "Rush"
                },
                "DMG": [
                    2.1
                ]
            },
            {
                "Name": {
                    "CH": "三小球 高频伤害",
                    "EN": "3 Balls DoT"
                },
                "DMG": [
                    1.12
                ]
            },
            {
                "Name": {
                    "CH": "追踪激光 高频伤害",
                    "EN": "Chasing Laser DoT"
                },
                "DMG": [
                    2.8
                ]
            },
            {
                "Name": {
                    "CH": "脚踩 + 膝盖砸",
                    "EN": "Foot stomp + Knee smash"
                },
                "DMG": [
                    2.8,
                    4.9
                ]
            },
            {
                "Name": {
                    "CH": "手拍 + 短激光",
                    "EN": "Hand clap + Short laser"
                },
                "DMG": [
                    3.5,
                    5.6
                ]
            },
            {
                "Name": {
                    "CH": "虚弱恢复冲击 内圈",
                    "EN": "Weakness Recover Shock: Inner Circle"
                },
                "DMG": [
                    7
                ]
            },
            {
                "Name": {
                    "CH": "虚弱恢复冲击 外圈",
                    "EN": "Weakness Recover Shock: Outer Circle"
                },
                "DMG": [
                    5.6
                ]
            }
        ],
        "Drop": [
            {
                "Type": 17,
                "Amount": [
                    0,
                    5.2526,
                    1.4006,
                    0.3502
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/遗迹重机"
        ]
    },
    "40104": {
        "_id": 40104,
        "KingdomID": 4,
        "ClassID": 104011,
        "Name": {
            "CH": "遗迹巨蛇",
            "EN": "Ruin Serpent"
        },
        "Title": [
            {
                "CH": "进路开掘 · SI/Glp",
                "EN": "Excavator: SI/Glp"
            }
        ],
        "Desc": {
            "CH": "古老的异形自律机关。\n传说是已经覆灭的国度留下的工程重机，「头部」的齿盘结构能够轻松挖开坚硬的岩盘。果真如此的话，借由这种能力，应该能开凿出足以让大军通行大陆各地的黯道吧。",
            "EN": "An ancient, bizarrely-shaped autonomous machine.\nIt is said that this was heavy-duty engineering equipment left behind by a nation that has been destroyed. The gear system on its \"head\" can grind its way through even the hardest bedrock. If this was the case, then carving out tunnels with which the forces of this nation might access the whole continent would have been a thing of great ease."
        },
        "HP": 8.0,
        "HPCurve": 2,
        "ATK": 6.0,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": 0.7
        },
        "RESState": [
            {
                "State": "Nithhoggr_Ult",
                "All": 3.0
            }
        ],
        "Ball": {
            "Type": 0,
            "Point": 3,
            "Drop": "332"
        },
        "Icon": [
            "rs"
        ],
        "Grade": 3,
        "Weight": 1000,
        "EndureTemplateID": 4000,
        "SkillDMG": [
            {
                "Name": {
                    "CH": "冲过来钻地 + 尾巴出来钉地",
                    "EN": "Rush and dig + Tail nail ground"
                },
                "DMG": [
                    1.8,
                    4.2
                ]
            },
            {
                "Name": {
                    "CH": "身子砸下来 + 来回滚",
                    "EN": "Fall down + Roll back and forth"
                },
                "DMG": [
                    3,
                    2.4
                ]
            },
            {
                "Name": {
                    "CH": "沉沙之冢：巨大漩涡 每秒伤害",
                    "EN": "Sand Tomb: Huge Vortex DoT every second"
                },
                "DMG": [
                    0.7
                ]
            },
            {
                "Name": {
                    "CH": "沉沙之冢：结束时钻出 + 钻地",
                    "EN": "Sand Tomb: Skill end dig out and back in"
                },
                "DMG": [
                    4.9
                ]
            },
            {
                "Name": {
                    "CH": "大招一：天女散花（1黑泥伤害 / 2黑泥伤害）",
                    "EN": "Ultimate 1: A Thousand Bullets (1 Mud / 2 Mud)"
                },
                "DMG": [
                    2.1,
                    2.8
                ]
            },
            {
                "Name": {
                    "CH": "大招二：单次爆轰（1黑泥伤害 / 2黑泥伤害）",
                    "EN": "Ultimate 2: Single Blast (1 Mud / 2 Mud)"
                },
                "DMG": [
                    6,
                    12
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/遗迹巨蛇"
        ]
    },
    "40201": {
        "_id": 40201,
        "KingdomID": 4,
        "ClassID": 10402,
        "Name": {
            "CH": "遗迹巡弋者",
            "EN": "Ruin Cruiser"
        },
        "Title": [
            {
                "CH": "遗迹巡弋者",
                "EN": "Ruin Cruiser"
            }
        ],
        "Desc": {
            "CH": "为了适应特殊的目标，有着定制化外形与机能的异形机械。\n传说是已经覆灭的国度留下的战争机械。相比更为广泛存在的「遗迹守卫」，在形态上有着更强的仿生学意义。分散化的设计和战斗模式似乎模仿了蜂群，能够变换成各种形态从不同方向发起攻击。",
            "EN": "Various bizarrely-shaped machines that have taken different forms and functions to adapt to different goals.\nLegend has it that they are war machines left behind by a nation that has already been destroyed. Compared to the more common Ruin Guards, their forms have greater value in the study of biomimesis. The inspiration for its decentralized design and combat modes seem to have come from honeybee colonies, rendering it able to change forms and to attack from different directions."
        },
        "HP": 1.5,
        "HPCurve": 2,
        "ATK": 4.2,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.3
        },
        "Ball": {
            "Type": 0,
            "Point": 3,
            "Drop": "112"
        },
        "Icon": [
            "rcr2"
        ],
        "Grade": 2,
        "Weight": 300,
        "EndureTemplateID": 400,
        "SkillData": [
            "4/40201"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "后撤留球",
                    "EN": "Move back and leave ball"
                },
                "DMG": [
                    1.68
                ]
            },
            {
                "Name": {
                    "CH": "单射线",
                    "EN": "Single Laser"
                },
                "DMG": [
                    2.94
                ]
            },
            {
                "Name": {
                    "CH": "发射三小球",
                    "EN": "Shoot 3 Balls"
                },
                "DMG": [
                    4.2
                ]
            },
            {
                "Name": {
                    "CH": "区域覆盖炮（开启 / 后续）",
                    "EN": "Area Fire (Begin DMG / Followup DMG)"
                },
                "DMG": [
                    1.26,
                    1.68
                ]
            }
        ],
        "Drop": [
            {
                "Type": 18,
                "Amount": [
                    0,
                    2.6263,
                    0.7003,
                    0.1751
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/遗迹巡弋者"
        ]
    },
    "40202": {
        "_id": 40202,
        "KingdomID": 4,
        "ClassID": 10402,
        "Name": {
            "CH": "遗迹歼击者",
            "EN": "Ruin Destroyer"
        },
        "Title": [
            {
                "CH": "遗迹歼击者",
                "EN": "Ruin Destroyer"
            }
        ],
        "Desc": {
            "CH": "为了适应特殊的目标，有着定制化外形与机能的异形机械。\n传说是已经覆灭的国度留下的战争机械。相比更为广泛存在的「遗迹守卫」，在设计上有着更强的仿生学意义。由若干部件构成的巨大花冠像是某种可怕的植物，不知道经历过什么才能设计出这种可怕的机械。",
            "EN": "Various bizarrely-shaped machines that have taken different forms and functions to adapt to different goals.\nLegend has it that they are war machines left behind by a nation that has already been destroyed. Compared to the more common Ruin Guards, their design has greater value in the study of biomimesis. The giant crown, composed of several parts, looks like some sort of frightening plant. One wonders what one has to go through in order to design such scary machine."
        },
        "HP": 1.5,
        "HPCurve": 2,
        "ATK": 4.2,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.3
        },
        "Ball": {
            "Type": 0,
            "Point": 3,
            "Drop": "112"
        },
        "Icon": [
            "rdt2"
        ],
        "Grade": 2,
        "Weight": 300,
        "EndureTemplateID": 400,
        "SkillData": [
            "4/40202"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "甩头",
                    "EN": "Swing Head"
                },
                "DMG": [
                    1.26
                ]
            },
            {
                "Name": {
                    "CH": "黑洞（每0.4s持续伤害 / 结束爆炸）",
                    "EN": "Black Hole (DoT every 0.4s / End blast)"
                },
                "DMG": [
                    2.1,
                    6.3
                ]
            },
            {
                "Name": {
                    "CH": "烫地板 每0.4s持续伤害",
                    "EN": "Scorching Sands DoT every 0.4s"
                },
                "DMG": [
                    1.68
                ]
            },
            {
                "Name": {
                    "CH": "扔6子弹",
                    "EN": "Throw 6 Bullets"
                },
                "DMG": [
                    1.68
                ]
            }
        ],
        "Drop": [
            {
                "Type": 18,
                "Amount": [
                    0,
                    2.6263,
                    0.7003,
                    0.1751
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/遗迹歼击者"
        ]
    },
    "40203": {
        "_id": 40203,
        "KingdomID": 4,
        "ClassID": 10402,
        "Name": {
            "CH": "遗迹防卫者",
            "EN": "Ruin Defender"
        },
        "Title": [
            {
                "CH": "遗迹防卫者",
                "EN": "Ruin Defender"
            }
        ],
        "Desc": {
            "CH": "为了适应特殊的目标，有着定制化外形与机能的异形机械。\n传说是已经覆灭的国度留下的战争机械。相比更为广泛存在的「遗迹守卫」，在设计上有着更强的仿生学意义。模仿昆虫的肢体相较于拟人机器更加方便移动，各部件结成的护盾能够抵御任何来自正面的攻击。",
            "EN": "Various bizarrely-shaped machines that have taken different forms and functions to adapt to different goals.\nLegend has it that they are war machines left behind by a nation that has already been destroyed. Compared to the more common Ruin Guards, their design has greater value in the study of biomimesis. The insect-like limbs are more agile than those of humanoid machines. The shield formed by its various components can resist any frontal attack."
        },
        "HP": 2.1,
        "HPCurve": 2,
        "ATK": 4.5,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.5
        },
        "Ball": {
            "Type": 0,
            "Point": 3,
            "Drop": "112"
        },
        "Icon": [
            "rdf2"
        ],
        "Grade": 2,
        "Weight": 300,
        "EndureTemplateID": 400,
        "SkillData": [
            "4/40203"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "发射激光",
                    "EN": "Laser"
                },
                "DMG": [
                    1.197
                ]
            },
            {
                "Name": {
                    "CH": "前脚踹人",
                    "EN": "Kick with fore legs"
                },
                "DMG": [
                    4.5
                ]
            },
            {
                "Name": {
                    "CH": "跳砸",
                    "EN": "Jump and land"
                },
                "DMG": [
                    4.5
                ]
            },
            {
                "Name": {
                    "CH": "盾前喷",
                    "EN": "Shield burst"
                },
                "DMG": [
                    4.5
                ]
            }
        ],
        "Drop": [
            {
                "Type": 18,
                "Amount": [
                    0,
                    2.6263,
                    0.7003,
                    0.1751
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/遗迹防卫者"
        ]
    },
    "40204": {
        "_id": 40204,
        "KingdomID": 4,
        "ClassID": 10402,
        "Name": {
            "CH": "遗迹侦察者",
            "EN": "Ruin Scout"
        },
        "Title": [
            {
                "CH": "遗迹侦察者",
                "EN": "Ruin Scout"
            }
        ],
        "Desc": {
            "CH": "为了适应特殊的目标，有着定制化外形与机能的异形机械。\n传说是已经覆灭的国度留下的战争机械。相比更为广泛存在的「遗迹守卫」，在设计上有着更强的仿生学意义。不知道根据什么原理在形态和行动模式上都模仿了某种深海中的生物，总之无论如何也不想被它缠上。",
            "EN": "Various bizarrely-shaped machines that have taken different forms and functions to adapt to different goals.\nLegend has it that they are war machines left behind by a nation that has already been destroyed. Compared to the more common Ruin Guards, their design has greater value in the study of biomimesis. Exactly why the form and movements of certain deep-sea lifeforms are being imitated is not known. But either way, tangling with them is hardly the best idea."
        },
        "HP": 2.1,
        "HPCurve": 2,
        "ATK": 3.9,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.5
        },
        "Ball": {
            "Type": 0,
            "Point": 3,
            "Drop": "112"
        },
        "Icon": [
            "rsc2"
        ],
        "Grade": 2,
        "Weight": 300,
        "EndureTemplateID": 400,
        "SkillData": [
            "4/40204"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "触须旋转",
                    "EN": "Tentacle spin"
                },
                "DMG": [
                    1.1973
                ]
            },
            {
                "Name": {
                    "CH": "内爆",
                    "EN": "Energy blast"
                },
                "DMG": [
                    3
                ]
            },
            {
                "Name": {
                    "CH": "推进",
                    "EN": "Push"
                },
                "DMG": [
                    3.9
                ]
            },
            {
                "Name": {
                    "CH": "球 持续伤害",
                    "EN": "Ball DoT"
                },
                "DMG": [
                    1.4976
                ]
            }
        ],
        "Drop": [
            {
                "Type": 18,
                "Amount": [
                    0,
                    2.6263,
                    0.7003,
                    0.1751
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/遗迹侦察者"
        ]
    },
    "40211": {
        "_id": 40211,
        "KingdomID": 4,
        "ClassID": 10402,
        "Name": {
            "CH": "恒常机关阵列",
            "EN": "Perpetual Mechanical Array"
        },
        "Title": [
            {
                "CH": "永续谐振 · SI/Ald",
                "EN": "Perpetual Resonance SI/Ald"
            }
        ],
        "Desc": {
            "CH": "诡谲的异形战斗机械。\n传说是已经覆灭的国度留下的战争机械。由若干部件构成，能够适应战斗的环境变换成不同形态，施展各种攻击手段。\n由立方体构成的形态在某个层面而言，与所谓的「无相元素」有着不少相似之处。",
            "EN": "A strange, alien machine.\nThey say that it is a war machine left behind by a nation that has already been destroyed. Composed of several different parts, it can adapt to its combat environment and employ a variety of attacks.\nThis machine, comprised of cubic shapes, is on some level very much like the elemental hypostases."
        },
        "HP": 8.0,
        "HPCurve": 2,
        "ATK": 7.2,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.7
        },
        "RESState": [
            {
                "State": "Weak",
                "All": -0.5
            }
        ],
        "Ball": {
            "Type": 0,
            "Point": 3,
            "Drop": "333"
        },
        "Icon": [
            "pma"
        ],
        "Grade": 3,
        "Weight": 300,
        "EndureTemplateID": 4000,
        "SkillData": [
            "4/40211"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "飞盘",
                    "EN": "Flying Disk"
                },
                "DMG": [
                    4.2
                ]
            },
            {
                "Name": {
                    "CH": "大剑",
                    "EN": "Big Sword"
                },
                "DMG": [
                    5.4
                ]
            },
            {
                "Name": {
                    "CH": "火箭",
                    "EN": "Rocket Smash"
                },
                "DMG": [
                    6.6
                ]
            },
            {
                "Name": {
                    "CH": "聚焦重炮",
                    "EN": "Focus Blast"
                },
                "DMG": [
                    7.2
                ]
            },
            {
                "Name": {
                    "CH": "追踪激光（高频伤害）",
                    "EN": "Chasing Laser (DoT)"
                },
                "DMG": [
                    2.4
                ]
            },
            {
                "Name": {
                    "CH": "方块滚滚",
                    "EN": "Cube Chase"
                },
                "DMG": [
                    5.4
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/恒常机关阵列"
        ]
    },
    "40301": {
        "_id": 40301,
        "KingdomID": 4,
        "ClassID": 10403,
        "Name": {
            "CH": "遗迹龙兽 · 空巡",
            "EN": "Ruin Drake: Skywatch"
        },
        "Title": [
            {
                "CH": "遗迹龙兽 · 空巡",
                "EN": "Ruin Drake: Skywatch"
            }
        ],
        "Desc": {
            "CH": "令人望而生畏的龙形战斗机械。\n传说是已经覆灭的国度留下的战争机械。形态上模仿了「龙蜥」这一物种，但为了性能进行了结构的优化。较之真正的龙蜥，具有更强的「吸收」，抑或是说「反制」元素的能力。\n能创造出这样可怕兵器的文明，又会因为什么而覆灭呢…",
            "EN": "A horrifying dragon-shaped machine.\nIt is said to be a war machine left behind by a now-destroyed nation. It mimics the appearance of the beings known as Vishaps, with some functional enhancements. Such machines have an even greater ability to \"absorb\" — or perhaps the right word is \"counteract\" — elemental powers than the vishaps.\nHow, you wonder, did a civilization that could make such frightening machines of war collapse?"
        },
        "HP": 2.5,
        "HPCurve": 2,
        "ATK": 5.0,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": 0.5
        },
        "RESModify": [
            {
                "State": "Gargoyle_Elemental",
                "Show": {
                    "CH": "对应抗性提升40%",
                    "EN": "Corresponding Element increase 40%"
                }
            }
        ],
        "Ball": {
            "Type": 0,
            "Point": 3,
            "Drop": "332"
        },
        "Icon": [
            "rd"
        ],
        "Grade": 2,
        "Weight": 300,
        "EndureTemplateID": 4000,
        "SkillData": [
            "4/40301"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "翻跟头砸",
                    "EN": "Backflip"
                },
                "DMG": [
                    4
                ]
            },
            {
                "Name": {
                    "CH": "两排机关枪",
                    "EN": "Two sets of gun"
                },
                "DMG": [
                    3
                ]
            },
            {
                "Name": {
                    "CH": "发射一堆小炮",
                    "EN": "Launch dozens of missiles"
                },
                "DMG": [
                    3
                ]
            },
            {
                "Name": {
                    "CH": "大招：能量洪流",
                    "EN": "Ultiamte: Energy Torrent"
                },
                "DMG": [
                    5
                ]
            }
        ],
        "Drop": [
            {
                "Type": 19,
                "Amount": [
                    0,
                    3.9394,
                    1.0505,
                    0.2626
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/遗迹龙兽·空巡"
        ]
    },
    "40302": {
        "_id": 40302,
        "KingdomID": 4,
        "ClassID": 10403,
        "Name": {
            "CH": "遗迹龙兽 · 地巡",
            "EN": "Ruin Drake: Earthguard"
        },
        "Title": [
            {
                "CH": "遗迹龙兽 · 地巡",
                "EN": "Ruin Drake: Earthguard"
            }
        ],
        "Desc": {
            "CH": "令人望而生畏的龙形战斗机械。\n传说是已经覆灭的国度留下的战争机械。形态上模仿了「龙蜥」这一物种，但为了性能进行了结构的优化。较之真正的龙蜥，具有更强的「吸收」，抑或是说「反制」元素的能力。\n从这种能力来看，创造出这样可怕兵器的文明曾经有过相当宏大的野心吧…",
            "EN": "A horrifying dragon-shaped machine.\nIt is said to be a war machine left behind by a now-destroyed nation. It mimics the appearance of the beings known as Vishaps, with some functional enhancements. Such machines have an even greater ability to \"absorb\" — or perhaps the right word is \"counteract\" — elemental powers than the vishaps.\nFrom this ability, one can glimpse the scale of the ambition that the civilization that made these machines must have once held..."
        },
        "HP": 3.5,
        "HPCurve": 2,
        "ATK": 4.0,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": 0.5
        },
        "RESModify": [
            {
                "State": "Gargoyle_Elemental",
                "Show": {
                    "CH": "对应抗性提升40%",
                    "EN": "Corresponding Element increase 40%"
                }
            }
        ],
        "Ball": {
            "Type": 0,
            "Point": 3,
            "Drop": "332"
        },
        "Icon": [
            "rd"
        ],
        "Grade": 2,
        "Weight": 300,
        "EndureTemplateID": 4000,
        "SkillData": [
            "4/40302"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "冲刺 + 甩尾",
                    "EN": "Rush + Tail Slap"
                },
                "DMG": [
                    3.2,
                    2.4
                ]
            },
            {
                "Name": {
                    "CH": "甩尾",
                    "EN": "Tail Slap"
                },
                "DMG": [
                    2.4
                ]
            },
            {
                "Name": {
                    "CH": "拱鼻子x2",
                    "EN": "Nose Nudge x2"
                },
                "DMG": [
                    2.4,
                    2.4
                ]
            },
            {
                "Name": {
                    "CH": "大招：能量洪流",
                    "EN": "Ultiamte: Energy Torrent"
                },
                "DMG": [
                    4
                ]
            }
        ],
        "Drop": [
            {
                "Type": 19,
                "Amount": [
                    0,
                    3.9394,
                    1.0505,
                    0.2626
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/遗迹龙兽·地巡"
        ]
    },
    "40303": {
        "_id": 40303,
        "KingdomID": 4,
        "ClassID": 10403,
        "Name": {
            "CH": "兆载永劫龙兽",
            "EN": "Aeonblight Drake"
        },
        "Title": [
            {
                "CH": "永续永动 · GL-Faf",
                "EN": "Perpetual Motion GL-Faf"
            }
        ],
        "Desc": {
            "CH": "如同古时曾制御大地的君王一般，令人望而生畏的龙形战斗机械。\n传说已经覆灭的国度曾渴求着禁忌的技术，试图拟造出可以匹敌，乃至超越亘古生命的永续机关。这架不知疲倦的神秘机械巨兽似乎证明，他们确已触及了人所不应抵达的领域。",
            "EN": "This dragon-shaped combat machine is a being that inspires fear, much like the lord who once ruled these vast lands.\nIt is said that the now-destroyed realm once sought forbidden knowledge and attempted to create perpetual-motion machines that could match or even surpass primordial lifeforms. This mysterious, tireless mechanical monstrosity seems to be proof that they did indeed reach heights that mortals should never have attained."
        },
        "HP": 8.5,
        "HPCurve": 2,
        "ATK": 6.0,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": 0.7
        },
        "RESState": [
            {
                "State": "Alert",
                "All": 1.0
            }
        ],
        "RESModify": [
            {
                "State": "Gargoyle_Elemental",
                "Show": {
                    "CH": "对应抗性提升60%",
                    "EN": "Corresponding Element increase 60%"
                }
            }
        ],
        "Ball": {
            "Type": 0,
            "Point": 3,
            "Drop": "332"
        },
        "Icon": [
            "abd"
        ],
        "Grade": 3,
        "Weight": 300,
        "EndureTemplateID": 4000,
        "SkillData": [
            "4/40303"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "空中：冲刺 + 甩尾",
                    "EN": "Air: Rush + Tail Slap"
                },
                "DMG": [
                    3.6,
                    3.6
                ]
            },
            {
                "Name": {
                    "CH": "空中：两排机关枪",
                    "EN": "Air: Two sets of gun"
                },
                "DMG": [
                    3.6
                ]
            },
            {
                "Name": {
                    "CH": "空中：发射一堆小炮",
                    "EN": "Air: Launch dozens of missiles"
                },
                "DMG": [
                    4.8
                ]
            },
            {
                "Name": {
                    "CH": "空中大招：能量洪流",
                    "EN": "Air Ultiamte: Energy Torrent"
                },
                "DMG": [
                    6
                ]
            },
            {
                "Name": {
                    "CH": "空中：落地伤害",
                    "EN": "Air: Landing DMG"
                },
                "DMG": [
                    1.8
                ]
            },
            {
                "Name": {
                    "CH": "地：冲刺 + 甩尾",
                    "EN": "Ground: Rush + Tail Slap"
                },
                "DMG": [
                    4.5,
                    4.5
                ]
            },
            {
                "Name": {
                    "CH": "地面：甩尾",
                    "EN": "Ground: Tail Slap"
                },
                "DMG": [
                    4.5
                ]
            },
            {
                "Name": {
                    "CH": "地面：翅膀砸x2",
                    "EN": "Ground: Wing Smash x2"
                },
                "DMG": [
                    4.5,
                    4.5
                ]
            },
            {
                "Name": {
                    "CH": "地面：拱鼻子x2",
                    "EN": "Ground: Nose Nudge x2"
                },
                "DMG": [
                    3.6,
                    3.6
                ]
            },
            {
                "Name": {
                    "CH": "地面大招：能量洪流",
                    "EN": "Ground Ultiamte: Energy Torrent"
                },
                "DMG": [
                    6
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/兆载永劫龙兽"
        ]
    },
    "40401": {
        "_id": 40401,
        "KingdomID": 4,
        "ClassID": 10404,
        "Name": {
            "CH": "元能构装体 · 力场发生器",
            "EN": "Primal Construct: Repulsor"
        },
        "Title": [
            {
                "CH": "砂原王都的建造者",
                "EN": "Architect of the Desert Palace"
            }
        ],
        "Desc": {
            "CH": "失落在荒漠中的古老遗迹的守护者，其力量足以惩罚一切敢于惊扰君王清梦之人。\n这些机械最初的用途并非是把守宫殿与神庙，而是要在沙海之上重现痴愚之王梦想中乐园。\n如今「乐园」早已湮灭，只有蚀刻在这些机械上的咒文还记录着那些已被世人忘却的梦想与承诺。",
            "EN": "The guardians of the lost ancient desert ruins have power enough to punish any who would presume to disturb the pure dreams of their lord.\nThese machines were not first built to be temple guards or palace defenders, but instead to rebirth the paradise that the foolish desert ruler dreamed of.\nToday, that utopia is long gone, and the dreams and oaths, forgotten by the world, are now only found in the mantras inscribed upon these machines."
        },
        "HP": 1.7,
        "HPCurve": 2,
        "ATK": 3.6,
        "ATKCurve": 4,
        "RESState": [
            {
                "State": "Monolith_Invisible",
                "All": 0.5
            }
        ],
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "1111"
        },
        "Icon": [
            "pc"
        ],
        "Grade": 2,
        "Weight": 101,
        "EndureTemplateID": 400,
        "SkillDMG": [
            {
                "Name": {
                    "CH": "力场推进",
                    "EN": "Replusive Barrier"
                },
                "DMG": [
                    1.08
                ]
            },
            {
                "Name": {
                    "CH": "元件追踪",
                    "EN": "Part Chase Player"
                },
                "DMG": [
                    1.8
                ]
            }
        ],
        "Drop": [
            {
                "Type": 25,
                "Amount": [
                    0,
                    2.6263,
                    0.7003,
                    0.1751
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/元能构装体·力场发生器"
        ]
    },
    "40402": {
        "_id": 40402,
        "KingdomID": 4,
        "ClassID": 10404,
        "Name": {
            "CH": "元能构装体 · 重塑仪",
            "EN": "Primal Construct: Reshaper"
        },
        "Title": [
            {
                "CH": "砂原王都的建造者",
                "EN": "Architect of the Desert Palace"
            }
        ],
        "Desc": {
            "CH": "失落在荒漠中的古老遗迹的守护者，其力量足以惩罚一切敢于惊扰君王清梦之人。\n统治沙海的烈日之王曾向他的子民许诺过彼岸的乐土，然而事实证明那不过是君王愚痴的妄念。\n如今「乐园」早已湮灭，只有蚀刻在这些机械上的咒文还记录着那些已被世人忘却的梦想与承诺。",
            "EN": "The guardians of the lost ancient desert ruins have power enough to punish any who would presume to disturb the pure dreams of their lord.\nThe lord of the blazing sun who once ruled this desert once promised his people a utopia on the other side of existence, though it ultimately turned out to be a mirage made of folly.\nToday, that utopia is long gone, and the dreams and oaths, forgotten by the world, are now only found in the mantras inscribed upon these machines."
        },
        "HP": 1.7,
        "HPCurve": 2,
        "ATK": 3.6,
        "ATKCurve": 4,
        "RESState": [
            {
                "State": "Monolith_Invisible",
                "All": 0.5
            }
        ],
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "1111"
        },
        "Icon": [
            "pc"
        ],
        "Grade": 2,
        "Weight": 101,
        "EndureTemplateID": 400,
        "SkillDMG": [
            {
                "Name": {
                    "CH": "重塑仪锥地",
                    "EN": "Reshaper Hack Ground"
                },
                "DMG": [
                    1.8,
                    2.52
                ]
            },
            {
                "Name": {
                    "CH": "元件追踪",
                    "EN": "Part Hack Ground"
                },
                "DMG": [
                    1.8
                ]
            },
            {
                "Name": {
                    "CH": "元件之间激光 高频伤害",
                    "EN": "Laser Between Parts DoT"
                },
                "DMG": [
                    0.72
                ]
            }
        ],
        "Drop": [
            {
                "Type": 25,
                "Amount": [
                    0,
                    2.6263,
                    0.7003,
                    0.1751
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/元能构装体·重塑仪"
        ]
    },
    "40403": {
        "_id": 40403,
        "KingdomID": 4,
        "ClassID": 10404,
        "Name": {
            "CH": "元能构装体 · 勘探机",
            "EN": "Primal Construct: Prospector"
        },
        "Title": [
            {
                "CH": "砂原王都的建造者",
                "EN": "Architect of the Desert Palace"
            }
        ],
        "Desc": {
            "CH": "失落在荒漠中的古老遗迹的守护者，其力量足以惩罚一切敢于惊扰君王清梦之人。\n为了构筑梦想中的乐园，沙海的主人曾探究禁忌的知识，这些神秘的机械或许正是其造物。\n如今「乐园」早已湮灭，只有蚀刻在这些机械上的咒文还记录着那些已被世人忘却的梦想与承诺。",
            "EN": "The guardians of the lost ancient desert ruins have power enough to punish any who would presume to disturb the pure dreams of their lord.\nThe master of the sands once took to studying taboo knowledge in an effort to create a dream paradise, and these machines may or may not have been the fruits of that research.\nToday, that utopia is long gone, and the dreams and oaths, forgotten by the world, are now only found in the mantras inscribed upon these machines."
        },
        "HP": 1.7,
        "HPCurve": 2,
        "ATK": 3.6,
        "ATKCurve": 4,
        "RESState": [
            {
                "State": "Monolith_Invisible",
                "All": 0.5
            }
        ],
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "1111"
        },
        "Icon": [
            "pc"
        ],
        "Grade": 2,
        "Weight": 101,
        "EndureTemplateID": 400,
        "SkillDMG": [
            {
                "Name": {
                    "CH": "勘探激光",
                    "EN": "Prospector Laser"
                },
                "DMG": [
                    1.8
                ]
            },
            {
                "Name": {
                    "CH": "元件激光",
                    "EN": "Part Laser"
                },
                "DMG": [
                    1.8
                ]
            }
        ],
        "Drop": [
            {
                "Type": 25,
                "Amount": [
                    0,
                    2.6263,
                    0.7003,
                    0.1751
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/元能构装体·勘探机"
        ]
    },
    "40501": {
        "_id": 40501,
        "KingdomID": 4,
        "ClassID": 10404,
        "Name": {
            "CH": "半永恒统辖矩阵",
            "EN": "Algorithm of Semi-Intransient Matrix of Overseer Network"
        },
        "Title": [
            {
                "CH": "砂原王之梦的守墓者",
                "EN": "Tomb Guard of the Desert King"
            }
        ],
        "Desc": {
            "CH": "统辖着散落在沙漠中的众多遗迹构装体的永恒机关，其中似乎仍残留着孤独君王的执着意念。\n或许是因为梦中的友人曾经的教诲，又或是因为觊觎着天上诸圣的权能，王创造出了本应不属于地上的神秘机关，并以此构建砂原上的国都。\n在千柱之城的废墟之上，古老的晶石中仍折射着黄金乐园的美景，即便王的梦想早已无人铭记。",
            "EN": "A perpetual motion machine that rules over the many constructs scattered throughout the desert. It seems that the lonely ruler's will still indwells this device.\nPerhaps due to a friend's teachings in a dream or perhaps because he coveted the power of the holy ones in the skies, the king created mysterious devices that should not exist in this world before founding his realm in these lands.\nAmidst the ruins of the City of the Pillars, the ancient crystals still reflect the glory of the golden paradise, though none now live who remember the king's dreams."
        },
        "UseCustomColorName": true,
        "RevertName": true,
        "HP": 8.0,
        "HPCurve": 2,
        "ATK": 6.6,
        "ATKCurve": 4,
        "RESState": [
            {
                "State": "Alert",
                "All": 1.0
            },
            {
                "State": "Monolith_Invisible",
                "All": 2.0
            }
        ],
        "Ball": {
            "Type": 0,
            "Point": 3,
            "Drop": "332"
        },
        "Icon": [
            "spcm"
        ],
        "Grade": 3,
        "Weight": 200,
        "EndureTemplateID": 400,
        "SkillData": [
            "4/40501"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "力场推进",
                    "EN": "Barrier Push"
                },
                "DMG": [
                    2.64
                ]
            },
            {
                "Name": {
                    "CH": "锥子锥地",
                    "EN": "Lance Hack Ground"
                },
                "DMG": [
                    3.3,
                    4.62
                ]
            },
            {
                "Name": {
                    "CH": "单次激光",
                    "EN": "Single Laser"
                },
                "DMG": [
                    4.62
                ]
            },
            {
                "Name": {
                    "CH": "元件：盘子",
                    "EN": "Parts: Spinning Frisbees"
                },
                "DMG": [
                    3.3
                ]
            },
            {
                "Name": {
                    "CH": "元件：激光发射器",
                    "EN": "Parts: Laser Generator"
                },
                "DMG": [
                    3.3
                ]
            },
            {
                "Name": {
                    "CH": "元件：钉子",
                    "EN": "Parts: Nails"
                },
                "DMG": [
                    3.3
                ]
            },
            {
                "Name": {
                    "CH": "元件：钉子之间激光 高频伤害",
                    "EN": "Parts: Laser Between Nails DoT"
                },
                "DMG": [
                    1.32
                ]
            },
            {
                "Name": {
                    "CH": "超频冲击",
                    "EN": "Overclocking Impact"
                },
                "DMG": [
                    6.6
                ]
            },
            {
                "Name": {
                    "CH": "持续激光",
                    "EN": "Chasing Laser"
                },
                "DMG": [
                    4.62
                ]
            },
            {
                "Name": {
                    "CH": "瞬间发射好多激光 地上有印记",
                    "EN": "Lots of lasers, Mark on ground"
                },
                "DMG": [
                    3.3
                ]
            },
            {
                "Name": {
                    "CH": "单次发射大锥子",
                    "EN": "Launch big cone"
                },
                "DMG": [
                    6.6
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/半永恒统辖矩阵"
        ]
    },
    "40601": {
        "_id": 40601,
        "KingdomID": 4,
        "ClassID": 104060,
        "Name": {
            "CH": "机关 · 侦察记录型 · 荒",
            "EN": "Recon Log Mek - Ousia"
        },
        "Title": [
            {
                "CH": "机关 · 侦察记录型",
                "EN": "Recon Log Mek"
            }
        ],
        "Desc": {
            "CH": "最早设计制造的新式发条机关机械之一。\n据说「奇械公」吉约丹在自然哲学学院的时期，曾经做出了能够真正模拟智慧思考的机械。\n尽管他从未正式承认过这种说法，这一传奇仍在日后激起了许多工程师的斗志。数十年前曾有人宣布创造了具有基础思考能力，并能够解读人类情感的发条机关，但最后相关的成果并未得到认同。",
            "EN": "One of the earliest new-type clockwork meka to be made.\nIt is said that during his time at the Institute of Natural Philosophy, the Lord-Artificer Guillotin created machines that could simulate genuine intelligence.\nThough he never openly admitted it, this rumor nonetheless spurred on subsequent generations of ambitious engineers. Decades ago, one of them announced that they had created a machine that not only possessed basic thinking abilities but could also read human emotions, but in the end the results went unacknowledged."
        },
        "Color": "Elec",
        "HP": 0.8,
        "HPCurve": 2,
        "ATK": 1.8,
        "ATKCurve": 4,
        "Ball": {
            "Type": 0,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "UI_MonsterIcon_MachinaIustitia_Sentry"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 210,
        "Tutorial": {
            "CH": "- 低级荒性机关，驱使<color style='color:#7030A0;'>雷元素</color>。<br>- 使用1次芒性攻击，可以使其弱化25s。<br>- 弱化期间只能进行低强度物理攻击，无法使用元素。",
            "EN": "- Small Ousia mek, masters the <color style='color:#7030A0;'>Electro</color> element.<br>- Attacking it with Pneuma 1 time will weaken it for 25s.<br>- While weakened, loses its element and can only perform basic Physical attacks."
        },
        "Drop": [
            {
                "Type": 29,
                "Amount": [
                    1.2606,
                    0.3362,
                    0.084,
                    0
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/机关·侦察记录型"
        ]
    },
    "40602": {
        "_id": 40602,
        "KingdomID": 4,
        "ClassID": 104060,
        "Name": {
            "CH": "机关 · 算力增幅器 · 荒",
            "EN": "Arithmetic Enhancer Mek - Ousia"
        },
        "Title": [
            {
                "CH": "机关 · 算力增幅器",
                "EN": "Arithmetic Enhancer Mek"
            }
        ],
        "Desc": {
            "CH": "以中继并增幅信号为目的，设计制造的发条机关。\n驱动发条机关的能源是通过荒性与芒性在动力核心内部进行可控湮灭反应而产生的。因此向其释放过度的能量会导致原本的平衡被打破。",
            "EN": "A clockwork meka designed to transmit and enhance signals.\nThe power that drives this clockwork meka is generated from a controlled annihilation reaction between ousia and pneuma in the kinetic core. As a result, overloading it with excessive energy will cause its original equilibrium to be disrupted."
        },
        "HP": 0.9,
        "HPCurve": 2,
        "ATK": 1.6,
        "ATKCurve": 4,
        "Ball": {
            "Type": 0,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "UI_MonsterIcon_MachinaIustitia_Pylon"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 210,
        "Tutorial": {
            "CH": "- 荒性增幅机关，可以使其他机关的攻击力提升20%，并恢复弱化的机关。<br>- 使用1次芒性攻击，可以使其弱化5s。",
            "EN": "- Ousia enhancement mek. Increases nearby meks' ATK by 20% and recovers nearby weakened meks.<br>- Attacking it with Pneuma 1 time will weaken it for 5s."
        },
        "Drop": [
            {
                "Type": 29,
                "Amount": [
                    1.2606,
                    0.3362,
                    0.084,
                    0
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/机关·算力增幅器"
        ]
    },
    "40622": {
        "_id": 40622,
        "KingdomID": 10,
        "ClassID": 11041,
        "Name": {
            "CH": "自律超算型场力发生装置",
            "EN": "Automated Supercomputing Field Generator"
        },
        "Title": [
            {
                "CH": "地方传奇 · 机关 · 算力增幅器",
                "EN": "Local Legend: Arithmetic Enhancer Mek"
            }
        ],
        "Color": "Rock",
        "HP": 19,
        "HPCurve": 2,
        "ATK": 12,
        "ATKCurve": 4,
        "RESState": [
            {
                "State": "FauneAbyssale_Shield",
                "All": 2
            }
        ],
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "1116"
        },
        "Icon": [
            "UI_MonsterIcon_MachinaIustitia_Pylon"
        ],
        "Grade": 3,
        "Weight": 150,
        "EndureTemplateID": 210,
        "Tutorial": {
            "CH": "初始处于<b>荒性激发态</b>，<b><color style='color:#D4A000;'>具有20U岩元素护甲</color></b>，使其全部抗性提升200%。破除护甲后，使用1次芒性攻击可以强制退出荒性激发态，使其瘫痪5s。护甲存在期间芒荒湮灭无效。<br>- 荒性激发态免疫冻结和石化。湮灭后仍然具有90%冻结抵抗。<br>- 湮灭后12s会恢复荒性激发。护甲破除后15s会重生护甲。",
            "EN": "Initially has <b>Ousia Power</b>, granting <b><color style='color:#D4A000;'>a 20GU Geo Armor</color></b> which increases all RES by 200%. After the armor is destroyed, using 1 Pneuma attack can cause annihilation: terminate Ousia Power and immobilize it for 5s. Cannot annihilate when armor is still on.<br>- While in Ousia Power, immune to Freeze and Petrification. After Pneumosia annihilation, still has 90% Freeze RES.<br>- Regains Ousia 12s after annihilation. Regains armor 15s after destruction of armor."
        },
        "SkillDMG": [
            {
                "Name": {
                    "CH": "（荒性激发）岩晶浪潮 / 闪避无效",
                    "EN": "(Ousia Power) Crystal Waves *Undodgable"
                },
                "DMG": [
                    12,
                    12,
                    12
                ]
            },
            {
                "Name": {
                    "CH": "（荒性激发）地面连续喷砂",
                    "EN": "(Ousia Power) Continuous Ground Spurt"
                },
                "DMG": [
                    10.8
                ]
            }
        ],
        "Csx": [
            "ECLegends/地方传奇·算力增幅器"
        ]
    },
    "40632": {
        "_id": 40632,
        "KingdomID": 10,
        "ClassID": 11006,
        "Name": {
            "CH": "自律超算型场力发生装置",
            "EN": "Automated Supercomputing Field Generator"
        },
        "Title": [
            {
                "CH": "地方传奇 · 机关 · 算力增幅器",
                "EN": "Local Legend: Arithmetic Enhancer Mek"
            }
        ],
        "Color": "Rock",
        "HP": 7.6,
        "HPCurve": 2,
        "ATK": 7.2,
        "ATKCurve": 4,
        "RESState": [
            {
                "State": "FauneAbyssale_Shield",
                "All": 2
            }
        ],
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "1116"
        },
        "Icon": [
            "UI_MonsterIcon_MachinaIustitia_Pylon"
        ],
        "Grade": 3,
        "Weight": 150,
        "EndureTemplateID": 210,
        "Tutorial": {
            "CH": "初始处于<b>荒性激发态</b>，<b><color style='color:#D4A000;'>具有20U岩元素护甲</color></b>，使其全部抗性提升200%。破除护甲后，使用1次芒性攻击可以强制退出荒性激发态，使其瘫痪5s。护甲存在期间芒荒湮灭无效。<br>- 荒性激发态免疫冻结和石化。湮灭后仍然具有90%冻结抵抗。<br>- 湮灭后12s会恢复荒性激发。护甲破除后15s会重生护甲。",
            "EN": "Initially has <b>Ousia Power</b>, granting <b><color style='color:#D4A000;'>a 20GU Geo Armor</color></b> which increases all RES by 200%. After the armor is destroyed, using 1 Pneuma attack can cause annihilation: terminate Ousia Power and immobilize it for 5s. Cannot annihilate when armor is still on.<br>- While in Ousia Power, immune to Freeze and Petrification. After Pneumosia annihilation, still has 90% Freeze RES.<br>- Regains Ousia 12s after annihilation. Regains armor 15s after destruction of armor."
        },
        "SkillDMG": [
            {
                "Name": {
                    "CH": "（荒性激发）岩晶浪潮 / 闪避无效",
                    "EN": "(Ousia Power) Crystal Waves *Undodgable"
                },
                "DMG": [
                    4.8,
                    4.8,
                    4.8
                ]
            },
            {
                "Name": {
                    "CH": "（荒性激发）地面连续喷砂",
                    "EN": "(Ousia Power) Continuous Ground Spurt"
                },
                "DMG": [
                    3.84
                ]
            }
        ],
        "Csx": [
            "ECLegends/地方传奇·算力增幅器"
        ]
    },
    "40603": {
        "_id": 40603,
        "KingdomID": 4,
        "ClassID": 104060,
        "Name": {
            "CH": "机关 · 水下勘测型 · 荒",
            "EN": "Underwater Survey Mek - Ousia"
        },
        "Title": [
            {
                "CH": "机关 · 水下勘测型",
                "EN": "Underwater Survey Mek"
            }
        ],
        "Desc": {
            "CH": "以水下探测为目的，设计制造的发条机关。\n在发条机关的设计上，在动力学无法得到突破的情况下，运动学是最重要的研究对象。在特定目的的机械设计上，仿生学有时能提供最便捷的解法。",
            "EN": "A clockwork meka designed for underwater surveying.\nIn the absence of significant breakthroughs in dynamics research, kinematics research has had the most significant impact on clockwork meka design. For machines designed to perform specific tasks, biomimesis has sometimes provided the most convenient solutions."
        },
        "HP": 0.6,
        "HPCurve": 7,
        "ATK": 45,
        "ATKCurve": 40,
        "Icon": [
            "UI_MonsterIcon_MachinaIustitia_Recon"
        ],
        "Grade": 1,
        "Weight": 125,
        "EndureTemplateID": 210,
        "Tutorial": {
            "CH": "- 低级水下荒性机关。<br>- 使用1次芒性攻击，可以使其弱化50s。<br>- 弱化期间无法进行攻击。",
            "EN": "- Small underwater Ousia mek.<br>- Attacking it with Pneuma 1 time will weaken it for 50s.<br>- While weakened, cannot perform any action."
        },
        "Drop": [
            {
                "Type": 29,
                "Amount": [
                    1.2606,
                    0.3362,
                    0.084,
                    0
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/机关·水下勘测型"
        ]
    },
    "40604": {
        "_id": 40604,
        "KingdomID": 4,
        "ClassID": 104060,
        "Name": {
            "CH": "机关 · 地质勘探型 · 荒",
            "EN": "Geological Survey Mek - Ousia"
        },
        "Title": [
            {
                "CH": "机关 · 地质勘探型",
                "EN": "Geological Survey Mek"
            }
        ],
        "Desc": {
            "CH": "以探测地下资源及特定矿物为目的，设计制造的发条机关。\n因为能够设置搜寻的金属类型，所以也很受盗宝团与冒险家的欢迎。",
            "EN": "A clockwork meka designed for surveying underground resources and certain minerals.\nDue to the fact that the desired metal type can be set, this device has proven very popular among Treasure Hoarders and Adventurers alike."
        },
        "Color": "Rock",
        "HP": 1.0,
        "HPCurve": 2,
        "ATK": 1.6,
        "ATKCurve": 4,
        "Ball": {
            "Type": 0,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "UI_MonsterIcon_MachinaIustitia_Probe"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 50,
        "Tutorial": {
            "CH": "- 低级荒性机关，驱使<color style='color:#D4A000;'>岩元素</color>。<br>- 使用1次芒性攻击，可以使其弱化25s。<br>- 弱化期间只能进行低强度物理攻击，无法使用元素。",
            "EN": "- Small Ousia mek, masters the <color style='color:#D4A000;'>Geo</color> element.<br>- Attacking it with Pneuma 1 time will weaken it for 25s.<br>- While weakened, loses its element and can only perform basic Physical attacks."
        },
        "Drop": [
            {
                "Type": 29,
                "Amount": [
                    1.2606,
                    0.3362,
                    0.084,
                    0
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/机关·地质勘探型"
        ]
    },
    "40605": {
        "_id": 40605,
        "KingdomID": 4,
        "ClassID": 104060,
        "Name": {
            "CH": "机关 · 水下巡游型 · 荒",
            "EN": "Underwater Patrol Mek - Ousia"
        },
        "Title": [
            {
                "CH": "机关 · 水下巡游型",
                "EN": "Underwater Patrol Mek"
            }
        ],
        "Desc": {
            "CH": "这种发条机关的设计目的为搜寻、驱逐、消灭对水下设施有不利影响的水生动物。\n自第一次涨水期以来，枫丹科学院便一直忧心于或许有一天终将发生的第二次涨水期。其中一部分人开始构想收复丧失在水中的遗迹并建设海底之城，另一部分人则望着永远浮在不可触及高度的空中岛屿，梦想天空的都会。\n「水下巡游型」来自前者，但从结果而言，近些年后者似乎取得了上风。",
            "EN": "This type of clockwork meka was designed to search out, repel, and if need be exterminate aquatic life forms that posed a threat to underwater equipment.\nEver since the first flood period, the Fontaine Research Institute has been preoccupied with the thought that there might be a second one. Some had the idea of taking back the ruins lost to the waves and building an underwater city, whereas others looked up to the untouchable sky islands overhead with the intention of building a metropolis in the clouds.\nUnderwater Patrol Meks were conceived by the former, but judging from the results, it is the latter that have gained the upper hand in recent years."
        },
        "HP": 1,
        "HPCurve": 7,
        "ATK": 50,
        "ATKCurve": 40,
        "Icon": [
            "UI_MonsterIcon_MachinaIustitia_Nautilus"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 150,
        "Tutorial": {
            "CH": "- 低级水下荒性机关。<br>- 使用1次芒性攻击，可以使其弱化50s。<br>- 弱化期间无法进行攻击。",
            "EN": "- Small underwater Ousia mek.<br>- Attacking it with Pneuma 1 time will weaken it for 50s.<br>- While weakened, cannot perform any action."
        },
        "Drop": [
            {
                "Type": 29,
                "Amount": [
                    1.2606,
                    0.3362,
                    0.084,
                    0
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/机关·水下巡游型"
        ]
    },
    "40606": {
        "_id": 40606,
        "KingdomID": 4,
        "ClassID": 104060,
        "Name": {
            "CH": "机关 · 深海攻击型 · 荒",
            "EN": "Deepwater Assault Mek - Ousia"
        },
        "Title": [
            {
                "CH": "机关 · 深海攻击型",
                "EN": "Deepwater Assault Mek"
            }
        ],
        "Desc": {
            "CH": "设计目的是进行水下攻击的发条机关。与「水下巡游型」机关不同的是，预期中它的敌人会更大、更凶暴。当然性能与造价的代价是，面对小体型的敌人时便无法完全发挥威力。\n过去在枫丹曾经发生了巨大魔兽入侵的悲剧，在陆上大湖受到景仰的白色铁甲舰队为了护国全军覆没。设计者希望如有此类事件再次发生，至少能在水中以强大火力出击的发条机关能先于将官与士兵成为第一道防线。",
            "EN": "A clockwork meka designed to conducting underwater attacks. Unlike \"Underwater Patrol Mek\" models, it is expected to take on much larger and fiercer opponents. Of course, the price it pays for such capabilities and its cost of manufacture is that is unable to fully unleash its capabilities against smaller opponents.\nLong ago, Fontaine was once scarred by the tragic invasion of a massive monster. The revered White Armada that sailed upon the great terrestrial lake was completely destroyed in defense of the nation. The designer hoped that were such a thing to happen again, the clockwork meka would possess enough naval firepower to be the first line of defense, going before the soldiers and generals."
        },
        "HP": 2,
        "HPCurve": 7,
        "ATK": 60,
        "ATKCurve": 40,
        "Icon": [
            "UI_MonsterIcon_MachinaIustitia_Frigate"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 150,
        "Tutorial": {
            "CH": "- 低级水下荒性机关。<br>- 使用1次芒性攻击，可以使其弱化50s。<br>- 弱化期间无法进行攻击。",
            "EN": "- Small underwater Ousia mek.<br>- Attacking it with Pneuma 1 time will weaken it for 50s.<br>- While weakened, cannot perform any action."
        },
        "Drop": [
            {
                "Type": 29,
                "Amount": [
                    1.2606,
                    0.3362,
                    0.084,
                    0
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/机关·深海攻击型"
        ]
    },
    "40607": {
        "_id": 40607,
        "KingdomID": 4,
        "ClassID": 104060,
        "Name": {
            "CH": "机关 · 灵活采集型 · 荒",
            "EN": "Nimble Harvester Mek - Ousia"
        },
        "Title": [
            {
                "CH": "机关 · 灵活采集型",
                "EN": "Nimble Harvester Mek"
            }
        ],
        "Desc": {
            "CH": "以发掘、输送砂土为目的设计制造的发条机关。\n基于荒性与芒性湮灭产生能量的发条机关在枫丹具有得天独厚的优势，因为枫丹的水脉与土质本身就有储存、传导荒性与芒性的特性。这也是冒险中有时会见到荒性与芒性能量块的原因。",
            "EN": "A clockwork meka designed to dig up and transport soil.\nDue to the inherent ability of the local water and soil to store and transmit ousia and pneuma, clockwork meka powered by the annihilation of these energies are highly favored in Fontaine. This also explains why adventurers may come across Ousia and Pneuma Blocks from time to time."
        },
        "Color": "Rock",
        "HP": 1.2,
        "HPCurve": 2,
        "ATK": 1.4,
        "ATKCurve": 4,
        "Ball": {
            "Type": 0,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "UI_MonsterIcon_MachinaIustitia_Assimilator"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 50,
        "Tutorial": {
            "CH": "- 低级荒性机关，驱使<color style='color:#D4A000;'>岩元素</color>。<br>- 使用1次芒性攻击，可以使其弱化25s。<br>- 弱化期间只能进行低强度物理攻击，无法使用元素。",
            "EN": "- Small Ousia mek, masters the <color style='color:#D4A000;'>Geo</color> element.<br>- Attacking it with Pneuma 1 time will weaken it for 25s.<br>- While weakened, loses its element and can only perform basic Physical attacks."
        },
        "Drop": [
            {
                "Type": 29,
                "Amount": [
                    1.2606,
                    0.3362,
                    0.084,
                    0
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/机关·灵活采集型"
        ]
    },
    "40608": {
        "_id": 40608,
        "KingdomID": 4,
        "ClassID": 104061,
        "Name": {
            "CH": "攻坚特化型机关 · 荒",
            "EN": "Assault Specialist Mek - Ousia"
        },
        "Title": [
            {
                "CH": "谢尔比乌斯式机关",
                "EN": "Meka, Cal. Scherbius"
            }
        ],
        "Desc": {
            "CH": "与「侦察记录型」一样，是最早设计制造的新式发条机关机械之一（数百年后的如今已经无人将其称为「新式」。这在当时是用以区分传统发条势能动力的机械机关用的说法）。\n名称是用来纪念发明者的友人而起的，最初设计的目的是帮助人。但后来的使用者普遍认为用它攻击人较之用人攻击人的相对优势更大。",
            "EN": "Alongside the Recon Log Mek, this was one of the first new-type clockwork meka to be made (though of course, after hundreds of years, no one refers to them as \"new-type\" anymore – this was simply to differentiate them from traditional clockwork mechanisms that were powered by potential energy.)\nIt was named to commemorate a friend of its inventor, and was originally designed to help people. However, its ability to attack people – thereby avoiding the need to put friendly personnel at risk – subsequently came to be seen as its real strength."
        },
        "Color": "Elec",
        "HP": 2.4,
        "HPCurve": 2,
        "ATK": 5.0,
        "ATKCurve": 4,
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "1113"
        },
        "Icon": [
            "UI_MonsterIcon_MachinaIustitia_Ruggiero_01"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 210,
        "Tutorial": {
            "CH": "- 中级荒性机关，驱使<color style='color:#7030A0;'>雷元素</color>。<br>- 使用2次芒性攻击，可以使其弱化25s。<br>- 弱化期间只能进行低强度物理攻击，无法使用元素。",
            "EN": "- Mid-tier Ousia mek, masters the <color style='color:#7030A0;'>Electro</color> element.<br>- Attacking it with Pneuma 2 times will weaken it for 25s.<br>- While weakened, loses its element and can only perform basic Physical attacks."
        },
        "Drop": [
            {
                "Type": 29,
                "Amount": [
                    3.3616,
                    0.8964,
                    0.224,
                    0
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/攻坚特化型机关"
        ]
    },
    "40609": {
        "_id": 40609,
        "KingdomID": 4,
        "ClassID": 104061,
        "Name": {
            "CH": "压制特化型机关 · 荒",
            "EN": "Suppression Specialist Mek - Ousia"
        },
        "Title": [
            {
                "CH": "布莱切利式机关",
                "EN": "Meka, Cal. Bletchley"
            }
        ],
        "Desc": {
            "CH": "基于「谢尔比乌斯式」，进行了调整的发条机关。很遗憾，并不具备攻击以外的能力。\n基于湮灭能量的发条机关通常会利用特殊晶体构成的中继器从枫丹的环境中提炼荒性与芒性产生能量，但在枫丹以外的区域也能通过预先准备的能量块进行短程短时的行动。这两者的区别体现在机关上晶体部件的形状。",
            "EN": "A clockwork meka adapted from the Cal. Scherbius type. Unfortunately, aside from its ability to attack, it has no other capabilities.\nClockwork meka based on annihilation energy normally produce their energy by utilizing Repeaters composed of special crystals to refine ousia and pneuma from Fontaine's environment. But outside of Fontaine, they can use pre-prepared Energy Blocks to move over short distances for short periods of time. The difference can be observed from the shape of the crystal components on the meka."
        },
        "Color": "Fire",
        "HP": 2.0,
        "HPCurve": 2,
        "ATK": 6.0,
        "ATKCurve": 4,
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "1113"
        },
        "Icon": [
            "UI_MonsterIcon_MachinaIustitia_Ruggiero_02"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 210,
        "Tutorial": {
            "CH": "- 中级荒性机关，驱使<color style='color:#FF0000;'>火元素</color>。<br>- 使用2次芒性攻击，可以使其弱化25s。<br>- 弱化期间只能进行低强度物理攻击，无法使用元素。",
            "EN": "- Mid-tier Ousia mek, masters the <color style='color:#FF0000;'>Pyro</color> element.<br>- Attacking it with Pneuma 2 times will weaken it for 25s.<br>- While weakened, loses its element and can only perform basic Physical attacks."
        },
        "Drop": [
            {
                "Type": 29,
                "Amount": [
                    3.3616,
                    0.8964,
                    0.224,
                    0
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/压制特化型机关"
        ]
    },
    "40610": {
        "_id": 40610,
        "KingdomID": 4,
        "ClassID": 104061,
        "Name": {
            "CH": "歼灭特化型机关 · 荒",
            "EN": "Annihilation Specialist Mek - Ousia"
        },
        "Title": [
            {
                "CH": "拜伦伯爵夫人式机关",
                "EN": "Meka, Cal. Countess Byron"
            }
        ],
        "Desc": {
            "CH": "尽管枫丹动能工程科学研究院是最为主要的发条机械研究与设计机构，坊间也有许多不隶属于科学院的工程师与独立工坊。\n这种机械起初是为一名独立工程师为纪念情人及赞助者而设计制造的。它最终被执律庭选中并采购作为城市安全或称公共暴力机构的武器之一。",
            "EN": "Although the Fontaine Research Institute of Kinetic Energy Engineering is the main organization involved in clockwork meka research and design, there are also many independent engineers and workshops that are not affiliated with the Research Institute.\nThe first ever machine of this type was designed by an independent engineer to commemorate their lover and sponsor. In the end, it was acquired by the Maison Gardiennage as a tool to uphold municipal security – or, in the eyes of some, as a weapon to administer public violence."
        },
        "Color": "Water",
        "HP": 2.6,
        "HPCurve": 2,
        "ATK": 4.6,
        "ATKCurve": 4,
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "1113"
        },
        "Icon": [
            "UI_MonsterIcon_MachinaIustitia_Bradamante"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 210,
        "Tutorial": {
            "CH": "- 中级荒性机关，驱使<color style='color:#0070C0;'>水元素</color>。<br>- 使用2次芒性攻击，可以使其弱化25s。<br>- 弱化期间只能进行低强度物理攻击，无法使用元素。",
            "EN": "- Mid-tier Ousia mek, masters the <color style='color:#0070C0;'>Hydro</color> element.<br>- Attacking it with Pneuma 2 times will weaken it for 25s.<br>- While weakened, loses its element and can only perform basic Physical attacks."
        },
        "Drop": [
            {
                "Type": 29,
                "Amount": [
                    3.3616,
                    0.8964,
                    0.224,
                    0
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/歼灭特化型机关"
        ]
    },
    "40611": {
        "_id": 40611,
        "KingdomID": 4,
        "ClassID": 104060,
        "Name": {
            "CH": "机关 · 区域警戒型 · 荒",
            "EN": "Area Alert Mek - Ousia"
        },
        "Title": [
            {
                "CH": "机关 · 区域警戒型",
                "EN": "Area Alert Mek"
            }
        ],
        "Desc": {
            "CH": "能够将湮灭产生的能量投射出去的机关。严格来说，是在枫丹科学院建立之前，自然哲学学院的相关研究中产生的试验装置。\n因此，尽管作为警戒与防御设备取得了相当的商业成功，大多数工程师并不将其视为严格意义的发条机关机械。",
            "EN": "A mechanism that is able to fire out the energy produced by an annihilation reaction. Strictly speaking, it was originally a test device that emerged from research undertaken by the School of Natural Philosophy before the Fontaine Research Institute had been established.\nAs such, despite its considerable commercial success as a sentinel and defensive device, most engineers do not consider it to be a true clockwork meka."
        },
        "Color": "Ice",
        "HP": 0.7,
        "HPCurve": 2,
        "ATK": 2.0,
        "ATKCurve": 4,
        "Ball": {
            "Type": 0,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "UI_MonsterIcon_MachinaIustitia_LengthShifter"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 210,
        "Tutorial": {
            "CH": "- 低级荒性机关，驱使<color style='color:#00B0F0;'>冰元素</color>。<br>- 使用1次芒性攻击，可以使其弱化25s。<br>- 弱化期间只能进行低强度物理攻击，无法使用元素。",
            "EN": "- Small Ousia mek, masters the <color style='color:#00B0F0;'>Cryo</color> element.<br>- Attacking it with Pneuma 1 time will weaken it for 25s.<br>- While weakened, loses its element and can only perform basic Physical attacks."
        },
        "Drop": [
            {
                "Type": 29,
                "Amount": [
                    1.2606,
                    0.3362,
                    0.084,
                    0
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/机关·区域警戒型"
        ]
    },
    "40612": {
        "_id": 40612,
        "KingdomID": 4,
        "ClassID": 104061,
        "Name": {
            "CH": "建造特化型机关 · 荒",
            "EN": "Construction Specialist Mek - Ousia"
        },
        "Title": [
            {
                "CH": "艾蒂安-路易式机关",
                "EN": "Meka, Cal. Etienne-Louis"
            }
        ],
        "Desc": {
            "CH": "在建筑施工、考古发掘上同样有不可估量价值的大型发条机关，其名来自修建克莱门汀线时卡雷斯雇佣的工程师。\n这名工程师在卡雷斯的资助下为打通白淞隧道建造了颇具传奇性的「黄金乐船」巨型工程机械，并在竣工后据此设计了这类结构相似，但尺寸缩小的发条机关。",
            "EN": "A type of large clockwork meka which also has immeasurable value when utilized in building construction and archeological excavation. Its name comes from an engineer employed by Callas during the construction of the Clementine Line.\nWith Callas' financial backing, this engineer built the legendary \"Golden Note\" – a gigantic piece of construction machinery which was used to dig the Poisson Tunnel. Once the job was complete, this type of clockwork meka was designed with a similar structure but reduced to a smaller scale."
        },
        "Color": "Ice",
        "HP": 3.5,
        "HPCurve": 2,
        "ATK": 3.0,
        "ATKCurve": 4,
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "1113"
        },
        "Icon": [
            "UI_MonsterIcon_MachinaIustitia_Excavator"
        ],
        "Grade": 1,
        "Weight": 300,
        "EndureTemplateID": 4000,
        "Tutorial": {
            "CH": "- 中级荒性机关，驱使<color style='color:#00B0F0;'>冰元素</color>。<br>- 使用2次芒性攻击，可以使其弱化25s。<br>- 弱化期间只能进行低强度物理攻击，无法使用元素。",
            "EN": "- Mid-tier Ousia mek, masters the <color style='color:#00B0F0;'>Cryo</color> element.<br>- Attacking it with Pneuma 2 times will weaken it for 25s.<br>- While weakened, loses its element and can only perform basic Physical attacks."
        },
        "Drop": [
            {
                "Type": 29,
                "Amount": [
                    3.3616,
                    0.8964,
                    0.224,
                    0
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/建造特化型机关"
        ]
    },
    "40651": {
        "_id": 40651,
        "KingdomID": 4,
        "ClassID": 104065,
        "Name": {
            "CH": "机关 · 侦察记录型 · 芒",
            "EN": "Recon Log Mek - Pneuma"
        },
        "Title": [
            {
                "CH": "机关 · 侦察记录型",
                "EN": "Recon Log Mek"
            }
        ],
        "Desc": {
            "CH": "最早设计制造的新式发条机关机械之一。\n据说「奇械公」吉约丹在自然哲学学院的时期，曾经做出了能够真正模拟智慧思考的机械。\n尽管他从未正式承认过这种说法，这一传奇仍在日后激起了许多工程师的斗志。数十年前曾有人宣布创造了具有基础思考能力，并能够解读人类情感的发条机关，但最后相关的成果并未得到认同。",
            "EN": "One of the earliest new-type clockwork meka to be made.\nIt is said that during his time at the Institute of Natural Philosophy, the Lord-Artificer Guillotin created machines that could simulate genuine intelligence.\nThough he never openly admitted it, this rumor nonetheless spurred on subsequent generations of ambitious engineers. Decades ago, one of them announced that they had created a machine that not only possessed basic thinking abilities but could also read human emotions, but in the end the results went unacknowledged."
        },
        "Color": "Elec",
        "HP": 0.8,
        "HPCurve": 2,
        "ATK": 1.8,
        "ATKCurve": 4,
        "Ball": {
            "Type": 0,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "UI_MonsterIcon_MachinaIustitia_Sentry"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 210,
        "Tutorial": {
            "CH": "- 低级芒性机关，驱使<color style='color:#7030A0;'>雷元素</color>。<br>- 使用1次荒性攻击，可以使其弱化25s。<br>- 弱化期间只能进行低强度物理攻击，无法使用元素。",
            "EN": "- Small Pneuma mek, masters the <color style='color:#7030A0;'>Electro</color> element.<br>- Attacking it with Ousia 1 time will weaken it for 25s.<br>- While weakened, loses its element and can only perform basic Physical attacks."
        },
        "Drop": [
            {
                "Type": 29,
                "Amount": [
                    1.2606,
                    0.3362,
                    0.084,
                    0
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/机关·侦察记录型"
        ]
    },
    "40652": {
        "_id": 40652,
        "KingdomID": 4,
        "ClassID": 104065,
        "Name": {
            "CH": "机关 · 算力增幅器 · 芒",
            "EN": "Arithmetic Enhancer Mek - Pneuma"
        },
        "Title": [
            {
                "CH": "机关 · 算力增幅器",
                "EN": "Arithmetic Enhancer Mek"
            }
        ],
        "Desc": {
            "CH": "以中继并增幅信号为目的，设计制造的发条机关。\n驱动发条机关的能源是通过荒性与芒性在动力核心内部进行可控湮灭反应而产生的。因此向其释放过度的能量会导致原本的平衡被打破。",
            "EN": "A clockwork meka designed to transmit and enhance signals.\nThe power that drives this clockwork meka is generated from a controlled annihilation reaction between ousia and pneuma in the kinetic core. As a result, overloading it with excessive energy will cause its original equilibrium to be disrupted."
        },
        "HP": 0.9,
        "HPCurve": 2,
        "ATK": 1.6,
        "ATKCurve": 4,
        "Ball": {
            "Type": 0,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "UI_MonsterIcon_MachinaIustitia_Pylon"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 210,
        "Tutorial": {
            "CH": "- 芒性增幅机关，可以使其他机关的全部抗性提升20%，并恢复弱化的机关。<br>- 使用1次荒性攻击，可以使其弱化5s。",
            "EN": "- Pneuma enhancement mek. Increases nearby meks' ATK by 20% and recovers nearby weakened meks.<br>- Attacking it with Ousia 1 time will weaken it for 5s."
        },
        "Drop": [
            {
                "Type": 29,
                "Amount": [
                    1.2606,
                    0.3362,
                    0.084,
                    0
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/机关·算力增幅器"
        ]
    },
    "40653": {
        "_id": 40653,
        "KingdomID": 4,
        "ClassID": 104065,
        "Name": {
            "CH": "机关 · 水下勘测型 · 芒",
            "EN": "Underwater Survey Mek - Pneuma"
        },
        "Title": [
            {
                "CH": "机关 · 水下勘测型",
                "EN": "Underwater Survey Mek"
            }
        ],
        "Desc": {
            "CH": "以水下探测为目的，设计制造的发条机关。\n在发条机关的设计上，在动力学无法得到突破的情况下，运动学是最重要的研究对象。在特定目的的机械设计上，仿生学有时能提供最便捷的解法。",
            "EN": "A clockwork meka designed for underwater surveying.\nIn the absence of significant breakthroughs in dynamics research, kinematics research has had the most significant impact on clockwork meka design. For machines designed to perform specific tasks, biomimesis has sometimes provided the most convenient solutions."
        },
        "HP": 0.6,
        "HPCurve": 7,
        "ATK": 45,
        "ATKCurve": 40,
        "Icon": [
            "UI_MonsterIcon_MachinaIustitia_Recon"
        ],
        "Grade": 1,
        "Weight": 125,
        "EndureTemplateID": 210,
        "Tutorial": {
            "CH": "- 低级水下芒性机关。<br>- 使用1次荒性攻击，可以使其弱化50s。<br>- 弱化期间无法进行攻击。",
            "EN": "- Small underwater Pneuma mek.<br>- Attacking it with Ousia 1 time will weaken it for 50s.<br>- While weakened, cannot perform any action."
        },
        "Drop": [
            {
                "Type": 29,
                "Amount": [
                    1.2606,
                    0.3362,
                    0.084,
                    0
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/机关·水下勘测型"
        ]
    },
    "40654": {
        "_id": 40654,
        "KingdomID": 4,
        "ClassID": 104065,
        "Name": {
            "CH": "机关 · 地质勘探型 · 芒",
            "EN": "Geological Survey Mek - Pneuma"
        },
        "Title": [
            {
                "CH": "机关 · 地质勘探型",
                "EN": "Geological Survey Mek"
            }
        ],
        "Desc": {
            "CH": "以探测地下资源及特定矿物为目的，设计制造的发条机关。\n因为能够设置搜寻的金属类型，所以也很受盗宝团与冒险家的欢迎。",
            "EN": "A clockwork meka designed for surveying underground resources and certain minerals.\nDue to the fact that the desired metal type can be set, this device has proven very popular among Treasure Hoarders and Adventurers alike."
        },
        "Color": "Water",
        "HP": 1.0,
        "HPCurve": 2,
        "ATK": 1.6,
        "ATKCurve": 4,
        "Ball": {
            "Type": 0,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "UI_MonsterIcon_MachinaIustitia_Probe"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 50,
        "Tutorial": {
            "CH": "- 低级芒性机关，驱使<color style='color:#0070C0;'>水元素</color>。<br>- 使用1次荒性攻击，可以使其弱化25s。<br>- 弱化期间只能进行低强度物理攻击，无法使用元素。",
            "EN": "- Small Pneuma mek, masters the <color style='color:#0070C0;'>Hydro</color> element.<br>- Attacking it with Ousia 1 time will weaken it for 25s.<br>- While weakened, loses its element and can only perform basic Physical attacks."
        },
        "Drop": [
            {
                "Type": 29,
                "Amount": [
                    1.2606,
                    0.3362,
                    0.084,
                    0
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/机关·地质勘探型"
        ]
    },
    "40655": {
        "_id": 40655,
        "KingdomID": 4,
        "ClassID": 104065,
        "Name": {
            "CH": "机关 · 水下巡游型 · 芒",
            "EN": "Underwater Patrol Mek - Pneuma"
        },
        "Title": [
            {
                "CH": "机关 · 水下巡游型",
                "EN": "Underwater Patrol Mek"
            }
        ],
        "Desc": {
            "CH": "这种发条机关的设计目的为搜寻、驱逐、消灭对水下设施有不利影响的水生动物。\n自第一次涨水期以来，枫丹科学院便一直忧心于或许有一天终将发生的第二次涨水期。其中一部分人开始构想收复丧失在水中的遗迹并建设海底之城，另一部分人则望着永远浮在不可触及高度的空中岛屿，梦想天空的都会。\n「水下巡游型」来自前者，但从结果而言，近些年后者似乎取得了上风。",
            "EN": "This type of clockwork meka was designed to search out, repel, and if need be exterminate aquatic life forms that posed a threat to underwater equipment.\nEver since the first flood period, the Fontaine Research Institute has been preoccupied with the thought that there might be a second one. Some had the idea of taking back the ruins lost to the waves and building an underwater city, whereas others looked up to the untouchable sky islands overhead with the intention of building a metropolis in the clouds.\nUnderwater Patrol Meks were conceived by the former, but judging from the results, it is the latter that have gained the upper hand in recent years."
        },
        "HP": 1,
        "HPCurve": 7,
        "ATK": 50,
        "ATKCurve": 40,
        "Icon": [
            "UI_MonsterIcon_MachinaIustitia_Nautilus"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 150,
        "Tutorial": {
            "CH": "- 低级水下芒性机关。<br>- 使用1次荒性攻击，可以使其弱化50s。<br>- 弱化期间无法进行攻击。",
            "EN": "- Small underwater Pneuma mek.<br>- Attacking it with Ousia 1 time will weaken it for 50s.<br>- While weakened, cannot perform any action."
        },
        "Drop": [
            {
                "Type": 29,
                "Amount": [
                    1.2606,
                    0.3362,
                    0.084,
                    0
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/机关·水下巡游型"
        ]
    },
    "40656": {
        "_id": 40656,
        "KingdomID": 4,
        "ClassID": 104060,
        "Name": {
            "CH": "机关 · 深海攻击型 · 芒",
            "EN": "Deepwater Assault Mek - Pneuma"
        },
        "Title": [
            {
                "CH": "机关 · 深海攻击型",
                "EN": "Deepwater Assault Mek"
            }
        ],
        "Desc": {
            "CH": "设计目的是进行水下攻击的发条机关。与「水下巡游型」机关不同的是，预期中它的敌人会更大、更凶暴。当然性能与造价的代价是，面对小体型的敌人时便无法完全发挥威力。\n过去在枫丹曾经发生了巨大魔兽入侵的悲剧，在陆上大湖受到景仰的白色铁甲舰队为了护国全军覆没。设计者希望如有此类事件再次发生，至少能在水中以强大火力出击的发条机关能先于将官与士兵成为第一道防线。",
            "EN": "A clockwork meka designed to conducting underwater attacks. Unlike \"Underwater Patrol Mek\" models, it is expected to take on much larger and fiercer opponents. Of course, the price it pays for such capabilities and its cost of manufacture is that is unable to fully unleash its capabilities against smaller opponents.\nLong ago, Fontaine was once scarred by the tragic invasion of a massive monster. The revered White Armada that sailed upon the great terrestrial lake was completely destroyed in defense of the nation. The designer hoped that were such a thing to happen again, the clockwork meka would possess enough naval firepower to be the first line of defense, going before the soldiers and generals."
        },
        "HP": 2,
        "HPCurve": 7,
        "ATK": 60,
        "ATKCurve": 40,
        "Icon": [
            "UI_MonsterIcon_MachinaIustitia_Frigate"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 150,
        "Tutorial": {
            "CH": "- 低级水下芒性机关。<br>- 使用1次荒性攻击，可以使其弱化50s。<br>- 弱化期间无法进行攻击。",
            "EN": "- Small underwater Pneuma mek.<br>- Attacking it with Ousia 1 time will weaken it for 50s.<br>- While weakened, cannot perform any action."
        },
        "Drop": [
            {
                "Type": 29,
                "Amount": [
                    1.2606,
                    0.3362,
                    0.084,
                    0
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/机关·深海攻击型"
        ]
    },
    "40657": {
        "_id": 40657,
        "KingdomID": 4,
        "ClassID": 104065,
        "Name": {
            "CH": "机关 · 灵活采集型 · 芒",
            "EN": "Nimble Harvester Mek - Pneuma"
        },
        "Title": [
            {
                "CH": "机关 · 灵活采集型",
                "EN": "Nimble Harvester Mek"
            }
        ],
        "Desc": {
            "CH": "以发掘、输送砂土为目的设计制造的发条机关。\n基于荒性与芒性湮灭产生能量的发条机关在枫丹具有得天独厚的优势，因为枫丹的水脉与土质本身就有储存、传导荒性与芒性的特性。这也是冒险中有时会见到荒性与芒性能量块的原因。",
            "EN": "A clockwork meka designed to dig up and transport soil.\nDue to the inherent ability of the local water and soil to store and transmit ousia and pneuma, clockwork meka powered by the annihilation of these energies are highly favored in Fontaine. This also explains why adventurers may come across Ousia and Pneuma Blocks from time to time."
        },
        "Color": "Wind",
        "HP": 1.2,
        "HPCurve": 2,
        "ATK": 1.4,
        "ATKCurve": 4,
        "Ball": {
            "Type": 0,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "UI_MonsterIcon_MachinaIustitia_Assimilator"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 50,
        "Tutorial": {
            "CH": "- 低级芒性机关，驱使<color style='color:#00B050;'>风元素</color>。<br>- 使用1次荒性攻击，可以使其弱化25s。<br>- 弱化期间只能进行低强度物理攻击，无法使用元素。",
            "EN": "- Small Pneuma mek, masters the <color style='color:#00B050;'>Anemo</color> element.<br>- Attacking it with Ousia 1 time will weaken it for 25s.<br>- While weakened, loses its element and can only perform basic Physical attacks."
        },
        "Drop": [
            {
                "Type": 29,
                "Amount": [
                    1.2606,
                    0.3362,
                    0.084,
                    0
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/机关·灵活采集型"
        ]
    },
    "40658": {
        "_id": 40658,
        "KingdomID": 4,
        "ClassID": 104066,
        "Name": {
            "CH": "攻坚特化型机关 · 芒",
            "EN": "Assault Specialist Mek - Pneuma"
        },
        "Title": [
            {
                "CH": "谢尔比乌斯式机关",
                "EN": "Meka, Cal. Scherbius"
            }
        ],
        "Desc": {
            "CH": "与「侦察记录型」一样，是最早设计制造的新式发条机关机械之一（数百年后的如今已经无人将其称为「新式」。这在当时是用以区分传统发条势能动力的机械机关用的说法）。\n名称是用来纪念发明者的友人而起的，最初设计的目的是帮助人。但后来的使用者普遍认为用它攻击人较之用人攻击人的相对优势更大。",
            "EN": "Alongside the Recon Log Mek, this was one of the first new-type clockwork meka to be made (though of course, after hundreds of years, no one refers to them as \"new-type\" anymore – this was simply to differentiate them from traditional clockwork mechanisms that were powered by potential energy.)\nIt was named to commemorate a friend of its inventor, and was originally designed to help people. However, its ability to attack people – thereby avoiding the need to put friendly personnel at risk – subsequently came to be seen as its real strength."
        },
        "Color": "Fire",
        "HP": 2.4,
        "HPCurve": 2,
        "ATK": 5.0,
        "ATKCurve": 4,
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "1113"
        },
        "Icon": [
            "UI_MonsterIcon_MachinaIustitia_Ruggiero_01"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 210,
        "Tutorial": {
            "CH": "- 中级芒性机关，驱使<color style='color:#FF0000;'>火元素</color>。<br>- 使用2次荒性攻击，可以使其弱化25s。<br>- 弱化期间只能进行低强度物理攻击，无法使用元素。",
            "EN": "- Mid-tier Pneuma mek, masters the <color style='color:#FF0000;'>Pyro</color> element.<br>- Attacking it with Ousia 2 times will weaken it for 25s.<br>- While weakened, loses its element and can only perform basic Physical attacks."
        },
        "Drop": [
            {
                "Type": 29,
                "Amount": [
                    3.3616,
                    0.8964,
                    0.224,
                    0
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/攻坚特化型机关"
        ]
    },
    "40659": {
        "_id": 40659,
        "KingdomID": 4,
        "ClassID": 104066,
        "Name": {
            "CH": "压制特化型机关 · 芒",
            "EN": "Suppression Specialist Mek - Pneuma"
        },
        "Title": [
            {
                "CH": "布莱切利式机关",
                "EN": "Meka, Cal. Bletchley"
            }
        ],
        "Desc": {
            "CH": "基于「谢尔比乌斯式」，进行了调整的发条机关。很遗憾，并不具备攻击以外的能力。\n基于湮灭能量的发条机关通常会利用特殊晶体构成的中继器从枫丹的环境中提炼荒性与芒性产生能量，但在枫丹以外的区域也能通过预先准备的能量块进行短程短时的行动。这两者的区别体现在机关上晶体部件的形状。",
            "EN": "A clockwork meka adapted from the Cal. Scherbius type. Unfortunately, aside from its ability to attack, it has no other capabilities.\nClockwork meka based on annihilation energy normally produce their energy by utilizing Repeaters composed of special crystals to refine ousia and pneuma from Fontaine's environment. But outside of Fontaine, they can use pre-prepared Energy Blocks to move over short distances for short periods of time. The difference can be observed from the shape of the crystal components on the meka."
        },
        "Color": "Grass",
        "HP": 2.0,
        "HPCurve": 2,
        "ATK": 6.0,
        "ATKCurve": 4,
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "1113"
        },
        "Icon": [
            "UI_MonsterIcon_MachinaIustitia_Ruggiero_02"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 210,
        "Tutorial": {
            "CH": "- 中级芒性机关，驱使<color style='color:#70AD47;'>草元素</color>。<br>- 使用2次荒性攻击，可以使其弱化25s。<br>- 弱化期间只能进行低强度物理攻击，无法使用元素。",
            "EN": "- Mid-tier Pneuma mek, masters the <color style='color:#70AD47;'>Dendro</color> element.<br>- Attacking it with Ousia 2 times will weaken it for 25s.<br>- While weakened, loses its element and can only perform basic Physical attacks."
        },
        "Drop": [
            {
                "Type": 29,
                "Amount": [
                    3.3616,
                    0.8964,
                    0.224,
                    0
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/压制特化型机关"
        ]
    },
    "40660": {
        "_id": 40660,
        "KingdomID": 4,
        "ClassID": 104066,
        "Name": {
            "CH": "歼灭特化型机关 · 芒",
            "EN": "Annihilation Specialist Mek - Pneuma"
        },
        "Title": [
            {
                "CH": "拜伦伯爵夫人式机关",
                "EN": "Meka, Cal. Countess Byron"
            }
        ],
        "Desc": {
            "CH": "尽管枫丹动能工程科学研究院是最为主要的发条机械研究与设计机构，坊间也有许多不隶属于科学院的工程师与独立工坊。\n这种机械起初是为一名独立工程师为纪念情人及赞助者而设计制造的。它最终被执律庭选中并采购作为城市安全或称公共暴力机构的武器之一。",
            "EN": "Although the Fontaine Research Institute of Kinetic Energy Engineering is the main organization involved in clockwork meka research and design, there are also many independent engineers and workshops that are not affiliated with the Research Institute.\nThe first ever machine of this type was designed by an independent engineer to commemorate their lover and sponsor. In the end, it was acquired by the Maison Gardiennage as a tool to uphold municipal security – or, in the eyes of some, as a weapon to administer public violence."
        },
        "Color": "Elec",
        "HP": 2.6,
        "HPCurve": 2,
        "ATK": 4.6,
        "ATKCurve": 4,
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "1113"
        },
        "Icon": [
            "UI_MonsterIcon_MachinaIustitia_Bradamante"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 210,
        "Tutorial": {
            "CH": "- 中级芒性机关，驱使<color style='color:#7030A0;'>雷元素</color>。<br>- 使用2次荒性攻击，可以使其弱化25s。<br>- 弱化期间只能进行低强度物理攻击，无法使用元素。",
            "EN": "- Mid-tier Pneuma mek, masters the <color style='color:#7030A0;'>Electro</color> element.<br>- Attacking it with Ousia 2 times will weaken it for 25s.<br>- While weakened, loses its element and can only perform basic Physical attacks."
        },
        "Drop": [
            {
                "Type": 29,
                "Amount": [
                    3.3616,
                    0.8964,
                    0.224,
                    0
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/歼灭特化型机关"
        ]
    },
    "40661": {
        "_id": 40661,
        "KingdomID": 4,
        "ClassID": 104065,
        "Name": {
            "CH": "机关 · 区域警戒型 · 芒",
            "EN": "Area Alert Mek - Pneuma"
        },
        "Title": [
            {
                "CH": "机关 · 区域警戒型",
                "EN": "Area Alert Mek"
            }
        ],
        "Desc": {
            "CH": "能够将湮灭产生的能量投射出去的机关。严格来说，是在枫丹科学院建立之前，自然哲学学院的相关研究中产生的试验装置。\n因此，尽管作为警戒与防御设备取得了相当的商业成功，大多数工程师并不将其视为严格意义的发条机关机械。",
            "EN": "A mechanism that is able to fire out the energy produced by an annihilation reaction. Strictly speaking, it was originally a test device that emerged from research undertaken by the School of Natural Philosophy before the Fontaine Research Institute had been established.\nAs such, despite its considerable commercial success as a sentinel and defensive device, most engineers do not consider it to be a true clockwork meka."
        },
        "Color": "Water",
        "HP": 0.7,
        "HPCurve": 2,
        "ATK": 2.0,
        "ATKCurve": 4,
        "Ball": {
            "Type": 0,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "UI_MonsterIcon_MachinaIustitia_LengthShifter"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 210,
        "Tutorial": {
            "CH": "- 低级芒性机关，驱使<color style='color:#0070C0;'>水元素</color>。<br>- 使用1次荒性攻击，可以使其弱化25s。<br>- 弱化期间只能进行低强度物理攻击，无法使用元素。",
            "EN": "- Small Pneuma mek, masters the <color style='color:#0070C0;'>Hydro</color> element.<br>- Attacking it with Ousia 1 time will weaken it for 25s.<br>- While weakened, loses its element and can only perform basic Physical attacks."
        },
        "Drop": [
            {
                "Type": 29,
                "Amount": [
                    1.2606,
                    0.3362,
                    0.084,
                    0
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/机关·区域警戒型"
        ]
    },
    "40662": {
        "_id": 40662,
        "KingdomID": 4,
        "ClassID": 104066,
        "Name": {
            "CH": "建造特化型机关 · 芒",
            "EN": "Construction Specialist Mek - Pneuma"
        },
        "Title": [
            {
                "CH": "艾蒂安-路易式机关",
                "EN": "Meka, Cal. Etienne-Louis"
            }
        ],
        "Desc": {
            "CH": "在建筑施工、考古发掘上同样有不可估量价值的大型发条机关，其名来自修建克莱门汀线时卡雷斯雇佣的工程师。\n这名工程师在卡雷斯的资助下为打通白淞隧道建造了颇具传奇性的「黄金乐船」巨型工程机械，并在竣工后据此设计了这类结构相似，但尺寸缩小的发条机关。",
            "EN": "A type of large clockwork meka which also has immeasurable value when utilized in building construction and archeological excavation. Its name comes from an engineer employed by Callas during the construction of the Clementine Line.\nWith Callas' financial backing, this engineer built the legendary \"Golden Note\" – a gigantic piece of construction machinery which was used to dig the Poisson Tunnel. Once the job was complete, this type of clockwork meka was designed with a similar structure but reduced to a smaller scale."
        },
        "Color": "Fire",
        "HP": 3.5,
        "HPCurve": 2,
        "ATK": 3.0,
        "ATKCurve": 4,
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "1113"
        },
        "Icon": [
            "UI_MonsterIcon_MachinaIustitia_Excavator"
        ],
        "Grade": 1,
        "Weight": 300,
        "EndureTemplateID": 4000,
        "Tutorial": {
            "CH": "- 中级芒性机关，驱使<color style='color:#FF0000;'>火元素</color>。<br>- 使用2次荒性攻击，可以使其弱化25s。<br>- 弱化期间只能进行低强度物理攻击，无法使用元素。",
            "EN": "- Mid-tier Pneuma mek, masters the <color style='color:#FF0000;'>Pyro</color> element.<br>- Attacking it with Ousia 2 times will weaken it for 25s.<br>- While weakened, loses its element and can only perform basic Physical attacks."
        },
        "Drop": [
            {
                "Type": 29,
                "Amount": [
                    3.3616,
                    0.8964,
                    0.224,
                    0
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/建造特化型机关"
        ]
    },
    "40701": {
        "_id": 40701,
        "KingdomID": 4,
        "ClassID": 10407,
        "Name": {
            "CH": "冰风组曲 · 科培琉司的劫罚",
            "EN": "Icewind Suite: Nemesis of Coppelius"
        },
        "Title": [
            {
                "CH": "冰风组曲 · 阳面",
                "EN": "Icewind Suite: Duality, Side B"
            }
        ],
        "Desc": {
            "CH": "由枫丹科学院研发的奇械舞者。\n在博絮埃最初的设想中，奇械舞者是超越机械复制时代的艺术品。但在博絮埃被迫离任后，该项目改由梅雅德接手。梅雅德对于奇械和舞者均有自己独到的见解，因此在迭代过程中，奇械舞者的各项功能在臻于完备的同时，也逐渐偏离了博絮埃的设计初衷。\n\n来自枫丹精密发条机关的华丽演出，这一幕的主角为发条机关贵族「科培琉司」。\n在演出的高潮阶段，科培琉司将停留在舞池的中央，露出能源特征为芒性的奇械发条，极大提升出力效率，为自己施加护罩，产生劫罚的三枚霜心。\n在这一阶段借由荒性的攻击，能有效破除护罩，并最终使其供能过载，推进演出流程。",
            "EN": "Artificed Dancers developed by the Fontaine Research Institute.\nBossuet originally intended for Artificed Dancers to be works of art that transcended the era of mechanical reproduction. But after Bossuet was forced to resign, the project was taken over by Maillardet, who had his own unique understanding of what he wanted to achieve. As a result, while their features were perfected during the course of its development, the Artificed Dancers gradually strayed from Bossuet's original vision.\n\nNext up is a fantastical performance to be delivered by the clockwork meka Coppelius. \nAt the climax of the performance, Coppelius will halt at the center of the dance floor and expose his spare clockwork component charged with Pneuma, thereby drastically increasing the efficiency of his exertions, granting him a shield, and generating three retributive Frosthearts. \nDuring this phase, using Ousia-imbued attacks will effectively break his shield, thus progressing the performance."
        },
        "Color": "Ice",
        "HP": 7.5,
        "HPCurve": 2,
        "ATK": 5.0,
        "ATKCurve": 4,
        "RESBase": {
            "Ice": 0.7,
            "Wind": 0.7
        },
        "RESState": [
            {
                "State": "Alert",
                "All": 3.0
            }
        ],
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "3336"
        },
        "Icon": [
            "UI_MonsterIcon_MachinaIustitia_Nutcracker"
        ],
        "Grade": 3,
        "Weight": 300,
        "EndureTemplateID": 4000,
        "Tutorial": {
            "CH": "- 科培琉司是演出《冰风组曲 · 科培琉司的劫罚》的主角，演出会在30s后（或血量降至71%后）进入高潮。<br>- 演出进入高潮时，科培琉司会移动到舞台中央，产生劫罚的三枚霜心，并在每个乐段开启时为自己刷新8元素量冰盾。<br>- <b>使用1次荒性攻击</b>可以直接湮灭其护盾。<b>累计使用3次荒性攻击</b>可提前结束高潮。<br>- 若未提前结束，高潮会在3个乐段后自行结束，每个乐段时长12秒。结束后30s会再次进入高潮。",
            "EN": "- Coppelius is the main character of the performance <i>Icewind Suite: Nemesis of Coppelius</i>. The performance reaches it climax after 30s or after HP falls below 71%.<br>- At the climax of the performance, Coppelius will move to the center of the stage and summon three Frosthearts of Retribution. At the start of every section of the climax, he refreshes an 8U Cryo Shield.<br>- <b>Attacking him with Ousia 1 time</b> will annihilate his Shield. <b>Attacking him with Ousia 3 times</b> will terminate the climax.<br>- If not terminated, the climax will finish after 3 sections. Each section lasts for 12s. The next climax starts 30s after the previous one finishes."
        },
        "Csx": [
            "ECAutomatons/冰风组曲·科培琉司的劫罚"
        ]
    },
    "40702": {
        "_id": 40702,
        "KingdomID": 4,
        "ClassID": 10407,
        "Name": {
            "CH": "冰风组曲 · 歌裴莉娅的葬送",
            "EN": "Icewind Suite: Dirge of Coppelia"
        },
        "Title": [
            {
                "CH": "冰风组曲 · 阴面",
                "EN": "Icewind Suite: Duality, Side A"
            }
        ],
        "Desc": {
            "CH": "由枫丹科学院研发的奇械舞者。\n在博絮埃最初的设想中，奇械舞者是超越机械复制时代的艺术品。但在博絮埃被迫离任后，该项目改由梅雅德接手。梅雅德对于奇械和舞者均有自己独到的见解，因此在迭代过程中，奇械舞者的各项功能在臻于完备的同时，也逐渐偏离了博絮埃的设计初衷。\n\n来自枫丹精密发条机关的华丽演出，这一幕的主角为发条机关女郎「歌裴莉娅」。\n在演出的高潮阶段，歌裴莉娅将停留在舞池的中央，露出荒性的奇械发条，极大提升出力效率，召来葬送的无数旋风。\n在这一阶段借由芒性的攻击，能有效削弱其攻击能力，并最终使其供能过载，推进演出流程。",
            "EN": "Artificed Dancers developed by the Fontaine Research Institute.\nBossuet originally intended for Artificed Dancers to be works of art that transcended the era of mechanical reproduction. But after Bossuet was forced to resign, the project was taken over by Maillardet, who had his own unique understanding of what he wanted to achieve. As a result, while their features were perfected during the course of its development, the Artificed Dancers gradually strayed from Bossuet's original vision.\n\nNext up is a fantastical performance to be delivered by the clockwork meka Geppelia. \nAt the climax of the performance, Geppelia will halt at the center of the dance floor and expose her spare clockwork component charged with Ousia, thereby drastically increasing the efficiency of her exertions and summoning countless whirlwinds of dirge. \nDuring this phase, using Pneuma-imbued attacks will reduce her offensive capabilities and overload her, thus progressing the performance."
        },
        "Color": "Wind",
        "HP": 7.5,
        "HPCurve": 2,
        "ATK": 5.0,
        "ATKCurve": 4,
        "RESBase": {
            "Ice": 0.7,
            "Wind": 0.7
        },
        "RESState": [
            {
                "State": "Alert",
                "All": 3.0
            },
            {
                "State": "Coppelia_Route",
                "All": 0.25
            }
        ],
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "3336"
        },
        "Icon": [
            "UI_MonsterIcon_MachinaIustitia_Nutcracker"
        ],
        "Grade": 3,
        "Weight": 300,
        "EndureTemplateID": 4000,
        "Tutorial": {
            "CH": "- 歌裴莉娅是演出《冰风组曲 · 歌裴莉娅的葬送》的主角，演出会在30s后（或血量降至71%后）进入高潮。<br>- 演出进入高潮时，歌裴莉娅会移动到舞台中央，召来葬送的无数旋风，<b>使全部抗性提升25%</b>。<br>- <b>使用1次芒性攻击</b>可移除抗性提升效果，并减弱其攻势。<b>累计使用3次芒性攻击</b>可提前结束高潮。<br>- 若未提前结束，高潮会在3个乐段后自行结束，每个乐段时长12秒。结束后30s会再次进入高潮。",
            "EN": "- Coppelia is the main character of the performance <i>Icewind Suite: Dirge of Coppelia</i>. The performance reaches it climax after 30s or after HP falls below 71%.<br>- At the climax of the performance, Coppelia will move to the center of the stage, summon countless Whirlwinds of Dirge and <b>increase all-type RES by 25%</b>. <br>- <b>Attacking her with Pnuema 1 time</b> will remove the RES increase and reduce her DMG. <b>Attacking her with Pnuema 3 times</b> will terminate the climax.<br>- If not terminated, the climax will finish after 3 sections. Each section lasts for 12s. The next climax starts 30s after the previous one finishes."
        },
        "Csx": [
            "ECAutomatons/冰风组曲·歌裴莉娅的葬送"
        ]
    },
    "40801": {
        "_id": 40801,
        "KingdomID": 4,
        "ClassID": 10408,
        "Name": {
            "CH": "实验性场力发生装置",
            "EN": "Experimental Field Generator"
        },
        "Title": [
            {
                "CH": "布雷阁原型机器",
                "EN": "Prototype Cal. Breguet"
            }
        ],
        "Desc": {
            "CH": "艾德温·伊斯丁豪斯生前设计的大型装置。依照原本的设计意图，拥有「抵消」重力的效果；依照原本的设计意图，这将是将枫丹建设成天空之都的第一步，从此以后再也没有人需要为曾经荡涤大地的浪潮而忧心，即使七国尽没深海，瑰丽的枫丹廷也将如天空岛那样在空中漂游。就连宇宙的秘密也将变得不那么遥不可及。\n装置核心的始基矿拥有「分离」或是「沉淀」的特性，能够将物质，或是某些物质以外的事物分离；传说这样的性质也能被反转，但枫丹科学院只醉心于它所能产生的物理效用。",
            "EN": "A large device designed by the late Edwin Eastinghouse. It has the ability to \"counteract\" the effects of gravity. According to the original design intentions, this was the first step in a plan to turn Fontaine into a city in the sky, so never again would the people have to worry about the waves that once swept over the land. Even if all seven nations were to be swallowed by the deeps, the elegant Court of Fontaine would float above it all, just like Celestia. Even the secrets of the universe would no longer seem so distant.\nThe Arkhium in the device's core has the characteristics of \"splitting\" or \"precipitation,\" and can take matter, or certain things outside of matter, and separate them. It is said that this process can be reversed, but the Fontaine Research Institute is infatuated only with the physical effects it can produce."
        },
        "Color": "Rock",
        "HP": 7.85,
        "HPCurve": 2,
        "ATK": 9.0,
        "ATKCurve": 4,
        "RESBase": {
            "Rock": 0.7
        },
        "RESState": [],
        "Ball": {
            "Type": 0,
            "Point": 3,
            "Drop": "333"
        },
        "Icon": [
            "UI_MonsterIcon_MachinaIustitia_Gravitas"
        ],
        "Grade": 3,
        "Weight": 300,
        "EndureTemplateID": 4000,
        "Tutorial": {
            "CH": "- 在战斗中，会周期性启动<b>荒性激发态</b>，在周围创造反重力场，使角色跳跃高度大幅提升（参数和魈大招等同）。在此期间，会在地面展开猛烈的攻势。<br>- 利用反重力场可以跳跃至空中躲避其攻势。<b>累计使用3次芒性攻击</b>可使其强制退出荒性激发态。荒性激发态会在一分钟后自行消去，并会在30s后再次启动。<br>大世界版本会在生命中降至70%时强制启动激发态；深渊版本开局就会启动。",
            "EN": "- In battle, it will periodically launch <b>Ousia Power</b>, creating anti-gravitational fields around it, greatly increasing characters' jump height (Equivalent to Xiao's Burst). While Powered, it will launch dangerous attacks on the ground.<br>- Use the anti-gravitational field to jump high into the air and avoid its attacks. <b>Using 3 Pneuma attacks</b> will terminate the Ousia Power. Ousia Power will fade after 1 minute, but will relaunch after 30s.<br>- The Overworld version will launch Power after its HP drops below 70%. The Abyss version will launch immediately after entering battle."
        },
        "SkillDMG": [
            {
                "Name": {
                    "CH": "（荒性激发）岩晶浪潮 / 闪避无效",
                    "EN": "(Ousia Power) Crystal Waves *Undodgable"
                },
                "DMG": [
                    9,
                    4.5
                ]
            },
            {
                "Name": {
                    "CH": "（荒性激发）斫地凿心",
                    "EN": "(Ousia Power) Groundbreaker *Undodgable"
                },
                "DMG": [
                    9
                ]
            },
            {
                "Name": {
                    "CH": "（荒性激发）地面连续喷砂",
                    "EN": "(Ousia Power) Continuous Ground Spurt"
                },
                "DMG": [
                    7.2
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/实验性场力发生装置"
        ]
    },
    "50102": {
        "_id": 50102,
        "KingdomID": 5,
        "ClassID": 105010,
        "Name": {
            "CH": "盗宝团 · 斥候",
            "EN": "Treasure Hoarders: Scout"
        },
        "Title": [
            {
                "CH": "盗宝团 · 斥候",
                "EN": "Treasure Hoarders: Scout"
            }
        ],
        "Desc": {
            "CH": "足迹遍布大陆各处，甚至深入未知秘境的松散盗贼组织中的一员。\n在团队协作中，职责通常是踩点与盯梢。落草为寇通常因为贫困与缺乏教育，但从他们的装备来看，即使选择了盗贼之路，也没有因此变得富裕…",
            "EN": "A member of the decentralized criminal organization that has footprints all over the continent and even deep within unknown domains.\nTheirs is the legwork and the tailing of targets. They usually wind up in criminal circles due to poverty or lack of education, but they don't seem to have gotten any richer after choosing a life of crime..."
        },
        "HP": 1.2,
        "HPCurve": 1,
        "ATK": 1.2,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": -0.2
        },
        "Ball": {
            "Type": 0,
            "Point": 3,
            "Drop": "111"
        },
        "Icon": [
            "thsc1"
        ],
        "Grade": 1,
        "Weight": 80,
        "EndureTemplateID": 50,
        "Drop": [
            {
                "Type": 9,
                "Amount": [
                    1.2606,
                    0.3362,
                    0.084,
                    0
                ]
            }
        ],
        "Csx": [
            "ECOtherHumanFactions/盗宝团·斥候"
        ]
    },
    "50103": {
        "_id": 50103,
        "KingdomID": 5,
        "ClassID": 105011,
        "Name": {
            "CH": "盗宝团 · 火之药剂师",
            "EN": "Treasure Hoarders: Pyro Potioneer"
        },
        "Title": [
            {
                "CH": "盗宝团 · 火之药剂师",
                "EN": "Treasure Hoarders: Pyro Potioneer"
            }
        ],
        "Desc": {
            "CH": "足迹遍布大陆各处，甚至深入未知秘境的松散盗贼组织中的一员。\n利用化学的知识，铤而走险的人。会调配出易燃易爆的物质，与团伙中其他的同伴一同作案。",
            "EN": "A member of the decentralized criminal organization that has footprints all over the continent and even deep within unknown domains.\nRisk-takers who utilize their knowledge of chemistry to mix volatile and flammable brews that aid in their comrades' dastardly deeds."
        },
        "Color": "Fire",
        "HP": 1.6,
        "HPCurve": 1,
        "ATK": 1.6,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": -0.2
        },
        "Ball": {
            "Type": 0,
            "Point": 3,
            "Drop": "112"
        },
        "Icon": [
            "thpp1"
        ],
        "Grade": 1,
        "Weight": 80,
        "EndureTemplateID": 50,
        "Drop": [
            {
                "Type": 9,
                "Amount": [
                    1.2606,
                    0.3362,
                    0.084,
                    0
                ]
            }
        ],
        "Csx": [
            "ECOtherHumanFactions/盗宝团·火之药剂师"
        ]
    },
    "50104": {
        "_id": 50104,
        "KingdomID": 5,
        "ClassID": 105011,
        "Name": {
            "CH": "盗宝团 · 水之药剂师",
            "EN": "Treasure Hoarders: Hydro Potioneer"
        },
        "Title": [
            {
                "CH": "盗宝团 · 水之药剂师",
                "EN": "Treasure Hoarders: Hydro Potioneer"
            }
        ],
        "Desc": {
            "CH": "足迹遍布大陆各处，甚至深入未知秘境的松散盗贼组织中的一员。\n据说部分盗宝团的药剂师，在加入盗宝团之前，曾经是在城镇间漫游，兜售自己调配的…「灵药」、「圣水」的江湖骗子。这些灵药、圣水显然在战斗中更能大放异彩。",
            "EN": "A member of the decentralized criminal organization that has footprints all over the continent and even deep within unknown domains.\nThey say that some Treasure Hoarder Potioneers used to be charlatans who would wander villages, towns, and cities, hawking their special blends of elixirs, holy waters, and the like. These cure-alls certainly seem to serve better in combat than they ever did in chicanery."
        },
        "Color": "Water",
        "HP": 1.6,
        "HPCurve": 1,
        "ATK": 1.6,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": -0.2
        },
        "Ball": {
            "Type": 0,
            "Point": 3,
            "Drop": "112"
        },
        "Icon": [
            "thhp1"
        ],
        "Grade": 1,
        "Weight": 80,
        "EndureTemplateID": 50,
        "Drop": [
            {
                "Type": 9,
                "Amount": [
                    1.2606,
                    0.3362,
                    0.084,
                    0
                ]
            }
        ],
        "Csx": [
            "ECOtherHumanFactions/盗宝团·水之药剂师"
        ]
    },
    "50105": {
        "_id": 50105,
        "KingdomID": 5,
        "ClassID": 105011,
        "Name": {
            "CH": "盗宝团 · 雷之药剂师",
            "EN": "Treasure Hoarders: Electro Potioneer"
        },
        "Title": [
            {
                "CH": "盗宝团 · 雷之药剂师",
                "EN": "Treasure Hoarders: Electro Potioneer"
            }
        ],
        "Desc": {
            "CH": "足迹遍布大陆各处，甚至深入未知秘境的松散盗贼组织中的一员。\n「药剂师」是盗宝团成员当中，为这些具有调配危险化学物质知识的人物取的诨名。\n<i>「嗯？这宝物上写了你们冒险家协会的名字么？」</i>\n——某名盗宝团的「药剂师」与冒险家对峙时说的话",
            "EN": "A member of the decentralized criminal organization that has footprints all over the continent and even deep within unknown domains.\n\"Potioneer\" is the nickname given to members of the Treasure Hoarder organization who know how to brew up dangerous chemicals.\n<i>\"What? Did you think this treasure has 'Adventurers' Guild' written all over it or something?\"</i>\n—A certain Treasure Hoarder Potioneer during a confrontation with adventurers"
        },
        "Color": "Elec",
        "HP": 1.6,
        "HPCurve": 1,
        "ATK": 1.6,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": -0.2
        },
        "Ball": {
            "Type": 0,
            "Point": 3,
            "Drop": "112"
        },
        "Icon": [
            "thep1"
        ],
        "Grade": 1,
        "Weight": 80,
        "EndureTemplateID": 50,
        "Drop": [
            {
                "Type": 9,
                "Amount": [
                    1.2606,
                    0.3362,
                    0.084,
                    0
                ]
            }
        ],
        "Csx": [
            "ECOtherHumanFactions/盗宝团·雷之药剂师"
        ]
    },
    "50106": {
        "_id": 50106,
        "KingdomID": 5,
        "ClassID": 105011,
        "Name": {
            "CH": "盗宝团 · 冰之药剂师",
            "EN": "Treasure Hoarders: Cryo Potioneer"
        },
        "Title": [
            {
                "CH": "盗宝团 · 冰之药剂师",
                "EN": "Treasure Hoarders: Cryo Potioneer"
            }
        ],
        "Desc": {
            "CH": "足迹遍布大陆各处，甚至深入未知秘境的松散盗贼组织中的一员。\n冰之药剂师在盗宝团当中相当受欢迎，因为他们调配的冰之灵药有快速降温的作用。即使是在没有冰雾花的野外，也可以制作冰镇酒类饮料。",
            "EN": "A member of the decentralized criminal organization that has footprints all over the continent and even deep within unknown domains.\nCryo Potioneers are quite the welcome sight amongst the Treasure Hoarders due to their elixirs having the ability to rapidly cool things down. Even out in the wild where there are no Mist Flowers to be found, they can have some cold liquor to make merry with."
        },
        "Color": "Ice",
        "HP": 1.6,
        "HPCurve": 1,
        "ATK": 1.6,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": -0.2
        },
        "Ball": {
            "Type": 0,
            "Point": 3,
            "Drop": "112"
        },
        "Icon": [
            "thcp1"
        ],
        "Grade": 1,
        "Weight": 80,
        "EndureTemplateID": 50,
        "Drop": [
            {
                "Type": 9,
                "Amount": [
                    1.2606,
                    0.3362,
                    0.084,
                    0
                ]
            }
        ],
        "Csx": [
            "ECOtherHumanFactions/盗宝团·冰之药剂师"
        ]
    },
    "50107": {
        "_id": 50107,
        "KingdomID": 5,
        "ClassID": 105012,
        "Name": {
            "CH": "盗宝团 · 杂工",
            "EN": "Treasure Hoarders: Handyman"
        },
        "Title": [
            {
                "CH": "盗宝团 · 杂工",
                "EN": "Treasure Hoarders: Handyman"
            }
        ],
        "Desc": {
            "CH": "足迹遍布大陆各处，甚至深入未知秘境的松散盗贼组织中的一员。\n在盗宝团当中从事着体力工作，地位比较低下的成员，因为自己的身材还经常被同伙取笑。但在团队遇袭时，会展现出人意料的身手与力量！",
            "EN": "A member of the decentralized criminal organization that has footprints all over the continent and even deep within unknown domains.\nThese members do physical work and are lower on the pecking order. They are often laughed at for their physique — but when the gang comes under attack, they get a chance to show their unexpectedly high strength and agility."
        },
        "HP": 2.0,
        "HPCurve": 1,
        "ATK": 1.6,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": -0.2
        },
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "1112"
        },
        "Icon": [
            "thha1"
        ],
        "Grade": 1,
        "Weight": 100,
        "EndureTemplateID": 100,
        "Drop": [
            {
                "Type": 9,
                "Amount": [
                    1.2606,
                    0.3362,
                    0.084,
                    0
                ]
            }
        ],
        "Csx": [
            "ECOtherHumanFactions/盗宝团·杂工"
        ]
    },
    "50108": {
        "_id": 50108,
        "KingdomID": 10,
        "ClassID": 11042,
        "Name": {
            "CH": "「利亚姆」",
            "EN": "Liam"
        },
        "Title": [
            {
                "CH": "地方传奇 · 盗宝团药剂师",
                "EN": "Local Legend: Potioneer"
            }
        ],
        "Desc": {
            "CH": "",
            "EN": ""
        },
        "HP": 30.5,
        "HPCurve": 2,
        "ATK": 3,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": -0.2
        },
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "1116"
        },
        "Icon": [
            "thep1"
        ],
        "Grade": 3,
        "Weight": 125,
        "EndureTemplateID": 210,
        "Csx": [
            "ECLegends/地方传奇·药剂师"
        ]
    },
    "50202": {
        "_id": 50202,
        "KingdomID": 5,
        "ClassID": 105010,
        "Name": {
            "CH": "盗宝团 · 神射手",
            "EN": "Treasure Hoarders: Marksman"
        },
        "Title": [
            {
                "CH": "盗宝团 · 神射手",
                "EN": "Treasure Hoarders: Marksman"
            }
        ],
        "Desc": {
            "CH": "足迹遍布大陆各处，甚至深入未知秘境的松散盗贼组织中的一员。\n虽然被称作「神射手」，但其实那是带有讥笑性质的诨名。毕竟他们使用的是弩，而不是弓这样需要长期训练的武器。要知道，就连丘丘人也会用弩。只要把比较宽的那头对准敌人，然后扣下扳机就行了。",
            "EN": "A member of the decentralized criminal organization that has footprints all over the continent and even deep within unknown domains.\nThough they are called \"marksmen,\" this name has an undercurrent of mockery to it, as they use crossbows instead of bows, which require a long period of training to use. It must be said that even hilichurls know how to operate a crossbow. Just point the wider end at the enemy, pull the trigger, and bam — that's all there is to it."
        },
        "HP": 1.2,
        "HPCurve": 1,
        "ATK": 1.2,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": -0.2
        },
        "Ball": {
            "Type": 0,
            "Point": 3,
            "Drop": "111"
        },
        "Icon": [
            "thma1"
        ],
        "Grade": 1,
        "Weight": 70,
        "EndureTemplateID": 50,
        "Drop": [
            {
                "Type": 9,
                "Amount": [
                    1.2606,
                    0.3362,
                    0.084,
                    0
                ]
            }
        ],
        "Csx": [
            "ECOtherHumanFactions/盗宝团·神射手"
        ]
    },
    "50302": {
        "_id": 50302,
        "KingdomID": 5,
        "ClassID": 105012,
        "Name": {
            "CH": "盗宝团 · 掘墓者",
            "EN": "Treasure Hoarders: Gravedigger"
        },
        "Title": [
            {
                "CH": "盗宝团 · 掘墓者",
                "EN": "Treasure Hoarders: Gravedigger"
            }
        ],
        "Desc": {
            "CH": "足迹遍布大陆各处，甚至深入未知秘境的松散盗贼组织中的一员。\n<i>「站在巨人的肩膀上，意思就是把前人积累的财富挖出来，拿走，好让我们…让它们回归经济循环！这是多么高尚的事情！」</i>\n——某名盗宝团的「掘墓者」在挖掘遗迹中的财宝时说的话",
            "EN": "A member of the decentralized criminal organization that has footprints all over the continent and even deep within unknown domains.\n<i>\"They say that we stand on the shoulders of giants. That means digging up the accumulated wealth of our forebears, taking them with us, and... uh, returning them to the economic cycle! What a noble endeavor, am I right?\"</i>\n—A certain Treasure Hoarder Gravedigger, while delving deep into a ruin."
        },
        "HP": 2.0,
        "HPCurve": 1,
        "ATK": 1.6,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": -0.2
        },
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "1112"
        },
        "Icon": [
            "thgr1"
        ],
        "Grade": 1,
        "Weight": 100,
        "EndureTemplateID": 100,
        "Drop": [
            {
                "Type": 9,
                "Amount": [
                    1.2606,
                    0.3362,
                    0.084,
                    0
                ]
            }
        ],
        "Csx": [
            "ECOtherHumanFactions/盗宝团·掘墓者"
        ]
    },
    "50303": {
        "_id": 50303,
        "KingdomID": 5,
        "ClassID": 105012,
        "Name": {
            "CH": "盗宝团 · 海上男儿",
            "EN": "Treasure Hoarders: Seaman"
        },
        "Title": [
            {
                "CH": "盗宝团 · 海上男儿",
                "EN": "Treasure Hoarders: Seaman"
            }
        ],
        "Desc": {
            "CH": "足迹遍布大陆各处，甚至深入未知秘境的松散盗贼组织中的一员。\n<i>「我们的活动怎么能叫偷？怎么能叫盗？那叫重新发现，让宝藏回到赏识宝藏的鉴赏家怀抱中！大家都开心，多好的事儿！」</i>\n——某名盗宝团的「海上男儿」对怀有疑问的新人说的话",
            "EN": "A member of the decentralized criminal organization that has footprints all over the continent and even deep within unknown domains.\n<i>\"Stealing? What do you mean, stealing? It's re-discovery, I tell you — the return of treasure to those who appreciate it! Makes everyone happy, to boot! How great is that, eh?\"</i>\n—A certain Treasure Hoarder Seaman, in response to a question posed by a doubtful recruit"
        },
        "HP": 2.0,
        "HPCurve": 1,
        "ATK": 1.6,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": -0.2
        },
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "1112"
        },
        "Icon": [
            "thse1"
        ],
        "Grade": 1,
        "Weight": 100,
        "EndureTemplateID": 100,
        "Drop": [
            {
                "Type": 9,
                "Amount": [
                    1.2606,
                    0.3362,
                    0.084,
                    0
                ]
            }
        ],
        "Csx": [
            "ECOtherHumanFactions/盗宝团·海上男儿"
        ]
    },
    "50601": {
        "_id": 50601,
        "KingdomID": 5,
        "ClassID": 105012,
        "Name": {
            "CH": "盗宝团 · 拳术家",
            "EN": "Treasure Hoarders: Pugilist"
        },
        "Title": [
            {
                "CH": "盗宝团 · 拳术家",
                "EN": "Treasure Hoarders: Pugilist"
            }
        ],
        "Desc": {
            "CH": "足迹遍布大陆各处，甚至深入未知秘境的松散盗贼组织中的一员。\n热衷于肌肉和战斗的人。对食物很挑剔，并且总是向周围的人宣扬肌肉的好处，所以其他盗宝团成员通常对这种人多有微辞。",
            "EN": "A member of the decentralized criminal organization that has footprints all over the continent and even deep within unknown domains.\nPassionate about battle and muscles, they are quite picky about food and will often boast of the benefits of having a fine musculature, leading to some dissatisfaction from their fellow Treasure Hoarders."
        },
        "HP": 2.0,
        "HPCurve": 1,
        "ATK": 1.6,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": -0.2
        },
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "1112"
        },
        "Icon": [
            "thpu1"
        ],
        "Grade": 1,
        "Weight": 100,
        "EndureTemplateID": 100,
        "Drop": [
            {
                "Type": 9,
                "Amount": [
                    1.2606,
                    0.3362,
                    0.084,
                    0
                ]
            }
        ],
        "Csx": [
            "ECOtherHumanFactions/盗宝团·拳术家"
        ]
    },
    "50602": {
        "_id": 50602,
        "KingdomID": 10,
        "ClassID": 11042,
        "Name": {
            "CH": "「罗基 · 艾威尔森」",
            "EN": "Rocky Avildsen"
        },
        "Title": [
            {
                "CH": "地方传奇 · 盗宝团 · 拳术家",
                "EN": "Local Legend: Pugilist"
            }
        ],
        "Desc": {
            "CH": "",
            "EN": ""
        },
        "HP": 32,
        "HPCurve": 2,
        "ATK": 11,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": -0.2
        },
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "1116"
        },
        "Icon": [
            "thpu1"
        ],
        "Grade": 3,
        "Weight": 125,
        "EndureTemplateID": 210,
        "Csx": [
            "ECLegends/地方传奇·拳术家"
        ]
    },
    "50701": {
        "_id": 50701,
        "KingdomID": 5,
        "ClassID": 105012,
        "Name": {
            "CH": "盗宝团 · 粉碎者",
            "EN": "Treasure Hoarders: Crusher"
        },
        "Title": [
            {
                "CH": "盗宝团 · 粉碎者",
                "EN": "Treasure Hoarders: Crusher"
            }
        ],
        "Desc": {
            "CH": "足迹遍布大陆各处，甚至深入未知秘境的松散盗贼组织中的一员。\n原本是在矿下工作的人。层岩巨渊封锁后，尽管领取着津贴，还是出于各种缘由加入了盗宝团。",
            "EN": "A member of the decentralized criminal organization that has footprints all over the continent and even deep within unknown domains.\nThis person was originally a miner — one who, even with the allowance they were provided with after the closure of the Chasm, ended up with the Treasure Hoarders for various reasons."
        },
        "HP": 2.0,
        "HPCurve": 1,
        "ATK": 1.6,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": -0.2
        },
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "1112"
        },
        "Icon": [
            "thcr1"
        ],
        "Grade": 1,
        "Weight": 100,
        "EndureTemplateID": 100,
        "Drop": [
            {
                "Type": 9,
                "Amount": [
                    1.2606,
                    0.3362,
                    0.084,
                    0
                ]
            }
        ],
        "Csx": [
            "ECOtherHumanFactions/盗宝团·粉碎者"
        ]
    },
    "50801": {
        "_id": 50801,
        "KingdomID": 5,
        "ClassID": 10508,
        "Name": {
            "CH": "野伏 · 阵刀番",
            "EN": "Nobushi: Jintouban"
        },
        "Title": [
            {
                "CH": "野伏 · 阵刀番",
                "EN": "Nobushi: Jintouban"
            }
        ],
        "Desc": {
            "CH": "落草为寇的流浪武人，虽然常被称为「野伏」，但并不隶属于某个统一的团体。\n作为常年行走在武道上的人，有着精湛的剑术，如今却不将其用于正道。\n为了生计与财富，有时候甚至会与盗宝团、愚人众沆瀣一气。",
            "EN": "Samurai who have fallen into banditry. Though they are called the Nobushi, they do not have a centralized organization.\nHaving pursued the path of a martial artist for a long time, they possess highly advanced sword skills but no longer use them for good.\nSometimes, they even collude with the Treasure Hoarders or Fatui out of a greed for wealth — or simply to survive."
        },
        "HP": 1.0,
        "HPCurve": 2,
        "ATK": 1.6,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": -0.2
        },
        "Ball": {
            "Type": 0,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "njtb"
        ],
        "Grade": 1,
        "Weight": 110,
        "EndureTemplateID": 100,
        "Drop": [
            {
                "Type": 10,
                "Amount": [
                    0.8404,
                    0.2241,
                    0.056,
                    0
                ]
            }
        ],
        "Csx": [
            "ECOtherHumanFactions/野伏·阵刀番"
        ]
    },
    "50802": {
        "_id": 50802,
        "KingdomID": 5,
        "ClassID": 10508,
        "Name": {
            "CH": "野伏 · 火付番",
            "EN": "Nobushi: Hitsukeban"
        },
        "Title": [
            {
                "CH": "野伏 · 火付番",
                "EN": "Nobushi: Hitsukeban"
            }
        ],
        "Desc": {
            "CH": "落草为寇的流浪武人，虽然常被称为「野伏」，但并不隶属于某个统一的团体。\n不仅掌握着精湛的剑术，还会使用具有元素的焰硝药粉攻击。从客观来说，或许这是背离武之道，转而寻求胜之途的体现吧。\n为了生计与财富，有时候甚至会与盗宝团、愚人众沆瀣一气。",
            "EN": "Samurai who have fallen into banditry. Though they are called the Nobushi, they do not have a centralized organization.\nThey not only possess highly advanced sword skills, but can also carry out flame powder attacks using the elements. Objectively speaking, perhaps this method of winning goes against the path of a martial artist.\nSometimes they even collude with the Treasure Hoarders or Fatui out of a greed for wealth — or simply to survive."
        },
        "Color": "Fire",
        "HP": 1.0,
        "HPCurve": 2,
        "ATK": 1.6,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": -0.2
        },
        "Ball": {
            "Type": 0,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "nhsb"
        ],
        "Grade": 1,
        "Weight": 110,
        "EndureTemplateID": 100,
        "Drop": [
            {
                "Type": 10,
                "Amount": [
                    0.8404,
                    0.2241,
                    0.056,
                    0
                ]
            }
        ],
        "Csx": [
            "ECOtherHumanFactions/野伏·火付番"
        ]
    },
    "50803": {
        "_id": 50803,
        "KingdomID": 5,
        "ClassID": 10508,
        "Name": {
            "CH": "野伏 · 机巧番",
            "EN": "Nobushi: Kikouban"
        },
        "Title": [
            {
                "CH": "野伏 · 机巧番",
                "EN": "Nobushi: Kikouban"
            }
        ],
        "Desc": {
            "CH": "落草为寇的流浪武人，虽然常被称为「野伏」，但并不隶属于某个统一的团体。\n除了作为武者必修的剑术之外，还会熟练地使用手弩偷袭对手，只要能够取胜便会不择手段。抛弃了武家之名的人，也常常会抛弃武家的自矜。\n为了生计与财富，有时候甚至会与盗宝团、愚人众沆瀣一气。",
            "EN": "Samurai who have fallen into banditry. Though they are called the Nobushi, they do not have a centralized organization.\nThey possess highly-advanced sword skills and also use crossbows to ambush opponents, using any means to win. Those who have abandoned the title of a martial artist often abandon their dignity as well.\nSometimes they even collude with the Treasure Hoarders or Fatui out of a greed for wealth — or simply to survive."
        },
        "Color": "Elec",
        "HP": 1.0,
        "HPCurve": 2,
        "ATK": 1.6,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": -0.2
        },
        "Ball": {
            "Type": 0,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "nkkb"
        ],
        "Grade": 1,
        "Weight": 110,
        "EndureTemplateID": 100,
        "Drop": [
            {
                "Type": 10,
                "Amount": [
                    0.8404,
                    0.2241,
                    0.056,
                    0
                ]
            }
        ],
        "Csx": [
            "ECOtherHumanFactions/野伏·机巧番"
        ]
    },
    "50901": {
        "_id": 50901,
        "KingdomID": 4,
        "ClassID": 10509,
        "Name": {
            "CH": "魔偶剑鬼",
            "EN": "Maguu Kenki"
        },
        "Title": [
            {
                "CH": "万般机巧 · 剑豪人形",
                "EN": "Ingenious Machine: Marionette Swordmaster"
            }
        ],
        "Desc": {
            "CH": "自律的人形机关剑士。\n似乎没有说话的能力与意愿，只愿意用剑沟通。\n据说在试作时，集成了某个剑道流派初代宗主的记忆，却又因为不明的原因而失控，最终被废弃了。\n根据歌人的描述，剑鬼会徘徊在因缘断绝的地方，而守护它的凶恶之面似乎取自那个时代有名的鬼人。",
            "EN": "A disciplined mechanical swordsman.\nDoesn't seem to have the ability nor will to speak, but instead only seeks to communicate with crossed swords.\nIt is said that this machine was made using the memories of the first-generation master of a certain sword school. However, it malfunctioned, lost control, and was ultimately discarded.\nSome sing that the machine's spirit still lingers in the place where its fate was ended. They also say that the vicious mask that serves as its guard was inspired by a famous oni from that era."
        },
        "HP": 20.0,
        "HPCurve": 1,
        "ATK": 8.0,
        "ATKCurve": 4,
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "3334"
        },
        "Icon": [
            "mk"
        ],
        "Grade": 3,
        "Weight": 99999,
        "EndureTemplateID": 4000,
        "SkillData": [
            "5/50901"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "普通攻击x3",
                    "EN": "Normal Attack x3"
                },
                "DMG": [
                    4,
                    4,
                    4
                ]
            },
            {
                "Name": {
                    "CH": "普通攻击 幻影协同",
                    "EN": "Normal Attack (Shadow Coordinates)"
                },
                "DMG": [
                    2.4,
                    2.4,
                    2.4
                ]
            },
            {
                "Name": {
                    "CH": "推面具",
                    "EN": "Mask Push"
                },
                "DMG": [
                    8
                ]
            },
            {
                "Name": {
                    "CH": "圆形斩",
                    "EN": "Circular Strike"
                },
                "DMG": [
                    8
                ]
            },
            {
                "Name": {
                    "CH": "双剑插地板",
                    "EN": "Double Sword Strike Ground"
                },
                "DMG": [
                    6.4,
                    6.4
                ]
            },
            {
                "Name": {
                    "CH": "突刺",
                    "EN": "Sword Rush"
                },
                "DMG": [
                    4.8
                ]
            },
            {
                "Name": {
                    "CH": "追踪面具",
                    "EN": "Chasing Mask"
                },
                "DMG": [
                    8
                ]
            },
            {
                "Name": {
                    "CH": "剑风",
                    "EN": "Wind Blade"
                },
                "DMG": [
                    4
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/魔偶剑鬼"
        ]
    },
    "50902": {
        "_id": 50902,
        "KingdomID": 4,
        "ClassID": 10509,
        "Name": {
            "CH": "魔偶剑鬼 · 孤风",
            "EN": "Maguu Kenki: Lone Gale"
        },
        "Title": [
            {
                "CH": "如剑迅疾之风",
                "EN": "Like the Rushing Wind"
            }
        ],
        "Color": "Wind",
        "HP": 16.0,
        "HPCurve": 1,
        "ATK": 5.0,
        "ATKCurve": 3,
        "RESBase": {
            "Wind": 0.3
        },
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "3334"
        },
        "Icon": [
            "lg"
        ],
        "Grade": 3,
        "Weight": 600,
        "EndureTemplateID": 4000,
        "SkillData": [
            "5/50902"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "普通攻击x2",
                    "EN": "Normal Attack x2"
                },
                "DMG": [
                    2.5,
                    2.5
                ]
            },
            {
                "Name": {
                    "CH": "剑风",
                    "EN": "Wind Blade"
                },
                "DMG": [
                    2.5
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/魔偶剑鬼·孤风"
        ]
    },
    "50903": {
        "_id": 50903,
        "KingdomID": 4,
        "ClassID": 10509,
        "Name": {
            "CH": "魔偶剑鬼 · 霜驰",
            "EN": "Maguu Kenki: Galloping Frost"
        },
        "Title": [
            {
                "CH": "如剑刺骨之霜",
                "EN": "Like the Biting Frost"
            }
        ],
        "Color": "Ice",
        "HP": 16.0,
        "HPCurve": 1,
        "ATK": 5.0,
        "ATKCurve": 3,
        "RESBase": {
            "Ice": 0.3
        },
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "3334"
        },
        "Icon": [
            "gf"
        ],
        "Grade": 3,
        "Weight": 600,
        "EndureTemplateID": 4000,
        "SkillData": [
            "5/50903"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "普通攻击x3",
                    "EN": "Normal Attack x3"
                },
                "DMG": [
                    2.5,
                    2.5,
                    2.5
                ]
            },
            {
                "Name": {
                    "CH": "持剑突刺",
                    "EN": "Sword Rush"
                },
                "DMG": [
                    3
                ]
            },
            {
                "Name": {
                    "CH": "圆形斩",
                    "EN": "Circular Strike"
                },
                "DMG": [
                    5
                ]
            },
            {
                "Name": {
                    "CH": "插地板",
                    "EN": "Strike Ground"
                },
                "DMG": [
                    8
                ]
            },
            {
                "Name": {
                    "CH": "插地板产生冰场 每秒伤害",
                    "EN": "Ground Strike Created Ice Area (DoT every second)"
                },
                "DMG": [
                    1.25
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/魔偶剑鬼·霜驰"
        ]
    },
    "50904": {
        "_id": 50904,
        "KingdomID": 4,
        "ClassID": 10509,
        "Name": {
            "CH": "魔偶剑鬼 · 凶面",
            "EN": "Maguu Kenki: Mask of Terror"
        },
        "Title": [
            {
                "CH": "如业狰狞之相",
                "EN": "Like a Karmic Terror"
            }
        ],
        "HP": 16.0,
        "HPCurve": 1,
        "ATK": 5.0,
        "ATKCurve": 3,
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "3334"
        },
        "Icon": [
            "mot"
        ],
        "Grade": 3,
        "Weight": 600,
        "EndureTemplateID": 4000,
        "SkillData": [
            "5/50904"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "推面具",
                    "EN": "Mask Push"
                },
                "DMG": [
                    5
                ]
            },
            {
                "Name": {
                    "CH": "追踪面具",
                    "EN": "Chasing Mask"
                },
                "DMG": [
                    5
                ]
            }
        ],
        "Csx": [
            "ECAutomatons/魔偶剑鬼·凶面"
        ]
    },
    "51001": {
        "_id": 51001,
        "KingdomID": 5,
        "ClassID": 10510,
        "Name": {
            "CH": "海乱鬼 · 雷腾",
            "EN": "Kairagi: Dancing Thunder"
        },
        "Title": [
            {
                "CH": "海乱鬼 · 雷腾",
                "EN": "Kairagi: Dancing Thunder"
            }
        ],
        "Desc": {
            "CH": "落草为寇的流浪武人。\n有着高强的武道修养，却因为种种原因而没有用武之地，走投无路中踏上了恶逆之道。会利用本应在数百年前失传的惟神技术制造的符纸，为刀剑附上雷霆的力量。",
            "EN": "Samurai who have fallen into banditry.\nThey possess highly advanced martial art skills but found no use for them for various reasons. Out of desperation, they have fallen onto the path of evil. They use paper seals, created from a Kamuna art that has been lost for centuries, to infuse their blades with the power of lightning."
        },
        "Color": "Elec",
        "HP": 2.4,
        "HPCurve": 2,
        "ATK": 7.5,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": -0.2
        },
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "1112"
        },
        "Icon": [
            "kdt"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 210,
        "Drop": [
            {
                "Type": 10,
                "Amount": [
                    1.6808,
                    0.4482,
                    0.112,
                    0
                ]
            }
        ],
        "Csx": [
            "ECOtherHumanFactions/海乱鬼·雷腾"
        ]
    },
    "51002": {
        "_id": 51002,
        "KingdomID": 5,
        "ClassID": 10510,
        "Name": {
            "CH": "海乱鬼 · 炎威",
            "EN": "Kairagi: Fiery Might"
        },
        "Title": [
            {
                "CH": "海乱鬼 · 炎威",
                "EN": "Kairagi: Fiery Might"
            }
        ],
        "Desc": {
            "CH": "落草为寇的流浪武人。\n自幼时便专注于武道从未懈怠，却因为种种原因失去了奉公的机会，背离了正道，走上恶逆之路。刀刃不似一般的钢铁那般冰冷，而是充满了忿恨的炽热怒火——似乎是利用了本应失传的惟神技术制造的符纸，为刀刃附上火焰。",
            "EN": "Samurai who have fallen into banditry.\nThey practiced martial arts since they were young, but lost the opportunity to serve the people due to certain reasons and turned from the righteous path. Their blade's edges are not cold like ordinary steel, but instead burn with a bitter flame. They seem to have used paper seals, created from a lost Kamuna art, to set their blades afire."
        },
        "Color": "Fire",
        "HP": 2.4,
        "HPCurve": 2,
        "ATK": 7.5,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": -0.2
        },
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "1112"
        },
        "Icon": [
            "kfm"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 210,
        "Drop": [
            {
                "Type": 10,
                "Amount": [
                    1.6808,
                    0.4482,
                    0.112,
                    0
                ]
            }
        ],
        "Csx": [
            "ECOtherHumanFactions/海乱鬼·炎威"
        ]
    },
    "51003": {
        "_id": 51003,
        "KingdomID": 8,
        "ClassID": 11001,
        "Name": {
            "CH": "「公义」",
            "EN": "Masanori"
        },
        "Title": [
            {
                "CH": "海乱鬼 · 雷腾",
                "EN": "Kairagi: Dancing Thunder"
            }
        ],
        "Color": "Elec",
        "HP": 3.0,
        "HPCurve": 2,
        "ATK": 8.25,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": -0.2
        },
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "1112"
        },
        "Icon": [
            "kdt"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 210,
        "EndureExtra": 100,
        "Csx": [
            "ECOtherHumanFactions/海乱鬼·雷腾"
        ]
    },
    "51004": {
        "_id": 51004,
        "KingdomID": 8,
        "ClassID": 11001,
        "Name": {
            "CH": "午夜「公义」",
            "EN": "Midnight Masanori"
        },
        "Title": [
            {
                "CH": "海乱鬼 · 雷腾",
                "EN": "Kairagi: Dancing Thunder"
            }
        ],
        "Color": "Elec",
        "HP": 5.4,
        "HPCurve": 2,
        "ATK": 12.0,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": -0.2
        },
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "1112"
        },
        "Icon": [
            "kdt"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 210,
        "EndureExtra": 100,
        "Csx": [
            "ECOtherHumanFactions/海乱鬼·雷腾"
        ]
    },
    "51101": {
        "_id": 51101,
        "KingdomID": 5,
        "ClassID": 10511,
        "Name": {
            "CH": "厄灵 · 岩之魔蜥",
            "EN": "Spirit of Omen: Stone Drake"
        },
        "Title": [
            {
                "CH": "厄灵 · 岩之魔蜥",
                "EN": "Spirit of Omen: Stone Drake"
            }
        ],
        "Color": "Rock",
        "HP": 0.75,
        "HPCurve": 2,
        "ATK": 3.2,
        "ATKCurve": 4,
        "RESBase": {
            "Rock": -0.5
        },
        "Ball": {
            "Type": 7,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "keq"
        ],
        "Grade": 1,
        "Weight": 10000,
        "EndureTemplateID": 400,
        "SkillData": [
            "5/51101"
        ],
        "Csx": [
            "ECNonCategorized/厄灵·岩之魔蜥"
        ]
    },
    "51102": {
        "_id": 51102,
        "KingdomID": 5,
        "ClassID": 10511,
        "Name": {
            "CH": "厄灵 · 风之魔鸢",
            "EN": "Spirit of Omen: Wind Glede"
        },
        "Title": [
            {
                "CH": "厄灵 · 风之魔鸢",
                "EN": "Spirit of Omen: Wind Glede"
            }
        ],
        "Color": "Wind",
        "HP": 0.66,
        "HPCurve": 2,
        "ATK": 4.0,
        "ATKCurve": 4,
        "RESBase": {
            "Wind": -0.5
        },
        "Ball": {
            "Type": 6,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "keq"
        ],
        "Grade": 1,
        "Weight": 10000,
        "EndureTemplateID": 400,
        "SkillData": [
            "5/51102"
        ],
        "Csx": [
            "ECNonCategorized/厄灵·风之魔鸢"
        ]
    },
    "51103": {
        "_id": 51103,
        "KingdomID": 5,
        "ClassID": 10511,
        "Name": {
            "CH": "厄灵 · 炎之魔蝎",
            "EN": "Spirit of Omen: Pyro Scorpion"
        },
        "Title": [
            {
                "CH": "厄灵 · 炎之魔蝎",
                "EN": "Spirit of Omen: Pyro Scorpion"
            }
        ],
        "Color": "Fire",
        "HP": 0.66,
        "HPCurve": 2,
        "ATK": 4.0,
        "ATKCurve": 4,
        "RESBase": {
            "Fire": -0.5
        },
        "Ball": {
            "Type": 1,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "keq"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 400,
        "SkillData": [
            "5/51103"
        ],
        "Csx": [
            "ECNonCategorized/厄灵·炎之魔蝎"
        ]
    },
    "51104": {
        "_id": 51104,
        "KingdomID": 5,
        "ClassID": 10511,
        "Name": {
            "CH": "厄灵 · 草之灵蛇",
            "EN": "Spirit of Omen: Dendro Spirit-Serpent"
        },
        "Title": [
            {
                "CH": "厄灵 · 草之灵蛇",
                "EN": "Spirit of Omen: Dendro Spirit-Serpent"
            }
        ],
        "Color": "Grass",
        "HP": 0.75,
        "HPCurve": 2,
        "ATK": 3.2,
        "ATKCurve": 4,
        "RESBase": {
            "Grass": -0.5
        },
        "Ball": {
            "Type": 5,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "keq"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 400,
        "SkillData": [
            "5/51104"
        ],
        "Csx": [
            "ECNonCategorized/厄灵·草之灵蛇"
        ]
    },
    "52101": {
        "_id": 52101,
        "KingdomID": 5,
        "ClassID": 10521,
        "Name": {
            "CH": "镀金旅团 · 阵前斧手",
            "EN": "Eremite Axe Vanguard"
        },
        "Title": [
            {
                "CH": "以武谋生的沙之遗民",
                "EN": "Martial Desert Descendant"
            }
        ],
        "Desc": {
            "CH": "来自黄金一般的沙漠，是松散佣兵团体中的一员。为了报酬，愿意为任何人效力。\n使用手斧的女战士在厮杀时往往一往无前，以此打乱敌人的阵脚，为战友创造机会。\n在沙漠的故乡里，她们有时会坐在篝火边，弹起那些失落在沙海中的悠远歌谣。",
            "EN": "A member of a loosely-organized mercenary corps from the golden desert sands. Will work for anyone as long as the pay is good.\nThese ax-wielding ladies often wade into the slaughter without fear, creating openings for their comrades to capitalize on.\nThey will sometimes sit by the fireside in their sand-swept homeland and play songs that were lost in that golden sea."
        },
        "HP": 0.8,
        "HPCurve": 2,
        "ATK": 1.6,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": -0.2
        },
        "Ball": {
            "Type": 0,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "emav"
        ],
        "Grade": 1,
        "Weight": 70,
        "EndureTemplateID": 50,
        "Drop": [
            {
                "Type": 24,
                "Amount": [
                    1.0505,
                    0.2801,
                    0.07,
                    0
                ]
            }
        ],
        "Csx": [
            "ECOtherHumanFactions/镀金旅团·阵前斧手"
        ]
    },
    "52102": {
        "_id": 52102,
        "KingdomID": 5,
        "ClassID": 10521,
        "Name": {
            "CH": "镀金旅团 · 机弩兵",
            "EN": "Eremite Crossbow"
        },
        "Title": [
            {
                "CH": "以武谋生的沙之遗民",
                "EN": "Martial Desert Descendant"
            }
        ],
        "Desc": {
            "CH": "来自黄金一般的沙漠，是松散佣兵团体中的一员。为了报酬，愿意为任何人效力。\n传统上，镀金旅团其实更习惯使用弓箭对付敌人。不过自从教令院允许他们在巴扎采购机弩之后，就只有最骄傲的射手才会再使用弓箭这种需要常年训练的武器了。",
            "EN": "A member of a loosely-organized mercenary corps hailing from a place where the sands shine like gold. Will work for anyone as long as the pay is good.\nThe Eremite tradition is to use bows to engage their foes, but ever since the Akademiya permitted them to buy crossbows at the bazaars, only the proudest shooters would still use bows, which take many years of practice to master."
        },
        "HP": 0.6,
        "HPCurve": 2,
        "ATK": 2.0,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": -0.2
        },
        "Ball": {
            "Type": 0,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "emcr"
        ],
        "Grade": 1,
        "Weight": 70,
        "EndureTemplateID": 50,
        "Drop": [
            {
                "Type": 24,
                "Amount": [
                    1.0505,
                    0.2801,
                    0.07,
                    0
                ]
            }
        ],
        "Csx": [
            "ECOtherHumanFactions/镀金旅团·机弩兵"
        ]
    },
    "52103": {
        "_id": 52103,
        "KingdomID": 5,
        "ClassID": 10521,
        "Name": {
            "CH": "镀金旅团 · 鸦喙戟手",
            "EN": "Eremite Revenbeak Halberdier"
        },
        "Title": [
            {
                "CH": "以武谋生的沙之遗民",
                "EN": "Martial Desert Descendant"
            }
        ],
        "Desc": {
            "CH": "来自黄金一般的沙漠，是松散佣兵团体中的一员。为了报酬，愿意为任何人效力。\n使用名为「鸦喙戟」的武器进行战斗。传说在古代，侍奉沙漠神祇的隐士会携带这种战戟作为身份的象征。这种武器的真名已经失落在了沙海中。",
            "EN": "A member of a loosely-organized mercenary corps from the golden desert sands. Will work for anyone as long as the pay is good.\nThese warriors use Ravenbeak Halberds in battle. It is said that ascetics who served the deity of the desert would carry these halberds as symbols of their office. These weapons once had a true name, but it has long been lost to the desert sands."
        },
        "HP": 0.6,
        "HPCurve": 2,
        "ATK": 2.0,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": -0.2
        },
        "Ball": {
            "Type": 0,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "emrh"
        ],
        "Grade": 1,
        "Weight": 70,
        "EndureTemplateID": 50,
        "Drop": [
            {
                "Type": 24,
                "Amount": [
                    1.0505,
                    0.2801,
                    0.07,
                    0
                ]
            }
        ],
        "Csx": [
            "ECOtherHumanFactions/镀金旅团·鸦喙戟手"
        ]
    },
    "52104": {
        "_id": 52104,
        "KingdomID": 5,
        "ClassID": 10521,
        "Name": {
            "CH": "镀金旅团 · 破阵者",
            "EN": "Eremite Linebreaker"
        },
        "Title": [
            {
                "CH": "以武谋生的沙之遗民",
                "EN": "Martial Desert Descendant"
            }
        ],
        "Desc": {
            "CH": "来自黄金一般的沙漠，是松散佣兵团体中的一员。为了报酬，愿意为任何人效力。\n装备了独特拳刃的武道家，为了寻找更多更强的对手而加入了镀金旅团。战斗与其说是为了报酬，不如说是为了追求更高的武技。",
            "EN": "A member of a loosely-organized mercenary corps from the golden desert sands. Will work for anyone as long as the pay is good.\nThis warrior wields a unique fist blade and joined the Eremites to seek out stronger foes. Rather than the pay, these warriors fight more to attain greater martial might."
        },
        "HP": 0.8,
        "HPCurve": 2,
        "ATK": 1.6,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": -0.2
        },
        "Ball": {
            "Type": 0,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "emlb"
        ],
        "Grade": 1,
        "Weight": 70,
        "EndureTemplateID": 50,
        "Drop": [
            {
                "Type": 24,
                "Amount": [
                    1.0505,
                    0.2801,
                    0.07,
                    0
                ]
            }
        ],
        "Csx": [
            "ECOtherHumanFactions/镀金旅团·破阵者"
        ]
    },
    "52105": {
        "_id": 52105,
        "KingdomID": 5,
        "ClassID": 10521,
        "Name": {
            "CH": "镀金旅团 · 刀舞者",
            "EN": "Eremite Sword-Dancer"
        },
        "Title": [
            {
                "CH": "以武谋生的沙之遗民",
                "EN": "Martial Desert Descendant"
            }
        ],
        "Desc": {
            "CH": "来自黄金一般的沙漠，是松散佣兵团体中的一员。为了报酬，愿意为任何人效力。\n擅长使用大弯刀的沙漠武士。其所掌握的传承自失落文明的刀法不仅是一种战技，由其发展而来的刀舞更成为了在各国都广受欢迎的杂技表演。\n这种大弯刀据说在古时是用于骑乘战斗的。",
            "EN": "A member of a loosely-organized mercenary corps from the golden desert sands. Will work for anyone as long as the pay is good.\nThis desert warrior wields a large curved blade in battle. The skills such fighters boast of have been passed down since the days when their now-lost civilization was still extant, and are not only techniques to be used in battle, but also serve well as a performing art popular in various lands.\nIt is said that this sword art was once practiced from horseback."
        },
        "HP": 0.8,
        "HPCurve": 2,
        "ATK": 1.6,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": -0.2
        },
        "Ball": {
            "Type": 0,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "emsd"
        ],
        "Grade": 1,
        "Weight": 70,
        "EndureTemplateID": 50,
        "Drop": [
            {
                "Type": 24,
                "Amount": [
                    1.0505,
                    0.2801,
                    0.07,
                    0
                ]
            }
        ],
        "Csx": [
            "ECOtherHumanFactions/镀金旅团·刀舞者"
        ]
    },
    "53101": {
        "_id": 53101,
        "KingdomID": 5,
        "ClassID": 10531,
        "Name": {
            "CH": "镀金旅团 · 沙中净水",
            "EN": "Eremite Desert Clearwater"
        },
        "Title": [
            {
                "CH": "沙下泉流般跃动的刀舞者",
                "EN": "Fluid Sword-Dancer"
            }
        ],
        "Desc": {
            "CH": "来自黄金一般的沙漠，是松散佣兵团体中的一员。为了报酬，愿意为任何人效力。\n迷失在砂砾作歌的旧梦之中的少女，将厮杀的趣味视作执行委托时真正有价值的报偿。传说诞在这一族中的女子常受沙漠中的游魂青睐，因而往往早夭。或许只有血管中澎湃着战斗的旋律时，她才能摆脱魂灵的低语吧。\n使用的武器中封印着残破的厄灵，会在危急时苏醒。只有战败者的哀嚎才能平息其忿怒——不论败者是武器的主人，抑或是敌人。",
            "EN": "A member of a loosely-organized mercenary corps from the golden desert sands. Will work for anyone as long as the pay is good.\nA young lady lost in the ancient dream of the desert songs. To her, the thrill of bloody battle is her true reward. Legend has it that the women of this tribe are often afflicted by the wandering spirits of the desert, a condition that leads many of them to an early end. Perhaps this one can only be free of these apparitions when her blood is up in combat.\nAn ominous, fragmented spirit is sealed within the weapon she uses, awakening in moments of peril. Once it does, it can only be silenced by the cries of the vanquished — whether that be its wielder, or its wielder's foe."
        },
        "Color": "Water",
        "HP": 1.8,
        "HPCurve": 2,
        "ATK": 3.2,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": -0.2
        },
        "RESState": [
            {
                "State": "Weak",
                "Water": -0.6
            }
        ],
        "Ball": {
            "Type": 2,
            "Point": 4,
            "Drop": "1112"
        },
        "Icon": [
            "emdc"
        ],
        "Grade": 1,
        "Weight": 70,
        "EndureTemplateID": 150,
        "SkillData": [
            "5/53101"
        ],
        "Drop": [
            {
                "Type": 24,
                "Amount": [
                    2.101,
                    0.5602,
                    0.14,
                    0
                ]
            }
        ],
        "Csx": [
            "ECOtherHumanFactions/镀金旅团·沙中净水"
        ]
    },
    "53102": {
        "_id": 53102,
        "KingdomID": 5,
        "ClassID": 10531,
        "Name": {
            "CH": "镀金旅团 · 炽阳凝冰",
            "EN": "Eremite Sunfrost"
        },
        "Title": [
            {
                "CH": "深暮结霜般清冷的佣兵",
                "EN": "Frosty Mercenary"
            }
        ],
        "Desc": {
            "CH": "来自黄金一般的沙漠，是松散佣兵团体中的一员。为了报酬，愿意为任何人效力。\n对职责与任务之外的一切事宜漠不关心的职业佣兵。将完成委托作为唯一的行动纲领，在佣兵市场上的要价也是最高的。\n使用的武器中封印着残破的厄灵，会在危急时苏醒。只有战败者的哀嚎才能平息其忿怒——不论败者是武器的主人，抑或是敌人。",
            "EN": "A member of a loosely-organized mercenary corps from the golden desert sands. Will work for anyone as long as the pay is good.\nA professional mercenary who cares about nothing outside of work and the mission at hand. To such people, their only game plan is to complete the mission, and their high asking prices on the market match that committment.\nAn ominous, fragmented spirit is sealed within the weapon they use, awakening in moments of peril. Once it does, it can only be silenced by the cries of the vanquished — whether that be its wielder, or its wielder's foe."
        },
        "Color": "Ice",
        "HP": 1.8,
        "HPCurve": 2,
        "ATK": 3.2,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": -0.2
        },
        "RESState": [
            {
                "State": "Weak",
                "Ice": -0.6
            }
        ],
        "Ball": {
            "Type": 4,
            "Point": 4,
            "Drop": "1112"
        },
        "Icon": [
            "emsf"
        ],
        "Grade": 1,
        "Weight": 80,
        "EndureTemplateID": 150,
        "SkillData": [
            "5/53102"
        ],
        "Drop": [
            {
                "Type": 24,
                "Amount": [
                    2.101,
                    0.5602,
                    0.14,
                    0
                ]
            }
        ],
        "Csx": [
            "ECOtherHumanFactions/镀金旅团·炽阳凝冰"
        ]
    },
    "53103": {
        "_id": 53103,
        "KingdomID": 5,
        "ClassID": 10531,
        "Name": {
            "CH": "镀金旅团 · 白日鸣雷",
            "EN": "Eremite Daythunder"
        },
        "Title": [
            {
                "CH": "疾雷馀声般迅猛的战士",
                "EN": "Thunderclap Warrior"
            }
        ],
        "Desc": {
            "CH": "来自黄金一般的沙漠，是松散佣兵团体中的一员。为了报酬，愿意为任何人效力。\n以战斗为天职，真正的武士部族的后裔。与轻松挥舞巨斧的蛮人形象不同，这一族的战士实际上对沙中废都的古文与诗歌颇有研究。\n使用的武器中封印着残破的厄灵，会在危急时苏醒。只有战败者的哀嚎才能平息其忿怒——不论败者是武器的主人，抑或是敌人。",
            "EN": "A member of a loosely-organized mercenary corps from the golden desert sands. Will work for anyone as long as the pay is good.\nA true scion of a warrior tribe, such warriors see battle as their ordained purpose, and unlike some barbarian who wields a large ax with ease, these tribespeople are also quite well-versed in the lore and songs of the desert ruins.\nAn ominous, fragmented spirit is sealed within the weapon they use, awakening in moments of peril. Once it does, it can only be silenced by the cries of the vanquished — whether that be its wielder, or its wielder's foe."
        },
        "Color": "Elec",
        "HP": 2.0,
        "HPCurve": 2,
        "ATK": 2.4,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": -0.2
        },
        "RESState": [
            {
                "State": "Weak",
                "Elec": -0.6
            }
        ],
        "Ball": {
            "Type": 3,
            "Point": 4,
            "Drop": "1112"
        },
        "Icon": [
            "emdt"
        ],
        "Grade": 1,
        "Weight": 80,
        "EndureTemplateID": 150,
        "SkillData": [
            "5/53103"
        ],
        "Drop": [
            {
                "Type": 24,
                "Amount": [
                    2.101,
                    0.5602,
                    0.14,
                    0
                ]
            }
        ],
        "Csx": [
            "ECOtherHumanFactions/镀金旅团·白日鸣雷"
        ]
    },
    "54101": {
        "_id": 54101,
        "KingdomID": 5,
        "ClassID": 10541,
        "Name": {
            "CH": "镀金旅团 · 魔岩役使",
            "EN": "Eremite Stone Enchanter"
        },
        "Title": [
            {
                "CH": "使役岩之魔蜥的战士",
                "EN": "Servant of the Stone Drake"
            }
        ],
        "Desc": {
            "CH": "从沙漠中走出的松散佣兵团体中的一员，为了报酬愿意为任何人效力。\n在古代，这些重装的武士乃是烈日君王最为亲信的宿卫。或许正是由于这个缘故，他们身上至今仍流溢着太阳般的金色光辉。\n使用的武器中封印着残破的厄灵，会在危急时苏醒。只有战败者的哀嚎才能平息他们的愤怒。——不论败者是武器的主人，抑或是敌人。",
            "EN": "A person hailing from the loosely-organized mercenary bands of the desert. Will work for anyone as long as the pay is good.\nIn ancient days, these heavily-equipped warriors were the domain of the personal, trusted guards of the Lord of the Blazing Sun. It is perhaps on this account that they gleam like the golden sun itself to this day.\nOminous, fragmented spirits are sealed within the weapons they use, awakening in moments of peril. Once these weapons awaken, they can only be silenced by the cries of the vanquished — whether that be their wielders or their wielders' foes."
        },
        "Color": "Rock",
        "HP": 2.5,
        "HPCurve": 2,
        "ATK": 4.0,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": -0.2
        },
        "RESState": [
            {
                "State": "Eremite_Improve",
                "All": 0.5
            },
            {
                "State": "Weak",
                "Rock": -0.6
            }
        ],
        "Ball": {
            "Type": 7,
            "Point": 4,
            "Drop": "1111"
        },
        "Icon": [
            "emse"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 210,
        "SkillData": [
            "5/54101"
        ],
        "Drop": [
            {
                "Type": 24,
                "Amount": [
                    2.9414,
                    0.7844,
                    0.196,
                    0
                ]
            }
        ],
        "Csx": [
            "ECOtherHumanFactions/镀金旅团·魔岩役使"
        ]
    },
    "54102": {
        "_id": 54102,
        "KingdomID": 5,
        "ClassID": 10541,
        "Name": {
            "CH": "镀金旅团 · 灵风猎手",
            "EN": "Eremite Galehunter"
        },
        "Title": [
            {
                "CH": "使役风之魔鸢的猎手",
                "EN": "Retainer of the Wind Glede"
            }
        ],
        "Desc": {
            "CH": "从沙漠中走出的松散佣兵团体中的一员，为了报酬愿意为任何人效力。\n由于机弩的广泛使用，在如今的镀金部族中唯有最老练的战士才会使用长弓作战。她们无需依靠双目，仅凭风沙流动的声音便能分辨出敌人的所在。\n使用的武器中封印着残破的厄灵，会在危急时苏醒。只有战败者的哀嚎才能平息他们的愤怒。——不论败者是武器的主人，抑或是敌人。",
            "EN": "A person hailing from the loosely-organized mercenary bands of the desert. Will work for anyone as long as the pay is good.\nIn this age where crossbows are in widespread use, only the most experienced of Eremite warriors know how to use a longbow in battle. They do not even need their eyes — merely their ears with which they sense the flow of wind and sand alike — to judge the location of their foes.\nOminous, fragmented spirits are sealed within the weapons they use, awakening in moments of peril. Once these weapons awaken, they can only be silenced by the cries of the vanquished — whether that be their wielders or their wielders' foes."
        },
        "Color": "Wind",
        "HP": 2.2,
        "HPCurve": 2,
        "ATK": 5.0,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": -0.2
        },
        "RESState": [
            {
                "State": "Eremite_Improve",
                "All": 0.5
            },
            {
                "State": "Weak",
                "Wind": -0.6
            }
        ],
        "Ball": {
            "Type": 6,
            "Point": 4,
            "Drop": "1111"
        },
        "Icon": [
            "emgh"
        ],
        "Grade": 1,
        "Weight": 80,
        "EndureTemplateID": 210,
        "SkillData": [
            "5/54102"
        ],
        "Drop": [
            {
                "Type": 24,
                "Amount": [
                    2.9414,
                    0.7844,
                    0.196,
                    0
                ]
            }
        ],
        "Csx": [
            "ECOtherHumanFactions/镀金旅团·灵风猎手"
        ]
    },
    "54103": {
        "_id": 54103,
        "KingdomID": 5,
        "ClassID": 10541,
        "Name": {
            "CH": "镀金旅团 · 炽沙叙事人",
            "EN": "Eremite Scorching Loremaster"
        },
        "Title": [
            {
                "CH": "编织烈焰与史诗的神谕",
                "EN": "Augury of the Foretold Flame"
            }
        ],
        "Desc": {
            "CH": "从沙漠中走出的松散佣兵团体中的一员。即使为了报酬愿意为任何人效力，也不会忘记沙海中的根源。\n在文明崩解之后，沙漠中的故事如同风中的火星、云中的雨一样，是经由口口相传的诗与秘史一代又一代继承下去的。在流传的过程中有许多故事变化了，又有许多故事失落了。\n「镀金旅团」是无知或是怀着蔑视的心不去区分沙海中诸多部族的外人对他们的统称。在被称为镀金旅团的人当中，有许多人已经忘记了沙漠的故事。如今仍然能记起许多故事的人，是不会背叛流淌在体内的沙漠血脉的。",
            "EN": "A person hailing from the loosely-organized mercenary bands of the desert. Even though they will work for anyone as long as the pay is good, they will never forget their roots in the Great Red Sand.\nAfter the collapse of its civilizations, the stories of the desert were passed down generation after generation in oral tradition just like the sparks in the wind and the rain in the clouds. As time passed, some stories were lost while others survived, albeit with significant changes.\n\"The Eremites\" is the umbrella term given to the tribes of the desert by those too ignorant or disdainful to learn the tribes' myriad differences. Within those referred to as the Eremites, many have also indeed already forgotten the tales of the desert. Fear not, however, for those who still remember will never betray the spirit of the sands yet flowing through their veins."
        },
        "Color": "Fire",
        "HP": 2.2,
        "HPCurve": 2,
        "ATK": 5.0,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": -0.2
        },
        "RESState": [
            {
                "State": "Eremite_Improve",
                "All": 0.5
            },
            {
                "State": "Weak",
                "Fire": -0.6
            }
        ],
        "Ball": {
            "Type": 1,
            "Point": 4,
            "Drop": "1111"
        },
        "Icon": [
            "emsl"
        ],
        "Grade": 1,
        "Weight": 80,
        "EndureTemplateID": 210,
        "SkillData": [
            "5/54103"
        ],
        "Drop": [
            {
                "Type": 24,
                "Amount": [
                    2.9414,
                    0.7844,
                    0.196,
                    0
                ]
            }
        ],
        "Csx": [
            "ECOtherHumanFactions/镀金旅团·炽沙叙事人"
        ]
    },
    "54104": {
        "_id": 54104,
        "KingdomID": 5,
        "ClassID": 10541,
        "Name": {
            "CH": "镀金旅团 · 叶轮舞者",
            "EN": "Eremite Floral Ring-Dancer"
        },
        "Title": [
            {
                "CH": "与灵蛇共舞的轮刃武者",
                "EN": "Ring-Bladed Snake-Dancer"
            }
        ],
        "Desc": {
            "CH": "从沙漠中走出的松散佣兵团体中的一员，为了报酬愿意为任何人效力。\n沙之民有音乐与舞蹈的传统，起初是对神的礼赞，后来则是讨取王者欢心的演艺与战斗的技术。大漠的传说中，便有曾将王者谋刺于床榻或高塔的舞姬。如今即使他们的神与王都尽数沉入流沙，舞蹈的记忆依然留存在民族的血脉中。\n使用的武器中封印着残破的厄灵，会在危急时苏醒。只有战败者的哀嚎才能平息他们的愤怒。——不论败者是武器的主人，抑或是敌人。",
            "EN": "A person hailing from the loosely-organized mercenary bands of the desert. They will work for anyone as long as the pay is good.\nThe children of the desert have a rich tradition of song and dance. Originally used to praise and serve the gods, the crafts were later adapted for battle and the seduction of kings. Many tales of the sands feature dancers who murdered kings relaxed in their beds or standing atop high towers. Even though all such gods and kings have now faded into the flowing sands, the memory of dance yet persists in the desert-dwellers' veins.\nOminous, fragmented spirits are sealed within the weapons they use, awakening in moments of peril. Once these weapons awaken, they can only be silenced by the cries of the vanquished — whether that be their wielders or their wielders' foes."
        },
        "Color": "Grass",
        "HP": 2.5,
        "HPCurve": 2,
        "ATK": 4.0,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": -0.2
        },
        "RESState": [
            {
                "State": "Eremite_Improve",
                "All": 0.5
            },
            {
                "State": "Weak",
                "Grass": -0.6
            }
        ],
        "Ball": {
            "Type": 5,
            "Point": 4,
            "Drop": "1111"
        },
        "Icon": [
            "emfd"
        ],
        "Grade": 1,
        "Weight": 80,
        "EndureTemplateID": 210,
        "SkillData": [
            "5/54104"
        ],
        "Drop": [
            {
                "Type": 24,
                "Amount": [
                    2.9414,
                    0.7844,
                    0.196,
                    0
                ]
            }
        ],
        "Csx": [
            "ECOtherHumanFactions/镀金旅团·叶轮舞者"
        ]
    },
    "60101": {
        "_id": 60101,
        "KingdomID": 6,
        "ClassID": 10601,
        "Name": {
            "CH": "冰霜骗骗花",
            "EN": "Cryo Whopperflower"
        },
        "Title": [
            {
                "CH": "伺机而动的拟态植物",
                "EN": "Opportunistic Mimetic Plant"
            }
        ],
        "Desc": {
            "CH": "神出鬼没的拟态植物魔物，会利用冰元素战斗。\n有时会隐藏在地表下，捕猎浑然不觉的猎物。如果有足够的年月、养分与环境，或许能扎根某处，长成巨大的急冻树…",
            "EN": "A mimetic plant monster that appears out of nowhere and uses Cryo in combat.\nThese plants lurk under the surface and ambush unsuspecting prey. With the passage of years, sufficient nutrition, and the right environment, it could even take root somewhere and grow into a giant Cryo Regisvine..."
        },
        "Color": "Ice",
        "HP": 3.0,
        "HPCurve": 1,
        "ATK": 2.0,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.35,
            "Fire": 0.35,
            "Water": 0.35,
            "Elec": 0.35,
            "Ice": 0.75,
            "Grass": 0.35,
            "Wind": 0.35,
            "Rock": 0.35
        },
        "RESState": [
            {
                "State": "Weak",
                "All": -0.25
            }
        ],
        "Ball": {
            "Type": 4,
            "Point": 4,
            "Drop": "1113"
        },
        "Icon": [
            "cwp"
        ],
        "Grade": 1,
        "Weight": 60,
        "EndureTemplateID": 50,
        "Drop": [
            {
                "Type": 3,
                "Amount": [
                    4.202,
                    1.1205,
                    0.28,
                    0
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/冰霜骗骗花"
        ]
    },
    "60102": {
        "_id": 60102,
        "KingdomID": 6,
        "ClassID": 10601,
        "Name": {
            "CH": "炽热骗骗花",
            "EN": "Pyro Whopperflower"
        },
        "Title": [
            {
                "CH": "伺机而动的拟态植物",
                "EN": "Opportunistic Mimetic Plant"
            }
        ],
        "Desc": {
            "CH": "古灵精怪的拟态植物魔物，会用火元素攻击猎物。\n提瓦特的世界中，存在着元素凭附的植物。因此获得了智性的它们，对「养分」的需求也大大提升了…",
            "EN": "A mimetic plant monster that attacks without warning and uses Pyro to hunt its prey.\nThe world of Teyvat plays host to plants infused with the elements. Having gained a measure of intellect, their \"nutritional needs\" have also greatly increased..."
        },
        "Color": "Fire",
        "HP": 3.0,
        "HPCurve": 1,
        "ATK": 3.0,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.35,
            "Fire": 0.75,
            "Water": 0.35,
            "Elec": 0.35,
            "Ice": 0.35,
            "Grass": 0.35,
            "Wind": 0.35,
            "Rock": 0.35
        },
        "RESState": [
            {
                "State": "Weak",
                "All": -0.25
            }
        ],
        "Ball": {
            "Type": 1,
            "Point": 4,
            "Drop": "1113"
        },
        "Icon": [
            "pwp"
        ],
        "Grade": 1,
        "Weight": 60,
        "EndureTemplateID": 50,
        "Drop": [
            {
                "Type": 3,
                "Amount": [
                    4.202,
                    1.1205,
                    0.28,
                    0
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/炽热骗骗花"
        ]
    },
    "60103": {
        "_id": 60103,
        "KingdomID": 6,
        "ClassID": 10601,
        "Name": {
            "CH": "电气骗骗花",
            "EN": "Electro Whopperflower"
        },
        "Title": [
            {
                "CH": "伺机而动的拟态植物",
                "EN": "Opportunistic Mimetic Plant"
            }
        ],
        "Desc": {
            "CH": "元气满满的拟态植物魔物，会用雷元素攻击猎物。\n在元素能量充满的时候，可以从花冠中释放出强大的电流，因此也有学者试图将其用于一些其他方面的用途…",
            "EN": "Energetic mimetic Plant monsters that use Electro to attack their prey.\nWhen fully-charged with Electro energy, they can unleash a strong electric current from their crowns. As such, some researchers have tried to apply them in other areas..."
        },
        "Color": "Elec",
        "HP": 3.0,
        "HPCurve": 1,
        "ATK": 2.0,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.35,
            "Fire": 0.35,
            "Water": 0.35,
            "Elec": 0.75,
            "Ice": 0.35,
            "Grass": 0.35,
            "Wind": 0.35,
            "Rock": 0.35
        },
        "RESState": [
            {
                "State": "Weak",
                "All": -0.25
            }
        ],
        "Ball": {
            "Type": 3,
            "Point": 4,
            "Drop": "1113"
        },
        "Icon": [
            "ewp"
        ],
        "Grade": 1,
        "Weight": 60,
        "EndureTemplateID": 50,
        "Drop": [
            {
                "Type": 3,
                "Amount": [
                    4.202,
                    1.1205,
                    0.28,
                    0
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/电气骗骗花"
        ]
    },
    "60201": {
        "_id": 60201,
        "KingdomID": 6,
        "ClassID": 10602,
        "Name": {
            "CH": "急冻树",
            "EN": "Cryo Regisvine"
        },
        "Title": [
            {
                "CH": "极寒的冰之花",
                "EN": "Evercold Iceflower"
            },
            {
                "CH": "吐息冰霜的巨藤",
                "EN": "Giant Frostbreath Calamus"
            },
            {
                "CH": "静静绽放的霜花",
                "EN": "Silent Frost Bloom"
            }
        ],
        "Desc": {
            "CH": "地脉中的寒霜附着在藤蔓上，由此形成的魔物。\n有研究认为，植物就像世界的器官一样，调和着元素奔涌的地脉；而这种生态在地表上的结晶就是冰雾花、骗骗花等，充满了元素能量的产物。\n而生长在特定环境的特定植物，经年累月或许就会成为急冻树这样的，如同魔兽一般庞大而凶猛的生命体吧…",
            "EN": "A monster formed from a vine that was imbued with the essence of biting frost within the ley lines.\nSome studies suggest that plants are like the organs of the world, harmonizing the turbulent elemental energies of the ley lines. Concrete examples of this phenomena are Mist Flowers, Whopperflowers, and the like, which brim over with elemental energy.\nIn certain circumstances, certain plants will turn into creatures of monstrous size and intent — such as the Cryo Regisvines — in the course of many years."
        },
        "Color": "Ice",
        "HP": 7.0,
        "HPCurve": 1,
        "ATK": 5.0,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 1.3,
            "Fire": 1.1,
            "Water": 1.1,
            "Elec": 1.1,
            "Ice": 1.7,
            "Grass": 1.1,
            "Wind": 1.1,
            "Rock": 1.1
        },
        "RESState": [
            {
                "State": "Weak",
                "All": -1.0
            }
        ],
        "Ball": {
            "Type": 4,
            "Point": 4,
            "Drop": "3333"
        },
        "Icon": [
            "cr"
        ],
        "Grade": 3,
        "Weight": 300,
        "EndureTemplateID": 4000,
        "Csx": [
            "ECMysticalBeasts/急冻树"
        ]
    },
    "60202": {
        "_id": 60202,
        "KingdomID": 6,
        "ClassID": 10602,
        "Name": {
            "CH": "爆炎树",
            "EN": "Pyro Regisvine"
        },
        "Title": [
            {
                "CH": "怒放的火之花",
                "EN": "Raging Fireflower"
            },
            {
                "CH": "恒常燃烧的巨藤",
                "EN": "Giant Everblaze Calamus"
            },
            {
                "CH": "摇曳的炽热火花",
                "EN": "Flickering Blaze Bloom"
            }
        ],
        "Desc": {
            "CH": "吸收了地脉中奔涌的古老烈焰的巨大藤蔓。\n如同充满怒火一般，永不止息的植物。\n诗人、歌手乃至一些学者认为「元素」中蕴含着情感和愿望。如果属实，又是什么情感与愿望驱使着爆炎树恒常燃烧，如同渴求着挣脱大地的束缚一般挣扎舞动呢…",
            "EN": "A giant vine that has absorbed the ancient flame that rages within the ley lines.\nIt is restless as if filled with endless fury.\nPoets, bards, and even some academics believe that elements also contain emotions and hopes. If this is true, then one can only wonder what emotions cause the Pyro Regisvine to burn eternally, writhing like one longing to be free of the confines of the earth..."
        },
        "Color": "Fire",
        "HP": 7.0,
        "HPCurve": 1,
        "ATK": 5.0,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 1.3,
            "Fire": 1.7,
            "Water": 1.1,
            "Elec": 1.1,
            "Ice": 1.1,
            "Grass": 1.1,
            "Wind": 1.1,
            "Rock": 1.1
        },
        "RESState": [
            {
                "State": "Weak",
                "All": -1.0
            }
        ],
        "Ball": {
            "Type": 1,
            "Point": 4,
            "Drop": "3333"
        },
        "Icon": [
            "pr"
        ],
        "Grade": 3,
        "Weight": 300,
        "EndureTemplateID": 4000,
        "Csx": [
            "ECMysticalBeasts/爆炎树"
        ]
    },
    "60203": {
        "_id": 60203,
        "KingdomID": 6,
        "ClassID": 10602,
        "Name": {
            "CH": "掣电树",
            "EN": "Electro Regisvine"
        },
        "Title": [
            {
                "CH": "震霆的雷之花",
                "EN": "Mournful Rumbleflower"
            },
            {
                "CH": "电荷喷薄的巨藤",
                "EN": "Giant Sparksurge Calamus"
            },
            {
                "CH": "雷殛的天电之花",
                "EN": "Thunderous Blast Bloom"
            }
        ],
        "Desc": {
            "CH": "汲取了地脉中传导的高压电流的巨大藤蔓形成的魔物。\n从树冠中源源不断释放出电力，如同发泄着大地的愤怒。\n曾经有妙论派的学者提出，如果围绕掣电树建立起一套复杂的电路矩阵，便能够将其中的电能输送到城市里使用。这一计划遭到了素论派学者的激烈反对，为此他们不惜牺牲了十头蕈猪以证明使用电力的危险性。",
            "EN": "A huge vine that has morphed into a monster through the conduction of high-voltage currents through the Ley Lines.\nElectricity surges constantly from its canopy like the unending rage of the earth itself.\nKshahrewar researchers once submitted that a complex circuit matrix might be set up around this creature to carry electrical energy to the city for use there. This plan was stridently opposed by scholars from Spantamad, who sacrificed 10 Shroomboars to drive home the point about the dangers of utilizing electricity."
        },
        "Color": "Elec",
        "HP": 7.0,
        "HPCurve": 1,
        "ATK": 5.0,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": 3.3,
            "Fire": 3.1,
            "Water": 3.1,
            "Elec": 3.7,
            "Ice": 3.1,
            "Grass": 3.1,
            "Wind": 3.1,
            "Rock": 3.1
        },
        "RESState": [
            {
                "State": "Weak",
                "All": -3.0
            }
        ],
        "Ball": {
            "Type": 3,
            "Point": 4,
            "Drop": "3333"
        },
        "Icon": [
            "er"
        ],
        "Grade": 3,
        "Weight": 300,
        "EndureTemplateID": 4000,
        "Csx": [
            "ECMysticalBeasts/掣电树"
        ]
    },
    "60301": {
        "_id": 60301,
        "KingdomID": 6,
        "ClassID": 10603,
        "Name": {
            "CH": "幼岩龙蜥",
            "EN": "Geovishap Hatchling"
        },
        "Title": [
            {
                "CH": "梦想成为巨龙的幼兽",
                "EN": "A Hatchling Dreaming of Dragonhood"
            }
        ],
        "Desc": {
            "CH": "有着坚硬岩质甲壳的小型龙蜥。\n行动非常灵活，是不容小觑的对手。须弥教令院的研究认为，即使是这种幼兽，也有成为「龙」的潜质。假以时日，如果不受天灾与敌害，或许也有一天能成为撼动高山的巨大魔物吧。",
            "EN": "A young vishap with a hard Geo exoskeleton.\nVishaps are exceptionally agile creatures that one would be wise not to underestimate in battle. Experts from Sumeru Akademiya believe that they become dragons when fully grown. By avoiding predators and natural disasters, perhaps they can even live long enough to become formidable beasts that reign over entire mountains."
        },
        "Color": "Rock",
        "HP": 3.0,
        "HPCurve": 1,
        "ATK": 4.32,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.3,
            "Rock": 0.5
        },
        "Ball": {
            "Type": 7,
            "Point": 3,
            "Drop": "113"
        },
        "Icon": [
            "gvh"
        ],
        "Grade": 2,
        "Weight": 150,
        "EndureTemplateID": 150,
        "Drop": [
            {
                "Type": 23,
                "Amount": [
                    0,
                    5.2526,
                    1.4006,
                    0.3502
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/幼岩龙蜥"
        ]
    },
    "60401": {
        "_id": 60401,
        "KingdomID": 6,
        "ClassID": 10603,
        "Name": {
            "CH": "(水)岩龙蜥",
            "EN": "Hydro Geovishap"
        },
        "Title": [
            {
                "CH": "坚若磐石的岩生猛兽",
                "EN": "Rock-Hard Geo Beast"
            }
        ],
        "Desc": {
            "CH": "成熟的岩之龙蜥。\n据称，幼岩龙蜥历经漫长岁月的积累后，会蜕去原本保护自己的装甲，成为「岩龙蜥」。但这两者，以及更古老、更伟大的「古岩龙蜥」，在过去的许多年中都蛰伏在山下，直到近期才开始活动。因此这种说法没有第一手观测记录证明。",
            "EN": "A mature Geovishap.\nIt is said that after many years have passed, Geovishap Hatchlings will shed the armor that originally protected them and become Geovishaps. However, these two and the older, greater Primo Geovishaps have spent many years hibernating under the mountains, and have only emerged and become active in recent times. As such, none can personally attest to having witnessed or recorded such a thing in person."
        },
        "UseCustomColorName": true,
        "HP": 8.0,
        "HPCurve": 1,
        "ATK": 6.0,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.3,
            "Rock": 0.5
        },
        "RESState": [
            {
                "State": "Drake_Absorb",
                "Water": 0.2
            }
        ],
        "Ball": {
            "Type": 7,
            "Point": 3,
            "Drop": "113"
        },
        "Icon": [
            "gv"
        ],
        "Grade": 2,
        "Weight": 150,
        "EndureTemplateID": 150,
        "SkillData": [
            "6/60401"
        ],
        "Drop": [
            {
                "Type": 23,
                "Amount": [
                    0,
                    5.2526,
                    1.4006,
                    0.3502
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/岩龙蜥"
        ]
    },
    "60402": {
        "_id": 60402,
        "KingdomID": 6,
        "ClassID": 10603,
        "Name": {
            "CH": "(火)岩龙蜥",
            "EN": "Pyro Geovishap"
        },
        "Title": [
            {
                "CH": "坚若磐石的岩生猛兽",
                "EN": "Rock-Hard Geo Beast"
            }
        ],
        "Desc": {
            "CH": "成熟的岩之龙蜥。\n据称，幼岩龙蜥历经漫长岁月的积累后，会蜕去原本保护自己的装甲，成为「岩龙蜥」。但这两者，以及更古老、更伟大的「古岩龙蜥」，在过去的许多年中都蛰伏在山下，直到近期才开始活动。因此这种说法没有第一手观测记录证明。",
            "EN": "A mature Geovishap.\nIt is said that after many years have passed, Geovishap Hatchlings will shed the armor that originally protected them and become Geovishaps. However, these two and the older, greater Primo Geovishaps have spent many years hibernating under the mountains, and have only emerged and become active in recent times. As such, none can personally attest to having witnessed or recorded such a thing in person."
        },
        "UseCustomColorName": true,
        "HP": 8.0,
        "HPCurve": 1,
        "ATK": 6.0,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.3,
            "Rock": 0.5
        },
        "RESState": [
            {
                "State": "Drake_Absorb",
                "Fire": 0.2
            }
        ],
        "Ball": {
            "Type": 7,
            "Point": 3,
            "Drop": "113"
        },
        "Icon": [
            "gv"
        ],
        "Grade": 2,
        "Weight": 150,
        "EndureTemplateID": 150,
        "SkillData": [
            "6/60401"
        ],
        "Drop": [
            {
                "Type": 23,
                "Amount": [
                    0,
                    5.2526,
                    1.4006,
                    0.3502
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/岩龙蜥"
        ]
    },
    "60403": {
        "_id": 60403,
        "KingdomID": 6,
        "ClassID": 10603,
        "Name": {
            "CH": "(冰)岩龙蜥",
            "EN": "Cryo Geovishap"
        },
        "Title": [
            {
                "CH": "坚若磐石的岩生猛兽",
                "EN": "Rock-Hard Geo Beast"
            }
        ],
        "Desc": {
            "CH": "成熟的岩之龙蜥。\n据称，幼岩龙蜥历经漫长岁月的积累后，会蜕去原本保护自己的装甲，成为「岩龙蜥」。但这两者，以及更古老、更伟大的「古岩龙蜥」，在过去的许多年中都蛰伏在山下，直到近期才开始活动。因此这种说法没有第一手观测记录证明。",
            "EN": "A mature Geovishap.\nIt is said that after many years have passed, Geovishap Hatchlings will shed the armor that originally protected them and become Geovishaps. However, these two and the older, greater Primo Geovishaps have spent many years hibernating under the mountains, and have only emerged and become active in recent times. As such, none can personally attest to having witnessed or recorded such a thing in person."
        },
        "UseCustomColorName": true,
        "HP": 8.0,
        "HPCurve": 1,
        "ATK": 6.0,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.3,
            "Rock": 0.5
        },
        "RESState": [
            {
                "State": "Drake_Absorb",
                "Ice": 0.2
            }
        ],
        "Ball": {
            "Type": 7,
            "Point": 3,
            "Drop": "113"
        },
        "Icon": [
            "gv"
        ],
        "Grade": 2,
        "Weight": 150,
        "EndureTemplateID": 150,
        "SkillData": [
            "6/60401"
        ],
        "Drop": [
            {
                "Type": 23,
                "Amount": [
                    0,
                    5.2526,
                    1.4006,
                    0.3502
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/岩龙蜥"
        ]
    },
    "60404": {
        "_id": 60404,
        "KingdomID": 6,
        "ClassID": 10603,
        "Name": {
            "CH": "(雷)岩龙蜥",
            "EN": "Electro Geovishap"
        },
        "Title": [
            {
                "CH": "坚若磐石的岩生猛兽",
                "EN": "Rock-Hard Geo Beast"
            }
        ],
        "Desc": {
            "CH": "成熟的岩之龙蜥。\n据称，幼岩龙蜥历经漫长岁月的积累后，会蜕去原本保护自己的装甲，成为「岩龙蜥」。但这两者，以及更古老、更伟大的「古岩龙蜥」，在过去的许多年中都蛰伏在山下，直到近期才开始活动。因此这种说法没有第一手观测记录证明。",
            "EN": "A mature Geovishap.\nIt is said that after many years have passed, Geovishap Hatchlings will shed the armor that originally protected them and become Geovishaps. However, these two and the older, greater Primo Geovishaps have spent many years hibernating under the mountains, and have only emerged and become active in recent times. As such, none can personally attest to having witnessed or recorded such a thing in person."
        },
        "UseCustomColorName": true,
        "HP": 8.0,
        "HPCurve": 1,
        "ATK": 6.0,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.3,
            "Rock": 0.5
        },
        "RESState": [
            {
                "State": "Drake_Absorb",
                "Elec": 0.2
            }
        ],
        "Ball": {
            "Type": 7,
            "Point": 3,
            "Drop": "113"
        },
        "Icon": [
            "gv"
        ],
        "Grade": 2,
        "Weight": 150,
        "EndureTemplateID": 150,
        "SkillData": [
            "6/60401"
        ],
        "Drop": [
            {
                "Type": 23,
                "Amount": [
                    0,
                    5.2526,
                    1.4006,
                    0.3502
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/岩龙蜥"
        ]
    },
    "60406": {
        "_id": 60406,
        "KingdomID": 10,
        "ClassID": 11044,
        "Name": {
            "CH": "「赤璋巡岳府君 · 移即」",
            "EN": "Ruler of the Chizhang Mountains: Yiji"
        },
        "Title": [
            {
                "CH": "「始起的熛火 · 移即」",
                "EN": "Local Legend: Pyro Geovishap"
            }
        ],
        "Desc": {
            "CH": "",
            "EN": ""
        },
        "Color": "Fire",
        "HP": 23.9,
        "HPCurve": 2,
        "ATK": 7.0,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": 0.3,
            "Rock": 0.5,
            "Fire": 0.7
        },
        "Ball": {
            "Type": 1,
            "Point": 4,
            "Drop": "1114"
        },
        "Icon": [
            "gv"
        ],
        "Grade": 3,
        "Weight": 250,
        "EndureTemplateID": 4000,
        "Tutorial": {
            "CH": "<b><color style='color:#FF0000;'>「赤璋巡岳府君 · 移即」</color></b> 和 <b><color style='color:#00B0F0;'>「赤璋巡岳府君 · 天虞」</color></b>是在璃月成对出现的一对岩龙蜥地方传奇。二者<b>免疫石化</b>，并且永久具有冰元素增幅和火元素增幅状态，具有70%火/冰元素抗性、50%岩元素抗性、30%物理抗性。<br>- 击杀其中一只岩龙蜥后，如果在15秒内未能击败第二只，先被击败的岩龙蜥会复活。<br>- 在战斗中，二者会协力释放强大的组合技。部分技能如果没有被护盾抵挡，则会进行追加攻击。",
            "EN": "<b><color style='color:#FF0000;'>\"The Rising Fire: Yiji\"</color></b> and <b><color style='color:#00B0F0;'>\"The Imminent Freeze: Tianyu\"</color></b> are a twin Local Legend of Liyue. They are <b>immune to Petrification</b>, and are permanently enhanced with Pyro and Cryo. They have 70% Pyro/Cryo RES, 50% Geo RES and 30% Physical RES.<br>- If one of the Geovishaps is killed but the second is not killed within 15s, the first Geovishap will revive.<br>- In combat, they will unleash powerful cooperated attacks. If certain attacks are not blocked by a shield, they will perform extra follow-up attacks."
        }
    },
    "60407": {
        "_id": 60407,
        "KingdomID": 10,
        "ClassID": 11044,
        "Name": {
            "CH": "「赤璋巡岳府君 · 天虞」",
            "EN": "Ruler of the Chizhang Mountains: Tianyu"
        },
        "Title": [
            {
                "CH": "「将近的荒寒 · 天虞」",
                "EN": "Local Legend: Cryo Geovishap"
            }
        ],
        "Desc": {
            "CH": "",
            "EN": ""
        },
        "Color": "Ice",
        "HP": 23.9,
        "HPCurve": 2,
        "ATK": 7.0,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": 0.3,
            "Rock": 0.5,
            "Ice": 0.7
        },
        "Ball": {
            "Type": 4,
            "Point": 4,
            "Drop": "1114"
        },
        "Icon": [
            "gv"
        ],
        "Grade": 3,
        "Weight": 250,
        "EndureTemplateID": 4000,
        "Tutorial": {
            "CH": "<b><color style='color:#FF0000;'>「赤璋巡岳府君 · 移即」</color></b> 和 <b><color style='color:#00B0F0;'>「赤璋巡岳府君 · 天虞」</color></b>是在璃月成对出现的一对岩龙蜥地方传奇。二者<b>免疫石化</b>，并且永久具有冰元素增幅和火元素增幅状态，具有70%火/冰元素抗性、50%岩元素抗性、30%物理抗性。<br>- 击杀其中一只岩龙蜥后，如果在15秒内未能击败第二只，先被击败的岩龙蜥会复活。<br>- 在战斗中，二者会协力释放强大的组合技。部分技能如果没有被护盾抵挡，则会进行追加攻击。",
            "EN": "<b><color style='color:#FF0000;'>\"The Rising Fire: Yiji\"</color></b> and <b><color style='color:#00B0F0;'>\"The Imminent Freeze: Tianyu\"</color></b> are a twin Local Legend of Liyue. They are <b>immune to Petrification</b>, and are permanently enhanced with Pyro and Cryo. They have 70% Pyro/Cryo RES, 50% Geo RES and 30% Physical RES.<br>- If one of the Geovishaps is killed but the second is not killed within 15s, the first Geovishap will revive.<br>- In combat, they will unleash powerful cooperated attacks. If certain attacks are not blocked by a shield, they will perform extra follow-up attacks."
        }
    },
    "60410": {
        "_id": 60410,
        "KingdomID": 10,
        "ClassID": 11006,
        "Name": {
            "CH": "「赤璋巡岳府君 · 移即」",
            "EN": "Ruler of the Chizhang Mountains: Yiji"
        },
        "Title": [
            {
                "CH": "地方传奇 · 岩龙蜥",
                "EN": "Local Legend: Pyro Geovishap"
            }
        ],
        "Desc": {
            "CH": "",
            "EN": ""
        },
        "Color": "Fire",
        "HP": 7.17,
        "HPCurve": 2,
        "ATK": 7.0,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": 0.3,
            "Rock": 0.5,
            "Fire": 0.7
        },
        "Ball": {
            "Type": 1,
            "Point": 4,
            "Drop": "1114"
        },
        "Icon": [
            "gv"
        ],
        "Grade": 3,
        "Weight": 250,
        "EndureTemplateID": 4000
    },
    "60411": {
        "_id": 60411,
        "KingdomID": 10,
        "ClassID": 11006,
        "Name": {
            "CH": "「赤璋巡岳府君 · 天虞」",
            "EN": "Ruler of the Chizhang Mountains: Tianyu"
        },
        "Title": [
            {
                "CH": "地方传奇 · 岩龙蜥",
                "EN": "Local Legend: Cryo Geovishap"
            }
        ],
        "Desc": {
            "CH": "",
            "EN": ""
        },
        "Color": "Ice",
        "HP": 7.17,
        "HPCurve": 2,
        "ATK": 7.0,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": 0.3,
            "Rock": 0.5,
            "Ice": 0.7
        },
        "Ball": {
            "Type": 4,
            "Point": 4,
            "Drop": "1114"
        },
        "Icon": [
            "gv"
        ],
        "Grade": 3,
        "Weight": 250,
        "EndureTemplateID": 4000
    },
    "60501": {
        "_id": 60501,
        "KingdomID": 6,
        "ClassID": 10603,
        "Name": {
            "CH": "(水)古岩龙蜥",
            "EN": "Hydro Primo Geovishap"
        },
        "Title": [
            {
                "CH": "曾与「龙王」摇撼山岳的岩龙",
                "EN": "Geo Dragon Who Once Raged With the King"
            },
            {
                "CH": "自漫长蛰眠中苏醒的撼地之龙",
                "EN": "Long-Slumbering Earthshaker Dragon"
            }
        ],
        "Desc": {
            "CH": "经历了长久岁月，拥有伟大力量的巨大岩龙蜥，能够适应元素环境的异变。\n根据民话，导致天遒谷毁灭的「龙灾」发生后，岩龙蜥与古岩龙蜥的王者被禁锢在大地深处，而它们则在地壳之下休眠，等待着复起的时机…",
            "EN": "After many years, the awesomely powerful Primo Geovishaps grow accustomed to changes in their elemental environment.\nFolktales hold that after the great \"draconic calamity\" that led to the ruination of Tianqiu Valley, the overlord of the Geovishaps and Primo Geovishaps was imprisoned deep beneath the earth, and so too did they burrow into deep and unseen places, awaiting their chance to rise once more..."
        },
        "UseCustomColorName": true,
        "HP": 30.0,
        "HPCurve": 1,
        "ATK": 8.0,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.3,
            "Water": 0.3,
            "Rock": 0.5
        },
        "RESState": [
            {
                "State": "Alert",
                "All": 2.0
            },
            {
                "State": "Drake_Backfire",
                "All": -0.5
            }
        ],
        "Ball": {
            "Type": 7,
            "Point": 3,
            "Drop": "113"
        },
        "Icon": [
            "pg"
        ],
        "Grade": 3,
        "Weight": 300,
        "EndureTemplateID": 4000,
        "SkillData": [
            "6/60501"
        ],
        "Csx": [
            "ECMysticalBeasts/古岩龙蜥"
        ]
    },
    "60502": {
        "_id": 60502,
        "KingdomID": 6,
        "ClassID": 10603,
        "Name": {
            "CH": "(火)古岩龙蜥",
            "EN": "Pyro Primo Geovishap"
        },
        "Title": [
            {
                "CH": "曾与「龙王」摇撼山岳的岩龙",
                "EN": "Geo Dragon Who Once Raged With the King"
            },
            {
                "CH": "自漫长蛰眠中苏醒的撼地之龙",
                "EN": "Long-Slumbering Earthshaker Dragon"
            }
        ],
        "Desc": {
            "CH": "经历了长久岁月，拥有伟大力量的巨大岩龙蜥，能够适应元素环境的异变。\n根据民话，导致天遒谷毁灭的「龙灾」发生后，岩龙蜥与古岩龙蜥的王者被禁锢在大地深处，而它们则在地壳之下休眠，等待着复起的时机…",
            "EN": "After many years, the awesomely powerful Primo Geovishaps grow accustomed to changes in their elemental environment.\nFolktales hold that after the great \"draconic calamity\" that led to the ruination of Tianqiu Valley, the overlord of the Geovishaps and Primo Geovishaps was imprisoned deep beneath the earth, and so too did they burrow into deep and unseen places, awaiting their chance to rise once more..."
        },
        "UseCustomColorName": true,
        "HP": 30.0,
        "HPCurve": 1,
        "ATK": 8.0,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.3,
            "Fire": 0.3,
            "Rock": 0.5
        },
        "RESState": [
            {
                "State": "Alert",
                "All": 2.0
            },
            {
                "State": "Drake_Backfire",
                "All": -0.5
            }
        ],
        "Ball": {
            "Type": 7,
            "Point": 3,
            "Drop": "113"
        },
        "Icon": [
            "pg"
        ],
        "Grade": 3,
        "Weight": 300,
        "EndureTemplateID": 4000,
        "SkillData": [
            "6/60501"
        ],
        "Csx": [
            "ECMysticalBeasts/古岩龙蜥"
        ]
    },
    "60503": {
        "_id": 60503,
        "KingdomID": 6,
        "ClassID": 10603,
        "Name": {
            "CH": "(冰)古岩龙蜥",
            "EN": "Cryo Primo Geovishap"
        },
        "Title": [
            {
                "CH": "曾与「龙王」摇撼山岳的岩龙",
                "EN": "Geo Dragon Who Once Raged With the King"
            },
            {
                "CH": "自漫长蛰眠中苏醒的撼地之龙",
                "EN": "Long-Slumbering Earthshaker Dragon"
            }
        ],
        "Desc": {
            "CH": "经历了长久岁月，拥有伟大力量的巨大岩龙蜥，能够适应元素环境的异变。\n根据民话，导致天遒谷毁灭的「龙灾」发生后，岩龙蜥与古岩龙蜥的王者被禁锢在大地深处，而它们则在地壳之下休眠，等待着复起的时机…",
            "EN": "After many years, the awesomely powerful Primo Geovishaps grow accustomed to changes in their elemental environment.\nFolktales hold that after the great \"draconic calamity\" that led to the ruination of Tianqiu Valley, the overlord of the Geovishaps and Primo Geovishaps was imprisoned deep beneath the earth, and so too did they burrow into deep and unseen places, awaiting their chance to rise once more..."
        },
        "UseCustomColorName": true,
        "HP": 30.0,
        "HPCurve": 1,
        "ATK": 8.0,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.3,
            "Ice": 0.3,
            "Rock": 0.5
        },
        "RESState": [
            {
                "State": "Alert",
                "All": 2.0
            },
            {
                "State": "Drake_Backfire",
                "All": -0.5
            }
        ],
        "Ball": {
            "Type": 7,
            "Point": 3,
            "Drop": "113"
        },
        "Icon": [
            "pg"
        ],
        "Grade": 3,
        "Weight": 300,
        "EndureTemplateID": 4000,
        "SkillData": [
            "6/60501"
        ],
        "Csx": [
            "ECMysticalBeasts/古岩龙蜥"
        ]
    },
    "60504": {
        "_id": 60504,
        "KingdomID": 6,
        "ClassID": 10603,
        "Name": {
            "CH": "(雷)古岩龙蜥",
            "EN": "Electro Primo Geovishap"
        },
        "Title": [
            {
                "CH": "曾与「龙王」摇撼山岳的岩龙",
                "EN": "Geo Dragon Who Once Raged With the King"
            },
            {
                "CH": "自漫长蛰眠中苏醒的撼地之龙",
                "EN": "Long-Slumbering Earthshaker Dragon"
            }
        ],
        "Desc": {
            "CH": "经历了长久岁月，拥有伟大力量的巨大岩龙蜥，能够适应元素环境的异变。\n根据民话，导致天遒谷毁灭的「龙灾」发生后，岩龙蜥与古岩龙蜥的王者被禁锢在大地深处，而它们则在地壳之下休眠，等待着复起的时机…",
            "EN": "After many years, the awesomely powerful Primo Geovishaps grow accustomed to changes in their elemental environment.\nFolktales hold that after the great \"draconic calamity\" that led to the ruination of Tianqiu Valley, the overlord of the Geovishaps and Primo Geovishaps was imprisoned deep beneath the earth, and so too did they burrow into deep and unseen places, awaiting their chance to rise once more..."
        },
        "UseCustomColorName": true,
        "HP": 30.0,
        "HPCurve": 1,
        "ATK": 8.0,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.3,
            "Elec": 0.3,
            "Rock": 0.5
        },
        "RESState": [
            {
                "State": "Alert",
                "All": 2.0
            },
            {
                "State": "Drake_Backfire",
                "All": -0.5
            }
        ],
        "Ball": {
            "Type": 7,
            "Point": 3,
            "Drop": "113"
        },
        "Icon": [
            "pg"
        ],
        "Grade": 3,
        "Weight": 300,
        "EndureTemplateID": 4000,
        "SkillData": [
            "6/60501"
        ],
        "Csx": [
            "ECMysticalBeasts/古岩龙蜥"
        ]
    },
    "60506": {
        "_id": 60506,
        "KingdomID": 6,
        "ClassID": 10605,
        "Name": {
            "CH": "深海龙蜥 · 原种",
            "EN": "Primordial Bathysmal Vishap"
        },
        "Title": [
            {
                "CH": "盘踞深海的水栖猛兽",
                "EN": "Deep Sea-Dwelling Beast"
            }
        ],
        "Desc": {
            "CH": "生活在海渊之下的龙蜥。\n它们曾经是海渊之下的支配种族——其实在更加久远的过去，龙们就是这个世界的支配者。曾经的七王被高天之力打败，相继凋零了。\n如今似乎有龙蜥出现在了远离海渊的地方。或许它们无法融入人所主宰的世界，但这仍然是它们的世界。",
            "EN": "A vishap that dwells deep beneath the abyssal depths.\nThey were once the dominant race in the depths — and indeed, dragons were the overlords of the whole world in an even more ancient age. But their former Seven Sovereigns were defeated by a power from the heavens, withering one by one.\nRecently, vishaps have appeared away from the depths. While they may have trouble assimilating in a world ruled by humans, this is still their world."
        },
        "Color": "Water",
        "HP": 4.0,
        "HPCurve": 2,
        "ATK": 4.0,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.3,
            "Water": 0.3
        },
        "Ball": {
            "Type": 2,
            "Point": 4,
            "Drop": "1112"
        },
        "Icon": [
            "pbv"
        ],
        "Grade": 2,
        "Weight": 150,
        "EndureTemplateID": 150,
        "Drop": [
            {
                "Type": 23,
                "Amount": [
                    0,
                    5.2526,
                    1.4006,
                    0.3502
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/深海龙蜥·原种"
        ]
    },
    "60507": {
        "_id": 60507,
        "KingdomID": 6,
        "ClassID": 10605,
        "Name": {
            "CH": "深海龙蜥 · 啮冰",
            "EN": "Rimebiter Bathysmal Vishap"
        },
        "Title": [
            {
                "CH": "珊瑚的守护者",
                "EN": "Coral Defenders"
            }
        ],
        "Desc": {
            "CH": "生活在海渊之下的龙蜥。因为社会分工，而出现了冰元素的形态。\n人类的到来引发了对抗，深海龙蜥开始了进化。正如蚂蚁会因性别和分工不同，而产生不同形态。这些龙蜥比起普通深海龙蜥更加强大。",
            "EN": "A vishap that dwells deep beneath the oceans. Due to certain social adaptations, it has manifested a form aligned with the Cryo element.\nThe coming of humans sparked a conflict and a resultant evolution within the Bathysmal Vishaps, just as ants might divide up labor and biological characteristics, thus appearing in different forms. These vishaps are stronger than their ordinary kin."
        },
        "Color": "Ice",
        "HP": 6.0,
        "HPCurve": 2,
        "ATK": 5.0,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.3,
            "Ice": 0.3
        },
        "Ball": {
            "Type": 4,
            "Point": 4,
            "Drop": "1113"
        },
        "Icon": [
            "cd"
        ],
        "Grade": 3,
        "Weight": 250,
        "EndureTemplateID": 4000,
        "Csx": [
            "ECMysticalBeasts/深海龙蜥·啮冰",
            "ECMysticalBeasts/深海龙蜥之群"
        ]
    },
    "60508": {
        "_id": 60508,
        "KingdomID": 6,
        "ClassID": 10605,
        "Name": {
            "CH": "深海龙蜥 · 吞雷",
            "EN": "Bolteater Bathysmal Vishap"
        },
        "Title": [
            {
                "CH": "珊瑚的守护者",
                "EN": "Coral Defenders"
            }
        ],
        "Desc": {
            "CH": "生活在海渊之下的龙蜥。因为社会分工，而出现了雷元素的形态。\n古七王凋零之后，新一代的七王正在陆续诞生。只是深海龙蜥如此进化，失去了纯粹。水之龙已经不会在它们族群中诞生了。根据预言，新的水之龙一定会以人形降临…\n如今回归完全形态的水之龙虽仍把它们视作同族，但是却与另一种族走得更近。或许是因为选择了与人携手并进，「眷属」自然是能与人相容为好；又或许是因为看到了她们的潜力。",
            "EN": "A vishap that dwells deep beneath the abyssal depths. Due to the social division of labor, it has manifested a form aligned with the Electro element.\nFollowing the demise of the Ancient Seven Sovereigns, a new generation of Sovereigns is presently being born. But now that the Bathysmal Vishaps have evolved in this manner, they have lost their purity. As such, the Dragon of Water will no longer be born from among their ranks. Prophecy holds that the new Dragon of Water will descend in the form of a human...\nThough the Dragons of Water that have returned to their complete form still see them as one of their own, they are closer to another race. Perhaps they figured that it is better to be closer to humans, since they chose to work with them, or perhaps they see their potential..."
        },
        "Color": "Elec",
        "HP": 6.0,
        "HPCurve": 2,
        "ATK": 5.0,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.3,
            "Elec": 0.3
        },
        "Ball": {
            "Type": 3,
            "Point": 4,
            "Drop": "1113"
        },
        "Icon": [
            "cd"
        ],
        "Grade": 3,
        "Weight": 250,
        "EndureTemplateID": 4000,
        "Csx": [
            "ECMysticalBeasts/深海龙蜥·吞雷",
            "ECMysticalBeasts/深海龙蜥之群"
        ]
    },
    "60509": {
        "_id": 60509,
        "KingdomID": 6,
        "ClassID": 10605,
        "Name": {
            "CH": "深海龙蜥幼体 · 原种",
            "EN": "Primordial Bathysmal Vishap Hatchling"
        },
        "Title": [
            {
                "CH": "盘踞深海的水栖猛兽",
                "EN": "Deep Sea-Dwelling Beast"
            }
        ],
        "Desc": {
            "CH": "生活在海渊之下的龙蜥。\n它们曾经是海渊之下的支配种族——其实在更加久远的过去，龙们就是这个世界的支配者。曾经的七王被高天之力打败，相继凋零了。\n如今似乎有龙蜥出现在了远离海渊的地方。或许它们无法融入人所主宰的世界，但这仍然是它们的世界。",
            "EN": "A vishap that dwells deep beneath the abyssal depths.\nThey were once the dominant race in the depths — and indeed, dragons were the overlords of the whole world in an even more ancient age. But their former Seven Sovereigns were defeated by a power from the heavens, withering one by one.\nRecently, vishaps have appeared away from the depths. While they may have trouble assimilating in a world ruled by humans, this is still their world."
        },
        "Color": "Water",
        "HP": 2.0,
        "HPCurve": 2,
        "ATK": 2.0,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.3,
            "Water": 0.2
        },
        "Ball": {
            "Type": 2,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "pbv"
        ],
        "Grade": 2,
        "Weight": 150,
        "EndureTemplateID": 150,
        "Drop": [
            {
                "Type": 23,
                "Amount": [
                    0,
                    5.2526,
                    1.4006,
                    0.3502
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/深海龙蜥·原种"
        ]
    },
    "60510": {
        "_id": 60510,
        "KingdomID": 6,
        "ClassID": 10605,
        "Name": {
            "CH": "深海龙蜥幼体 · 啮冰",
            "EN": "Rimebiter Bathysmal Vishap Hatchling"
        },
        "Title": [
            {
                "CH": "盘踞深海的水栖猛兽",
                "EN": "Deep Sea-Dwelling Beast"
            }
        ],
        "Desc": {
            "CH": "生活在海渊之下的龙蜥。因为社会分工，而出现了冰元素的形态。\n人类的到来引发了对抗，深海龙蜥开始了进化。正如蚂蚁会因性别和分工不同，而产生不同形态。这些龙蜥比起普通深海龙蜥更加强大。",
            "EN": "A vishap that dwells deep beneath the oceans. Due to certain social adaptations, it has manifested a form aligned with the Cryo element.\nThe coming of humans sparked a conflict and a resultant evolution within the Bathysmal Vishaps, just as ants might divide up labor and biological characteristics, thus appearing in different forms. These vishaps are stronger than their ordinary kin."
        },
        "Color": "Ice",
        "HP": 2.0,
        "HPCurve": 2,
        "ATK": 2.0,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.3,
            "Ice": 0.2
        },
        "Ball": {
            "Type": 4,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "rbv"
        ],
        "Grade": 2,
        "Weight": 150,
        "EndureTemplateID": 150,
        "Drop": [
            {
                "Type": 23,
                "Amount": [
                    0,
                    5.2526,
                    1.4006,
                    0.3502
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/深海龙蜥·啮冰"
        ]
    },
    "60511": {
        "_id": 60511,
        "KingdomID": 6,
        "ClassID": 10605,
        "Name": {
            "CH": "深海龙蜥幼体 · 吞雷",
            "EN": "Bolteater Bathysmal Vishap Hatchling"
        },
        "Title": [
            {
                "CH": "盘踞深海的水栖猛兽",
                "EN": "Deep Sea-Dwelling Beast"
            }
        ],
        "Desc": {
            "CH": "生活在海渊之下的龙蜥。因为社会分工，而出现了雷元素的形态。\n古七王凋零之后，新一代的七王正在陆续诞生。只是深海龙蜥如此进化，失去了纯粹。水之龙已经不会在它们族群中诞生了。根据预言，新的水之龙一定会以人形降临…\n如今回归完全形态的水之龙虽仍把它们视作同族，但是却与另一种族走得更近。或许是因为选择了与人携手并进，「眷属」自然是能与人相容为好；又或许是因为看到了她们的潜力。",
            "EN": "A vishap that dwells deep beneath the abyssal depths. Due to the social division of labor, it has manifested a form aligned with the Electro element.\nFollowing the demise of the Ancient Seven Sovereigns, a new generation of Sovereigns is presently being born. But now that the Bathysmal Vishaps have evolved in this manner, they have lost their purity. As such, the Dragon of Water will no longer be born from among their ranks. Prophecy holds that the new Dragon of Water will descend in the form of a human...\nThough the Dragons of Water that have returned to their complete form still see them as one of their own, they are closer to another race. Perhaps they figured that it is better to be closer to humans, since they chose to work with them, or perhaps they see their potential..."
        },
        "Color": "Elec",
        "HP": 2.0,
        "HPCurve": 2,
        "ATK": 2.0,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.3,
            "Elec": 0.2
        },
        "Ball": {
            "Type": 3,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "bbv"
        ],
        "Grade": 2,
        "Weight": 150,
        "EndureTemplateID": 150,
        "Drop": [
            {
                "Type": 23,
                "Amount": [
                    0,
                    5.2526,
                    1.4006,
                    0.3502
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/深海龙蜥·吞雷"
        ]
    },
    "60601": {
        "_id": 60601,
        "KingdomID": 6,
        "ClassID": 10606,
        "Name": {
            "CH": "雷萤",
            "EN": "Electro Cicin"
        },
        "Title": [
            {
                "CH": "酥酥麻麻",
                "EN": "Zap-and-Tingle"
            }
        ],
        "Desc": {
            "CH": "能够稍微导引雷元素的小型生物。\n与其他的萤族生物通称「飞萤」。尽管力量十分弱小，但在某些环境下也能造成可观的伤害。",
            "EN": "Little creatures that can ever so slightly manipulate Electro.\nThey and other such creatures are known collectively as Cicins, and though they are individually weak, they can cause significant damage under the right circumstances."
        },
        "Color": "Elec",
        "HP": 0.5,
        "HPCurve": 1,
        "ATK": 0.8,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": -0.5,
            "Elec": 0.5
        },
        "Ball": {
            "Type": 3,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "ecic"
        ],
        "Grade": 1,
        "Weight": 60,
        "EndureTemplateID": 20,
        "Csx": [
            "ECMysticalBeasts/雷萤"
        ]
    },
    "60602": {
        "_id": 60602,
        "KingdomID": 6,
        "ClassID": 10606,
        "Name": {
            "CH": "水萤",
            "EN": "Hydro Cicin"
        },
        "Title": [
            {
                "CH": "湿湿滑滑",
                "EN": "Slip-and-Slide"
            }
        ],
        "Desc": {
            "CH": "能够稍微导引水元素的小型生物。\n与其他的萤族生物通称「飞萤」。飞萤对「雾虚草」这种罕见而奇妙的植物情有独钟，而愚人众的术士据此开发了利用飞萤进行战斗的方法。",
            "EN": "Little creatures that can ever so slightly manipulate Hydro.\nThey and other such creatures are known collectively as Cicins, and they truly adore the rare and wonderful Mist Grass plant. It is by exploiting this relationship that the Fatui Mages have sought to commandeer Cicins in battle."
        },
        "Color": "Water",
        "HP": 0.5,
        "HPCurve": 1,
        "ATK": 0.8,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": -0.5,
            "Water": 0.5
        },
        "Ball": {
            "Type": 2,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "hcic"
        ],
        "Grade": 1,
        "Weight": 60,
        "EndureTemplateID": 20,
        "Csx": [
            "ECMysticalBeasts/水萤"
        ]
    },
    "60603": {
        "_id": 60603,
        "KingdomID": 6,
        "ClassID": 10606,
        "Name": {
            "CH": "冰萤",
            "EN": "Cryo Cicin"
        },
        "Title": [
            {
                "CH": "冰冰凉凉",
                "EN": "Chilly-and-Cold"
            }
        ],
        "Desc": {
            "CH": "能够稍微导引冰元素的小型生物。\n与其他的萤族生物通称「飞萤」。尽管力量非常弱小，但也因为力量弱小，所以进化出了奇特的移动方式，以避开掠食者的攻击。",
            "EN": "Little creatures that can ever so slightly manipulate Cryo.\nThey and other such creatures are known collectively as Cicins. They are individually very weak, but it is precisely because they are weak that they have evolved unique methods of movement that allow them to avoid predators."
        },
        "Color": "Ice",
        "HP": 0.5,
        "HPCurve": 1,
        "ATK": 0.8,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": -0.5,
            "Ice": 0.5
        },
        "Ball": {
            "Type": 4,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "ccic"
        ],
        "Grade": 1,
        "Weight": 60,
        "EndureTemplateID": 20,
        "Csx": [
            "ECMysticalBeasts/冰萤"
        ]
    },
    "60901": {
        "_id": 60901,
        "KingdomID": 6,
        "ClassID": 10609,
        "Name": {
            "CH": "浮游水蕈兽",
            "EN": "Floating Hydro Fungus"
        },
        "Title": [
            {
                "CH": "孢子飘浮，所以蕈兽也飘浮？",
                "EN": "Spores Float, So Do Fungi Float Too?"
            }
        ],
        "Desc": {
            "CH": "拥有一定智能的孢子生物。具有强大的环境适应能力。\n据说很难吃。但这种蕈兽因为含有丰富的水分，所以也能成为应急水源…",
            "EN": "A spore organism of some intelligence. It possesses extreme adaptability.\nIt is said that it tastes awful. However, such fungi have lots of moisture within them, and so can be used as an emergency water source..."
        },
        "Color": "Water",
        "HP": 0.5,
        "HPCurve": 2,
        "ATK": 1.8,
        "ATKCurve": 4,
        "RESBase": {
            "Water": 0.2,
            "Grass": 0.2
        },
        "Ball": {
            "Type": 0,
            "Point": 1,
            "Drop": "1"
        },
        "Icon": [
            "fhf"
        ],
        "Grade": 1,
        "Weight": 75,
        "EndureTemplateID": 20,
        "Drop": [
            {
                "Type": 11,
                "Amount": [
                    0.8404,
                    0.2241,
                    0.056,
                    0
                ]
            },
            {
                "Type": 12,
                "Amount": [
                    0.2101,
                    0.056,
                    0.014,
                    0
                ]
            },
            {
                "Type": 13,
                "Amount": [
                    0,
                    1.0505,
                    0.2801,
                    0.07
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/浮游水蕈兽"
        ]
    },
    "60902": {
        "_id": 60902,
        "KingdomID": 6,
        "ClassID": 10609,
        "Name": {
            "CH": "浮游草蕈兽",
            "EN": "Floating Dendro Fungus"
        },
        "Title": [
            {
                "CH": "孢子飘浮，所以蕈兽也飘浮？",
                "EN": "Spores Float, So Do Fungi Float Too?"
            }
        ],
        "Desc": {
            "CH": "拥有一定智能的孢子生物。具有强大的环境适应能力。\n曾经有学者撰文宣称它们是草飘浮灵的一种。在受到批驳后，他便改口说这是一种诗性的「修辞」。",
            "EN": "A spore organism of some intelligence. It possesses extreme adaptability.\nA researcher once said that these are some species of Dendro Specter. After this view was criticized, he changed his tack, claiming that this was just a literary turn of phrase."
        },
        "Color": "Grass",
        "HP": 0.5,
        "HPCurve": 2,
        "ATK": 1.8,
        "ATKCurve": 4,
        "RESBase": {
            "Grass": 0.25
        },
        "Ball": {
            "Type": 0,
            "Point": 1,
            "Drop": "1"
        },
        "Icon": [
            "fdf"
        ],
        "Grade": 1,
        "Weight": 75,
        "EndureTemplateID": 20,
        "Drop": [
            {
                "Type": 11,
                "Amount": [
                    0.8404,
                    0.2241,
                    0.056,
                    0
                ]
            },
            {
                "Type": 12,
                "Amount": [
                    0.2101,
                    0.056,
                    0.014,
                    0
                ]
            },
            {
                "Type": 13,
                "Amount": [
                    0,
                    1.0505,
                    0.2801,
                    0.07
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/浮游草蕈兽"
        ]
    },
    "60903": {
        "_id": 60903,
        "KingdomID": 6,
        "ClassID": 10609,
        "Name": {
            "CH": "浮游风蕈兽",
            "EN": "Floating Anemo Fungus"
        },
        "Title": [
            {
                "CH": "孢子飘浮，所以蕈兽也飘浮？",
                "EN": "Spores Float, So Do Fungi Float Too?"
            }
        ],
        "Desc": {
            "CH": "拥有一定智能的孢子生物。具有强大的环境适应能力。\n虽然有着不小的菌帽，但这些伞状组织的结构其实是中空的，因此难以作为食材使用。",
            "EN": "A spore organism of some intelligence. It possesses extreme adaptability.\nThis mushroom may have a sizable cap, but the umbrella-shaped structure is in fact hollow, and thus does not make for a particularly good meal."
        },
        "Color": "Wind",
        "HP": 0.5,
        "HPCurve": 2,
        "ATK": 1.8,
        "ATKCurve": 4,
        "RESBase": {
            "Grass": 0.2,
            "Wind": 0.2
        },
        "Ball": {
            "Type": 0,
            "Point": 1,
            "Drop": "1"
        },
        "Icon": [
            "faf"
        ],
        "Grade": 1,
        "Weight": 75,
        "EndureTemplateID": 20,
        "Drop": [
            {
                "Type": 11,
                "Amount": [
                    0.8404,
                    0.2241,
                    0.056,
                    0
                ]
            },
            {
                "Type": 12,
                "Amount": [
                    0.2101,
                    0.056,
                    0.014,
                    0
                ]
            },
            {
                "Type": 13,
                "Amount": [
                    0,
                    1.0505,
                    0.2801,
                    0.07
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/浮游风蕈兽"
        ]
    },
    "60904": {
        "_id": 60904,
        "KingdomID": 6,
        "ClassID": 10609,
        "Name": {
            "CH": "旋转雷蕈兽",
            "EN": "Whirling Electro Fungus"
        },
        "Title": [
            {
                "CH": "旋转，并以此通往胜利",
                "EN": "Spin to Win"
            }
        ],
        "Desc": {
            "CH": "拥有一定智能的孢子生物。具有强大的环境适应能力。\n有研究表明，这类蕈兽内部电流的方向会随时间发生周期性的变化，如果加以开发说不定会有更广泛的用途…",
            "EN": "A spore organism of some intelligence. It possesses extreme adaptability.\nResearch has shown that this sort of fungal beast's internal electric current will change periodically. If further developed, perhaps this study may give rise to a wide range of uses..."
        },
        "Color": "Elec",
        "HP": 0.5,
        "HPCurve": 2,
        "ATK": 1.8,
        "ATKCurve": 4,
        "RESBase": {
            "Elec": 0.2,
            "Grass": 0.2
        },
        "Ball": {
            "Type": 0,
            "Point": 1,
            "Drop": "1"
        },
        "Icon": [
            "wef"
        ],
        "Grade": 1,
        "Weight": 75,
        "EndureTemplateID": 20,
        "Drop": [
            {
                "Type": 11,
                "Amount": [
                    0.8404,
                    0.2241,
                    0.056,
                    0
                ]
            },
            {
                "Type": 12,
                "Amount": [
                    0.2101,
                    0.056,
                    0.014,
                    0
                ]
            },
            {
                "Type": 13,
                "Amount": [
                    0,
                    1.0505,
                    0.2801,
                    0.07
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/旋转雷蕈兽"
        ]
    },
    "60905": {
        "_id": 60905,
        "KingdomID": 6,
        "ClassID": 10609,
        "Name": {
            "CH": "旋转冰蕈兽",
            "EN": "Whirling Cryo Fungus"
        },
        "Title": [
            {
                "CH": "旋转，并以此通往胜利",
                "EN": "Spin to Win"
            }
        ],
        "Desc": {
            "CH": "拥有一定智能的孢子生物。具有强大的环境适应能力。\n经过冰镇之后的蕈兽也不会变得更好吃，不过倒也因此有着消暑的功效。",
            "EN": "A spore organism of some intelligence. It possesses extreme adaptability.\nPutting Fungi on ice does not make them taste any better, but it does at least help with relieving the heat."
        },
        "Color": "Ice",
        "HP": 0.5,
        "HPCurve": 2,
        "ATK": 1.8,
        "ATKCurve": 4,
        "RESBase": {
            "Ice": 0.2,
            "Grass": 0.2
        },
        "Ball": {
            "Type": 0,
            "Point": 1,
            "Drop": "1"
        },
        "Icon": [
            "wcf"
        ],
        "Grade": 1,
        "Weight": 75,
        "EndureTemplateID": 20,
        "Drop": [
            {
                "Type": 11,
                "Amount": [
                    0.8404,
                    0.2241,
                    0.056,
                    0
                ]
            },
            {
                "Type": 12,
                "Amount": [
                    0.2101,
                    0.056,
                    0.014,
                    0
                ]
            },
            {
                "Type": 13,
                "Amount": [
                    0,
                    1.0505,
                    0.2801,
                    0.07
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/旋转冰蕈兽"
        ]
    },
    "60906": {
        "_id": 60906,
        "KingdomID": 6,
        "ClassID": 10609,
        "Name": {
            "CH": "旋转火蕈兽",
            "EN": "Whirling Pyro Fungus"
        },
        "Title": [
            {
                "CH": "旋转，并以此通往胜利",
                "EN": "Spin to Win"
            }
        ],
        "Desc": {
            "CH": "拥有一定智能的孢子生物。具有强大的环境适应能力。\n因为含有富集的火元素，在干燥的季节容易引发林火，对森林来说是非常危险的入侵物种。",
            "EN": "A spore organism of some intelligence. It possesses extreme adaptability.\nDue to the abundant amount of Pyro these creatures contain, they have a tendency to cause forest fires during dry seasons, and thus are considered a dangerous invasive species to the forest."
        },
        "Color": "Fire",
        "HP": 0.5,
        "HPCurve": 2,
        "ATK": 1.8,
        "ATKCurve": 4,
        "RESBase": {
            "Fire": 0.2,
            "Grass": 0.2
        },
        "Ball": {
            "Type": 0,
            "Point": 1,
            "Drop": "1"
        },
        "Icon": [
            "wpf"
        ],
        "Grade": 1,
        "Weight": 75,
        "EndureTemplateID": 20,
        "Drop": [
            {
                "Type": 11,
                "Amount": [
                    0.8404,
                    0.2241,
                    0.056,
                    0
                ]
            },
            {
                "Type": 12,
                "Amount": [
                    0.2101,
                    0.056,
                    0.014,
                    0
                ]
            },
            {
                "Type": 13,
                "Amount": [
                    0,
                    1.0505,
                    0.2801,
                    0.07
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/旋转火蕈兽"
        ]
    },
    "60907": {
        "_id": 60907,
        "KingdomID": 6,
        "ClassID": 10609,
        "Name": {
            "CH": "伸缩火蕈兽",
            "EN": "Stretchy Pyro Fungus"
        },
        "Title": [
            {
                "CH": "格外具有弹性",
                "EN": "Super Elastic"
            }
        ],
        "Desc": {
            "CH": "拥有一定智能的孢子生物。具有强大的环境适应能力。\n虽然看上去很火爆，但其实性格相当温和，只要不要靠得太近应该不会有什么危险…？",
            "EN": "A spore organism of some intelligence. It possesses extreme adaptability.\nThis creature only looks explosive — it is, in fact, a most gentle soul, so long as you don't get too close..."
        },
        "Color": "Fire",
        "HP": 0.5,
        "HPCurve": 2,
        "ATK": 1.8,
        "ATKCurve": 4,
        "RESBase": {
            "Fire": 0.2,
            "Grass": 0.2
        },
        "Ball": {
            "Type": 0,
            "Point": 1,
            "Drop": "1"
        },
        "Icon": [
            "spf"
        ],
        "Grade": 1,
        "Weight": 75,
        "EndureTemplateID": 20,
        "Drop": [
            {
                "Type": 11,
                "Amount": [
                    0.8404,
                    0.2241,
                    0.056,
                    0
                ]
            },
            {
                "Type": 12,
                "Amount": [
                    0.2101,
                    0.056,
                    0.014,
                    0
                ]
            },
            {
                "Type": 13,
                "Amount": [
                    0,
                    1.0505,
                    0.2801,
                    0.07
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/伸缩火蕈兽"
        ]
    },
    "60908": {
        "_id": 60908,
        "KingdomID": 6,
        "ClassID": 10609,
        "Name": {
            "CH": "伸缩风蕈兽",
            "EN": "Stretchy Anemo Fungus"
        },
        "Title": [
            {
                "CH": "格外具有弹性",
                "EN": "Super Elastic"
            }
        ],
        "Desc": {
            "CH": "拥有一定智能的孢子生物。具有强大的环境适应能力。\n通过伸长的菌柱快速吸收养分的蕈兽，伞状的菌盖会随着风的流向转动。",
            "EN": "A spore organism of some intelligence. It possesses extreme adaptability.\nThis mushroom quickly absorbs nutrients through its extended column, and its umbrella-shaped cap turns where the wind's flow carries it."
        },
        "Color": "Wind",
        "HP": 0.5,
        "HPCurve": 2,
        "ATK": 1.8,
        "ATKCurve": 4,
        "RESBase": {
            "Grass": 0.2,
            "Wind": 0.2
        },
        "Ball": {
            "Type": 0,
            "Point": 1,
            "Drop": "1"
        },
        "Icon": [
            "saf"
        ],
        "Grade": 1,
        "Weight": 75,
        "EndureTemplateID": 20,
        "Drop": [
            {
                "Type": 11,
                "Amount": [
                    0.8404,
                    0.2241,
                    0.056,
                    0
                ]
            },
            {
                "Type": 12,
                "Amount": [
                    0.2101,
                    0.056,
                    0.014,
                    0
                ]
            },
            {
                "Type": 13,
                "Amount": [
                    0,
                    1.0505,
                    0.2801,
                    0.07
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/伸缩风蕈兽"
        ]
    },
    "60909": {
        "_id": 60909,
        "KingdomID": 6,
        "ClassID": 10609,
        "Name": {
            "CH": "伸缩岩蕈兽",
            "EN": "Stretchy Geo Fungus"
        },
        "Title": [
            {
                "CH": "格外具有弹性",
                "EN": "Super Elastic"
            }
        ],
        "Desc": {
            "CH": "拥有一定智能的孢子生物。具有强大的环境适应能力。\n富集的岩元素在它们的菌盖上形成了坚硬的外壳。为了挥舞沉重的头部，想必其身体结构相比同类也更加紧密。",
            "EN": "A spore organism of some intelligence. It possesses extreme adaptability.\nThese shrooms' caps have hard outer layers derived from concretions of Geo energy. Its bodily structure is also quite likely to be denser than others of its kind so as to withstand the heavy weight of its cap."
        },
        "Color": "Rock",
        "HP": 0.5,
        "HPCurve": 2,
        "ATK": 1.8,
        "ATKCurve": 4,
        "RESBase": {
            "Grass": 0.2,
            "Rock": 0.2
        },
        "Ball": {
            "Type": 0,
            "Point": 1,
            "Drop": "1"
        },
        "Icon": [
            "sgf"
        ],
        "Grade": 1,
        "Weight": 75,
        "EndureTemplateID": 20,
        "Drop": [
            {
                "Type": 11,
                "Amount": [
                    0.8404,
                    0.2241,
                    0.056,
                    0
                ]
            },
            {
                "Type": 12,
                "Amount": [
                    0.2101,
                    0.056,
                    0.014,
                    0
                ]
            },
            {
                "Type": 13,
                "Amount": [
                    0,
                    1.0505,
                    0.2801,
                    0.07
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/伸缩岩蕈兽"
        ]
    },
    "60910": {
        "_id": 60910,
        "KingdomID": 6,
        "ClassID": 10609,
        "Name": {
            "CH": "伸缩雷蕈兽",
            "EN": "Stretchy Electro Fungus"
        },
        "Title": [
            {
                "CH": "格外具有弹性",
                "EN": "Super Elastic"
            }
        ],
        "Desc": {
            "CH": "拥有一定智能的孢子生物。具有强大的环境适应能力。\n不同于其他蕈兽，除了菌盖外还长有会发光的触角。遇到危险的话会释放出小电球来保护自己。",
            "EN": "A spore organism of some intelligence. It possesses extreme adaptability.\nUnlike other Fungi, this creature possesses a cap with a luminous tendril sticking out of it. When in danger, it will release small electric orbs as a form of self-defense."
        },
        "Color": "Elec",
        "HP": 0.5,
        "HPCurve": 2,
        "ATK": 1.8,
        "ATKCurve": 4,
        "RESBase": {
            "Elec": 0.2,
            "Grass": 0.2
        },
        "Ball": {
            "Type": 0,
            "Point": 1,
            "Drop": "1"
        },
        "Icon": [
            "sef"
        ],
        "Grade": 1,
        "Weight": 75,
        "EndureTemplateID": 20,
        "Drop": [
            {
                "Type": 11,
                "Amount": [
                    0.8404,
                    0.2241,
                    0.056,
                    0
                ]
            },
            {
                "Type": 12,
                "Amount": [
                    0.2101,
                    0.056,
                    0.014,
                    0
                ]
            },
            {
                "Type": 13,
                "Amount": [
                    0,
                    1.0505,
                    0.2801,
                    0.07
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/伸缩雷蕈兽"
        ]
    },
    "61001": {
        "_id": 61001,
        "KingdomID": 6,
        "ClassID": 10610,
        "Name": {
            "CH": "沧流 · 圣骸角鳄",
            "EN": "Consecrated Horned Crocodile"
        },
        "Title": [
            {
                "CH": "啃噬伟大生命的水之兽",
                "EN": "The Surging One, Devourer of the Mighty"
            }
        ],
        "Desc": {
            "CH": "因为啃噬伟大的生命体，而扭曲异变的爬行动物。\n姿态威严的猛兽有时会受到人们的敬畏，被视为伟大力量的化身，并借由他们的形象彰显自己。而真的摄入「伟大力量」的野兽大多因此丧命，仅有少数进化出了新的形态。",
            "EN": "A reptile that has mutated after feeding from greater lifeforms.\nMajestic beasts are sometimes revered by human beings as the embodiment of a greater power, their visages turned to analogy to feed in reference to a person, feeding their ego. However, the majority of beasts that have absorbed the \"greater power\" were slain by the overwhelming nature of the power itself. Only a few among their number evolved new forms."
        },
        "Color": "Water",
        "HP": 3.5,
        "HPCurve": 2,
        "ATK": 6.0,
        "ATKCurve": 4,
        "RESState": [
            {
                "State": "Sacred_Enhance",
                "All": 0.3,
                "Water": 0.3
            }
        ],
        "Ball": {
            "Type": 2,
            "Point": 3,
            "Drop": "222"
        },
        "Icon": [
            "cshc"
        ],
        "Grade": 2,
        "Weight": 150,
        "EndureTemplateID": 400,
        "SkillData": [
            "6/61001"
        ],
        "Drop": [
            {
                "Type": 26,
                "Amount": [
                    0,
                    6.5658,
                    1.7508,
                    0.4378
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/圣骸角鳄"
        ]
    },
    "61002": {
        "_id": 61002,
        "KingdomID": 6,
        "ClassID": 10610,
        "Name": {
            "CH": "狂蔓 · 圣骸牙兽",
            "EN": "Consecrated Fanged Beast"
        },
        "Title": [
            {
                "CH": "啃噬伟大生命的草之兽",
                "EN": "The Lush One, Devourer of the Mighty"
            }
        ],
        "Desc": {
            "CH": "因为啃噬伟大的生命体，而扭曲异变的掠食者。\n自然的规律是弱肉强食，宠物和圣骸兽则是例外。人类的饵食与不朽的遗骸，本质都是有意或无意的圈套。贪图至强者的馈赠，代价比被关进笼子要惨痛得多。",
            "EN": "A predator that has mutated after feeding from greater lifeforms.\nSurvival of the fittest is written in the law of nature. But pets and Consecrated Beasts are exceptions. Human beings' baits and the remains of immortal beings are essentially deliberate or unintentional traps. The price of coveting the gift of the strongest is much more painful than being locked in a cage."
        },
        "Color": "Grass",
        "HP": 3.5,
        "HPCurve": 2,
        "ATK": 6.0,
        "ATKCurve": 4,
        "RESState": [
            {
                "State": "Sacred_Enhance",
                "All": 0.3,
                "Grass": 0.3
            }
        ],
        "Ball": {
            "Type": 5,
            "Point": 3,
            "Drop": "222"
        },
        "Icon": [
            "csfb"
        ],
        "Grade": 2,
        "Weight": 150,
        "EndureTemplateID": 400,
        "SkillData": [
            "6/61002"
        ],
        "Drop": [
            {
                "Type": 26,
                "Amount": [
                    0,
                    6.5658,
                    1.7508,
                    0.4378
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/圣骸牙兽"
        ]
    },
    "61003": {
        "_id": 61003,
        "KingdomID": 6,
        "ClassID": 10610,
        "Name": {
            "CH": "赤炎 · 圣骸赤鹫",
            "EN": "Consecrated Red Vulture"
        },
        "Title": [
            {
                "CH": "啃噬伟大生命的火之兽",
                "EN": "The Burning One, Devourer of the Mighty"
            }
        ],
        "Desc": {
            "CH": "因为啃噬伟大的生命体，而扭曲异变的食腐猛禽。\n圣骸兽所吞食的尸骸当中，常常夹杂着有永远无法陷入沉寂的意志。而这些意志与力量又会驱使它无穷尽地渴求厮杀与亡骸。",
            "EN": "A scavenging bird of prey that has mutated after feeding from greater lifeforms.\nThe greater lifeforms devoured by Consecrated Beasts often have nigh-immortal consciousness that can never be fully silenced or eradicated. The very power and consciousness gained by Consecrated Beasts devouring these lifeforms will, in turn, drive them to seek more violence and carrion."
        },
        "Color": "Fire",
        "HP": 3.5,
        "HPCurve": 2,
        "ATK": 6.0,
        "ATKCurve": 4,
        "RESState": [
            {
                "State": "Sacred_Enhance",
                "All": 0.3,
                "Fire": 0.3
            }
        ],
        "Ball": {
            "Type": 1,
            "Point": 3,
            "Drop": "222"
        },
        "Icon": [
            "csrv"
        ],
        "Grade": 2,
        "Weight": 150,
        "EndureTemplateID": 400,
        "SkillData": [
            "6/61003"
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "魔神能量攻击",
                    "EN": "Ultimate Attack"
                },
                "DMG": [
                    3
                ]
            },
            {
                "Name": {
                    "CH": "头戳一下+翅膀扇",
                    "EN": "Head pierce + wing hit"
                },
                "DMG": [
                    3,
                    4.2
                ]
            },
            {
                "Name": {
                    "CH": "跳砸",
                    "EN": "Jump and land"
                },
                "DMG": [
                    6
                ]
            },
            {
                "Name": {
                    "CH": "啄x2+拱+旋转",
                    "EN": "Peck*2 + push + spin"
                },
                "DMG": [
                    3,
                    3,
                    4.8,
                    6
                ]
            },
            {
                "Name": {
                    "CH": "飞天轰炸鸡",
                    "EN": "Fly and drop bombs"
                },
                "DMG": [
                    4.8
                ]
            },
            {
                "Name": {
                    "CH": "转身尾巴扫",
                    "EN": "Tail slash"
                },
                "DMG": [
                    4.8
                ]
            }
        ],
        "Drop": [
            {
                "Type": 26,
                "Amount": [
                    0,
                    6.5658,
                    1.7508,
                    0.4378
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/圣骸赤鹫"
        ]
    },
    "61004": {
        "_id": 61004,
        "KingdomID": 6,
        "ClassID": 10610,
        "Name": {
            "CH": "苍雷 · 圣骸毒蝎",
            "EN": "Consecrated Scorpion"
        },
        "Title": [
            {
                "CH": "啃噬伟大生命的雷之兽",
                "EN": "The Fulgurous One, Devourer of the Mighty"
            }
        ],
        "Desc": {
            "CH": "因为啃噬伟大的生命体，而扭曲异变的节肢动物。\n生物有其进化之法则，而圣骸兽的法则则是拥有足够的运气，能在同族之前发现在遥远过去就已亡殁的强大生灵遗骸。动物与人的共性往往比人所甘愿承认的更多。",
            "EN": "An arthropod that has mutated after feeding from greater lifeforms.\nLifeforms are governed by the laws of evolution, Consecrated Beasts exploited these rules by being fortunate enough to discover a long-dead carcass of a greater being before any of their competition ever did. Animals and humans often have far more in common than the latter is willing to acknowledge."
        },
        "Color": "Elec",
        "HP": 3.5,
        "HPCurve": 2,
        "ATK": 6.0,
        "ATKCurve": 4,
        "RESState": [
            {
                "State": "Sacred_Enhance",
                "All": 0.3,
                "Elec": 0.3
            }
        ],
        "Ball": {
            "Type": 3,
            "Point": 3,
            "Drop": "222"
        },
        "Icon": [
            "css"
        ],
        "Grade": 2,
        "Weight": 150,
        "EndureTemplateID": 400,
        "SkillData": [
            "6/61004"
        ],
        "Drop": [
            {
                "Type": 26,
                "Amount": [
                    0,
                    6.5658,
                    1.7508,
                    0.4378
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/圣骸毒蝎"
        ]
    },
    "61005": {
        "_id": 61005,
        "KingdomID": 6,
        "ClassID": 10610,
        "Name": {
            "CH": "驭风 · 圣骸飞蛇",
            "EN": "Consecrated Flying Serpent"
        },
        "Title": [
            {
                "CH": "啃噬伟大生命的风之兽",
                "EN": "The Whirling One, Devourer of the Mighty"
            }
        ],
        "Desc": {
            "CH": "因为啃噬伟大的生命体，而扭曲异变的爬行动物。\n即使获得了操使元素的异能，圣骸兽自身无法将体内不朽的亡骸彻底消化。其中的力量与动物本身融合，长出的骨质甲壳据说在成分上与主要产自稻妻的冶炼材料有相近之处。",
            "EN": "A reptile that has mutated after feeding from greater lifeforms.\nThough now imbued with the power of elemental manipulation, the Consecrated Beasts themselves still cannot fully digest the pieces of flesh they devoured from the immortal carcasses that they ate. Instead, the pieces slowly fused with the beasts themselves, weaving a bony shell over their body, one that shares a similar composition to a certain type of smelting material from Inazuma."
        },
        "Color": "Wind",
        "HP": 3.5,
        "HPCurve": 2,
        "ATK": 6.0,
        "ATKCurve": 4,
        "RESState": [
            {
                "State": "Sacred_Enhance",
                "All": 0.3,
                "Wind": 0.3
            }
        ],
        "Ball": {
            "Type": 6,
            "Point": 3,
            "Drop": "222"
        },
        "Icon": [
            "csfs"
        ],
        "Grade": 2,
        "Weight": 150,
        "EndureTemplateID": 400,
        "SkillData": [
            "6/61005"
        ],
        "Drop": [
            {
                "Type": 26,
                "Amount": [
                    0,
                    6.5658,
                    1.7508,
                    0.4378
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/圣骸飞蛇"
        ]
    },
    "61101": {
        "_id": 61101,
        "KingdomID": 6,
        "ClassID": 10611,
        "Name": {
            "CH": "翠翎恐蕈",
            "EN": "Jadeplume Terrorshroom"
        },
        "Title": [
            {
                "CH": "菌族拥趸的王者",
                "EN": "The Beloved King of Fungi"
            },
            {
                "CH": "深居幽林的蕈王",
                "EN": "Deepwood Shroomlord"
            }
        ],
        "Desc": {
            "CH": "经过漫长的岁月，演化出了独特形态的大型蕈兽。\n智能水平已经接近，甚至超越了真正的动物，会主动捕食猎物以获取养分。尾巴上长有漂亮的翠翎，与素论派尊崇的古代圣兽非常相似。以拟态演化的逻辑来看，想必它们曾经共同行走在大地之上吧。",
            "EN": "Over many long years, this large fungal beast has morphed into a unique form.\nIts intelligence has already reached and perhaps has even surpassed real animals, and it will actively hunt prey to obtain sustenance. Its tail has a lovely jade plumage that seems very similar to that of the ancient sacred beast that the Spantamad school reveres. As far as the logic of mimesis is concerned, it seems quite likely that the two beings once walked the earth together."
        },
        "Color": "Grass",
        "HP": 7.0,
        "HPCurve": 2,
        "ATK": 7.0,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": 0.25,
            "Fire": 0.25,
            "Water": 0.25,
            "Elec": 0.25,
            "Ice": 0.25,
            "Grass": 0.8,
            "Wind": 0.25,
            "Rock": 0.25
        },
        "RESState": [
            {
                "State": "Weak",
                "All": -0.25
            }
        ],
        "Ball": {
            "Type": 5,
            "Point": 3,
            "Drop": "334"
        },
        "Icon": [
            "jpt"
        ],
        "Grade": 3,
        "Weight": 250,
        "EndureTemplateID": 4000,
        "Csx": [
            "ECMysticalBeasts/翠翎恐蕈"
        ]
    },
    "61201": {
        "_id": 61201,
        "KingdomID": 6,
        "ClassID": 10612,
        "Name": {
            "CH": "有翼草本真蕈",
            "EN": "Winged Dendroshroom"
        },
        "Title": [
            {
                "CH": "菌族的守护者",
                "EN": "Shroom Sentinel"
            },
            {
                "CH": "孢子的牧者",
                "EN": "Spore Shepherd"
            },
            {
                "CH": "蕈群的超越者",
                "EN": "The Shroom Surpassing"
            }
        ],
        "Desc": {
            "CH": "孢子构成的拟态生物，具有强大的环境适应能力。\n不仅看上去像，连生活习性也模仿了鸟类。可以依靠菌类组织的翅膀飞上天空，将孢子播撒到更远的地方。\n在蕈兽群落中占有主导地位，显示出其拥有着更高的智能。",
            "EN": "A mimetic life form made out of spores that has extreme adaptability.\nGoing well past mere appearances, this being even imitates the habits of birds. It can use its spore-built wings to fly in the sky, spreading yet more spores to yet more distant places.\nThis creature has a position of command amongst colonies of Fungi and exhibits heightened intelligence to match."
        },
        "Color": "Grass",
        "HP": 1.0,
        "HPCurve": 2,
        "ATK": 3.0,
        "ATKCurve": 4,
        "RESBase": {
            "Grass": 0.4
        },
        "Ball": {
            "Type": 5,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "wds"
        ],
        "Grade": 1,
        "Weight": 125,
        "EndureTemplateID": 210,
        "Drop": [
            {
                "Type": 11,
                "Amount": [
                    1.6808,
                    0.4482,
                    0.112,
                    0
                ]
            },
            {
                "Type": 12,
                "Amount": [
                    0.4202,
                    0.112,
                    0.028,
                    0
                ]
            },
            {
                "Type": 13,
                "Amount": [
                    0,
                    2.101,
                    0.5602,
                    0.1401
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/有翼草本真蕈"
        ]
    },
    "61202": {
        "_id": 61202,
        "KingdomID": 6,
        "ClassID": 10612,
        "Name": {
            "CH": "有翼冰本真蕈",
            "EN": "Winged Cryoshroom"
        },
        "Title": [
            {
                "CH": "菌族的守护者",
                "EN": "Shroom Sentinel"
            },
            {
                "CH": "孢子的牧者",
                "EN": "Spore Shepherd"
            },
            {
                "CH": "蕈群的超越者",
                "EN": "The Shroom Surpassing"
            }
        ],
        "Desc": {
            "CH": "孢子构成的拟态生物，具有强大的环境适应能力。\n不仅看上去像，连生活习性也模仿了鸟类。飞舞的时候会散落出轻盈的冰花，这些冰晶的晶核其实也是孢子。\n在蕈兽群落中占有主导地位，显示出其拥有着更高的智能。",
            "EN": "A mimetic life form made out of spores that has extreme adaptability.\nGoing well past mere appearances, this being even imitates the habits of birds. When it flies about, it will scatter icy flowers in its wake. These crystal cores, too, are spores.\nThis creature has a position of command amongst colonies of fungal beasts and exhibits heightened intelligence to match."
        },
        "Color": "Ice",
        "HP": 1.4,
        "HPCurve": 2,
        "ATK": 2.4,
        "ATKCurve": 4,
        "RESBase": {
            "Ice": 0.3,
            "Grass": 0.3
        },
        "Ball": {
            "Type": 4,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "wcs"
        ],
        "Grade": 1,
        "Weight": 125,
        "EndureTemplateID": 210,
        "Drop": [
            {
                "Type": 11,
                "Amount": [
                    1.6808,
                    0.4482,
                    0.112,
                    0
                ]
            },
            {
                "Type": 12,
                "Amount": [
                    0.4202,
                    0.112,
                    0.028,
                    0
                ]
            },
            {
                "Type": 13,
                "Amount": [
                    0,
                    2.101,
                    0.5602,
                    0.1401
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/有翼冰本真蕈"
        ]
    },
    "61203": {
        "_id": 61203,
        "KingdomID": 6,
        "ClassID": 10612,
        "Name": {
            "CH": "陆行水本真蕈",
            "EN": "Grounded Hydroshroom"
        },
        "Title": [
            {
                "CH": "菌族的守护者",
                "EN": "Shroom Sentinel"
            },
            {
                "CH": "孢子的牧者",
                "EN": "Spore Shepherd"
            },
            {
                "CH": "蕈群的超越者",
                "EN": "The Shroom Surpassing"
            }
        ],
        "Desc": {
            "CH": "孢子构成的拟态生物，具有强大的环境适应能力。\n看上去像是鸟类，不过似乎并没有掌握飞行的能力。或许是因为水元素的影响，与同类相比其行动异常敏捷。也有研究推断这种蕈兽实际上是由无数细小的孢子集结构成的菌株，并不是纯粹单一的生命体。 \n在蕈兽群落中占有主导地位，显示出其拥有着更高的智能。",
            "EN": "A mimetic life form made out of spores that has extreme adaptability.\nThis creature may look like a bird, but it cannot fly. Due to the influence of Hydro energies, it is astoundingly agile compared to others of its kind. Some research has inferred that this creature is in fact an aggregation of countless tiny spores rather than a single entity.\nThis creature has a position of command amongst colonies of Fungi and exhibits heightened intelligence to match."
        },
        "Color": "Water",
        "HP": 1.0,
        "HPCurve": 2,
        "ATK": 3.0,
        "ATKCurve": 4,
        "RESBase": {
            "Water": 0.3,
            "Grass": 0.3
        },
        "Ball": {
            "Type": 2,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "ghs"
        ],
        "Grade": 1,
        "Weight": 125,
        "EndureTemplateID": 210,
        "Drop": [
            {
                "Type": 11,
                "Amount": [
                    1.6808,
                    0.4482,
                    0.112,
                    0
                ]
            },
            {
                "Type": 12,
                "Amount": [
                    0.4202,
                    0.112,
                    0.028,
                    0
                ]
            },
            {
                "Type": 13,
                "Amount": [
                    0,
                    2.101,
                    0.5602,
                    0.1401
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/陆行水本真蕈"
        ]
    },
    "61204": {
        "_id": 61204,
        "KingdomID": 6,
        "ClassID": 10612,
        "Name": {
            "CH": "陆行岩本真蕈",
            "EN": "Grounded Geoshroom"
        },
        "Title": [
            {
                "CH": "菌族的守护者",
                "EN": "Shroom Sentinel"
            },
            {
                "CH": "孢子的牧者",
                "EN": "Spore Shepherd"
            },
            {
                "CH": "蕈群的超越者",
                "EN": "The Shroom Surpassing"
            }
        ],
        "Desc": {
            "CH": "孢子构成的拟态生物，具有强大的环境适应能力。\n受到岩元素的影响，部分组织似乎已经完全角质化了。考虑到蕈类本就是从孢子进化而来，假以时日，或许可以以此为基础发展出另一种全新的生命形态。\n在蕈兽群落中占有主导地位，显示出其拥有着更高的智能。",
            "EN": "A mimetic life form made out of spores that has extreme adaptability.\nThis being has undergone almost complete keratinization due to the influence of Geo. When one considers that Fungi evolved from spores, perhaps this one might even be the foundation for a further evolution into some new life form altogether — if given time, that is.\nThis creature has a position of command amongst colonies of fungal beasts and exhibits heightened intelligence to match."
        },
        "Color": "Rock",
        "HP": 1.4,
        "HPCurve": 2,
        "ATK": 2.4,
        "ATKCurve": 4,
        "RESBase": {
            "Grass": 0.3,
            "Rock": 0.3
        },
        "Ball": {
            "Type": 7,
            "Point": 2,
            "Drop": "11"
        },
        "Icon": [
            "ggs"
        ],
        "Grade": 1,
        "Weight": 125,
        "EndureTemplateID": 210,
        "Drop": [
            {
                "Type": 11,
                "Amount": [
                    1.6808,
                    0.4482,
                    0.112,
                    0
                ]
            },
            {
                "Type": 12,
                "Amount": [
                    0.4202,
                    0.112,
                    0.028,
                    0
                ]
            },
            {
                "Type": 13,
                "Amount": [
                    0,
                    2.101,
                    0.5602,
                    0.1401
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/陆行岩本真蕈"
        ]
    },
    "61301": {
        "_id": 61301,
        "KingdomID": 6,
        "ClassID": 10613,
        "Name": {
            "CH": "风蚀沙虫",
            "EN": "Setekh Wenut"
        },
        "Title": [
            {
                "CH": "一万万砂砾最后的亲王",
                "EN": "The Last Prince of the Sands"
            }
        ],
        "Desc": {
            "CH": "生活在沙海深处的无目掠食者，会依据声音与大地的震动寻找猎物的踪迹。\n传说在时间开始之前的过去，它们所统辖地上遍布芳草与巨木，而它们的主宰与其子嗣一同，在湿润的泥土中穿梭，如同鱼之游于水中。\n许多纪元之后，天降的恩赐让泥土化为砂砾。许多生命陨落了，许多物种消失了，而依托土地而生的这一族则成为了「沙虫」。\n或许是因为生物习性中的共同之处，似乎有沙漠的居民错误地认为，「沙虫」是流沙鳗鳗蛹化成熟之后的完整形态。",
            "EN": "An eyeless predator that dwells within the depths of the desert and uses sound and vibrations to seek out its prey.\nLegend has it that in the ancient past before even the concept of time was created, these creatures thrived with their progenitors and offspring in a land of verdant grass and colossal trees, traversing through slick soil like fishes diving through the wet.\nWith the passage of eons, the gifts of heaven have long reduced the soils to sand, rendering a great many creatures extinct. This branch of their genus was able to survive by adapting more toward earth-based living, and became known as the \"Wenut.\"\nSome desert folk still mistakenly believe that Wenut is the final point of a quicksand eel's life cycle, likely due to superficial similarities between the species' behaviors."
        },
        "Color": "Wind",
        "HP": 7.0,
        "HPCurve": 2,
        "ATK": 6.0,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": 0.25,
            "Fire": 0.25,
            "Water": 0.25,
            "Elec": 0.25,
            "Ice": 0.25,
            "Grass": 0.25,
            "Wind": 0.6,
            "Rock": 0.25
        },
        "RESState": [
            {
                "State": "Sandworm_Tower",
                "All": 0.3
            }
        ],
        "RESModify": [
            {
                "State": "Weak",
                "Show": {
                    "CH": "对应抗性降低45%",
                    "EN": "Corresponding Element decrease 45%"
                }
            }
        ],
        "Ball": {
            "Type": 6,
            "Point": 4,
            "Drop": "3333"
        },
        "Icon": [
            "sw"
        ],
        "Grade": 3,
        "Weight": 500,
        "EndureTemplateID": 4000,
        "SkillData": [
            "6/61301",
            "6/613011"
        ],
        "Csx": [
            "ECMysticalBeasts/风蚀沙虫"
        ]
    },
    "61510": {
        "_id": 61510,
        "KingdomID": 8,
        "ClassID": 10805,
        "Name": {
            "CH": "猎刀鳐",
            "EN": "Hunter's Ray"
        },
        "Title": [
            {
                "CH": "因原海而进化的生命",
                "EN": "Life Evolved by the Fontemer"
            }
        ],
        "Desc": {
            "CH": "能够释放水刃的鳐鱼。\n在原海异种中，存在着诡谲的「异色」生物。据说这种异色体并不是生物，而是海洋中未曾经受肉身诞生，不曾吞下饵食的灵魂，会向原海所认同者或是驾驭原海者施以祝福。当然对自然哲学更有研究与心得的人是不会认同这种毫无依据的说法的。",
            "EN": "A stingray that can release blades of water.\nAmong the Fontemer Aberrants, there are certain strange \"Xenochromatic\" lifeforms. It is said that these are not actually living creatures, but rather spirits from the ocean that have neither taken physical form nor swallowed bait. They are said to bestow blessings upon those accepted by the Fontemer as well as those that are able to control it. Of course, those that have a deeper understanding of natural philosophy would discount such groundless speculation."
        },
        "Color": "Water",
        "HP": 1.0,
        "HPCurve": 7,
        "ATK": 50.0,
        "ATKCurve": 40,
        "Icon": [
            "UI_MonsterIcon_MantaRay_Larva_01",
            "UI_MonsterIcon_MantaRay_Larva_02"
        ],
        "Grade": 0,
        "Weight": 80,
        "EndureTemplateID": 20,
        "Drop": [
            {
                "Type": 28,
                "Amount": [
                    0.8404,
                    0.2241,
                    0.056,
                    0
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/猎刀鳐"
        ]
    },
    "61520": {
        "_id": 61520,
        "KingdomID": 8,
        "ClassID": 10805,
        "Name": {
            "CH": "重甲蟹",
            "EN": "Armored Crab"
        },
        "Title": [
            {
                "CH": "因原海而进化的生命",
                "EN": "Life Evolved by the Fontemer"
            }
        ],
        "Desc": {
            "CH": "寄居在螺与壳之中的螃蟹。\n几乎不具备野生动物常有的攻击性，因为食欲之外的攻击性常常源自领域意识，而重甲蟹的领土就是其背在身后的小小家园。",
            "EN": "A crab that makes its home in borrowed shells.\nIt seems not to have any of the aggressiveness possessed by many wild animals. Perhaps it is because animals who attack for purposes other than obtaining food possess some degree of territoriality, yet the territory of the Armored Crab is naught but the small home upon its back."
        },
        "Color": "Water",
        "HP": 0.39,
        "HPCurve": 2,
        "ATK": 45.0,
        "ATKCurve": 40,
        "Ball": {
            "Type": 0,
            "Point": 1,
            "Drop": "1"
        },
        "Icon": [
            "UI_MonsterIcon_HermitCrab_Larva_01",
            "UI_MonsterIcon_HermitCrab_Larva_02"
        ],
        "Grade": 0,
        "Weight": 75,
        "EndureTemplateID": 20,
        "Drop": [
            {
                "Type": 28,
                "Amount": [
                    0.8404,
                    0.2241,
                    0.056,
                    0
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/重甲蟹"
        ]
    },
    "61530": {
        "_id": 61530,
        "KingdomID": 8,
        "ClassID": 10805,
        "Name": {
            "CH": "膨膨兽",
            "EN": "Blubberbeast"
        },
        "Title": [
            {
                "CH": "因原海而进化的生命",
                "EN": "Life Evolved by the Fontemer"
            }
        ],
        "Desc": {
            "CH": "生活在枫丹水域的哺乳动物，能够发出悠扬的声音与同伴进行通讯。\n依据密度的原则，水作为声音传播的介质，效率远远高于空气。基于源自膨膨兽的灵感，科学院中有人发明了将图像转为数据并即时传输的技术，但由于实验的技术限制诸多所以并未在科学院外广泛使用。\n但事实上在更遥远的古代就有过基于液体与声音的数据传输技术了。世上没有新鲜事。",
            "EN": "A type of mammal that resides in the watery regions of Fontaine. It communicates with its companions through melodious sounds.\nDue to its density, water is a much more efficient medium of sound than air. Drawing inspiration from the Blubberbeast, someone from the Research Institute invented a type of technology that converted images into data to facilitate instant transmission. But due to the technical limitations of the experiment, it never became widely adopted outside the Institute.\nIn reality, however, data transmission technology based on fluid and sound has existed since ancient times. As the saying goes, there's nothing new under the sun."
        },
        "Color": "Water",
        "HP": 0.65,
        "HPCurve": 2,
        "ATK": 50.0,
        "ATKCurve": 40,
        "Ball": {
            "Type": 0,
            "Point": 1,
            "Drop": "2"
        },
        "Icon": [
            "UI_MonsterIcon_Seal_Larva_01"
        ],
        "Grade": 0,
        "Weight": 300,
        "EndureTemplateID": 20,
        "Drop": [
            {
                "Type": 28,
                "Amount": [
                    0.8404,
                    0.2241,
                    0.056,
                    0
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/膨膨兽"
        ]
    },
    "61540": {
        "_id": 61540,
        "KingdomID": 8,
        "ClassID": 10805,
        "Name": {
            "CH": "帽子水母",
            "EN": "Hat Jellyfish"
        },
        "Title": [
            {
                "CH": "因原海而进化的生命",
                "EN": "Life Evolved by the Fontemer"
            }
        ],
        "Desc": {
            "CH": "通过发射水弹进行狩猎的水生动物。\n虽然看起来像帽子，但进食的口器在「帽子」的底部，所以将其戴在头上实在是很不明智的行为。过去曾经有人证实过这一点：「事后回想起来，那确实是很不明智的行为。」",
            "EN": "An aquatic creature that hunts by firing watery projectiles.\nAlthough it looks like a hat, it actually eats from its underside, so attempting to wear it is strongly disadvised. In the words of someone who once did this: \"That wasn't very clever, was it?\""
        },
        "Color": "Water",
        "HP": 1.0,
        "HPCurve": 7,
        "ATK": 50.0,
        "ATKCurve": 40,
        "Icon": [
            "UI_MonsterIcon_JellyFish_Larva_01",
            "UI_MonsterIcon_JellyFish_Larva_02"
        ],
        "Grade": 0,
        "Weight": 80,
        "EndureTemplateID": 20,
        "Drop": [
            {
                "Type": 28,
                "Amount": [
                    0.8404,
                    0.2241,
                    0.056,
                    0
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/帽子水母"
        ]
    },
    "61550": {
        "_id": 61550,
        "KingdomID": 8,
        "ClassID": 10805,
        "Name": {
            "CH": "泡泡海马",
            "EN": "Bubbly Seahorse"
        },
        "Title": [
            {
                "CH": "因原海而进化的生命",
                "EN": "Life Evolved by the Fontemer"
            }
        ],
        "Desc": {
            "CH": "通过发射水弹进行狩猎的水生动物。\n在（为数不多的）原海异种动物爱好者当中，泡泡海马有着相当多的拥趸。当然，原海异种爱好者和发条机关爱好者一样，当中的细分群体对其他群体有很强的攻击性。",
            "EN": "An aquatic creature that hunts by firing watery projectiles.\nAmong the (admittedly somewhat sparse) ranks of Fontemer Aberrant enthusiasts, Bubbly Seahorses have many fans. As with clockwork meka enthusiasts, a small minority of Fontemer Aberrant enthusiasts can be very standoffish when it comes to people whose opinions differ from their own."
        },
        "Color": "Water",
        "HP": 0.6,
        "HPCurve": 7,
        "ATK": 45.0,
        "ATKCurve": 40,
        "Icon": [
            "UI_MonsterIcon_SeaHorse_Larva_01",
            "UI_MonsterIcon_SeaHorse_Larva_02",
            "UI_MonsterIcon_SeaHorse_Larva_03"
        ],
        "Grade": 0,
        "Weight": 60,
        "EndureTemplateID": 20,
        "Drop": [
            {
                "Type": 28,
                "Amount": [
                    0.8404,
                    0.2241,
                    0.056,
                    0
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/泡泡海马"
        ]
    },
    "61560": {
        "_id": 61560,
        "KingdomID": 8,
        "ClassID": 10805,
        "Name": {
            "CH": "球球章鱼",
            "EN": "Ball Octopus"
        },
        "Title": [
            {
                "CH": "因原海而进化的生命",
                "EN": "Life Evolved by the Fontemer"
            }
        ],
        "Desc": {
            "CH": "通过发射水弹进行狩猎的水生动物。\n当然了，发射水弹在原海异种当中并不是稀罕的能力，但对以游客为首的大意潜水者而言，这种高压水流能够造成不小的伤害。潜水护具的厚重是为了两个原因：抵抗水下的压力，以及抵御原海异种的攻击。",
            "EN": "An aquatic creature that hunts by firing watery projectiles.\nThe ability to fire water bombs is of course not uncommon among Fontemer Aberrants. But for your average careless diver, most of whom are tourists, a high-pressure water current of this kind can pose a significant threat. The thickness of diving equipment is therefore necessary for two reasons – firstly, to resist the pressure of the water itself, but secondly, to protect against the attacks of Fontemer Aberrants."
        },
        "Color": "Water",
        "HP": 0.6,
        "HPCurve": 7,
        "ATK": 45.0,
        "ATKCurve": 40,
        "Icon": [
            "UI_MonsterIcon_Octopus_Larva_01",
            "UI_MonsterIcon_Octopus_Larva_02"
        ],
        "Grade": 0,
        "Weight": 60,
        "EndureTemplateID": 20,
        "Drop": [
            {
                "Type": 28,
                "Amount": [
                    0.8404,
                    0.2241,
                    0.056,
                    0
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/球球章鱼"
        ]
    },
    "61570": {
        "_id": 61570,
        "KingdomID": 8,
        "ClassID": 10805,
        "Name": {
            "CH": "天使海兔",
            "EN": "Angelic Sea Hare"
        },
        "Title": [
            {
                "CH": "因原海而进化的生命",
                "EN": "Life Evolved by the Fontemer"
            }
        ],
        "Desc": {
            "CH": "通过发射水弹进行狩猎的水生动物。\n从演化的角度而言，原海异种是因为枫丹特殊的环境而产生了变化的水生动物。也因此可以认为在过去的某个时点前，枫丹原始的水生动物与提瓦特其他地区的水生动物并没有多少区别。",
            "EN": "An aquatic creature that hunts by firing watery projectiles.\nFrom an evolutionary perspective, it is assumed that Fontemer Aberrants have developed from aquatic creatures due to environmental factors specific to Fontaine. As such, it can be inferred that at some point in the past, Fontaine's original aquatic lifeforms differed little from those native to the other regions of Teyvat."
        },
        "Color": "Water",
        "HP": 0.6,
        "HPCurve": 7,
        "ATK": 45.0,
        "ATKCurve": 40,
        "Icon": [
            "UI_MonsterIcon_Aplysia_Larva_01",
            "UI_MonsterIcon_Aplysia_Larva_02"
        ],
        "Grade": 0,
        "Weight": 60,
        "EndureTemplateID": 20,
        "Drop": [
            {
                "Type": 28,
                "Amount": [
                    0.8404,
                    0.2241,
                    0.056,
                    0
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/天使海兔"
        ]
    },
    "61601": {
        "_id": 61601,
        "KingdomID": 6,
        "ClassID": 10616,
        "Name": {
            "CH": "坚盾重甲蟹",
            "EN": "Sternshield Crab"
        },
        "Title": [
            {
                "CH": "因原海而进化的生命",
                "EN": "Life Evolved by the Fontemer"
            }
        ],
        "Desc": {
            "CH": "寄居在螺与壳之中的螃蟹。\n重甲蟹会在成长的阶段中不断寻找适合自己体格的外壳。曾经有畅销的童话书，描述了重甲蟹寻找属于自己的住所的坎坷历程。那是充满了童趣的故事，但事后的反响证明了它与不少别的童话一样，为成年人带来的感触更深。",
            "EN": "A crab that makes its home in borrowed shells.\nArmored Crabs are constantly searching for shells that fit their bodies as they grow. A storybook that told the tale of the trials of an Armored Crab searching for a suitable shell was extremely popular in Fontaine for a time. Though the story is filled with childlike wonder, the reaction after proved that, like many fairytales, it touched adults even more deeply."
        },
        "Color": "Fire",
        "HP": 1.3,
        "HPCurve": 2,
        "ATK": 60,
        "ATKCurve": 40,
        "RESState": [
            {
                "State": "FauneAbyssale_Shield",
                "All": 0.6
            }
        ],
        "Ball": {
            "Type": 0,
            "Point": 1,
            "Drop": "3"
        },
        "Icon": [
            "UI_MonsterIcon_HermitCrab_Mature"
        ],
        "Grade": 1,
        "Weight": 125,
        "EndureTemplateID": 210,
        "Tutorial": {
            "CH": "- 初始具有<b>10.8U火元素增幅</b>：存在期间，全部抗性提升60%，并释放更强大的攻击。<br>- 破除火元素增幅可使其战斗能力降低。",
            "EN": "- Initially has a <b>10.8U Pyro Enhancement</b>: While Enhanced, all RES increase 60% and unleash powerful attacks.<br>- Breaking the Pyro Enhancement will make its attacks less fierce."
        },
        "Drop": [
            {
                "Type": 28,
                "Amount": [
                    1.6808,
                    0.4482,
                    0.112,
                    0
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/坚盾重甲蟹"
        ]
    },
    "61602": {
        "_id": 61602,
        "KingdomID": 6,
        "ClassID": 10616,
        "Name": {
            "CH": "大天使海兔",
            "EN": "Cherubic Sea Hare"
        },
        "Title": [
            {
                "CH": "因原海而进化的生命",
                "EN": "Life Evolved by the Fontemer"
            }
        ],
        "Desc": {
            "CH": "天使海兔成年后的形态，背上的「双翼」实际上是储存元素能量的脂肪块。\n这种水生动物究其一生都在进食、储存实际活动中并不需要的大量能量。或许在大海干涸后的末日，它能够继续存活很长的时间，或是成为其他生物的重要养分。",
            "EN": "The adult form of an Angelic Sea Hare. The \"wings\" on its back are actually fat used for storing elemental energy.\nThis type of sea creature spends its whole life eating and conserving energy far beyond its actual requirements. Perhaps if the oceans dry out some day, they will be able to continue to survive – or simply become an important source of nutrition for other creatures..."
        },
        "Color": "Ice",
        "HP": 1.3,
        "HPCurve": 2,
        "ATK": 60,
        "ATKCurve": 40,
        "RESState": [
            {
                "State": "FauneAbyssale_Shield",
                "All": 0.6
            }
        ],
        "Ball": {
            "Type": 0,
            "Point": 1,
            "Drop": "3"
        },
        "Icon": [
            "UI_MonsterIcon_Aplysia_Mature"
        ],
        "Grade": 1,
        "Weight": 125,
        "EndureTemplateID": 210,
        "Tutorial": {
            "CH": "- 初始具有<b>10.8U冰元素增幅</b>：增幅存在期间，全部抗性提升60%，并释放更强大的攻击。<br>- 破除冰元素增幅可使其战斗能力降低。",
            "EN": "- Initially has a <b>10.8U Cryo Enhancement</b>: While Enhanced, all RES increase 60% and unleash powerful attacks.<br>- Breaking the Cryo Enhancement will make its attacks less fierce."
        },
        "Drop": [
            {
                "Type": 28,
                "Amount": [
                    1.6808,
                    0.4482,
                    0.112,
                    0
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/大天使海兔"
        ]
    },
    "61603": {
        "_id": 61603,
        "KingdomID": 6,
        "ClassID": 10616,
        "Name": {
            "CH": "泡泡雄海马",
            "EN": "Bubbler Seahorse"
        },
        "Title": [
            {
                "CH": "因原海而进化的生命",
                "EN": "Life Evolved by the Fontemer"
            }
        ],
        "Desc": {
            "CH": "雄性的泡泡海马成年后的形态，具有非常强的攻击性。\n曾经有人遇见过海马当中的王者，据说那是拥有修长四肢的高傲生灵。",
            "EN": "The adult form of a male Bubbly Seahorse. It is extremely aggressive.\nSome people even claim to have encountered champions among the seahorses. It is said that they are proud beings with four long limbs."
        },
        "Color": "Elec",
        "HP": 1.3,
        "HPCurve": 2,
        "ATK": 60,
        "ATKCurve": 40,
        "RESState": [
            {
                "State": "FauneAbyssale_Shield",
                "All": 0.6
            }
        ],
        "Ball": {
            "Type": 0,
            "Point": 1,
            "Drop": "3"
        },
        "Icon": [
            "UI_MonsterIcon_SeaHorse_Mature"
        ],
        "Grade": 1,
        "Weight": 125,
        "EndureTemplateID": 210,
        "Tutorial": {
            "CH": "- 初始具有<b>7.2U雷元素增幅</b>：增幅存在期间，全部抗性提升60%，并释放更强大的攻击。<br>- 破除雷元素增幅可使其战斗能力降低。",
            "EN": "- Initially has a <b>7.2U Electro Enhancement</b>: While Enhanced, all RES increase 60% and unleash powerful attacks.<br>- Breaking the Electro Enhancement will make its attacks less fierce."
        },
        "Drop": [
            {
                "Type": 28,
                "Amount": [
                    1.6808,
                    0.4482,
                    0.112,
                    0
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/泡泡雄海马"
        ]
    },
    "61611": {
        "_id": 61611,
        "KingdomID": 10,
        "ClassID": 11040,
        "Name": {
            "CH": "「海原巡回法官」",
            "EN": "Ocean Circuit Judge"
        },
        "Title": [
            {
                "CH": "地方传奇 · 重甲蟹",
                "EN": "Local Legend: Armored Crab"
            }
        ],
        "Desc": {
            "CH": "",
            "EN": ""
        },
        "Color": "Water",
        "HP": 54,
        "HPCurve": 7,
        "ATK": 175,
        "ATKCurve": 40,
        "Icon": [
            "lgamc"
        ],
        "Grade": 3,
        "Weight": 125,
        "EndureTemplateID": 210,
        "MultiPlayerID": 103,
        "Csx": [
            "ECLegends/地方传奇·重甲蟹"
        ]
    },
    "61612": {
        "_id": 61612,
        "KingdomID": 10,
        "ClassID": 11040,
        "Name": {
            "CH": "「深谷之剑」",
            "EN": "Swords of the Gorge"
        },
        "Title": [
            {
                "CH": "地方传奇 · 猎刀鳐",
                "EN": "Local Legend: Hunter's Ray"
            }
        ],
        "Desc": {
            "CH": "",
            "EN": ""
        },
        "Color": "Water",
        "HP": 50,
        "HPCurve": 7,
        "ATK": 225,
        "ATKCurve": 40,
        "Icon": [
            "lghtr"
        ],
        "Grade": 3,
        "Weight": 100,
        "EndureTemplateID": 210,
        "MultiPlayerID": 103,
        "Csx": [
            "ECLegends/地方传奇·猎刀鳐"
        ]
    },
    "61613": {
        "_id": 61613,
        "KingdomID": 10,
        "ClassID": 11040,
        "Name": {
            "CH": "「隐者王 · 多尔库」",
            "EN": "Dobharcu, Lord of the Hidden"
        },
        "Title": [
            {
                "CH": "地方传奇 · 膨膨兽",
                "EN": "Local Legend: Blubberbeast"
            }
        ],
        "Desc": {
            "CH": "",
            "EN": ""
        },
        "Color": "Water",
        "HP": 12,
        "HPCurve": 7,
        "ATK": 62.5,
        "ATKCurve": 40,
        "Icon": [
            "lgpfb"
        ],
        "Grade": 3,
        "Weight": 300,
        "EndureTemplateID": 210,
        "MultiPlayerID": 103,
        "Csx": [
            "ECLegends/地方传奇·膨膨兽"
        ]
    },
    "61621": {
        "_id": 61621,
        "KingdomID": 10,
        "ClassID": 11040,
        "Name": {
            "CH": "「铁皮子爵」",
            "EN": "Iron Viscount"
        },
        "Title": [
            {
                "CH": "地方传奇 · 重甲蟹",
                "EN": "Local Legend: Armored Crab"
            }
        ],
        "Desc": {
            "CH": "",
            "EN": ""
        },
        "Color": "Water",
        "HP": 54,
        "HPCurve": 7,
        "ATK": 175,
        "ATKCurve": 40,
        "Icon": [
            "lgamc"
        ],
        "Grade": 3,
        "Weight": 125,
        "EndureTemplateID": 210,
        "MultiPlayerID": 103,
        "Csx": [
            "ECLegends/地方传奇·重甲蟹"
        ]
    },
    "61622": {
        "_id": 61622,
        "KingdomID": 10,
        "ClassID": 11040,
        "Name": {
            "CH": "「双生妖精骑士」",
            "EN": "The Fairy Knight Twins"
        },
        "Title": [
            {
                "CH": "地方传奇 · 猎刀鳐",
                "EN": "Local Legend: Hunter's Ray"
            }
        ],
        "Desc": {
            "CH": "",
            "EN": ""
        },
        "Color": "Water",
        "HP": 50,
        "HPCurve": 7,
        "ATK": 225,
        "ATKCurve": 40,
        "Icon": [
            "lghtr"
        ],
        "Grade": 3,
        "Weight": 100,
        "EndureTemplateID": 210,
        "MultiPlayerID": 103,
        "Csx": [
            "ECLegends/地方传奇·猎刀鳐"
        ]
    },
    "61623": {
        "_id": 61623,
        "KingdomID": 10,
        "ClassID": 11040,
        "Name": {
            "CH": "「渐渐隐去的老兵」",
            "EN": "Fading Veteran"
        },
        "Title": [
            {
                "CH": "地方传奇 · 膨膨兽",
                "EN": "Local Legend: Blubberbeast"
            }
        ],
        "Desc": {
            "CH": "",
            "EN": ""
        },
        "Color": "Water",
        "HP": 12,
        "HPCurve": 7,
        "ATK": 62.5,
        "ATKCurve": 40,
        "Icon": [
            "lgpfb"
        ],
        "Grade": 3,
        "Weight": 300,
        "EndureTemplateID": 210,
        "MultiPlayerID": 103,
        "Csx": [
            "ECLegends/地方传奇·膨膨兽"
        ]
    },
    "61624": {
        "_id": 61624,
        "KingdomID": 10,
        "ClassID": 11041,
        "Name": {
            "CH": "「极彩的露珂菈」",
            "EN": "Luachra the Brilliant"
        },
        "Title": [
            {
                "CH": "地方传奇 · 大天使海兔",
                "EN": "Local Legend: Cherubic Sea Hare"
            }
        ],
        "Desc": {
            "CH": "",
            "EN": ""
        },
        "Color": "Ice",
        "HP": 54,
        "HPCurve": 7,
        "ATK": 175,
        "ATKCurve": 40,
        "Icon": [
            "UI_MonsterIcon_Aplysia_Mature"
        ],
        "Grade": 3,
        "Weight": 300,
        "EndureTemplateID": 210,
        "MultiPlayerID": 103,
        "Csx": [
            "ECLegends/地方传奇·大天使海兔"
        ]
    },
    "61625": {
        "_id": 61625,
        "KingdomID": 10,
        "ClassID": 11042,
        "Name": {
            "CH": "「夏萨尼翁」",
            "EN": "Chassanion"
        },
        "Title": [
            {
                "CH": "地方传奇 · 帽子水母",
                "EN": "Local Legend: Hat Jellyfish"
            }
        ],
        "Desc": {
            "CH": "",
            "EN": ""
        },
        "Color": "Water",
        "HP": 54,
        "HPCurve": 7,
        "ATK": 175,
        "ATKCurve": 40,
        "Icon": [
            "UI_MonsterIcon_JellyFish_Larva_01",
            "UI_MonsterIcon_JellyFish_Larva_02"
        ],
        "Grade": 3,
        "Weight": 120,
        "EndureTemplateID": 4000,
        "MultiPlayerID": 103,
        "Csx": [
            "ECLegends/地方传奇·帽子水母"
        ]
    },
    "61635": {
        "_id": 61635,
        "KingdomID": 10,
        "ClassID": 11042,
        "Name": {
            "CH": "「魔剑柯鲁日」",
            "EN": "Mageblade Corrouge"
        },
        "Title": [
            {
                "CH": "地方传奇 · 帽子水母",
                "EN": "Local Legend: Hat Jellyfish"
            }
        ],
        "Desc": {
            "CH": "",
            "EN": ""
        },
        "Color": "Water",
        "HP": 54,
        "HPCurve": 7,
        "ATK": 175,
        "ATKCurve": 40,
        "Icon": [
            "UI_MonsterIcon_JellyFish_Larva_01",
            "UI_MonsterIcon_JellyFish_Larva_02"
        ],
        "Grade": 3,
        "Weight": 120,
        "EndureTemplateID": 4000,
        "MultiPlayerID": 103,
        "Csx": [
            "ECLegends/地方传奇·帽子水母"
        ]
    },
    "61701": {
        "_id": 61701,
        "KingdomID": 6,
        "ClassID": 10617,
        "Name": {
            "CH": "铁甲熔火帝皇",
            "EN": "Emperor of Fire and Iron"
        },
        "Title": [
            {
                "CH": "山峦之下鼓动的热潮",
                "EN": "Searing Tide Pulsing Beneath the Mountains"
            }
        ],
        "Desc": {
            "CH": "矗立在原海异种顶端的两位霸主之一，曾经是海员之间的逸闻。\n不遇天敌，能够一直狩猎、进食的重甲蟹会随着成长，不断寻找适合的螺与贝壳，作为新的甲胄。但在最后它长得实在太大了。在第一次涨水期前，曾有巨大的重甲蟹袭击船只，并最终被尚未复职的海军司令贝瑟 · 埃尔顿击退的记录。\n设想这样的世界：将提瓦特所有的人都消除，无限成长下去的铁甲熔火帝皇终究会将沫芒宫穿在身上，作为最为华美的白石铠甲吧。当然在那样的世界自然不会有人半是敬畏，半是戏谑地将它称为「铁甲熔火帝皇」了。",
            "EN": "One of the two sovereigns who once stood at the apex of the aberrant species of the Fontemer, and the subject of many a tale among sailors.\nAn Armored Crab that avoids predation, and can keep hunting and eating, will continue to grow, continually seeking out suitable shells to become its new armor. But eventually, it became simply too gargantuan. Before the first flood period, a massive Armored Crab once attacked a ship, and was finally forced back into the depths by Basil Elton, who had not yet been reinstated as a naval commander.\nImagine this world: All of humanity has been purged from Teyvat, and the ever-growing Emperor of Fire and Iron eventually wears the Palais Mermonia upon its body like a luxurious coat of white stone. Of course, in such a world, there naturally would be no one to call it, half-jokingly and half in awe, the \"Emperor of Fire and Iron.\""
        },
        "Color": "Fire",
        "HP": 6.0,
        "HPCurve": 2,
        "ATK": 9.0,
        "ATKCurve": 4,
        "ExtraSpiralAbyssCoeff": {
            "ATK": 1.33
        },
        "RESBase": {
            "Fire": 0.6
        },
        "RESState": [
            {
                "State": "FauneAbyssale_Horn_Normal",
                "All": 0.6
            },
            {
                "State": "FauneAbyssale_Horn_Abyss",
                "All": 0.8
            }
        ],
        "Ball": {
            "Type": 1,
            "Point": 4,
            "Drop": "3333"
        },
        "Icon": [
            "UI_MonsterIcon_HermitCrab_Primo"
        ],
        "Grade": 3,
        "Weight": 20000,
        "EndureTemplateID": 4000,
        "Tutorial": {
            "CH": "- 初始，<b>身躯上方高处的熔火双角含有<color style='color:#FF0000;'>16U火元素</color></b>：熔火双角存在期间，本体全部抗性提升60%（深渊版本80%）并释放极为危险的攻击，产生造成大量持续伤害的熔岩地板。<br>- 熔火双角免疫扩散的附着。在部分技能期间，熔火双角会更贴近地面；抓住时机使用元素反应将其摧毁。<br>- 摧毁熔火双角后，会失去上述强化效果，但会在30s（深渊版本20s）后再次生成熔火双角。<br>- 深渊版本的攻击力额外提高33%，产生的熔岩地板数量大幅提升。",
            "EN": "- Initially, <b>the molten horns located high up on its body contains <color style='color:#FF0000;'>16U Pyro</color></b>: The horn will increase its all RES by 60% (Abyss Version 80%), and it will unleash deadly attacks, creating Molten Areas that deal massive DoT.<br>- Molten horns are immune to Swirl's element application, and are only reachable when it unleashes certain attacks. Seize this opportunity and destroy it with reactions.<br>- After the horns are destroyed, it will lose all enhancements, but will regenerate its molten horns after 30s (Abyss Version 20s).<br>- Spiral Abyss Version will gain an extra 33% ATK boost, and will create even more Molten Areas."
        },
        "Csx": [
            "ECMysticalBeasts/铁甲熔火帝皇"
        ]
    },
    "61801": {
        "_id": 61801,
        "KingdomID": 6,
        "ClassID": 10617,
        "Name": {
            "CH": "千年珍珠骏麟",
            "EN": "Millennial Pearl Seahorse"
        },
        "Title": [
            {
                "CH": "陆上大湖的无名之主",
                "EN": "Nameless Lord of the Great Lake"
            }
        ],
        "Desc": {
            "CH": "矗立在原海异种顶端的两位霸主之一。\n诗人与作者常因其独行而称之孤傲而高洁，但能成为这类描绘的对象归根到底是因为它身姿修长优美。也因此它为「骏麟」，而重甲蟹的王是「帝皇」。\n在枫丹的传奇逸话中，原海异种当中曾经爆发过无比酷烈的战争，只留下「千年珍珠骏麟」与「铁甲熔火帝皇」在大湖南北分庭抗礼。在《多尔库之歌》中，作者曾经描绘了战争的结局：被称为风暴王的巨大猎刀鳐在死前掀起了巨浪，导致了人们口中所谓的「第一次涨水期」…\n所有的这一切当然只不过是人不愿习惯于毫无幻想可言的现实生活而进行的浪漫想象。原海异种归根到底不过是水生动物罢了，既没有华丽或破败的王庭，也没有昂扬或恶毒的话语。",
            "EN": "One of the two sovereigns standing at the apex of the Fontemer Aberrants.\nPoets and authors often proclaim it to be proud and noble due to its solitary tendencies, but the true reason behind such adulatory descriptions is its elegant and slender body. As such, it is a \"Pearl Seahorse,\" while the suzerain of the Armored Crabs is an \"Emperor.\"\nIn the legends and rumors of Fontaine, a brutal war was once waged among the Aberrants of the Fontemer, leaving only the \"Millennial Pearl Seahorse\" and \"Emperor of Fire and Iron\" splitting the great lake into two courts, one ruling over the north, the other the south. In the \"Song of Dobharcu,\" the writer describes the outcome of the war: the great Hunter's Ray known as the Stormlord stirred huge waves in its death throes, causing what is now known by the people as the \"first diluvian period\"...\nOf course, all of this is nothing more than the romantic imaginations of people who are unwilling to accept the bland realities of the world. For the Fontemer Aberrants are in fact just simple aquatic animals, and they have no grand courts nor fallen kingdoms, nor do they possess words meant to exalt or curse."
        },
        "Color": "Elec",
        "HP": 5.5,
        "HPCurve": 2,
        "ATK": 8.0,
        "ATKCurve": 4,
        "RESBase": {
            "Elec": 0.6
        },
        "RESState": [
            {
                "State": "Alert",
                "All": 2
            },
            {
                "State": "FauneAbyssale_Pearl_Normal",
                "All": 0.6
            },
            {
                "State": "FauneAbyssale_Pearl_Abyss",
                "All": 0.8
            }
        ],
        "Ball": {
            "Type": 3,
            "Point": 4,
            "Drop": "3333"
        },
        "Icon": [
            "UI_MonsterIcon_SeaHorse_Primo_Electric"
        ],
        "Grade": 3,
        "Weight": 250,
        "EndureTemplateID": 400,
        "Tutorial": {
            "CH": "- 初始，<b>头部下方的千年珍珠含有<color style='color:#7030A0;'>19U雷元素</color></b>：千年珍珠存在期间，本体全部抗性提升60%（深渊版本80%）并释放极为危险的攻击，产生造成大量持续伤害的溢雷地板。<br>- 千年珍珠的元素量降低至7U之后，千年珍珠骏麟将会导引「原海古雷」，呼来三枚雷殛的珊瑚珠。珊瑚珠含有0.4U雷元素。每破除一枚珊瑚珠，千年珍珠都会<b>流失三分之一总含量</b>。<br>- 在15s的役雷时间内，原海古雷将持续涤荡大地。若未能破除全部的珊瑚珠，每个存活的珊瑚珠都将为千年珍珠将<b>恢复三分之一总含量+2U</b>。<br>- 摧毁千年珍珠后，会在30s（深渊版本20s）后再次生成千年珍珠。重生的千年珍珠元素量<b>降低至11.4U</b>。",
            "EN": "- Initially, <b>the Millenial Pearl under its head contains<color style='color:#7030A0;'>19U Electro</color></b>: The Pearl will increase its all RES by 60% (Abyss Version 80%), and it will unleash deadly attacks, creating Electrified Areas that deal massive DoT.<br>- When the Pearl's Element drops below 7U, it will call for the Fontemer Hoarthunder, summoning three Hearts of Storm, which contain 0.4U Electro. Destroying each Heart will cause the Pearl to <b>lose a third of its durability</b><br>- During the 15s call, the Fontemer Hoarthunder will continuously descend. If not all Hearts are destroyed in time, each remaining Heart <b>reviving a third of the Pearl's durability plus 2GU</b>.<br>- After the Pearl is destroyed, it will regenerate its Pearl after 30s (Abyss Version 20s). Regenerated Pearl's durability <b>decreases to 11.4GU</b>."
        },
        "SkillDMG": [
            {
                "Name": {
                    "CH": "原海古雷",
                    "EN": "Fontemer Hoarthunder"
                },
                "DMG": [
                    8
                ]
            }
        ],
        "Csx": [
            "ECMysticalBeasts/千年珍珠骏麟"
        ]
    },
    "61901": {
        "_id": 61901,
        "KingdomID": 6,
        "ClassID": 10619,
        "Name": {
            "CH": "隐山猊兽",
            "EN": "Solitary Suanni"
        },
        "Title": [
            {
                "CH": "悠古的群山之主",
                "EN": "Ancient Lord of the Mountains"
            }
        ],
        "Desc": {
            "CH": "优雅而高傲的灵兽，只栖息于仙力萦绕的深山中。\n自古以来，这种仙兽便被认为是福运的象征，其隐居山林、远离世俗纷扰的特质也为历代文人墨客所赞赏。在沉玉谷当地的古老传说中，猊兽的首领名为「山主」，其地位往往被认为与仙人等同。\n另有说法认为，如今伏行山林的玄文兽亦是这些仙兽的后嗣，只是因仙缘浅薄，难以继承如此高贵的血脉，从而退化为凶暴的野兽。这种说法与另一种关于玄文兽起源的说法相违背，但尽管如此，关于猊兽为数不多的记载中，其出现往往伴随着大量的玄文兽作为随从。\n如今，沉玉谷家喻户晓的「舞兽戏」，便是模仿传说中猊兽的动作与神态进行的表演。然而，真正见到过猊兽的人少之又少，绝大多数人只能通过想象来描述这种生灵的模样。数千年的流变与艺术加工后，「兽首」最终也固定成了现今的样式。",
            "EN": "These elegant and proud mystical beasts only live in mountains shrouded in adeptal energy.\nSince ancient times, they have been considered a symbol of good fortune, and their preference for living aloof from the material world has been admired by intellectuals for generations. In ancient legends from Chenyu Vale, the leader of the suanni was called the Mountain Lord and was considered to be of the same standing as the adepti.\nAnother theory claims that the Xuanwen Beasts that roam the mountains these days are the descendants of the suanni, but that the thinning of their blood over the years has caused them to devolve into violent creatures. Although this contradicts another theory regarding the origin of Xuanwen Beasts, the few records of the suanni often show them attended by a large number of Xuanwen Beasts.\nThese days, the well-known Wushou Dance in Chenyu Vale came from the movements of the suanni. But since only a handful of people have actually seen such creatures, most can only imagine what they look like. After thousands of years of changes and different artistic interpretations, the Wushou Dance costume head has developed into its current style."
        },
        "HP": 8.3,
        "HPCurve": 2,
        "ATK": 8.0,
        "ATKCurve": 4,
        "RESBase": {
            "Water": 0.7,
            "Wind": 0.7
        },
        "RESState": [
            {
                "State": "Alert",
                "All": 2
            },
            {
                "State": "Adeptal_Power",
                "All": 2
            }
        ],
        "SkillDMG": [
            {
                "Name": {
                    "CH": "水终结技持续",
                    "EN": "Hydro Ult Continuous"
                },
                "DMG": [
                    5.6
                ]
            },
            {
                "Name": {
                    "CH": "水终结技最终（七圆）",
                    "EN": "Hydro Ult Seven Circles"
                },
                "DMG": [
                    16
                ]
            },
            {
                "Name": {
                    "CH": "风终结技持续",
                    "EN": "Anemo Ult Continuous"
                },
                "DMG": [
                    5.6
                ]
            },
            {
                "Name": {
                    "CH": "风终结技最终（全场）",
                    "EN": "Anemo Ult Entire Arena"
                },
                "DMG": [
                    16
                ]
            }
        ],
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "3333"
        },
        "Icon": [
            "UI_MonsterIcon_Hermit"
        ],
        "Grade": 3,
        "Weight": 20000,
        "EndureTemplateID": 4000,
        "Tutorial": {
            "CH": "栖息于深山中的仙兽，会周期性凝聚仙力，交替释放<b>水终结技</b>和<b>风终结技</b>，期间全部抗性提升200%，持续10秒。<br><br>水终结技：在周围随机地段，持续造成水元素伤害。结束时，在7个大型圆形区域内翻起巨浪，造成巨额水元素伤害。<br><b>期间自身会附着水元素。利用冰元素使其冻结，再使用<color style='color:#f29e38;'>超导、融化、扩散、结晶、碎冰反应</color>破坏其冻结状态，可以解除其抗性提升效果，并使其短暂瘫痪。</b><br><br>风终结技：召唤3枚移动的灵风珠，并在周围随机地段，持续造成风元素伤害。结束时，对全场造成巨额风元素伤害，剩余的灵风珠将转化为涡眼，持续牵引角色并造成伤害。<br><b>利用<color style='color:#f29e38;'>雷、水、冰、火元素</color>触发扩散反应破坏全部灵风珠，可以解除其抗性提升效果，并使其短暂瘫痪。</b><br><br>水终结技和风终结技会交替释放，间隔30s~60s，第一次必定为水。生命值降低至70%时，无视间隔使用一次水终结技。生命值降低至40%时，无视间隔使用一次风终结技。<br><b>深渊特化版：入战后立刻使用水终结技，然后立刻使用风终结技。</b>",
            "EN": "A beast dwelling in the deep mountains. It will periodically draw upon the adeptal powers and alternately unleash <b>Hydro Ult</b> and <b>Anemo Ult</b>. During its Ults, all-type RES is increased by 200% for 10s.<br><br>Hydro Ult: Continuously deal Hydro DMG in random surrounding areas. In the end, summon deadly tides in 7 huge circles, dealing massive Hydro DMG.<br><b>During the Ult it will apply Hydro to itself. Freeze it with Cryo attacks and destroy its Frozen state with the <color style='color:#f29e38;'>Superconduct, Melt, Swirl, Crystallize or Shatter</color> Reactions to remove its RES increase and temporarily paralyze it.</b><br><br>Anemo Ult: Summon 3 moving Gale Orbs and continuously deal Anemo DMG in random surrounding areas. In the end, deal massive Anemo DMG to the entire arena. The Gale Orbs will transform into vertexes, pulling characters in and dealing Anemo DoT. <br><b>Swirl all three Gale Orbs with the <color style='color:#f29e38;'>Electro, Hydro, Cryo or Pyro Elements</color> to remove its RES increase and temporarily paralyze it.</b><br><br>Hydro and Anemo Adeptal Powers are used alternately, at intervals of 30s~60s. The first is always Hydro. When HP falls below 70%, uses Hydro Ult ignoring interval. When HP falls below 40%, uses Anemo Ult ignoring interval.<br><b>Abyss Version: Upon entering battle, immediately use Hydro Ult, and then immediately use Anemo Ult.</b>"
        }
    },
    "61902": {
        "_id": 61902,
        "KingdomID": 6,
        "ClassID": 10619,
        "Name": {
            "CH": "玄文兽",
            "EN": "Xuanwen Beast"
        },
        "Title": [
            {
                "CH": "伏行的玄影",
                "EN": "Assailing Shadow"
            }
        ],
        "Desc": {
            "CH": "栖息在沉玉谷山林间的凶猛野兽，因其深色的纹理而得名「玄文」。\n沉玉谷本地的民话中，在遥远的过去，这些猛兽曾是某位仙人的眷属，降妖伏魔、庇护一方，在仙人故去后，方才退化为凶暴的野兽。传说早已无从查考，仙人的身份也众说纷纭，唯一可以确定的是，出于安全考虑，最好还是不要贸然靠近这些暴躁的生灵。\n千年间，伴随着人类不懈的开拓，原本属于这些野兽的生存空间变得越来越小。时至今日，只有在沉玉谷人迹罕至的崇山峻岭中，才能偶尔看到这些猛兽的身影…",
            "EN": "Xuanwen Beasts, named for the dark patterns on their bodies, are fierce creatures that inhabit the mountains of Chenyu Vale.\nIn the region's folklore, these beasts were the servants of an adeptus in ancient times, tasked with vanquishing evil and protecting the region. However, they devolved into brutal creatures after said adeptus passed away. These legends cannot be verified, and no one knows the true identity of this adeptus. This much is for sure, however — one should stay away from these aggressive creatures for one's safety.\nFor over a thousand years, these beasts have lost a tremendous amount of territory that once belonged to them to relentless human encroachment. These days, they can only occasionally be spotted in the remote mountains of Chenyu Vale..."
        },
        "HP": 2.1,
        "HPCurve": 2,
        "ATK": 4.4,
        "ATKCurve": 4,
        "RESBase": {
            "Water": 0.5,
            "Wind": 0.5
        },
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "1113"
        },
        "Icon": [
            "UI_MonsterIcon_Striped"
        ],
        "Grade": 2,
        "Weight": 150,
        "EndureTemplateID": 150,
        "Tutorial": {
            "CH": "栖息在沉玉谷山林间的凶猛野兽。<br>- 通过元素反应将其冻结，再通过元素反应解除其冻结状态，能使其暂时瘫痪。",
            "EN": "A beast dwelling in the deep mountains.<br>- Freezing it, and then destroying its Frozen state with Elemental Reactions, will temporarily paralyze it."
        },
        "Drop": [
            {
                "Type": 33,
                "Amount": [
                    0,
                    3.9394,
                    1.0505,
                    0.2626
                ]
            }
        ]
    },
    "80203": {
        "_id": 80203,
        "KingdomID": 8,
        "ClassID": 10801,
        "Name": {
            "CH": "大雪猪王",
            "EN": "The Great Snowboar King"
        },
        "Title": [
            {
                "CH": "龙脊雪山的真正王者…？",
                "EN": "The true ruler of Dragonspine?"
            }
        ],
        "Desc": {
            "CH": "生活在冰雪之中，等待着被冰封的同族们复苏的「王者」。\n在无比遥远的过去，荒野的猎人中间流传着这样的传说：当人迹尽没的冰封之境恢复生机，当不速之客搅扰冰穴的安宁，带领雪猪的族群走向壮大的传奇「君王」便会从沉睡中苏醒，抖掉身上积压的冰雪，将那不懂礼数的客人好生教训一顿。",
            "EN": "Amidst the snow and ice dwells an ancient king waiting to be awakened by its ice-entombed brethren.\nIn the distant past beyond memory, a certain legend circulated among hunters out in the wild: when the land sealed by ice returns to life and when an uninvited guest disturbs the deep, frozen silence, the lord and leader of the snowboars unto greatness will awaken from slumber, shake off the ice that shackles it, and bring judgment upon those disrespectful interlopers."
        },
        "Color": "Ice",
        "HP": 11.0,
        "HPCurve": 1,
        "ATK": 5.0,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0.5,
            "Ice": 0.3
        },
        "Ball": {
            "Type": 0,
            "Point": 4,
            "Drop": "6666"
        },
        "Icon": [
            "gsbk"
        ],
        "Grade": 3,
        "Weight": 10000,
        "EndureTemplateID": 400,
        "Csx": [
            "ECAnimals/大雪猪王"
        ]
    },
    "80209": {
        "_id": 80209,
        "KingdomID": 8,
        "ClassID": 11002,
        "Name": {
            "CH": "帽子刀子狗子",
            "EN": "Hat Blade Pup"
        },
        "Title": [
            {
                "CH": "武士狗狗",
                "EN": "Samurai Puppy"
            }
        ],
        "HP": 30.0,
        "HPCurve": 2,
        "ATK": 6.0,
        "ATKCurve": 3,
        "Icon": [
            "cb"
        ],
        "Grade": 3,
        "Weight": 101,
        "EndureTemplateID": 400,
        "Csx": [
            "ECNonCategorized/帽子刀子狗子"
        ]
    },
    "80221": {
        "_id": 80221,
        "KingdomID": 8,
        "ClassID": 10806,
        "Name": {
            "CH": "柔柔羊",
            "EN": "Fluff-Fleece Sheep"
        },
        "Title": [
            {
                "CH": "柔柔羊",
                "EN": "Fluff-Fleece Sheep"
            }
        ],
        "Desc": {
            "CH": "生活在提瓦特大陆各处的兽类。性情温顺，是最早被人类驯服的家畜之一。\n正如其名所示，柔柔羊的绒毛又暖又软，是用来编织衣物的优质材料，而口感柔顺的羊奶，也是深受人们喜爱的美味饮品。传说在遥远的过去，先民的祭祀活动中，这种被称作「柔毛」的生灵也扮演着相当重要的角色——不过，那就是与今日无关的另一个故事了。",
            "EN": "These gentle beasts can be found all over Teyvat and were among the earliest livestock tamed by humans.\nFluff-Fleece Goat, as their name indicates, have soft and warm fur that is used to weave clothing. Humans also love them for their delicious milk. In the distant past, these beasts, often called \"Soft-Furs,\" played an important role in the ceremonial affairs of the region's ancestral people, but that's a story for another time."
        },
        "HP": 0.8,
        "HPCurve": 1,
        "ATK": 1.2,
        "ATKCurve": 3,
        "Icon": [
            "UI_AnimalIcon_Goat_01"
        ],
        "Grade": 0,
        "Weight": 80,
        "EndureTemplateID": 50
    },
    "80601": {
        "_id": 80601,
        "KingdomID": 8,
        "ClassID": 10803,
        "Name": {
            "CH": "棘冠鳄",
            "EN": "Spinocrocodile"
        },
        "Title": [
            {
                "CH": "棘冠鳄",
                "EN": "Spinocrocodile"
            }
        ],
        "Desc": {
            "CH": "栖息于湖泊沼泽的古老生灵。性情凶猛，会从水下发起攻击。\n在须弥这片土地繁衍的历史要比雨林的存在更加悠久。在兽面的神祇仍旧斜倚王座之时，它们就被人们当作神明的使者来供奉。直到今天，沙漠的居民仍然将它们头上赤色的棘冠视为旧日神明的遗物。",
            "EN": "An ancient life form that dwells in the lake swamp. Ferocious by nature, it will attack from underwater.\nIts evolutionary history in this land precedes even the advent of Sumeru's forest. In the age when gods with beast's faces still had dominion over the world, these creatures were worshiped as gods by humans. Even today, the desert dwellers see their spiny red crowns as relics of ancient deities."
        },
        "HP": 0.8,
        "HPCurve": 1,
        "ATK": 0.8,
        "ATKCurve": 3,
        "Icon": [
            "anicr"
        ],
        "Grade": 0,
        "Weight": 80,
        "EndureTemplateID": 50,
        "Csx": [
            "ECAnimals/棘冠鳄"
        ]
    },
    "80602": {
        "_id": 80602,
        "KingdomID": 8,
        "ClassID": 10803,
        "Name": {
            "CH": "长鬓虎",
            "EN": "Rishboland Tiger"
        },
        "Title": [
            {
                "CH": "长鬓虎",
                "EN": "Rishboland Tiger"
            }
        ],
        "Desc": {
            "CH": "栖息在山林之间的猛兽，是森林中的王者。\n将丛林视为自己的领地，平时会像国王一样巡视自己的疆域。如果遇到入侵者，它们会毫不留情的发起进攻。\n鬓角上长着夸张的绒毛，根据学者的研究，似乎有着测量距离的功能。\n传说在丛林中曾经有一匹真正的「虎」，是保卫丛林的灵兽。在它死后，这种形态与猫更加接近的猛兽继承了「虎」的威名。",
            "EN": "A beast that resides in the woods. The king of the jungle.\nIt regards the entire woodland realm as its territory, and will often survey its domain even as a true king might. Any intruder shall be set upon with extreme prejudice.\nIt has almost exaggerated amount of fluff on its temple, and research indicates that this allows it to measure distances.\nIt is said that the forests once played host to a true \"tiger,\" a divine beast that defended the woods. Upon its death, these creatures that resemble cats far more became the successors to its mighty name."
        },
        "HP": 0.8,
        "HPCurve": 1,
        "ATK": 1.2,
        "ATKCurve": 3,
        "Icon": [
            "anirt"
        ],
        "Grade": 0,
        "Weight": 80,
        "EndureTemplateID": 50,
        "Csx": [
            "ECAnimals/长鬓虎"
        ]
    },
    "80603": {
        "_id": 80603,
        "KingdomID": 8,
        "ClassID": 10804,
        "Name": {
            "CH": "赤鹫",
            "EN": "Red Vulture"
        },
        "Title": [
            {
                "CH": "赤鹫",
                "EN": "Red Vulture"
            }
        ],
        "Desc": {
            "CH": "以残骸为食的大型鸟类，赤红色的羽毛在阳光下显得有些稀疏。\n这些鹫鸟并非原生于荒漠中的物种，而是在猩红之王统治的古老文明消亡后才出现在沙漠之中的。同时由于其特殊的生活习性，在某些沙漠遗民看来，它们乃是带来死亡与灾厄的生物。",
            "EN": "This large bird consumes remains, and its red feathers often seem a little sparse under the sunlight.\nThese members of the eagle family are not actually native to the desert, instead appearing in the desert after the disappearance of the ancient civilization ruled by King Deshret. They have also come to be seen as harbingers of death or disaster by some migrants from the desert due to their unique living habits."
        },
        "HP": 0.6,
        "HPCurve": 1,
        "ATK": 1.0,
        "ATKCurve": 3,
        "Icon": [
            "anirv"
        ],
        "Grade": 0,
        "Weight": 80,
        "EndureTemplateID": 50,
        "Csx": [
            "ECAnimals/赤鹫"
        ]
    },
    "80604": {
        "_id": 80604,
        "KingdomID": 8,
        "ClassID": 10804,
        "Name": {
            "CH": "毒蝎",
            "EN": "Scorpion"
        },
        "Title": [
            {
                "CH": "毒蝎",
                "EN": "Scorpion"
            }
        ],
        "Desc": {
            "CH": "生活在砂岩之下的大型节肢动物，擅长利用毒液捕猎。\n因为在受到攻击时会分泌出含有剧毒的液体，因此在很久之前就被诸多阴谋家和炼金术士们盯上，成为了各类毒药的常用原料。果然比起毒蝎，还是人心要更加可怕。",
            "EN": "Large arthropods that live beneath the sandstone that specialize in using their venom to hunt.\nDue to their ability to secrete highly toxic fluids when under attack, they have long been much sought-after by plotters and alchemists alike, with these fluids becoming common ingredients in poison-making. Indeed, human hearts are yet more dangerous than venomous scorpions."
        },
        "HP": 0.8,
        "HPCurve": 1,
        "ATK": 1.2,
        "ATKCurve": 3,
        "Icon": [
            "anisc"
        ],
        "Grade": 0,
        "Weight": 80,
        "EndureTemplateID": 50,
        "Csx": [
            "ECAnimals/毒蝎"
        ]
    },
    "806051": {
        "_id": 806051,
        "KingdomID": 8,
        "ClassID": 10802,
        "Name": {
            "CH": "驮兽-小",
            "EN": "Small Sumpter Beast"
        },
        "Title": [
            {
                "CH": "驮兽-小",
                "EN": "Small Sumpter Beast"
            }
        ],
        "Desc": {
            "CH": "驮兽是人类的朋友。它们性格温顺，是重要的生产运输工具。\n在须弥，拥有驮兽的数量是财富的象征。大型的商队和镀金旅团往往拥有一支庞大的驮兽队伍，这使他们得以装载足够的补给与货物以跨越沙漠与丛林。\n驮兽能够适应各种不同的自然环境。早在雨林中的商贾注意到它们之前，这些勤劳的生灵便已是沙漠部族赖以生存的家畜。",
            "EN": "Sumpter Beasts are friends of humanity. These gentle-hearted creatures are indispensable elements of production and transportation.\nIn Sumeru, your wealth is symbolized by the number of Sumpter Beasts you own. Large merchant caravans and Eremites usually come with very sizable trains of Sumpter Beasts indeed, and this allows them to carry ample supplies and goods across both desert and forest alike.\nSumpter Beasts can adapt to various different natural environments. Long before the forest merchants ever noticed them, these hardworking animals were already the much-relied-upon domesticated beasts of the desert peoples."
        },
        "HP": 0.2,
        "HPCurve": 1,
        "ATK": 1.0,
        "ATKCurve": 3,
        "Icon": [
            "anisb"
        ],
        "Grade": 0,
        "Weight": 300,
        "EndureTemplateID": 4000,
        "Csx": [
            "ECAnimals/驮兽"
        ]
    },
    "806052": {
        "_id": 806052,
        "KingdomID": 8,
        "ClassID": 10802,
        "Name": {
            "CH": "驮兽-中",
            "EN": "Medium Sumpter Beast"
        },
        "Title": [
            {
                "CH": "驮兽-中",
                "EN": "Medium Sumpter Beast"
            }
        ],
        "Desc": {
            "CH": "驮兽是人类的朋友。它们性格温顺，是重要的生产运输工具。\n在须弥，拥有驮兽的数量是财富的象征。大型的商队和镀金旅团往往拥有一支庞大的驮兽队伍，这使他们得以装载足够的补给与货物以跨越沙漠与丛林。\n驮兽能够适应各种不同的自然环境。早在雨林中的商贾注意到它们之前，这些勤劳的生灵便已是沙漠部族赖以生存的家畜。",
            "EN": "Sumpter Beasts are friends of humanity. These gentle-hearted creatures are indispensable elements of production and transportation.\nIn Sumeru, your wealth is symbolized by the number of Sumpter Beasts you own. Large merchant caravans and Eremites usually come with very sizable trains of Sumpter Beasts indeed, and this allows them to carry ample supplies and goods across both desert and forest alike.\nSumpter Beasts can adapt to various different natural environments. Long before the forest merchants ever noticed them, these hardworking animals were already the much-relied-upon domesticated beasts of the desert peoples."
        },
        "HP": 0.6,
        "HPCurve": 1,
        "ATK": 1.4,
        "ATKCurve": 3,
        "Icon": [
            "anisb"
        ],
        "Grade": 0,
        "Weight": 300,
        "EndureTemplateID": 4000,
        "Csx": [
            "ECAnimals/驮兽"
        ]
    },
    "806053": {
        "_id": 806053,
        "KingdomID": 8,
        "ClassID": 10802,
        "Name": {
            "CH": "驮兽-大",
            "EN": "Large Sumpter Beast"
        },
        "Title": [
            {
                "CH": "驮兽-大",
                "EN": "Large Sumpter Beast"
            }
        ],
        "Desc": {
            "CH": "驮兽是人类的朋友。它们性格温顺，是重要的生产运输工具。\n在须弥，拥有驮兽的数量是财富的象征。大型的商队和镀金旅团往往拥有一支庞大的驮兽队伍，这使他们得以装载足够的补给与货物以跨越沙漠与丛林。\n驮兽能够适应各种不同的自然环境。早在雨林中的商贾注意到它们之前，这些勤劳的生灵便已是沙漠部族赖以生存的家畜。",
            "EN": "Sumpter Beasts are friends of humanity. These gentle-hearted creatures are indispensable elements of production and transportation.\nIn Sumeru, your wealth is symbolized by the number of Sumpter Beasts you own. Large merchant caravans and Eremites usually come with very sizable trains of Sumpter Beasts indeed, and this allows them to carry ample supplies and goods across both desert and forest alike.\nSumpter Beasts can adapt to various different natural environments. Long before the forest merchants ever noticed them, these hardworking animals were already the much-relied-upon domesticated beasts of the desert peoples."
        },
        "HP": 1.0,
        "HPCurve": 1,
        "ATK": 1.8,
        "ATKCurve": 3,
        "Icon": [
            "anisb"
        ],
        "Grade": 0,
        "Weight": 300,
        "EndureTemplateID": 4000,
        "Csx": [
            "ECAnimals/驮兽"
        ]
    },
    "806061": {
        "_id": 806061,
        "KingdomID": 8,
        "ClassID": 10802,
        "Name": {
            "CH": "背包驮兽-小",
            "EN": "Small Sumpter Beast with Package"
        },
        "Title": [
            {
                "CH": "背包驮兽-小",
                "EN": "Small Sumpter Beast with Package"
            }
        ],
        "Desc": {
            "CH": "驮兽是人类的朋友。它们性格温顺，是重要的生产运输工具。\n在须弥，拥有驮兽的数量是财富的象征。大型的商队和镀金旅团往往拥有一支庞大的驮兽队伍，这使他们得以装载足够的补给与货物以跨越沙漠与丛林。\n驮兽能够适应各种不同的自然环境。早在雨林中的商贾注意到它们之前，这些勤劳的生灵便已是沙漠部族赖以生存的家畜。",
            "EN": "Sumpter Beasts are friends of humanity. These gentle-hearted creatures are indispensable elements of production and transportation.\nIn Sumeru, your wealth is symbolized by the number of Sumpter Beasts you own. Large merchant caravans and Eremites usually come with very sizable trains of Sumpter Beasts indeed, and this allows them to carry ample supplies and goods across both desert and forest alike.\nSumpter Beasts can adapt to various different natural environments. Long before the forest merchants ever noticed them, these hardworking animals were already the much-relied-upon domesticated beasts of the desert peoples."
        },
        "HP": 0.8,
        "HPCurve": 1,
        "ATK": 1.0,
        "ATKCurve": 3,
        "Icon": [
            "anisb"
        ],
        "Grade": 0,
        "Weight": 300,
        "EndureTemplateID": 4000,
        "Csx": [
            "ECAnimals/驮兽"
        ]
    },
    "806062": {
        "_id": 806062,
        "KingdomID": 8,
        "ClassID": 10802,
        "Name": {
            "CH": "背包驮兽-中",
            "EN": "Medium Sumpter Beast with Package"
        },
        "Title": [
            {
                "CH": "背包驮兽-中",
                "EN": "Medium Sumpter Beast with Package"
            }
        ],
        "Desc": {
            "CH": "驮兽是人类的朋友。它们性格温顺，是重要的生产运输工具。\n在须弥，拥有驮兽的数量是财富的象征。大型的商队和镀金旅团往往拥有一支庞大的驮兽队伍，这使他们得以装载足够的补给与货物以跨越沙漠与丛林。\n驮兽能够适应各种不同的自然环境。早在雨林中的商贾注意到它们之前，这些勤劳的生灵便已是沙漠部族赖以生存的家畜。",
            "EN": "Sumpter Beasts are friends of humanity. These gentle-hearted creatures are indispensable elements of production and transportation.\nIn Sumeru, your wealth is symbolized by the number of Sumpter Beasts you own. Large merchant caravans and Eremites usually come with very sizable trains of Sumpter Beasts indeed, and this allows them to carry ample supplies and goods across both desert and forest alike.\nSumpter Beasts can adapt to various different natural environments. Long before the forest merchants ever noticed them, these hardworking animals were already the much-relied-upon domesticated beasts of the desert peoples."
        },
        "HP": 1.2,
        "HPCurve": 1,
        "ATK": 1.4,
        "ATKCurve": 3,
        "Icon": [
            "anisb"
        ],
        "Grade": 0,
        "Weight": 300,
        "EndureTemplateID": 4000,
        "Csx": [
            "ECAnimals/驮兽"
        ]
    },
    "806063": {
        "_id": 806063,
        "KingdomID": 8,
        "ClassID": 10802,
        "Name": {
            "CH": "背包驮兽-大",
            "EN": "Large Sumpter Beast with Package"
        },
        "Title": [
            {
                "CH": "背包驮兽-大",
                "EN": "Large Sumpter Beast with Package"
            }
        ],
        "Desc": {
            "CH": "驮兽是人类的朋友。它们性格温顺，是重要的生产运输工具。\n在须弥，拥有驮兽的数量是财富的象征。大型的商队和镀金旅团往往拥有一支庞大的驮兽队伍，这使他们得以装载足够的补给与货物以跨越沙漠与丛林。\n驮兽能够适应各种不同的自然环境。早在雨林中的商贾注意到它们之前，这些勤劳的生灵便已是沙漠部族赖以生存的家畜。",
            "EN": "Sumpter Beasts are friends of humanity. These gentle-hearted creatures are indispensable elements of production and transportation.\nIn Sumeru, your wealth is symbolized by the number of Sumpter Beasts you own. Large merchant caravans and Eremites usually come with very sizable trains of Sumpter Beasts indeed, and this allows them to carry ample supplies and goods across both desert and forest alike.\nSumpter Beasts can adapt to various different natural environments. Long before the forest merchants ever noticed them, these hardworking animals were already the much-relied-upon domesticated beasts of the desert peoples."
        },
        "HP": 1.6,
        "HPCurve": 1,
        "ATK": 1.8,
        "ATKCurve": 3,
        "Icon": [
            "anisb"
        ],
        "Grade": 0,
        "Weight": 300,
        "EndureTemplateID": 4000,
        "Csx": [
            "ECAnimals/驮兽"
        ]
    },
    "80615": {
        "_id": 80615,
        "KingdomID": 8,
        "ClassID": 10804,
        "Name": {
            "CH": "飞蛇",
            "EN": "Flying Serpent"
        },
        "Title": [
            {
                "CH": "飞蛇",
                "EN": "Flying Serpent"
            }
        ],
        "Desc": {
            "CH": "一定程度上能够悬浮的爬行动物。\n飞蛇并不具备「飞行」的能力，悬浮的能力似乎是借助了与鸟类相同的中空骨骼，与其中风元素的残渣。沙漠不是风元素的国度，但其中常年呼啸着侵蚀一切的风。无论是城池或是历史，都会被沙海的风慢慢磨成细沙。只有能与这种风共生的动物，才能在一切都回归尘土后依然生存在沙海中。",
            "EN": "A reptile with a limited ability to hover.\nFlying serpents do not truly possess the ability to fly; their ability to hover seems to derive from their bird-like hollow bones and the sparkle of Anemo that imbues them with a subtle influence over the wind. The desert is a far way from the nation of Anemo, but its lands are still ever-graced by gales intent on eroding everything. Everything, from cities to history, will eventually be razed to sand grains by the winnowing currents that drift over the Great Red Sand. Only creatures such as the flying serpents, which have learned to live with the wind, can continue to survive in the desert long after all has been reduced to dust."
        },
        "HP": 0.6,
        "HPCurve": 1,
        "ATK": 0.8,
        "ATKCurve": 3,
        "Icon": [
            "anifs"
        ],
        "Grade": 0,
        "Weight": 80,
        "EndureTemplateID": 50,
        "Csx": [
            "ECAnimals/飞蛇"
        ]
    },
    "90101": {
        "_id": 90101,
        "KingdomID": 9,
        "ClassID": 10901,
        "Name": {
            "CH": "风魔龙 · 特瓦林",
            "EN": "Stormterror Dvalin"
        },
        "Title": [
            {
                "CH": "往日的天空之王",
                "EN": "Erstwhile King of the Skies"
            }
        ],
        "Desc": {
            "CH": "蒙德的「四风守护」之一，东风之龙特瓦林。\n在漫长的岁月与无边的黑暗中，清澈的宝石也会蒙尘而变得黯淡，高傲的龙也会被侵蚀而充满了忿恨。\n不过，要记住，尘埃能被轻易拭去，疑惧与毒也能被粉碎。\n\n<i>「自由地在深空中翱翔吧。</i>\n<i>「就如当年你我相遇时，我行走在大地上，唱着风的歌曲；</i>\n<i>「你则在大气稀薄的高天上，俯瞰世界。</i>\n<i>「如今的天空中没有风雪，大地上充满了绿草，会很美丽吧。</i>\n<i>「如果你也在地面上行走过，听过牧歌和风铃在远方错落的歌，尝过蜜一样甜的酒，就会明白吧。</i>\n<i>「如果你曾是我，看见过你在蓝蓝的天上滑翔的孤高模样，见识过你的美丽身姿就好了。</i>\n<i>「如此，你就会明白，这样的天空与大地，是值得为之奋战的。」</i>",
            "EN": "One of The Four Winds of Mondstadt: Dvalin, the Dragon of the East.\nWith the passage of long years and amid boundless darkness, even the purest gem will become dulled by dust, and even the noble dragon might be corrupted and cankered by hatred.\nBut never forget that dust can be wiped away, and the power of doubt and poison can be shattered.\n\n<i>\"Fly freely in the sky.</i>\n<i>\"Just like you did in those days when we met, when I walked upon this land and sang the songs of the wind,</i>\n<i>\"And you soared on high, looking down on the world from above.</i>\n<i>\"Today, the sky is no longer swept by wind and snow. The green grass grows all over the earth, and it is beautiful.</i>\n<i>\"If you too could walk the earth and listen to the songs of shepherds and the scattered sound of wind chimes in the distance, if you could have tasted wine as sweet as honey, you would understand.</i>\n<i>\"How wonderful it would have been if you were as I, witnessing your noble, beautiful form as you soared through the blue sky.</i>\n<i>\"Then you would know that this sky and this earth are both things worth fighting for.\"</i>"
        },
        "Color": "Wind",
        "HP": 9.0,
        "HPCurve": 1,
        "ATK": 6.0,
        "ATKCurve": 4,
        "Icon": [
            "dv"
        ],
        "Grade": 4,
        "Weight": 20000,
        "EndureTemplateID": 4000,
        "Csx": [
            "ECEnemiesOfNote/特瓦林"
        ]
    },
    "90201": {
        "_id": 90201,
        "KingdomID": 9,
        "ClassID": 10902,
        "Name": {
            "CH": "安德留斯",
            "EN": "Andrius"
        },
        "Title": [
            {
                "CH": "奔狼的领主",
                "EN": "Dominator of Wolves"
            }
        ],
        "Desc": {
            "CH": "守望着奔狼领的「卢皮卡」，无比崇高的伟大灵魂。\n当群狼受到威胁时，会以狼的姿态现世，展现自己的苍牙与利爪。\n\n奔狼领的狼对蒙德人而言，通常是诡谲的魅影：\n树木之间一闪而过的模糊形象、方位难辨的凄嚎，以及独行郊野林中时，时常能隐约感受到的视线。\n人与狼冰冷的双眸直视的机会很少，因为「王狼」为自己的族类订立了规矩。",
            "EN": "The noblest and greatest of souls, who watches over the lupical of Wolvendom.\nWhen the wolfpack is imperiled, it will emerge in the form of a wolf and show forth its fangs and claws.\n\nThe wolves of Wolvendom are frightening phantoms to the people of Mondstadt:\nA blurred shape flashing through the forest, a chilling howl in the indeterminate distance, the creeping sense of eyes constantly nailed to their backs...\nHumans rarely have the opportunity to lock their eyes with a wolf, for this is the law Lupus Boreas laid down for its race."
        },
        "HP": 18.0,
        "HPCurve": 1,
        "ATK": 7.2,
        "ATKCurve": 3,
        "RESBase": {
            "Ice": 0.5,
            "Wind": 0.3
        },
        "Immune": [
            "Ice",
            "Wind"
        ],
        "Icon": [
            "and"
        ],
        "Grade": 4,
        "Weight": 20000,
        "EndureTemplateID": 4000,
        "Csx": [
            "ECEnemiesOfNote/安德留斯"
        ]
    },
    "90301": {
        "_id": 90301,
        "KingdomID": 9,
        "ClassID": 10903,
        "Name": {
            "CH": "「公子」一阶段",
            "EN": "Childe P1"
        },
        "Title": [
            {
                "CH": "愚人众执行官末席",
                "EN": "Eleventh of the Fatui Harbingers"
            }
        ],
        "Desc": {
            "CH": "愚人众执行官第十一席，「公子」达达利亚。\n自不祥的「邪眼」之中汲取力量，以至暗之地习得的武艺战斗。\n\n拥有无穷争斗欲望的纯粹战士。\n每次浴血的纷争、每场生死攸关的挑战，于他而言皆是快意试炼。\n人皆言年轻的「公子」战绩远近闻名。\n但他本人却对这种大话从来不以为然。\n\n毕竟…\n<i>「你的命运将成为纷争的风暴之眼，</i>\n<i>「铁剑交错的声响是常伴你的奏乐。</i>\n<i>「踏入至暗之处的漆黑记忆，</i>\n<i>「终将成为你颠覆世界的力量吧。」</i>",
            "EN": "\"Childe\" Tartaglia, Eleventh of the Fatui Harbingers.\nHe draws power from the ominous Delusion he possesses and fights using martial arts that he learned in a land of darkness.\n\nHe is a pure warrior with an insatiable lust for battle.\nEach bloody conflict, each life-and-death struggle is a delightful trial to him.\nPeople say that the young Childe is famed throughout the land for his battle prowess.\nBut he has never taken big talk of this kind to heart.\n\nAfter all...\n<i>\"You shall ever be the eye of the storm,</i>\n<i>\"And the clashing of steel shall ever accompany you.</i>\n<i>\"The pitch-black memory of stepping into uttermost darkness,</i>\n<i>\"Shall, at last, become the strength by which you will overturn this world.\"</i>"
        },
        "Color": "Water",
        "HP": 6.0,
        "HPCurve": 1,
        "ATK": 10.0,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0,
            "Fire": 0,
            "Water": 0.5,
            "Elec": 0,
            "Ice": 0,
            "Grass": 0,
            "Wind": 0,
            "Rock": 0
        },
        "RESState": [
            {
                "State": "Tartaglia_Weak",
                "All": -0.3
            }
        ],
        "Icon": [
            "chi1"
        ],
        "Grade": 4,
        "Weight": 150,
        "EndureTemplateID": 500,
        "Csx": [
            "ECEnemiesOfNote/公子1"
        ]
    },
    "90302": {
        "_id": 90302,
        "KingdomID": 9,
        "ClassID": 10903,
        "Name": {
            "CH": "「公子」二阶段",
            "EN": "Childe P2"
        },
        "Title": [
            {
                "CH": "邪眼解放",
                "EN": "Delusion Unleashed"
            }
        ],
        "Desc": {
            "CH": "愚人众执行官第十一席，「公子」达达利亚。\n自不祥的「邪眼」之中汲取力量，以至暗之地习得的武艺战斗。\n\n拥有无穷争斗欲望的纯粹战士。\n每次浴血的纷争、每场生死攸关的挑战，于他而言皆是快意试炼。\n人皆言年轻的「公子」战绩远近闻名。\n但他本人却对这种大话从来不以为然。\n\n毕竟…\n<i>「你的命运将成为纷争的风暴之眼，</i>\n<i>「铁剑交错的声响是常伴你的奏乐。</i>\n<i>「踏入至暗之处的漆黑记忆，</i>\n<i>「终将成为你颠覆世界的力量吧。」</i>",
            "EN": "\"Childe\" Tartaglia, Eleventh of the Fatui Harbingers.\nHe draws power from the ominous Delusion he possesses and fights using martial arts that he learned in a land of darkness.\n\nHe is a pure warrior with an insatiable lust for battle.\nEach bloody conflict, each life-and-death struggle is a delightful trial to him.\nPeople say that the young Childe is famed throughout the land for his battle prowess.\nBut he has never taken big talk of this kind to heart.\n\nAfter all...\n<i>\"You shall ever be the eye of the storm,</i>\n<i>\"And the clashing of steel shall ever accompany you.</i>\n<i>\"The pitch-black memory of stepping into uttermost darkness,</i>\n<i>\"Shall, at last, become the strength by which you will overturn this world.\"</i>"
        },
        "Color": "Elec",
        "HP": 6.0,
        "HPCurve": 1,
        "ATK": 10.0,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0,
            "Fire": 0,
            "Water": 0,
            "Elec": 0.5,
            "Ice": 0,
            "Grass": 0,
            "Wind": 0,
            "Rock": 0
        },
        "RESState": [
            {
                "State": "Tartaglia_Weak",
                "All": -0.5
            }
        ],
        "Icon": [
            "chi2"
        ],
        "Grade": 4,
        "Weight": 150,
        "EndureTemplateID": 500,
        "Csx": [
            "ECEnemiesOfNote/公子2"
        ]
    },
    "90303": {
        "_id": 90303,
        "KingdomID": 9,
        "ClassID": 10903,
        "Name": {
            "CH": "「公子」三阶段",
            "EN": "Childe P3"
        },
        "Title": [
            {
                "CH": "魔王武装",
                "EN": "Foul Legacy Transformation"
            }
        ],
        "Desc": {
            "CH": "愚人众执行官第十一席，「公子」达达利亚。\n自不祥的「邪眼」之中汲取力量，以至暗之地习得的武艺战斗。\n\n拥有无穷争斗欲望的纯粹战士。\n每次浴血的纷争、每场生死攸关的挑战，于他而言皆是快意试炼。\n人皆言年轻的「公子」战绩远近闻名。\n但他本人却对这种大话从来不以为然。\n\n毕竟…\n<i>「你的命运将成为纷争的风暴之眼，</i>\n<i>「铁剑交错的声响是常伴你的奏乐。</i>\n<i>「踏入至暗之处的漆黑记忆，</i>\n<i>「终将成为你颠覆世界的力量吧。」</i>",
            "EN": "\"Childe\" Tartaglia, Eleventh of the Fatui Harbingers.\nHe draws power from the ominous Delusion he possesses and fights using martial arts that he learned in a land of darkness.\n\nHe is a pure warrior with an insatiable lust for battle.\nEach bloody conflict, each life-and-death struggle is a delightful trial to him.\nPeople say that the young Childe is famed throughout the land for his battle prowess.\nBut he has never taken big talk of this kind to heart.\n\nAfter all...\n<i>\"You shall ever be the eye of the storm,</i>\n<i>\"And the clashing of steel shall ever accompany you.</i>\n<i>\"The pitch-black memory of stepping into uttermost darkness,</i>\n<i>\"Shall, at last, become the strength by which you will overturn this world.\"</i>"
        },
        "HP": 10.0,
        "HPCurve": 1,
        "ATK": 14.0,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0,
            "Fire": 0,
            "Water": 0.7,
            "Elec": 0.7,
            "Ice": 0,
            "Grass": 0,
            "Wind": 0,
            "Rock": 0
        },
        "Icon": [
            "chi3"
        ],
        "Grade": 4,
        "Weight": 20000,
        "EndureTemplateID": 4000,
        "Csx": [
            "ECEnemiesOfNote/公子3"
        ]
    },
    "90401": {
        "_id": 90401,
        "KingdomID": 9,
        "ClassID": 10904,
        "Name": {
            "CH": "若陀龙王",
            "EN": "Azhdaha"
        },
        "Title": [
            {
                "CH": "被封印的岩龙之王",
                "EN": "Sealed Lord of Vishaps"
            }
        ],
        "Desc": {
            "CH": "如同山岳一般长寿的古老巨龙。\n在难以追忆的年月里曾与岩港的帝君并肩，但最终反目，遭镇压于黑暗的地底。在无比漫长的岁月中，它的力量渐渐流失，形态也因各种折磨而扭曲。\n枷锁的隐隐震响与龙祖低沉的怒吼，同记忆一般在山峦间回荡。",
            "EN": "An enormous dragon as ancient as the mountains themselves.\nIn an age that has all but faded from memory, he stood shoulder to shoulder with one who ruled over a harbor of stone. But in the end, the two came into conflict, and the dragon was banished to a dark place deep underground. Over the long years of his imprisonment, his power has slowly dissipated. He has also become disfigured from the various kinds of erosion he has been subjected to.\nThe faint rattling of this dragon lord's shackles and his deep, angry growl echo through the bowels of the mountains like memories of a bygone era."
        },
        "Color": "Rock",
        "HP": 36.0,
        "HPCurve": 1,
        "ATK": 9.0,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": 0.4,
            "Rock": 0.7
        },
        "RESModify": [
            {
                "State": "Drake_Absorb_1",
                "Show": {
                    "CH": "对应抗性提升60%",
                    "EN": "Corresponding Element increase 60%"
                }
            },
            {
                "State": "Drake_Absorb_2",
                "Show": {
                    "CH": "对应抗性提升50%",
                    "EN": "Corresponding Element increase 50%"
                }
            }
        ],
        "Icon": [
            "azh"
        ],
        "Grade": 4,
        "Weight": 20000,
        "EndureTemplateID": 4000,
        "MultiPlayerID": 103,
        "Csx": [
            "ECEnemiesOfNote/若陀龙王·岩",
            "ECEnemiesOfNote/若陀龙王·火",
            "ECEnemiesOfNote/若陀龙王·水",
            "ECEnemiesOfNote/若陀龙王·雷",
            "ECEnemiesOfNote/若陀龙王·冰"
        ]
    },
    "90501": {
        "_id": 90501,
        "KingdomID": 9,
        "ClassID": 10905,
        "Name": {
            "CH": "「女士」一阶段",
            "EN": "La Signora P1"
        },
        "Title": [
            {
                "CH": "愚人众执行官第八席",
                "EN": "Eighth of the Fatui Harbingers"
            }
        ],
        "Desc": {
            "CH": "愚人众执行官第八席的「女士」。\n与行使邪眼之权能的其他愚人众不同，邪眼是抑制啃噬一切的烈火的器物。比起厮杀用的兵器，说是刑拘道具才更为贴切。\n而这就是漫长的岁月中积郁的余烬，冲破邪眼霜封后为她涂抹的扭曲妆容。\n\n瞳仁中倒映着破晓的赤红，她最后展开烈焰之翼向黎明飞去。\n<i>「但那并不是曙光，亲爱的罗莎琳。那是焚尽一切的火海。」</i>\n在光芒中，一个声音这么对她说。\n\n但这也没什么所谓。因为她心中明白，自己早已被烈火吞没。",
            "EN": "Signora, the Eighth of the Fatui Harbingers.\nUnlike other Fatui who use the authority granted by Delusions, her Delusion is meant to shackle a flame that will devour everything. As opposed to a weapon used to kill, it might be more fitting to consider this a tool of imprisonment.\nAnd it was the embers of that flame, which had accumulated over many long years, that would break through the frosty seal of the Delusion and apply a warped rouge upon her.\n\nThe crimson dawn was reflected in her pupils, and at last, she unfolded her flaming wings and flew towards the light.\n<i>\"But that light is not the dawn, dear Rosalyne. That is a sea of flame that will consume everything.\"</i>\nA voice spoke to her amidst the light.\n\nYet it mattered not, for she knew in her heart that the flames had devoured her long ago."
        },
        "Color": "Ice",
        "HP": 5.0,
        "HPCurve": 2,
        "ATK": 4.8,
        "ATKCurve": 3,
        "RESBase": {
            "Ice": 0.5
        },
        "Icon": [
            "las2"
        ],
        "Grade": 4,
        "Weight": 150,
        "EndureTemplateID": 4000,
        "MultiPlayerID": 103,
        "Csx": [
            "ECEnemiesOfNote/女士1"
        ]
    },
    "90502": {
        "_id": 90502,
        "KingdomID": 9,
        "ClassID": 10905,
        "Name": {
            "CH": "焚尽的炽炎魔女",
            "EN": "Crimson Witch of Embers"
        },
        "Title": [
            {
                "CH": "罗莎琳 · 克鲁兹希卡 · 洛厄法特",
                "EN": "Rosalyne-Kruzchka Lohefalter"
            }
        ],
        "Desc": {
            "CH": "愚人众执行官第八席的「女士」。\n与行使邪眼之权能的其他愚人众不同，邪眼是抑制啃噬一切的烈火的器物。比起厮杀用的兵器，说是刑拘道具才更为贴切。\n而这就是漫长的岁月中积郁的余烬，冲破邪眼霜封后为她涂抹的扭曲妆容。\n\n瞳仁中倒映着破晓的赤红，她最后展开烈焰之翼向黎明飞去。\n<i>「但那并不是曙光，亲爱的罗莎琳。那是焚尽一切的火海。」</i>\n在光芒中，一个声音这么对她说。\n\n但这也没什么所谓。因为她心中明白，自己早已被烈火吞没。",
            "EN": "Signora, the Eighth of the Fatui Harbingers.\nUnlike other Fatui who use the authority granted by Delusions, her Delusion is meant to shackle a flame that will devour everything. As opposed to a weapon used to kill, it might be more fitting to consider this a tool of imprisonment.\nAnd it was the embers of that flame, which had accumulated over many long years, that would break through the frosty seal of the Delusion and apply a warped rouge upon her.\n\nThe crimson dawn was reflected in her pupils, and at last, she unfolded her flaming wings and flew towards the light.\n<i>\"But that light is not the dawn, dear Rosalyne. That is a sea of flame that will consume everything.\"</i>\nA voice spoke to her amidst the light.\n\nYet it mattered not, for she knew in her heart that the flames had devoured her long ago."
        },
        "Color": "Fire",
        "HP": 10.0,
        "HPCurve": 2,
        "ATK": 4.8,
        "ATKCurve": 3,
        "RESBase": {
            "Fire": 0.7
        },
        "Icon": [
            "las2"
        ],
        "Grade": 4,
        "Weight": 20000,
        "EndureTemplateID": 4000,
        "MultiPlayerID": 103,
        "Csx": [
            "ECEnemiesOfNote/女士2"
        ]
    },
    "90602": {
        "_id": 90602,
        "KingdomID": 9,
        "ClassID": 10906,
        "Name": {
            "CH": "祸津御建鸣神命",
            "EN": "Magatsu Mitake Narukami no Mikoto"
        },
        "Title": [
            {
                "CH": "雷电之稻妻殿",
                "EN": "Raiden no Inazuma Tono"
            }
        ],
        "Desc": {
            "CH": "代行雷电将军权能的人偶，在对抗威胁「永恒」之敌时展现出的形态。\n作为雷电影践行「永恒」的协助者，她从未质疑，亦无畏惧，一直以来全力以赴，心无杂念。\n她曾听闻在真与影的时代，影承担着与她类似的责任，经历了诸多惨痛失去。\n当两个影子交叠为一时，她却只能看见在「她」本应永恒的意志上留下的无数细密的凿痕吧。既然命运注定是一场轮回，就如约定那样，向其宣战吧。\n\n<i>「此身，象征无上威严，被授予统领一国之权，凝聚了『雷电将军』的一切。</i>\n<i>「自然也继承了影前行时不断失去的痛苦，以及，想要抵达永恒的决心。</i>\n<i>「决心、勇气、爱慕、仇恨…都会在时间长河中变质扭曲。</i>\n<i>「唯有『法则』永恒不变。」</i>",
            "EN": "This is the form that the puppet that inherited the Raiden Shogun's authority takes when she is opposing those who threaten Eternity.\nAs the Raiden Shogun's assistant in pursuing eternity, she has never doubted, never felt fear, never given anything less than her best, never been disturbed by idle thoughts.\nShe once heard about how Ei had shouldered a similar duty when Makoto was also present, suffering much agonizing loss in the process.\nWhen the two shadows overlapped once more, however, she could only see the countless fine fractures in what should have been \"her\" eternal will. Since fate is destined for cyclicality, she must fulfill her promise and declare war on \"her.\"\n\n<i>Mine is the most supreme and noble form. Power over the realm has been vested within me, and I am the essence of all that is the \"Raiden Shogun.\"</i>\n<i>\"Naturally, I have also inherited the pain of Ei's countless losses and her will to attain Eternity.</i>\n<i>\"Resolve, valor, love, hate... They will all twist in the river of time.</i>\n<i>\"But the 'rules' will never change.\"</i>"
        },
        "Color": "Elec",
        "HP": 20.0,
        "HPCurve": 2,
        "ATK": 12.0,
        "ATKCurve": 4,
        "RESState": [
            {
                "State": "MitakeNarukami",
                "All": 2.0
            }
        ],
        "Icon": [
            "mag"
        ],
        "Grade": 4,
        "Weight": 375,
        "EndureTemplateID": 500,
        "MultiPlayerID": 103,
        "Csx": [
            "ECEnemiesOfNote/祸津御建鸣神命1",
            "ECEnemiesOfNote/祸津御建鸣神命2",
            "ECEnemiesOfNote/祸津御建鸣神命3",
            "ECNonCategorized/雷电将军",
            "ECNonCategorized/影"
        ]
    },
    "90701": {
        "_id": 90701,
        "KingdomID": 9,
        "ClassID": 10907,
        "Name": {
            "CH": "「正机之神」一阶段",
            "EN": "Shouki no Kami P1"
        },
        "Title": [
            {
                "CH": "七叶寂照秘密主",
                "EN": "Everlasting Lord of Arcane Wisdom"
            }
        ],
        "Desc": {
            "CH": "汇聚须弥教令院六大学院力量制造出的人造正机神，拥有比肩真神的威能。\n世人若称之为伪神，则是不懂得「神」的真意。\n假如超人者为神，\n正机神已通晓万事背负无上力量，谓之超人。\n假如爱人者为神，\n正机神亦懂得赐予人平等的失败，谓之爱人。\n假如观人者为神，\n……\n…………\n正机神窥得无数爱憎别离怨嗔痴，谓之观人。\n\n<i>「汝等已拥有太多的幸福，</i>\n<i>「因此变得愚昧自大，贪婪可怖。</i>\n<i>「终结这个时代！</i>\n<i>「新的神明，宣告旧神将败北。</i>\n<i>「可悲的人类…现在还来得及跪下。」</i>",
            "EN": "Shouki no Kami, as an artificial divinity created using all the might of Sumeru's Six Darshans, has power that rivals that of true gods.\nBut those of this world who call such a being a \"false deity\" misunderstand the true meaning of the word.\nShouki no Kami now understands countless things and shoulders unassailable might, and may be said to surpass mortals.\nIf gods must love mortals,\nthen Shouki no Kami, who grants failure in equal measure to humanity, loves them also.\nIf gods must watch over humanity,\n...\n...\nthen Shouki no Kami, who has borne witness to infinite love, hatred, partings, grudges, and maddening anger — thus does he watch over mortals.\n\n<i>\"Too much happiness you have had!\"</i>\n<i>\"It has made you foolish. Arrogant. Horrifying in your greed.\"</i>\n<i>\"Let this era end!\"</i>\n<i>\"Let the new god announce the defeat of the old.\"</i>\n<i>\"Foolish mortal... There is yet time for you to kneel.\"</i>"
        },
        "HP": 10.0,
        "HPCurve": 2,
        "ATK": 5.0,
        "ATKCurve": 4,
        "RESBase": {
            "Elec": 0.5
        },
        "Icon": [
            "nad"
        ],
        "Grade": 4,
        "Weight": 20000,
        "EndureTemplateID": 4000,
        "MultiPlayerID": 104,
        "Csx": [
            "ECEnemiesOfNote/七叶寂照秘密主1"
        ]
    },
    "90702": {
        "_id": 90702,
        "KingdomID": 9,
        "ClassID": 10907,
        "Name": {
            "CH": "「正机之神」二阶段",
            "EN": "Shouki no Kami P2"
        },
        "Title": [
            {
                "CH": "七叶寂照秘密主",
                "EN": "Everlasting Lord of Arcane Wisdom"
            }
        ],
        "Desc": {
            "CH": "汇聚须弥教令院六大学院力量制造出的人造正机神，拥有比肩真神的威能。\n世人若称之为伪神，则是不懂得「神」的真意。\n假如超人者为神，\n正机神已通晓万事背负无上力量，谓之超人。\n假如爱人者为神，\n正机神亦懂得赐予人平等的失败，谓之爱人。\n假如观人者为神，\n……\n…………\n正机神窥得无数爱憎别离怨嗔痴，谓之观人。\n\n<i>「汝等已拥有太多的幸福，</i>\n<i>「因此变得愚昧自大，贪婪可怖。</i>\n<i>「终结这个时代！</i>\n<i>「新的神明，宣告旧神将败北。</i>\n<i>「可悲的人类…现在还来得及跪下。」</i>",
            "EN": "Shouki no Kami, as an artificial divinity created using all the might of Sumeru's Six Darshans, has power that rivals that of true gods.\nBut those of this world who call such a being a \"false deity\" misunderstand the true meaning of the word.\nShouki no Kami now understands countless things and shoulders unassailable might, and may be said to surpass mortals.\nIf gods must love mortals,\nthen Shouki no Kami, who grants failure in equal measure to humanity, loves them also.\nIf gods must watch over humanity,\n...\n...\nthen Shouki no Kami, who has borne witness to infinite love, hatred, partings, grudges, and maddening anger — thus does he watch over mortals.\n\n<i>\"Too much happiness you have had!\"</i>\n<i>\"It has made you foolish. Arrogant. Horrifying in your greed.\"</i>\n<i>\"Let this era end!\"</i>\n<i>\"Let the new god announce the defeat of the old.\"</i>\n<i>\"Foolish mortal... There is yet time for you to kneel.\"</i>"
        },
        "HP": 9.0,
        "HPCurve": 2,
        "ATK": 6.0,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": 0.3,
            "Fire": 0.3,
            "Water": 0.3,
            "Elec": 0.9,
            "Ice": 0.3,
            "Grass": 0.3,
            "Wind": 0.3,
            "Rock": 0.3
        },
        "RESState": [
            {
                "State": "Nada_Shield",
                "All": 2.0
            },
            {
                "State": "Nada_Weak",
                "All": -1.7
            }
        ],
        "Icon": [
            "nad"
        ],
        "Grade": 4,
        "Weight": 20000,
        "EndureTemplateID": 4000,
        "MultiPlayerID": 104,
        "Csx": [
            "ECEnemiesOfNote/七叶寂照秘密主2"
        ]
    },
    "90800": {
        "_id": 90800,
        "KingdomID": 9,
        "ClassID": 109080,
        "Name": {
            "CH": "起源绿洲的守望者",
            "EN": "Warden of Oasis Prime"
        },
        "Title": [
            {
                "CH": "经由「火种」而得以复苏的生命",
                "EN": "Revived By the Fire Seed"
            }
        ],
        "Desc": {
            "CH": "阿佩普曾与诸多子嗣一同在芳草之海、百尺巨木与千里之藤蔓中，如游鱼穿行于水。直到最终草木枯萎化为尘埃，如同砂砾般渺小的诸多生灵在它原本的土地上建立起脆弱的城国。\n阿佩普曾与诸多子嗣一同令沙海如沸水般翻腾，将城楼如落叶般吹散。直到新的沙海之王与失去故土的王相互和解，从此日光之下的沙丘都属于人与神，而龙的绿洲留在龙的心中。\n阿佩普曾与诸多子嗣一同分享进化的知识，因为流水不腐，而朽木必定倾倒。直到最终它践行了与沙之王的约定得到了终极的智慧，从此它只能看见终末的风景，漫无边际的末日。\n阿佩普曾独自沉溺于末日的风景当中。所有的人、神、龙、走兽、飞鸟与游鱼，所有记忆、智慧、话语与仇恨将都磨为无色尘粉，最后一轮明月之光则化作白焰之雨落在荒土之上。\n……\n阿佩普曾视沙海之底为自己的墓场，而非失乡之王的行宫。「智慧」的毒很快就会让它从无数个月亮的仇恨与愤怒中解脱。它已经对终末缺乏颜色的景象感到厌倦了。直到最终年轻的神明与金色的旅人让它再度回想起，即便自己曾经主宰的青绿土地已经化作饰金的荒原，即便自己与子嗣为了在其中生存而变得扭曲丑陋，但它的心中始终珍藏着那一角绿洲的景象。",
            "EN": "Apep walked through the water like a fish with its many offspring in a sea of fragrant grass, gigantic trees, and unimaginably long vines. Finally, the grass, trees, and vines withered and turned into dust, while countless beings as minute as grains of gravel built a fragile kingdom upon Apep's land.\nApep and its offspring let the sea of sand rage like boiling water and swept houses down as if they were falling leaves. Until the day when the new Lord of Sand reconciled with the king who lost their homeland, the dunes under the sun belonged to humanity, while the dragon's oasis remained in the hearts of the dragons.\nApep shared the knowledge of evolution with its many offspring, because spreading knowledge ensures dissemination while the stagnation of hoarded wisdom courted only decay. Till the day the promise between it and the Lord of Sand was finally attained, with ultimate wisdom claimed. Henceforth, it reaped only sights of oblivion, of an endless calamity.\nApep found itself swallowed alone amidst the scenery of the catastrophe. All the people, gods, dragons, beasts, birds, and fish. All memories, wisdom, words, and hatred were ground into colorless dust, and the last of the moonlight burned away into a rain of pale fire, falling on the barren land.\n...\nApep once deemed the bottom of the sea of sand to be its grave, not the palace of a dispossessed king. The poison of \"wisdom\" would soon free Apep from the hatred and anger festering within it over countless moons. Apep had grown weary of the colorless apocalypse. Then, finally, the young god and the golden Traveler let it recall once again, even though the verdant land it once ruled had been reduced to this gilded wasteland, even though Apep and its many offspring had grown twisted and wretched to secure their continued survival therein, Apep still treasured the sight and scenes of the oasis, sparkling in its heart."
        },
        "Color": "Grass",
        "HP": 8.0,
        "HPCurve": 2,
        "ATK": 10.0,
        "ATKCurve": 4,
        "RESBase": {
            "Grass": 0.7
        },
        "Icon": [
            "ap"
        ],
        "Grade": 4,
        "Weight": 20000,
        "EndureTemplateID": 4000,
        "MultiPlayerID": 103,
        "Csx": [
            "ECEnemiesOfNote/起源绿洲的守望者"
        ]
    },
    "90801": {
        "_id": 90801,
        "KingdomID": 9,
        "ClassID": 109080,
        "Name": {
            "CH": "终末绿洲的守望者",
            "EN": "Warden of the Last Oasis"
        },
        "Title": [
            {
                "CH": "历经千年之「末日」的进化生命",
                "EN": "Evolved From an Apocalyptic Millennium"
            }
        ],
        "Desc": {
            "CH": "阿佩普曾与诸多子嗣一同在芳草之海、百尺巨木与千里之藤蔓中，如游鱼穿行于水。直到最终草木枯萎化为尘埃，如同砂砾般渺小的诸多生灵在它原本的土地上建立起脆弱的城国。\n阿佩普曾与诸多子嗣一同令沙海如沸水般翻腾，将城楼如落叶般吹散。直到新的沙海之王与失去故土的王相互和解，从此日光之下的沙丘都属于人与神，而龙的绿洲留在龙的心中。\n阿佩普曾与诸多子嗣一同分享进化的知识，因为流水不腐，而朽木必定倾倒。直到最终它践行了与沙之王的约定得到了终极的智慧，从此它只能看见终末的风景，漫无边际的末日。\n阿佩普曾独自沉溺于末日的风景当中。所有的人、神、龙、走兽、飞鸟与游鱼，所有记忆、智慧、话语与仇恨将都磨为无色尘粉，最后一轮明月之光则化作白焰之雨落在荒土之上。\n……\n阿佩普曾视沙海之底为自己的墓场，而非失乡之王的行宫。「智慧」的毒很快就会让它从无数个月亮的仇恨与愤怒中解脱。它已经对终末缺乏颜色的景象感到厌倦了。直到最终年轻的神明与金色的旅人让它再度回想起，即便自己曾经主宰的青绿土地已经化作饰金的荒原，即便自己与子嗣为了在其中生存而变得扭曲丑陋，但它的心中始终珍藏着那一角绿洲的景象。",
            "EN": "Apep walked through the water like a fish with its many offspring in a sea of fragrant grass, gigantic trees, and unimaginably long vines. Finally, the grass, trees, and vines withered and turned into dust, while countless beings as minute as grains of gravel built a fragile kingdom upon Apep's land.\nApep and its offspring let the sea of sand rage like boiling water and swept houses down as if they were falling leaves. Until the day when the new Lord of Sand reconciled with the king who lost their homeland, the dunes under the sun belonged to humanity, while the dragon's oasis remained in the hearts of the dragons.\nApep shared the knowledge of evolution with its many offspring, because spreading knowledge ensures dissemination while the stagnation of hoarded wisdom courted only decay. Till the day the promise between it and the Lord of Sand was finally attained, with ultimate wisdom claimed. Henceforth, it reaped only sights of oblivion, of an endless calamity.\nApep found itself swallowed alone amidst the scenery of the catastrophe. All the people, gods, dragons, beasts, birds, and fish. All memories, wisdom, words, and hatred were ground into colorless dust, and the last of the moonlight burned away into a rain of pale fire, falling on the barren land.\n...\nApep once deemed the bottom of the sea of sand to be its grave, not the palace of a dispossessed king. The poison of \"wisdom\" would soon free Apep from the hatred and anger festering within it over countless moons. Apep had grown weary of the colorless apocalypse. Then, finally, the young god and the golden Traveler let it recall once again, even though the verdant land it once ruled had been reduced to this gilded wasteland, even though Apep and its many offspring had grown twisted and wretched to secure their continued survival therein, Apep still treasured the sight and scenes of the oasis, sparkling in its heart."
        },
        "Color": "Grass",
        "HP": 9.0,
        "HPCurve": 2,
        "ATK": 6.0,
        "ATKCurve": 4,
        "RESBase": {
            "Grass": 0.7
        },
        "Icon": [
            "ap"
        ],
        "Grade": 4,
        "Weight": 20000,
        "EndureTemplateID": 4000,
        "MultiPlayerID": 103,
        "Csx": [
            "ECEnemiesOfNote/终末绿洲的守望者"
        ]
    },
    "90802": {
        "_id": 90802,
        "KingdomID": 9,
        "ClassID": 109081,
        "Name": {
            "CH": "基础增殖生命体",
            "EN": "Preliminary Proliferating Organism"
        },
        "Title": [
            {
                "CH": "历经千年之「末日」的进化生命",
                "EN": "Evolved From an Apocalyptic Millennium"
            }
        ],
        "Color": "Grass",
        "HP": 0.8,
        "HPCurve": 2,
        "ATK": 0.2,
        "ATKCurve": 4,
        "RESBase": {
            "Grass": 0.5
        },
        "Icon": [
            "ap"
        ],
        "Grade": 1,
        "Weight": 75,
        "EndureTemplateID": 50,
        "Csx": [
            "ECNonCategorized/基础增殖生命体"
        ]
    },
    "90803": {
        "_id": 90803,
        "KingdomID": 9,
        "ClassID": 109081,
        "Name": {
            "CH": "掠食性增殖生命体",
            "EN": "Predatory Proliferating Organism"
        },
        "Title": [
            {
                "CH": "历经千年之「末日」的进化生命",
                "EN": "Evolved From an Apocalyptic Millennium"
            }
        ],
        "Color": "Grass",
        "HP": 1.2,
        "HPCurve": 2,
        "ATK": 4.0,
        "ATKCurve": 4,
        "RESBase": {
            "Grass": 0.5
        },
        "Icon": [
            "ap"
        ],
        "Grade": 1,
        "Weight": 100,
        "EndureTemplateID": 400,
        "Csx": [
            "ECNonCategorized/掠食性增殖生命体"
        ]
    },
    "90804": {
        "_id": 90804,
        "KingdomID": 9,
        "ClassID": 109081,
        "Name": {
            "CH": "守护的增殖生命体",
            "EN": "Protective Proliferating Organism"
        },
        "Title": [
            {
                "CH": "历经千年之「末日」的进化生命",
                "EN": "Evolved From an Apocalyptic Millennium"
            }
        ],
        "Color": "Grass",
        "HP": 1.6,
        "HPCurve": 2,
        "ATK": 1.0,
        "ATKCurve": 4,
        "RESBase": {
            "Grass": 0.5
        },
        "Icon": [
            "ap"
        ],
        "Grade": 1,
        "Weight": 300,
        "EndureTemplateID": 400,
        "Csx": [
            "ECNonCategorized/守护的增殖生命体"
        ]
    },
    "90805": {
        "_id": 90805,
        "KingdomID": 9,
        "ClassID": 109081,
        "Name": {
            "CH": "养育的增殖生命体",
            "EN": "Provender Proliferating Organism"
        },
        "Title": [
            {
                "CH": "历经千年之「末日」的进化生命",
                "EN": "Evolved From an Apocalyptic Millennium"
            }
        ],
        "Color": "Grass",
        "HP": 1.6,
        "HPCurve": 2,
        "ATK": 1.0,
        "ATKCurve": 4,
        "RESBase": {
            "Grass": 0.5
        },
        "RESState": [
            {
                "State": "Apep_Provender",
                "All": 99
            }
        ],
        "Icon": [
            "ap"
        ],
        "Grade": 1,
        "Weight": 300,
        "EndureTemplateID": 400,
        "Csx": [
            "ECNonCategorized/养育的增殖生命体"
        ]
    },
    "90806": {
        "_id": 90806,
        "KingdomID": 9,
        "ClassID": 109081,
        "Name": {
            "CH": "破坏的增殖生命体",
            "EN": "Pernicious Proliferating Organism"
        },
        "Title": [
            {
                "CH": "历经千年之「末日」的进化生命",
                "EN": "Evolved From an Apocalyptic Millennium"
            }
        ],
        "Color": "Grass",
        "HP": 1.0,
        "HPCurve": 2,
        "ATK": 1.0,
        "ATKCurve": 4,
        "RESBase": {
            "Grass": 0.5
        },
        "Icon": [
            "ap"
        ],
        "Grade": 2,
        "Weight": 100,
        "EndureTemplateID": 400,
        "Csx": [
            "ECNonCategorized/破坏的增殖生命体"
        ]
    },
    "90807": {
        "_id": 90807,
        "KingdomID": 9,
        "ClassID": 109081,
        "Name": {
            "CH": "筑巢的增殖生命体",
            "EN": "Perch Proliferating Organism"
        },
        "Title": [
            {
                "CH": "历经千年之「末日」的进化生命",
                "EN": "Evolved From an Apocalyptic Millennium"
            }
        ],
        "Color": "Grass",
        "HP": 1.2,
        "HPCurve": 2,
        "ATK": 1.0,
        "ATKCurve": 4,
        "RESBase": {
            "Grass": 0.5
        },
        "Icon": [
            "ap"
        ],
        "Grade": 1,
        "Weight": 100,
        "EndureTemplateID": 400,
        "Csx": [
            "ECNonCategorized/筑巢的增殖生命体"
        ]
    },
    "90901": {
        "_id": 90901,
        "KingdomID": 9,
        "ClassID": 10909,
        "Name": {
            "CH": "吞星之鲸",
            "EN": "All-Devouring Narwhal"
        },
        "Title": [
            {
                "CH": "无光星海彼端的访客",
                "EN": "Visitor From the Far Side of the Sea of Stars"
            }
        ],
        "Desc": {
            "CH": "神秘的异界巨兽。\n诗人常将星空比做海洋。他们见过或是梦见过无月的漆黑海面上闪着磷光的无数浮游生物，状似银河；又或者是洒落在海面上的月光被波浪绞碎，就如同闪烁的星光。在最魔幻的故事里或是最疯癫的诳语中，宇宙深处真正的星辰或许也如提瓦特一般充满了生机，而宇宙本身就如同海洋。但从现实的角度来看，假若存在深邃的、充满了生命的宇宙，为何这些生命至今也未与提瓦特人进行接触呢？\n或许宇宙渗入提瓦特的过程从未停止；也许更高的意志为它划定了边界是为了保护这个世界。",
            "EN": "A mysterious and gargantuan creature from another world.\nPoets often liken the starry sky to the ocean. They've seen or dreamt of countless bioluminescent plankton floating on a moonless sea, resembling a galaxy, or the moonlight ripped apart by waves on the sea's surface, like shimmering stars. In the most fantastical stories or outrageous lies, the stars found in the depths of the cosmos might be teeming with life, much like Teyvat, and the universe itself is akin to an ocean. Realistically speaking, however, if a profound universe full of life exists, why hasn't any of that life made contact with Teyvat?\nMaybe the universe has been constantly trying to infiltrate Teyvat, or maybe a higher power created borders to protect this world."
        },
        "Color": "Water",
        "HP": 20.0,
        "HPCurve": 2,
        "ATK": 10.0,
        "ATKCurve": 4,
        "RESBase": {
            "Water": 0.7
        },
        "Icon": [
            "UI_MonsterIcon_Ptahur_Devourer"
        ],
        "Grade": 4,
        "Weight": 20000,
        "EndureTemplateID": 4000,
        "MultiPlayerID": 103,
        "Tutorial": {
            "CH": "- 本体具有怒气条，<b>积攒至120点时将会吞噬角色，进入神秘空间</b>；此时无法对本体造成伤害。击败神秘空间内的「幻影」，会离开神秘空间，并<b>使本体损失20%生命值</b>，为所有角色恢复100点元素能量。<br>- 本体在战斗中每秒会积攒1点怒气，战斗80秒后将改为每秒积攒2点。本体受到伤害时，每失去1%生命值，将会积攒1点怒气。破坏「涡眼」将积攒30点怒气。<br>- 会周期性召唤「涡眼」：持续牵引角色，并对角色造成大量伤害。累计对其进行8次攻击，或<b>使用1次荒性或芒性攻击</b>，方可破坏涡眼。<br>- 神秘空间中的「幻影」具有本体60%的生命值，并会发动强大的攻击。在合适的时机<b>使用1次荒性或芒性攻击</b>，可使幻影<b>损失25%生命值</b>并短暂瘫痪。",
            "EN": "- The BOSS accumulates Rage during combat. <b>When Rage reaches 120, will devour you, taking you to another realm</b>. To return to reality, you must defeat the Phantom inside the realm. Upon defeating the Phantom, the BOSS will <b>lose 20% HP</b>, and every party member revives 100 Elemental Energy.<br>- The BOSS gathers 1 Rage every second; after 80 seconds, will instead gather 2 Rage every second. Each time it takes damage, every 1% HP lost will increase Rage by 1. Destroying an Eye of Maelstrom will gather 30 Rage.<br>- Eyes of Maelstrom will appear in battle, pulling characters in and dealing massive DMG. To destroy it, either attack it 8 times, or <b>attack it once with Ousia or Pneuma</b>.<br>- The Phantom in the other realm has 60% HP of the BOSS, and will unleash powerful attacks. <b>Correctly timed Ousia or Pneuma attacks</b> will paralyze it and cause it to <b>lose 25% HP</b>."
        },
        "Csx": [
            "ECEnemiesOfNote/吞星之鲸",
            "ECEnemiesOfNote/吞星之鲸·幻影"
        ]
    },
    "90902": {
        "_id": 90902,
        "KingdomID": 9,
        "ClassID": 10909,
        "Name": {
            "CH": "吞星之鲸 · 幻影",
            "EN": "All-Devouring Narwhal - Phantom"
        },
        "Title": [
            {
                "CH": "无光星海彼端的访客",
                "EN": "Visitor from the Far Side of the Sea of Stars"
            }
        ],
        "Desc": {
            "CH": "神秘的异界巨兽。\n诗人常将星空比做海洋。他们见过或是梦见过无月的漆黑海面上闪着磷光的无数浮游生物，状似银河；又或者是洒落在海面上的月光被波浪绞碎，就如同闪烁的星光。在最魔幻的故事里或是最疯癫的诳语中，宇宙深处真正的星辰或许也如提瓦特一般充满了生机，而宇宙本身就如同海洋。但从现实的角度来看，假若存在深邃的、充满了生命的宇宙，为何这些生命至今也未与提瓦特人进行接触呢？\n或许宇宙渗入提瓦特的过程从未停止；也许更高的意志为它划定了边界是为了保护这个世界。",
            "EN": "A mysterious and gargantuan creature from another world.\nPoets often liken the starry sky to the ocean. They've seen or dreamt of countless bioluminescent plankton floating on a moonless sea, resembling a galaxy, or the moonlight ripped apart by waves on the sea's surface, like shimmering stars. In the most fantastical stories or outrageous lies, the stars found in the depths of the cosmos might be teeming with life, much like Teyvat, and the universe itself is akin to an ocean. Realistically speaking, however, if a profound universe full of life exists, why hasn't any of that life made contact with Teyvat?\nMaybe the universe has been constantly trying to infiltrate Teyvat, or maybe a higher power created borders to protect this world."
        },
        "Color": "Elec",
        "HP": 12.0,
        "HPCurve": 2,
        "ATK": 8.0,
        "ATKCurve": 4,
        "RESBase": {
            "Elec": 0.7
        },
        "Icon": [
            "UI_MonsterIcon_Ptahur_Devourer"
        ],
        "Grade": 4,
        "Weight": 150,
        "EndureTemplateID": 4000,
        "MultiPlayerID": 103,
        "Tutorial": {
            "CH": "- 本体具有怒气条，<b>积攒至120点时将会吞噬角色，进入神秘空间</b>；此时无法对本体造成伤害。击败神秘空间内的「幻影」，会离开神秘空间，并<b>使本体损失20%生命值</b>，为所有角色恢复100点元素能量。<br>- 本体在战斗中每秒会积攒1点怒气，战斗80秒后将改为每秒积攒2点。本体受到伤害时，每失去1%生命值，将会积攒1点怒气。破坏「涡眼」将积攒30点怒气。<br>- 会周期性召唤「涡眼」：持续牵引角色，并对角色造成伤害。累计对其进行8次攻击，或<b>使用1次荒性或芒性攻击</b>，方可破坏涡眼。<br>- 神秘空间中的「幻影」具有本体60%的生命值，并会发动强大的攻击。在合适的时机<b>使用1次荒性或芒性攻击</b>，可使幻影<b>损失25%生命值</b>并短暂瘫痪。",
            "EN": "- The BOSS accumulates Rage during combat. <b>When Rage reaches 120, will devour you, taking you to another realm</b>. You cannot deal damage to the BOSS in the realm. To return to reality, you must defeat the Phantom inside the realm. Upon defeating the Phantom, the BOSS will <b>lose 20% HP</b>, and every party member revives 100 Elemental Energy.<br>- The BOSS gathers 1 Rage every second; after 80 seconds, will instead gather 2 Rage every second. Each time it takes damage, every 1% HP lost will increase Rage by 1. Destroying an Eye of Maelstrom will gather 30 Rage.<br>- Eyes of Maelstrom will appear in battle, pulling characters in and dealing DMG. To destroy it, either hit it 8 times, or <b>attack it once with Ousia or Pneuma</b>.<br>- The Phantom in the other realm has 60% HP of the BOSS, and will unleash powerful attacks. <b>Correctly timed Ousia or Pneuma attacks</b> will paralyze it and cause it to <b>lose 25% HP</b>."
        },
        "Csx": [
            "ECEnemiesOfNote/吞星之鲸",
            "ECEnemiesOfNote/吞星之鲸·幻影"
        ]
    },
    "90903": {
        "_id": 90903,
        "KingdomID": 9,
        "ClassID": 10909,
        "Name": {
            "CH": "吞星之鲸 · 涡眼",
            "EN": "Eye of the Maelstrom"
        },
        "Title": [
            {
                "CH": "无光星海彼端的访客",
                "EN": "Visitor from the Far Side of the Sea of Stars"
            }
        ],
        "Desc": {
            "CH": "神秘的异界巨兽。\n诗人常将星空比做海洋。他们见过或是梦见过无月的漆黑海面上闪着磷光的无数浮游生物，状似银河；又或者是洒落在海面上的月光被波浪绞碎，就如同闪烁的星光。在最魔幻的故事里或是最疯癫的诳语中，宇宙深处真正的星辰或许也如提瓦特一般充满了生机，而宇宙本身就如同海洋。但从现实的角度来看，假若存在深邃的、充满了生命的宇宙，为何这些生命至今也未与提瓦特人进行接触呢？\n或许宇宙渗入提瓦特的过程从未停止；也许更高的意志为它划定了边界是为了保护这个世界。",
            "EN": "A mysterious and gargantuan creature from another world.\nPoets often liken the starry sky to the ocean. They've seen or dreamt of countless bioluminescent plankton floating on a moonless sea, resembling a galaxy, or the moonlight ripped apart by waves on the sea's surface, like shimmering stars. In the most fantastical stories or outrageous lies, the stars found in the depths of the cosmos might be teeming with life, much like Teyvat, and the universe itself is akin to an ocean. Realistically speaking, however, if a profound universe full of life exists, why hasn't any of that life made contact with Teyvat?\nMaybe the universe has been constantly trying to infiltrate Teyvat, or maybe a higher power created borders to protect this world."
        },
        "Color": "Water",
        "HP": 10,
        "HPCurve": 2,
        "ATK": 2,
        "ATKCurve": 4,
        "Icon": [
            "UI_MonsterIcon_Ptahur_Devourer"
        ],
        "Grade": 4,
        "Weight": 500,
        "EndureTemplateID": 4000,
        "MultiPlayerID": 103,
        "Tutorial": {
            "CH": "- 具有怒气条，<b>积攒至120点时将会吞噬角色，进入神秘空间</b>；此时无法对本体造成伤害。击败神秘空间内的「幻影」，会离开神秘空间，并<b>使本体损失20%生命值</b>，为所有角色恢复100点元素能量。<br>- 本体在战斗中每1秒会积攒1点怒气。本体受到伤害时，每失去1%生命值，将会积攒1点怒气。破坏「涡眼」将积攒30点怒气。<br>- 会周期性召唤「涡眼」：持续牵引角色，并对角色造成大量伤害。累计对其进行8次攻击，或<b>使用1次荒性或芒性攻击</b>，方可破坏涡眼。<br>- 神秘空间中的「幻影」具有本体60%的生命值，并会发动强大的攻击。在合适的时机<b>使用1次荒性或芒性攻击</b>，可使幻影<b>损失25%生命值</b>并短暂瘫痪。",
            "EN": "- Accumulates Rage during combat. <b>When Rage reaches 120, will devour you, taking you to another realm</b>. To return to reality, you must defeat the Phantom inside the realm. Upon defeating the Phantom, the BOSS will <b>lose 20% HP</b>, and every party member revives 100 Elemental Energy.<br>- The BOSS gathers 1 Rage every second. Each time it takes damage, every 1% HP lost will increase Rage by 1. Destroying an Eye of Maelstrom will gather 30 Rage.<br>- Eyes of Maelstrom will appear in battle, pulling characters in and dealing massive DMG. To destroy it, either attack it 8 times, or <b>attack it once with Ousia or Pneuma</b>.<br>- The Phantom in the other realm has 60% HP of the BOSS, and will unleash powerful attacks. <b>Correctly timed Ousia or Pneuma attacks</b> will paralyze it and cause it to <b>lose 25% HP</b>."
        },
        "Csx": [
            "ECEnemiesOfNote/吞星之鲸",
            "ECEnemiesOfNote/吞星之鲸·幻影"
        ]
    },
    "70350450": {
        "_id": 70350450,
        "KingdomID": 8,
        "ClassID": 11003,
        "Name": {
            "CH": "秘境镇石",
            "EN": "Domain Monolith"
        },
        "Title": [
            {
                "CH": "秘境镇石",
                "EN": "Domain Monolith"
            }
        ],
        "HP": 90.0,
        "HPCurve": 3,
        "ATK": 0.0,
        "ATKCurve": 3,
        "RESBase": {
            "Phys": 0,
            "Fire": 0,
            "Water": 0,
            "Elec": 0,
            "Ice": 0,
            "Grass": 0,
            "Wind": 0,
            "Rock": 0
        },
        "NotAffectedBySpiralAbyss": true,
        "Icon": [
            "UI_UGCIcon_MonsterTarget_01"
        ],
        "Grade": 1,
        "Weight": 0,
        "EndureTemplateID": 0
    },
    "3700701": {
        "_id": 3700701,
        "KingdomID": 10,
        "ClassID": 11004,
        "Name": {
            "CH": "裂鸣的雷音权现",
            "EN": "Crackling Thunder Manifestation"
        },
        "Title": [
            {
                "CH": "除垢：凄响之悲雷",
                "EN": "Descaling: Thundering Grief"
            }
        ],
        "Desc": {
            "CH": "挑战「裂鸣的雷音权现」。\n深陷迷惘，在盘旋中凄鸣的雷音权现，不知有着何等惨痛的过往。无论感知范围内出现了何种活物，它强烈的恨意都将转化成凌厉的雷电，试图将对方彻底撕碎，只留自己那刺耳的静寂…",
            "EN": "Challenge the Crackling Thunder Manifestation.\nWho knows what miseries the grieving, confused Thunder Manifestation that wanders here has witnessed. It transforms its hatred into fierce lightning regardless of what living thing it detects, and it will try to completely destroy everything, until it is left alone amidst deafening silence..."
        },
        "Color": "Elec",
        "HP": 9.0,
        "HPCurve": 2,
        "ATK": 7.2,
        "ATKCurve": 4,
        "Ball": {
            "Type": 3,
            "Point": 4,
            "Drop": "3332"
        },
        "Icon": [
            "tm"
        ],
        "Grade": 3,
        "Weight": 150,
        "EndureTemplateID": 4000,
        "Csx": [
            "ECElementalLifeforms/雷音权现·裂鸣"
        ]
    },
    "3740303": {
        "_id": 3740303,
        "KingdomID": 10,
        "ClassID": 11004,
        "Name": {
            "CH": "重武装兆载永劫龙兽",
            "EN": "Aeonblight Drake (Heavy Assault Pack)"
        },
        "Title": [
            {
                "CH": "虔心：炽火坚钢万劫",
                "EN": "Piety: Calamity of Blazing Steel"
            }
        ],
        "Desc": {
            "CH": "挑战「重武装兆载永劫龙兽」。\n外形与同族的「兆载永劫龙兽」无异，却载有更为致命的武装，原本只在坊间传说和少量报告中被提及，如今却现身于谜一般的秘境深处，向侦测到的一切存在倾泻狂烈的火力…",
            "EN": "Challenge the Aeonblight Drake (Heavy Assault Pack).\nIt appears no different from an Aeonblight Drake, but comes with even more dangerous weaponry that heretofore has only appeared in legends and a small number of reports. Now, it roams this mysterious Domain, directing its fearsome firepower against all it detects..."
        },
        "HP": 7.5,
        "HPCurve": 2,
        "ATK": 7.2,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": 0.7
        },
        "RESState": [
            {
                "State": "Alert",
                "All": 1.0
            },
            {
                "State": "Gargoyle_Fafnir_Air",
                "All": 1.0
            }
        ],
        "RESModify": [
            {
                "State": "Gargoyle_Elemental",
                "Show": {
                    "CH": "对应抗性提升60%",
                    "EN": "Corresponding Element\nRES increased by 60%"
                }
            }
        ],
        "Ball": {
            "Type": 0,
            "Point": 3,
            "Drop": "332"
        },
        "Icon": [
            "abd"
        ],
        "Grade": 3,
        "Weight": 300,
        "EndureTemplateID": 4000,
        "Csx": [
            "ECAutomatons/重武装兆载永劫龙兽"
        ]
    },
    "3760507": {
        "_id": 3760507,
        "KingdomID": 10,
        "ClassID": 11004,
        "Name": {
            "CH": "深海龙蜥 · 唳霜",
            "EN": "Frostshrieker Bathysmal Vishap"
        },
        "Title": [
            {
                "CH": "羁影：龙嗣之绞旋",
                "EN": "Shadow: Twined Dragonheirs"
            }
        ],
        "Desc": {
            "CH": "挑战「深海龙蜥之群 · 轰旋」：「唳霜」与「啸霆」。\n误闯异域，难寻归途的深海龙蜥，捕捉到陌生环境中的危险气息，认定自己正被不可名状之物威胁，借由强大的战斗本能，对任何接近的生灵发起猛攻…",
            "EN": "Challenge the Booming Bathysmal Vishap Herd: the Frostshrieker and the Blitzroarer.\nHaving barged into this area, the Bathysmal Vishaps will struggle to return whence they came. Having sensed danger nearby, they will now attack any who would pose a threat to them with great might and savagery..."
        },
        "Color": "Ice",
        "HP": 185.185,
        "HPCurve": 20,
        "ATK": 5.0,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": 0.3,
            "Ice": 0.3
        },
        "Ball": {
            "Type": 4,
            "Point": 4,
            "Drop": "1113"
        },
        "Icon": [
            "cd"
        ],
        "Grade": 3,
        "Weight": 250,
        "EndureTemplateID": 4000,
        "Csx": [
            "ECMysticalBeasts/深海龙蜥之群·轰旋"
        ]
    },
    "3760508": {
        "_id": 3760508,
        "KingdomID": 10,
        "ClassID": 11004,
        "Name": {
            "CH": "深海龙蜥 · 啸霆",
            "EN": "Blitzroarer Bathysmal Vishap"
        },
        "Title": [
            {
                "CH": "羁影：龙嗣之绞旋",
                "EN": "Shadow: Twined Dragonheirs"
            }
        ],
        "Desc": {
            "CH": "挑战「深海龙蜥之群 · 轰旋」：「唳霜」与「啸霆」。\n误闯异域，难寻归途的深海龙蜥，捕捉到陌生环境中的危险气息，认定自己正被不可名状之物威胁，借由强大的战斗本能，对任何接近的生灵发起猛攻…",
            "EN": "Challenge the Booming Bathysmal Vishap Herd: the Frostshrieker and the Blitzroarer.\nHaving barged into this area, the Bathysmal Vishaps will struggle to return whence they came. Having sensed danger nearby, they will now attack any who would pose a threat to them with great might and savagery..."
        },
        "Color": "Elec",
        "HP": 185.185,
        "HPCurve": 20,
        "ATK": 5.0,
        "ATKCurve": 4,
        "RESBase": {
            "Phys": 0.3,
            "Elec": 0.3
        },
        "Ball": {
            "Type": 3,
            "Point": 4,
            "Drop": "1113"
        },
        "Icon": [
            "cd"
        ],
        "Grade": 3,
        "Weight": 250,
        "EndureTemplateID": 4000,
        "Csx": [
            "ECMysticalBeasts/深海龙蜥之群·轰旋"
        ]
    },
    "3700000": {
        "_id": 3700000,
        "KingdomID": 10,
        "ClassID": 11004,
        "Name": {
            "CH": "离垢者肃心旅宴",
            "EN": "Feast of the Solemn Departed"
        },
        "Title": [
            {
                "CH": "宴默行，三百由旬！",
                "EN": "Three-Realm Banquet of Stillness"
            }
        ],
        "Desc": {
            "CH": "连续挑战「裂鸣的雷音权现」、「重武装兆载永劫龙兽」与「深海龙蜥之群 · 轰旋」。\n以沉着清寂之姿，应对悖险之至的强敌，若是能以不染纤尘的姿态越过这重险阻，距离极致武艺的山巅，业已超出了五百之三百…",
            "EN": "Challenge the Crackling Thunder Manifestation, Aeonblight Drake (Heavy Assault Pack), and the Booming Bathysmal Vishap Herd in succession.\nApproach this battle against your deadly foes with absolute calm, and should you overcome, you will be well on your way toward the apex of martial prowess..."
        },
        "HP": 0.0,
        "HPCurve": 2,
        "ATK": 0.0,
        "ATKCurve": 4,
        "Icon": [
            "keq"
        ],
        "Grade": 3,
        "Weight": 0,
        "EndureTemplateID": 0
    },
    "30050701": {
        "_id": 30050701,
        "KingdomID": 12,
        "ClassID": 12138,
        "Name": {
            "CH": "友善的幻形花鼠",
            "EN": "Friendly Hydro Mimic Squirrel"
        },
        "Title": [
            {
                "CH": "无源之水",
                "EN": "Water Without Source"
            }
        ],
        "Desc": {
            "CH": "3.8新地图新增朋友",
            "EN": "3.8 new map new friend"
        },
        "Color": "Water",
        "HP": 180,
        "HPCurve": 3,
        "ATK": 0.1,
        "ATKCurve": 1,
        "RESBase": {
            "Phys": 0.15,
            "Fire": -0.4,
            "Water": 0.15,
            "Elec": 0.15,
            "Ice": 0.15,
            "Grass": 0.15,
            "Wind": 0.15,
            "Rock": 0.15
        },
        "Icon": [
            "hm"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 4000
    },
    "35210101": {
        "_id": 35210101,
        "KingdomID": 12,
        "ClassID": 12130,
        "Name": {
            "CH": "拉娜",
            "EN": "Rana"
        },
        "Title": [
            {
                "CH": "森林书的开端与终结",
                "EN": "Aranyaka's Beginning and End"
            }
        ],
        "Desc": {
            "CH": "道成林的巡林员。在森林书的结尾获得了草元素神之眼，和兰那罗一起踏上了旅途...",
            "EN": "A Forest Watcher from Avidya Forest. At the end of Aranyaka, Rana obtained a Dendro Vision and started on a journey with Aranara..."
        },
        "HP": 180,
        "HPCurve": 3,
        "ATK": 0.1,
        "ATKCurve": 1,
        "Icon": [
            "hm"
        ],
        "Grade": 1,
        "Weight": 70,
        "EndureTemplateID": 50
    },
    "35210201": {
        "_id": 35210201,
        "KingdomID": 12,
        "ClassID": 12135,
        "Name": {
            "CH": "伊德里西",
            "EN": "Idrisi"
        },
        "Title": [
            {
                "CH": "「阿赫玛尔之须」成员",
                "EN": "Member of Dakan Al-Ahmar"
            }
        ],
        "Desc": {
            "CH": "迪希雅的传说任务中，迪希雅的父亲的旅团「阿赫玛尔之须」的成员。",
            "EN": "A character from Dehya's Story Quest. A member of Dakan Al-Ahmar, the group of Eremites formerly lead by Dehya's father."
        },
        "HP": 1,
        "HPCurve": 1,
        "ATK": 1.8,
        "ATKCurve": 3,
        "Icon": [
            "emcr"
        ],
        "Grade": 1,
        "Weight": 70,
        "EndureTemplateID": 50
    },
    "35210402": {
        "_id": 35210402,
        "KingdomID": 12,
        "ClassID": 12135,
        "Name": {
            "CH": "提克里蒂",
            "EN": "Tikriti"
        },
        "Title": [
            {
                "CH": "「阿赫玛尔之须」成员",
                "EN": "Member of Dakan Al-Ahmar"
            }
        ],
        "Desc": {
            "CH": "迪希雅的传说任务中，迪希雅的父亲的旅团「阿赫玛尔之须」的成员。",
            "EN": "A character from Dehya's Story Quest. A member of Dakan Al-Ahmar, the group of Eremites formerly lead by Dehya's father."
        },
        "HP": 1,
        "HPCurve": 1,
        "ATK": 1.8,
        "ATKCurve": 3,
        "Icon": [
            "emlb"
        ],
        "Grade": 1,
        "Weight": 70,
        "EndureTemplateID": 50
    },
    "35210502": {
        "_id": 35210502,
        "KingdomID": 12,
        "ClassID": 12135,
        "Name": {
            "CH": "巴沙尔",
            "EN": "Bashar"
        },
        "Title": [
            {
                "CH": "「阿赫玛尔之须」成员",
                "EN": "Member of Dakan Al-Ahmar"
            }
        ],
        "Desc": {
            "CH": "迪希雅的传说任务中，迪希雅的父亲的旅团「阿赫玛尔之须」的成员。",
            "EN": "A character from Dehya's Story Quest. A member of Dakan Al-Ahmar, the group of Eremites formerly lead by Dehya's father."
        },
        "HP": 1,
        "HPCurve": 1,
        "ATK": 1.8,
        "ATKCurve": 3,
        "Icon": [
            "emsd"
        ],
        "Grade": 1,
        "Weight": 70,
        "EndureTemplateID": 50
    },
    "35210202": {
        "_id": 35210202,
        "KingdomID": 12,
        "ClassID": 12136,
        "Name": {
            "CH": "巴达维",
            "EN": "Badawi"
        },
        "Title": [
            {
                "CH": "卡维的甲方",
                "EN": "Kaveh's Client"
            }
        ],
        "Desc": {
            "CH": "卡维的邀约任务中，委托卡维在沙漠中建造一座图书馆。",
            "EN": "A character from Kaveh's Hangout Quest. Commissioned Kaveh to build a library in the desert."
        },
        "HP": 0.6,
        "HPCurve": 2,
        "ATK": 2,
        "ATKCurve": 4,
        "Icon": [
            "emcr"
        ],
        "Grade": 1,
        "Weight": 70,
        "EndureTemplateID": 50
    },
    "35310101": {
        "_id": 35310101,
        "KingdomID": 12,
        "ClassID": 12131,
        "Name": {
            "CH": "婕德",
            "EN": "Jeht"
        },
        "Title": [
            {
                "CH": "沙漠的明珠",
                "EN": "Pearl of the Desert"
            }
        ],
        "Desc": {
            "CH": "哲伯莱勒的女儿。和旅行者一起经历了诸多故事。",
            "EN": "Jebrael's daughter. Witnessed a great many stories with the traveller."
        },
        "Color": "Water",
        "HP": 360,
        "HPCurve": 3,
        "ATK": 0.05,
        "ATKCurve": 2,
        "Icon": [
            "emdc"
        ],
        "Grade": 1,
        "Weight": 70,
        "EndureTemplateID": 150
    },
    "35310301": {
        "_id": 35310301,
        "KingdomID": 12,
        "ClassID": 12131,
        "Name": {
            "CH": "哲伯莱勒",
            "EN": "Jebrael"
        },
        "Title": [
            {
                "CH": "「图特摩斯的双刃」",
                "EN": "\"The Twin Blades of Thutmose\""
            }
        ],
        "Desc": {
            "CH": "「图特摩斯的双刃」之一。婕德的父亲。多年前离开了图特摩斯部落，被另一位「图特摩斯的双刃」萨梅尔追猎。",
            "EN": "One of the Twin Blades of Thutmose. Jeht's father. Left Thutmose long ago and is hunted by Samail, the other Twin Blade of Thutmose."
        },
        "Color": "Elec",
        "HP": 360,
        "HPCurve": 3,
        "ATK": 0.05,
        "ATKCurve": 2,
        "Icon": [
            "emdt"
        ],
        "Grade": 1,
        "Weight": 80,
        "EndureTemplateID": 150
    },
    "35410101": {
        "_id": 35410101,
        "KingdomID": 12,
        "ClassID": 12134,
        "Name": {
            "CH": "阿萨里格",
            "EN": "Azariq"
        },
        "Title": [
            {
                "CH": "「叛徒」",
                "EN": "\"The Traitor\""
            }
        ],
        "Desc": {
            "CH": "塔尼特部族的一员。",
            "EN": "A member of the Tanit Tribe."
        },
        "Color": "Rock",
        "HP": 360,
        "HPCurve": 3,
        "ATK": 0.05,
        "ATKCurve": 2,
        "RESBase": {
            "Phys": -0.2
        },
        "Icon": [
            "emse"
        ],
        "Grade": 1,
        "Weight": 150,
        "EndureTemplateID": 210
    },
    "35410201": {
        "_id": 35410201,
        "KingdomID": 12,
        "ClassID": 12134,
        "Name": {
            "CH": "塔德菈",
            "EN": "Tadhla"
        },
        "Title": [
            {
                "CH": "塔尼特的「猎鹰」",
                "EN": "Tanit's \"Falcon\""
            }
        ],
        "Desc": {
            "CH": "塔尼特部族的一员。",
            "EN": "A member of the Tanit Tribe."
        },
        "Color": "Wind",
        "HP": 360,
        "HPCurve": 3,
        "ATK": 0.05,
        "ATKCurve": 2,
        "RESBase": {
            "Phys": -0.2
        },
        "Icon": [
            "emgh"
        ],
        "Grade": 1,
        "Weight": 80,
        "EndureTemplateID": 210
    },
    "42": {
        "_id": 42,
        "KingdomID": 1,
        "ClassID": 101012,
        "Name": {
            "CH": "刻晴",
            "EN": "Keqing"
        },
        "Title": [
            {
                "CH": "霆霓快雨",
                "EN": "Driving Thunder"
            }
        ],
        "Color": "Elec",
        "HP": 42,
        "HPCurve": 1,
        "ATK": 42,
        "ATKCurve": 3,
        "Icon": [
            "keq"
        ],
        "Grade": 4,
        "Weight": 0,
        "EndureTemplateID": 0
    },
    "4201": {
        "_id": 4201,
        "KingdomID": 1,
        "ClassID": 101012,
        "Name": {
            "CH": "刻晴 · 炽焰初绽",
            "EN": "Keqing - Blooming Flicker"
        },
        "Title": [
            {
                "CH": "霆霓快雨",
                "EN": "Driving Thunder"
            }
        ],
        "Color": "Fire",
        "HP": 42,
        "HPCurve": 1,
        "ATK": 42,
        "ATKCurve": 3,
        "Icon": [
            "keq"
        ],
        "Grade": 4,
        "Weight": 0,
        "EndureTemplateID": 0
    },
    "4202": {
        "_id": 4202,
        "KingdomID": 1,
        "ClassID": 101012,
        "Name": {
            "CH": "刻晴 · 幽谷涓流",
            "EN": "Keqing - Mountain Stream"
        },
        "Title": [
            {
                "CH": "霆霓快雨",
                "EN": "Driving Thunder"
            }
        ],
        "Color": "Water",
        "HP": 42,
        "HPCurve": 1,
        "ATK": 42,
        "ATKCurve": 3,
        "Icon": [
            "keq"
        ],
        "Grade": 4,
        "Weight": 0,
        "EndureTemplateID": 0
    },
    "4203": {
        "_id": 4203,
        "KingdomID": 1,
        "ClassID": 101012,
        "Name": {
            "CH": "刻晴 · 寂世霜寒",
            "EN": "Keqing - Descending Frost"
        },
        "Title": [
            {
                "CH": "霆霓快雨",
                "EN": "Driving Thunder"
            }
        ],
        "Color": "Ice",
        "HP": 42,
        "HPCurve": 1,
        "ATK": 42,
        "ATKCurve": 3,
        "Icon": [
            "keq"
        ],
        "Grade": 4,
        "Weight": 0,
        "EndureTemplateID": 0
    },
    "4204": {
        "_id": 4204,
        "KingdomID": 1,
        "ClassID": 101012,
        "Name": {
            "CH": "刻晴 · 亘古花海",
            "EN": "Keqing - Archean Flora"
        },
        "Title": [
            {
                "CH": "霆霓快雨",
                "EN": "Driving Thunder"
            }
        ],
        "Color": "Grass",
        "HP": 42,
        "HPCurve": 1,
        "ATK": 42,
        "ATKCurve": 3,
        "Icon": [
            "keq"
        ],
        "Grade": 4,
        "Weight": 0,
        "EndureTemplateID": 0
    },
    "4205": {
        "_id": 4205,
        "KingdomID": 1,
        "ClassID": 101012,
        "Name": {
            "CH": "刻晴 · 孤冢阴风",
            "EN": "Keqing - Sepulchral Wind"
        },
        "Title": [
            {
                "CH": "霆霓快雨",
                "EN": "Driving Thunder"
            }
        ],
        "Color": "Wind",
        "HP": 42,
        "HPCurve": 1,
        "ATK": 42,
        "ATKCurve": 3,
        "Icon": [
            "keq"
        ],
        "Grade": 4,
        "Weight": 0,
        "EndureTemplateID": 0
    },
    "4206": {
        "_id": 4206,
        "KingdomID": 1,
        "ClassID": 101012,
        "Name": {
            "CH": "刻晴 · 巍巍巉岩",
            "EN": "Keqing - Looming Monolith"
        },
        "Title": [
            {
                "CH": "霆霓快雨",
                "EN": "Driving Thunder"
            }
        ],
        "Color": "Rock",
        "HP": 42,
        "HPCurve": 1,
        "ATK": 42,
        "ATKCurve": 3,
        "Icon": [
            "keq"
        ],
        "Grade": 4,
        "Weight": 0,
        "EndureTemplateID": 0
    }
}

var _MonsterCustomColorNameConfig = {
    "21001": {
        "Name": {
            "CH": "深罪浸礼者 · <color style='color:#FF0000;'>火</color><color style='color:#00B0F0;'>冰</color><color style='color:#7030A0;'>雷</color>驱役者",
            "EN": "Iniquitous Baptist - Invoker of <color style='color:#FF0000;'>Flame</color>, <color style='color:#00B0F0;'>Frost</color>, and <color style='color:#7030A0;'>Fulmination</color>"
        }
    },
    "21002": {
        "Name": {
            "CH": "深罪浸礼者 · <color style='color:#00B0F0;'>冰</color><color style='color:#0070C0;'>水</color><color style='color:#FF0000;'>火</color>驱役者",
            "EN": "Iniquitous Baptist - Bringer of <color style='color:#00B0F0;'>Frost</color>, <color style='color:#0070C0;'>Flood</color>, and <color style='color:#FF0000;'>Flame</color>"
        }
    },
    "21003": {
        "Name": {
            "CH": "深罪浸礼者 · <color style='color:#7030A0;'>雷</color><color style='color:#FF0000;'>火</color><color style='color:#0070C0;'>水</color>驱役者",
            "EN": "Iniquitous Baptist - Caller of <color style='color:#7030A0;'>Fulmination</color>, <color style='color:#FF0000;'>Flame</color>, and <color style='color:#0070C0;'>Flood</color>"
        }
    },
    "21004": {
        "Name": {
            "CH": "深罪浸礼者 · <color style='color:#0070C0;'>水</color><color style='color:#00B0F0;'>冰</color><color style='color:#7030A0;'>雷</color>驱役者",
            "EN": "Iniquitous Baptist - Summoner of <color style='color:#0070C0;'>Flood</color>, <color style='color:#00B0F0;'>Frost</color>, and <color style='color:#7030A0;'>Fulmination</color>"
        }
    },
    "40501": {
        "Name": {
            "CH": "半永恒统辖矩阵",
            "EN": "ASIMON"
        }
    },
    "60401": {
        "Name": {
            "CH": "<color style='color:#0070C0;'>(水)</color><color style='color:#D4A000;'>岩龙蜥</color>",
            "EN": "<color style='color:#0070C0;'>Hydro</color> <color style='color:#D4A000;'>Geovishap</color>"
        }
    },
    "60402": {
        "Name": {
            "CH": "<color style='color:#FF0000;'>(火)</color><color style='color:#D4A000;'>岩龙蜥</color>",
            "EN": "<color style='color:#FF0000;'>Pyro</color> <color style='color:#D4A000;'>Geovishap</color>"
        }
    },
    "60403": {
        "Name": {
            "CH": "<color style='color:#00B0F0;'>(冰)</color><color style='color:#D4A000;'>岩龙蜥</color>",
            "EN": "<color style='color:#00B0F0;'>Cryo</color> <color style='color:#D4A000;'>Geovishap</color>"
        }
    },
    "60404": {
        "Name": {
            "CH": "<color style='color:#7030A0;'>(雷)</color><color style='color:#D4A000;'>岩龙蜥</color>",
            "EN": "<color style='color:#7030A0;'>Electro</color> <color style='color:#D4A000;'>Geovishap</color>"
        }
    },
    "60501": {
        "Name": {
            "CH": "<color style='color:#0070C0;'>(水)</color><color style='color:#D4A000;'>古岩龙蜥</color>",
            "EN": "<color style='color:#0070C0;'>Hydro</color> <color style='color:#D4A000;'>Primo Geovishap</color>"
        }
    },
    "60502": {
        "Name": {
            "CH": "<color style='color:#FF0000;'>(火)</color><color style='color:#D4A000;'>古岩龙蜥</color>",
            "EN": "<color style='color:#FF0000;'>Pyro</color> <color style='color:#D4A000;'>Primo Geovishap</color>"
        }
    },
    "60503": {
        "Name": {
            "CH": "<color style='color:#00B0F0;'>(冰)</color><color style='color:#D4A000;'>古岩龙蜥</color>",
            "EN": "<color style='color:#00B0F0;'>Cryo</color> <color style='color:#D4A000;'>Primo Geovishap</color>"
        }
    },
    "60504": {
        "Name": {
            "CH": "<color style='color:#7030A0;'>(雷)</color><color style='color:#D4A000;'>古岩龙蜥</color>",
            "EN": "<color style='color:#7030A0;'>Electro</color> <color style='color:#D4A000;'>Primo Geovishap</color>"
        }
    }
}

var _SpiralAbyssFloorEntryToLevelCoeffConfig = {
    "9-1": {
        "Level": 72,
        "HPCoeff": 2.0
    },
    "9-2": {
        "Level": 74,
        "HPCoeff": 2.0
    },
    "9-3": {
        "Level": 76,
        "HPCoeff": 2.0
    },
    "10-1": {
        "Level": 80,
        "HPCoeff": 2.0
    },
    "10-2": {
        "Level": 82,
        "HPCoeff": 2.0
    },
    "10-3": {
        "Level": 85,
        "HPCoeff": 2.0
    },
    "11-1": {
        "Level": 88,
        "HPCoeff": 2.0
    },
    "11-2": {
        "Level": 90,
        "HPCoeff": 2.0
    },
    "11-3": {
        "Level": 92,
        "HPCoeff": 2.0
    },
    "12-1": {
        "Level": 95,
        "HPCoeff": 2.5
    },
    "12-2": {
        "Level": 98,
        "HPCoeff": 2.5
    },
    "12-3": {
        "Level": 100,
        "HPCoeff": 2.5
    },
    "44-1": {
        "Level": 85,
        "HPCoeff": 1.69
    },
    "44-2": {
        "Level": 85,
        "HPCoeff": 1.61
    },
    "44-3": {
        "Level": 85,
        "HPCoeff": 1.65
    },
    "44-4": {
        "Level": 85,
        "HPCoeff": 1.45
    },
    "44-5": {
        "Level": 85,
        "HPCoeff": 1.85
    }
}